...
bool MemoryManager::open(const char* memoryRegionsFile, MM_Mode mode)
{
    if ( mode == MM_ReadOnly  )
    {
        ...
        if ( !mrv.empty() )
            CG_ASSERT("Previous contents were loaded");
...
void MemoryManager::_storeDataMemory(MemoryRegion* mr)
{
    ...
    if ( fileReadOnly )
        CG_ASSERT("File was opened in read-only mode");
...
{
    if ( !file.is_open() )
        CG_ASSERT("Memory file not opened");
    if ( fileReadOnly )
        CG_ASSERT("In read-only mode Memory Regions cannot be created");
...
uint MemoryManager::_generateChecksum(const MPtr data, uint bytes)
{
    if ( sizeof(unsigned short) != 2 )
        CG_ASSERT("sizeof(unsigned short) != 2");
...
{
    if ( fileReadOnly )
        CG_ASSERT("Searching by contents not available when read-only mode is selected");
...
bool MemoryManager::close()
{
    ...
    if ( file.is_open() )
    {
        CG_ASSERT("Memory file could not be closed");
        return false;
    }
    return true;
}
...
{
    if ( ptr == 0 || bytes == 0 )
        CG_ASSERT("Trying to create an empty memory region");
