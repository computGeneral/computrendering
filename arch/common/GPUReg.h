#ifndef __GPU_REGISTERS_H__
#define __GPU_REGISTERS_H__
...
static const U32 MIN_VIEWPORT_X = 0;        //  Minimum viewport x coordinate.  
static const U32 MIN_VIEWPORT_Y = 0;        //  Minimum viewport y coordinate.  
static const U32 MAX_VIEWPORT_X = 4096;     //  Maximum viewport x coordinate.  
static const U32 MAX_VIEWPORT_Y = 4096;     //  Maximum viewport y coordinate.  
/***  Maximun number of vertex shader constants. SHOULD BE THE SAME IN SHADEREMULATOR!!!  */
static const U32 MAX_VERTEX_CONSTANTS = 512;
/***  Maximun number of fragment shader constants.  SHOULD BE THE SAME IN SHADEREMULATOR!!!  */
static const U32 MAX_FRAGMENT_CONSTANTS = 1024;
/***  Maximun number of vertex input attributes.  SHOULD BE THE SAME IN SHADEREMULATOR!!!  */
 ...
 */
static const U32 MAX_TRIANGLE_ATTRIBUTES = 16;
static const U32 MAX_STREAM_BUFFERS = 32;   //  Maximum number of stream buffers.  
static const U32 MAX_USER_CLIP_PLANES = 6;  //  Maximum number of user clip planes.  
/**  Stream buffer ID for attribute default value (attribute inactive).  */
static const U32 ST_INACTIVE_ATTRIBUTE = 255;
static const U32 MAX_FRAGMENT_ATTRIBUTES = 16;//  Maximum number of fragment attributes.  
static const U32 MAX_MICROFRAGMENT_ATTRIBUTES = (MAX_FRAGMENT_ATTRIBUTES * 3);//  Maximum number of microtriangle fragment attributes.  
static const U32 MAX_TEXTURES = 16;//  Number of texture units.  
static const U32 MAX_TEXTURE_SIZE = 13;//  Max texture size (2^n).  
static const U32 CUBEMAP_IMAGES = 6;//  Number of images (2D images) in a cubemap.  
static const U32 MAX_ANISOTROPY = 16;//  Maximum level of anisotropy.  
static const U32 MAX_MSAA_SAMPLES = 16;//  Defines the maximum number of samples to be used by the MSAA (MultiSampling AntiAliasing) algorithm.  
static const F64 MSAA_SUBPIXEL_PRECISSION = 128.0f;//  Defines the subpixel precission for MSAA samples.  
static const U32 MSAA_PATTERNS = 2;//  Defines the number of patterns that can be stored for MSAA.  
static const U32 MAX_RENDER_TARGETS = 8;//  Defines the maximum number of render targets.  
/**
*  Defines the maximum number of bytes required to represent a color.
*/
static const U32 MAX_BYTES_PER_COLOR = 8;  //  RGBA16F.
/**
 *  Defines the maximum number of bytes required to store data for a pixel (depth or color).
 */
static const U32 MAX_BYTES_PER_PIXEL = 8;  // RGBA16F
 ...
 *
 */
static const U32 STAMP_WIDTH = 2;
static const U32 STAMP_HEIGHT = 2;
static const U32 STAMP_FRAGMENTS = 4;
...
static const U32 COLOR_ATTRIBUTE = 1;
static const U32 COLOR_ATTRIBUTE_SEC = 2;
static const U32 COLOR_ATTRIBUTE_BACK_PRI = 3;
static const U32 COLOR_ATTRIBUTE_BACK_SEC = 4;
static const U32 FACE_ATTRIBUTE = 15;

/**
 *
 *  Defines the attribute that will be used to pass the vertex index in cmoStreamController Loader
 *  bypass mode.
 *
 */
 
static const U32 INDEX_ATTRIBUTE = 0; 
 ...
 *
 */
static const U32 X_ATTRIBUTE = 0;
static const U32 Y_ATTRIBUTE = 1;
static const U32 Z_ATTRIBUTE = 2;
static const U32 W_ATTRIBUTE = 3;
static const U32 A_ATTRIBUTE = 0;
static const U32 B_ATTRIBUTE = 1;
static const U32 C_ATTRIBUTE = 2;
static const U32 AREA_ATTRIBUTE = 3;

//*  Instruction memory size (number of instructions) in the unified shader model. 
static const U32 UNIFIED_INSTRUCTION_MEMORY_SIZE = 2048;

//*  Number of registers in the input register bank for the unified shader model.  
static const U32 UNIFIED_INPUT_NUM_REGS = 16;

//*  Number of registers in the output register bank for the unified shader model.  
static const U32 UNIFIED_OUTPUT_NUM_REGS = 16;

//*  Number of registers in the temporary register bank for the unified shader model.  
static const U32 UNIFIED_TEMPORARY_NUM_REGS = 32;

//*  Number of registers in the address register bank (integer) for the unified shader model.  
static const U32 UNIFIED_ADDRESS_NUM_REGS = 2;

//*  Number of registers in the constant register bank for the unified shader model.  
static const U32 UNIFIED_CONSTANT_NUM_REGS = 512;

//*  Number of registers in the predicate register bank for the unified shader model.  
static const U32 UNIFIED_PREDICATE_NUM_REGS = 32;
...
enum cgeGpuStatus
{
    ...
    GPU_MEMORY_WRITE,           //  The command processor is writing data to memory (from MetaStream).  
...
struct cgsGpuRegister
{

    //  GPU state registers.  
    cgeGpuStatus statusRegister;       //  GPU status register.  
    //cgoStreamMultiprocConfig streamMultiprocConfig; // Stream Multiprocessor Config register.

    //  GPU display registers.  
    U32 displayResX;             //  Display horizontal resolution.  
    U32 displayResY;             //  Display vertical resolution.  

    //  GPU memory registers.  
    U32 frontBufferBaseAddr;     //  Frontbuffer GPU memory base address.  
    U32 backBufferBaseAddr;      //  Backbuffer GPU memory base address.  
    U32 zStencilBufferBaseAddr;  //  Z/Stencil Buffer GPU memory base address.  
    U32 textureMemoryBaseAddr;   //  Texture memory GPU memory base address.  
    U32 programMemoryBaseAddr;   //  Shader program memory GPU memory base address.  

    //  GPU vertex shader registers.  
    U32 vertexProgramAddr;       //  Active vertex program address in program memory.  
    U32 vertexProgramStartPC;    //  Active vertex program initial PC.  
    U32 vertexProgramSize;       //  Size of the vertex program to load.  
    U32 vertexThreadResources;   //  Amount of 'resources' used per vertex thread in the vertex shader.  
    Vec4FP32 vConstants[MAX_VERTEX_CONSTANTS];     //  Vertex shader float point vector constant bank.  
    ...
    U32 streamFrequency[MAX_STREAM_BUFFERS];     //  Frequency at which the stream is sampled: 0 -> per index/vertex, >0 -> MOD(instance, freq).  
    U32 streamStart;                             //  Start position (entry) from where to start streaming data.  
    U32 streamCount;                             //  Count number of elements to stream to the Vertex Shader.  
    U32 streamInstances;                         //  Count number of instances (loops) for the current stream.  
    bool indexedMode;                               //  Indexed primitive mode.  
    U32 indexStream;                             //  Index stream buffer.  
    ...
    F32 slopeFactor;             //  Depth offset as a factor of the triangle z slope.  
    F32 unitOffset;              //  Depth offset in depth precission units.  
    ...
    U32 msaaSamples;             //  Number of z samples to generate per fragment when MSAA is enabled.  
    ...
    U32 fragProgramAddr;         //  Current fragment program address in program memory.  
    U32 fragProgramStartPC;      //  Current fragment program initial PC.  
    U32 fragProgramSize;         //  Current fragment program size.  
    U32 fragThreadResources;     //  Amount of 'resources' used per fragment thread in the fragment shader.  
    Vec4FP32 fConstants[MAX_FRAGMENT_CONSTANTS];   //  Fragment shader float point vector constant bank.  
    bool modifyDepth;               //  Flag storing if the fragment shader modifies the fragment depth component.  

    //  Shader program registers.
    U32 programAddress;                          //  Address of memory from where to load the shader program.  
    U32 programSize;                             //  Size in bytes of the shader program to load.  
    ...
    U32 textureAddress[MAX_TEXTURES][MAX_TEXTURE_SIZE][CUBEMAP_IMAGES];  //  Address in GPU memory of the active texture mipmap levels.  
    ...
    U32 maxAnisotropy[MAX_TEXTURES];     //  Max anisotropy level for the texture.  

    //  GPU per fragment operations and tests registers.  

    //  Depth and stencil clear values.  
    U32 zBufferClear;            //  Z Buffer clear depth.  
    U32 stencilBufferClear;      //  Stencil Buffer clear value.  
    U32 zstencilStateBufferAddr; //  Z Stencil block state buffer memory address.  
    ...
    U32 rtAddress[MAX_RENDER_TARGETS];       //  Base address in memory of the render target.  
    
    //  Color buffer clear value.  
    Vec4FP32 colorBufferClear;     //  Color Buffer clear color.  
    U32 colorStateBufferAddr;    //  Color block state buffer memory address.  
    ...
    Vec4FP32 blendColor[MAX_RENDER_TARGETS];                   //  Blend constant color.  
    ...
    U32 mcSecondInterleavingStartAddr; //* Second interleaving start address (0 means not used) 
    
    // Framebuffer bit blit operation registers. 

    U32 blitIniX;                                 //  Framebuffer initial (lower left) X coordinate.  
    U32 blitIniY;                                 //  Framebuffer initial (lower left) Y coordinate.  
    U32 blitXOffset;                              //  Texture initial (lower left) X coordinate.  
    U32 blitYOffset;                              //  Texture initial (lower left) Y coordinate.  
    U32 blitHeight;                               //  Blit region height.  
    U32 blitWidth;                                //  Blit region width.  
