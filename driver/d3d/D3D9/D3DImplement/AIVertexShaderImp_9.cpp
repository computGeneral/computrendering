...
i_parent(_i_parent) 
{
    ...
    //D3D9State::instance().addVertexShader(this, pFunction);
...
libGAL::GALShaderProgram* AIVertexShaderImp9::getAcdVertexShader() 
{

    if (galVertexShader == NULL) {

        if (nativeVertexShader == NULL)
            nativeVertexShader = ShaderTranslator::get_instance().translate(programIR);

        galVertexShader = D3D9State::instance().createShaderProgram();
        
        galVertexShader->setCode(nativeVertexShader->bytecode, nativeVertexShader->lenght);

    }
    
    return galVertexShader;
