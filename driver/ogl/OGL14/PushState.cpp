...
void pushstate::pushAttrib(TstateStack& stateStack, const GLContext& ctx, const TextureManager& tm, GLbitfield mask)
{
    ...
    if ( mask > GL_ALL_ATTRIB_BITS )
        CG_ASSERT("unexpected Attrib Group");
...
{
    if (stateStack.empty())
        CG_ASSERT("Stack underflow. The PushState stack is empty");
...
void EnableGroup::syncState(GLContext& ctx) const
{
     ...
     *     lights     
     *
     * only affect the construction of vertex and fragment programs generated by OGL14
