...
// This call returns an interface and autogeneration doesn't handle this case.
inline void D3D9PixRunPlayer::Direct3DCreate9_SPECIFIC_PRE()
{
    U32 oip_Result;
    
    IDirect3D9 *sip_Result;    
    reader.readParameter<U32>(&oip_Result);
...
{
    if (result != D3D_OK)
        CG_ASSERT("CreateDevice returned an error.");
...
#define D3D9OP_IDIRECT3DSURFACE9_GETCONTAINER_SPECIFIC_PRE \
    U32 specific_ov_ppContainer;\
    reader.readParameter<U32>(&specific_ov_ppContainer);\
...
#define D3D9OP_IDIRECT3DTEXTURE9_QUERYINTERFACE_SPECIFIC_PRE \
    U32 ov_Return; \
    HRESULT sv_Return; \
    reader.readParameter<U32>(&ov_Return); \
    sv_Return = ov_Return; \
    \
    PIXPointer oip_This; \
...
{
    D3DTraceConfig &cfg = D3DTraceConfig::instance();
    cfg.load("../../driver/d3d/trace/D3DTracePlayer/D3D9PixRunPlayer.ini");
...
void D3D9PixRunPlayer::execute() {
    ...
    D3D9CID speId = static_cast<D3D9CID>(reader.getFetchedCID());
    switch (speId) {
    ...
    // Frame ended
    if((speId == D3D9CID_IDIRECT3DDEVICE9_PRESENT) || (speId == D3D9CID_IDIRECT3DSWAPCHAIN9_PRESENT) )
    {
        frameEnded = true;
    }
    else
...
U32 D3D9PixRunPlayer::getCurrentEventID()
{
...
D3DTracePlayer *createD3D9Player(HWND window) {
    D3D9PixRunPlayer *p = new D3D9PixRunPlayer();
    p->setWindow(window);
    return p;
}

D3DTrace *createD3D9Trace(char *file_name)
{
    D3D9PixRunPlayer *p = new D3D9PixRunPlayer();
    p->setWindow(0);
