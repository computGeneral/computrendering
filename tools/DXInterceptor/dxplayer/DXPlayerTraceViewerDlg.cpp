...
DXPlayerTraceViewerDlg::DXPlayerTraceViewerDlg(const CString& ProfilingFilePath, CWnd* pParent) :
...
bool DXPlayerTraceViewerDlg::DumpTrace()
{
  ...
  if (!traceman.OpenRead(m_ProfilingFilePath.GetString()))
  {
    CString str;
    str.Format("Could'nt open '%s'.", m_ProfilingFilePath);
