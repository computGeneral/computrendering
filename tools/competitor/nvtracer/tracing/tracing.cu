...
std::string traces_location = cwd + "/_traces_/";
std::string kernelslist_location = cwd + "/_traces_/kernellist";
std::string stats_location = cwd + "/_traces_/stats.csv";
...
void nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid,
  ...
  if (first_call == true) {
    ...
    if (mkdir("_traces_", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == -1) {
      if (errno == EEXIST) {
        // alredy exists
      } else {
        // something else
    ...
    if(user_defined_folders == 1)
    {
      ...
      std::string temp_kernelslist_location = usr_folder + "/kernellist";
