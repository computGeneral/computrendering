...
#ifndef GALx_GLSTATE_H
    #define GALx_GLSTATE_H
...
typedef GALxQuadReg<float> Quadf;
...
private:
    ...
    mutable GALxMatrixf matrixState[MATRICES_SIZE];
...
public:
    ...
    void setMatrix(MatrixId mid, GLuint unit, const GALxMatrixf& mat);
    void setMatrix(const GALxMatrixf& data, GLuint id);

    // Interface for getting a matrix easily 
    const GALxMatrixf& getMatrix(MatrixId mid, MatrixType mtype, GLuint unit = 0) const;
    
    // Compiler interface for getting a matrix 
    const GALxMatrixf& getMatrix(GLuint id) const;    
