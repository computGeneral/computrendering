/**************************************************************************
 *
 * cmoStreamController Commit mdu implementation file.
 *
 */
 ...
 *
 *  This file implements the cmoStreamController Commit mdu.
 *
 *
 */
...
StreamerCommit::StreamerCommit(U32 idxCycle, U32 oFIFOSize, U32 omSize, U32 nShaders,
    U32 maxOutLat, U32 vertCycle, U32 attrCycle, U32 outLat, char **shPrefixArray,
...

{
   ...
   //  Check shaders and shader signals prefixes.  
    ...
    for(i = 0; i < numShaders; i++)
    {
        //  cmoStreamController state signals to the shaders.  
        ...
        //  Set default StreamController state signal to the shaders.  
    ...
    //  Create commad signal from the main cmoStreamController mdu.  
    streamerCommitCommand = newInputSignal("StreamerCommitCommand", 1, 1, NULL);

    //  Create state signal to the cmoStreamController main mdu.  
    ...
    //  Create new index signal from the cmoStreamController Output Cache.  
    streamerCommitNewIndex = newInputSignal("StreamerCommitNewIndex", indicesCycle, 1, NULL);

    //  Create deallocation signal to the cmoStreamController Fetch.  
    streamerCommitDeAlloc = newOutputSignal("StreamerCommitDeAlloc", 4 * verticesCycle, 1, NULL);

    //  Create deallocation signal to the cmoStreamController Output Cache.  
    ...
    outputMemory = new Vec4FP32*[outputMemorySize];

    //  Check memory allocation.  
    CG_ASSERT_COND(!(outputMemory == NULL), "Error allocating memory for the output memory.");
    //  Allocate memory for the output memory lines.  
    for(i = 0; i < outputMemorySize; i++)
    {
        outputMemory[i] = new Vec4FP32[MAX_VERTEX_ATTRIBUTES];

        //  Check memory allocation.  
        CG_ASSERT_COND(!(outputMemory[i] == NULL), "Error allocating memory for the output memory lines.");    }

    //  Allocate memory for the last use table.  
    lastUseTable = new U32[outputMemorySize];

    //  Check memory allocation.  
    CG_ASSERT_COND(!(lastUseTable == NULL), "Error allocating memory for the last use table.");
    //  Allocate memory for the Output FIFO.  
    outputFIFO = new OFIFO[outputFIFOSize];

    //  Check memory allocation.  
    CG_ASSERT_COND(!(outputFIFO == NULL), "Error allocating memory for the output FIFO.");
    //  Allocate array of counter storing the remaining cycles for transmiting vertices to Primitive Assembly.  
    transCycles = new U32[verticesCycle];

    //  Check allocation.
    CG_ASSERT_COND(!(transCycles == NULL), "Error allocating the vertex transmission cycle counters.");
    ...
    outputMemoryDeAlloc = new U32*[verticesCycle];

    //  Check allocation.  
    CG_ASSERT_COND(!(outputMemoryDeAlloc == NULL), "Error allocating the array of the deallocated indices.");
    for(i = 0; i < verticesCycle; i++)
    {
        outputMemoryDeAlloc[i] = new U32[3];

        //  Check allocation.  
        CG_ASSERT_COND(!(outputMemoryDeAlloc[i] == NULL), "Error allocating the array of the deallocated indices.");    }
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(outputMemoryDeAllocHit == NULL), "Error allocating the table of hits in deallocated output memory lines.");
    //  Allocate the array of StreamController control commands of deallocated indices.  
    outputMemoryDeAllocCCom = new StreamerControlCommand*[verticesCycle];

    //  Check allocation.  
    CG_ASSERT_COND(!(outputMemoryDeAllocCCom == NULL), "Error allocating the array of StreamController control commands of deallocated indices.");
    //  Initialize the array of StreamController control commands of deallocated indices.  
    for (i = 0; i < verticesCycle; i++)
    {
    ...
    //  Create statistics.  
    indices = &getSM().getNumericStatistic("Indices", U32(0), "StreamerCommit", "StC");
    shOutputs = &getSM().getNumericStatistic("ShaderOutputs", U32(0), "StreamerCommit", "StC");
    paOutputs = &getSM().getNumericStatistic("Outputs", U32(0), "StreamerCommit", "StC");
    paRequests = &getSM().getNumericStatistic("Requests", U32(0), "StreamerCommit", "StC");
    outAttributes = &getSM().getNumericStatistic("OutputAttributes", U32(0), "StreamerCommit", "StC");
    avgVtxShadeLat = &getSM().getNumericStatistic("AvgShadeLatency", U32(0), "StreamerCommit", "StC");
    ...
    //  Create dummy last StreamController command.  
...
void StreamerCommit::clock(U64 cycle)
{
...
outputFIFO[nextOutput].stCCom);
    ...
    switch(state)
    {
        ...
        case ST_READY:
            /*  Ready state.  Wait for a start command from
                the StreamController main mdu.  */

            GPU_DEBUG_BOX( printf("StreamerCommit => READY state.\n"); )

            //  Get commands from the cmoStreamController main mdu.  
        ...
        case ST_STREAMING:
            ...
            //  Read new index command from the cmoStreamController Output Cache.  
            while (streamerCommitNewIndex->read(cycle, (DynamicObject *&) streamCCom))
            {
                CG_ASSERT_COND(!(freeOFIFOEntries == 0), "No free output FIFO entries.");
                ...
                if (lastUseTable[streamCCom->getOMLine()] != outputFIFOSize)
                {
                    /*  This output doesn't use an additional output memory line, so send
                        an already allocated command to cmoStreamController Fetch.  */
                    ...
                    //  Send already allocated command to the cmoStreamController Fetch.  
            ...
            for (i = 0; i < outputMemoryDeAllocs; i++)
            {
                ...
                if (!outputMemoryDeAllocHit[i])
                {
                    ...
                    //  Send output memory deallocation to the cmoStreamController Fetch.  
                    ...
                    //  Send output memory deallocation to the cmoStreamController Output Cache.  
                    streamerCommitDeAllocOC->write(cycle, streamCCom);

                    //  Delete new index StreamController control command of deallocated output memory line.  
                ...
                else
                {
                    /*  A new index this cycle matches with an index deallocated on the previous cycle.
                        ...
                        StreamController command is sent to cmoStreamController Fetch.  */
            ...
            for(i = 0; (i < verticesCycle) && (nextVertexBus < verticesCycle); i++)
            {
                ...
                if (outputFIFO[nextOutput].isShaded && (transCycles[nextVertexBus] == 0))
                {
                    ...
                    if (requestVertex > 0)
                    {
                        ...
                        oaux = new Vec4FP32[MAX_VERTEX_ATTRIBUTES];

                        //  Check memory allocation.  
                        CG_ASSERT_COND(!(oaux == NULL), "Primitive Assembly input buffer could not be allocated.");
                        ...
                        transCycles[nextVertexBus] = (U32) ceil(((F32) activeOutputs) / (F32) attributesCycle);
                        ...
                        //  Copy cookies from the source new index StreamController control command.  
                        ...
                        if(lastUseTable[oMemLine] == nextOutput)
                        {
                            ...
                            //  Add the StreamController control command of the deallocated index to the table.  
                            ...
                            //  This determines next if the StreamController control command must be deleted.  
                        ...
                        //  Send output FIFO deallocation signal to the cmoStreamController Fetch.  
                        streamerCommitDeAlloc->write(cycle, streamCCom);

                        //  Check if the index StreamController control command is no longer needed.  
                        if (!addedToDeAllocTable)
                        {
                            //  Delete new index StreamController control command for this index.  
                ...
                if (shOutputSignal[i]->read(cycle, (DynamicObject *&) output))
                {
                    ...
                    if (!output->getKill())
                    {
                        ...
                        if (validationMode)
                        {
                            ...
                            Vec4FP32 *vAttr = outputMemory[oMemLine];
                            for(U32 a = 0; a < MAX_VERTEX_ATTRIBUTES; a++)
                    ...
                    {
                        CG_ASSERT_COND(!(!output->isLast()), "Output for a hit index but not last index.");                    }
        ...
        case ST_FINISHED:
            /*  Finished state.  Wait for END command from the cmoStreamController
                main mdu.  */

            GPU_DEBUG_BOX( printf("StreamerCommit => FINISHED state.\n"); )

            //  Receive a command from the cmoStreamController main mdu 
            if (streamerCommitCommand->read(cycle, (DynamicObject *&) streamCom))
            {
                processStreamerCommand(streamCom);
            }
        ...
        default:
            CG_ASSERT("State not supported.");
            break;
    }

    //  Send current state to the cmoStreamController main mdu.  
    ...
    for(i = 0; i < numShaders; i++)
    {
        ...
        CG_ASSERT_COND(!(lastOutputSent && firstOutput), "Unexpected first output received and last output sent in the same cycle.");
        //  cmoStreamController state signals to the shaders.  
        if (lastOutputSent)
        {
...
void StreamerCommit::processStreamerCommand(StreamerCommand *streamCom)
{
    ...
    switch(streamCom->getCommand())
    {
        ...
        case STCOM_REG_WRITE:
            ...
            //  Check cmoStreamController Commit state.  
...
void StreamerCommit::processGPURegisterWrite(GPURegister gpuReg, U32 gpuSubReg,
     GPURegData gpuData)
{
    ...
    switch(gpuReg)
    {
        case GPU_VERTEX_OUTPUT_ATTRIBUTE:
            ...
            CG_ASSERT_COND(!(gpuSubReg >= MAX_VERTEX_ATTRIBUTES), "Illegal vertex output attribute identifier.");
