...
HRESULT D3D_CALL AIVolumeImp9 :: UnlockBox ( ) 
{
    ...
    //D3D9State::instance().updateVolume(this, &lockBox, Format, lockedData);

    volumeTexture->getGALVolumeTexture()->updateData(mipmap, leftLock, topLock, frontLock,
                                                    (rightLock - leftLock), (bottomLock - topLock),
                                                    (backLock - frontLock), D3D9State::instance().getGALFormat(Format),
                                                    getSurfacePitchGAL((rightLock - leftLock), Format),
                                                    (libGAL::gal_ubyte*) lockedData,
                                                    D3D9State::instance().isPreloadCall());
