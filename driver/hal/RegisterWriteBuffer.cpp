...
void RegisterWriteBuffer::writeRegister(GPURegister reg, U32 index,
                                        const GPURegData& data, U32 md)
{
    ...
    if (aux != registerStatus.end())
    {
        ...
        CG_ASSERT("GPU Register not registered.");
...
void RegisterWriteBuffer::unbufferedWriteRegister(GPURegister reg, U32 index, const GPURegData& data, U32 md)
{
    RegisterIdentifier auxri(reg, index);
    WriteBufferIt aux = registerStatus.find(auxri);
...
void RegisterWriteBuffer::flush()
{
    ...
    if ( it != writeBuffer.end() ) // register found, update contents
    {
        ...
        driver->_sendcgoMetaStream(new cgoMetaStream(GPU_INDEX_STREAM, 0, it->second.data, 0));

        //  Remove the register update from the write buffer.  
        writeBuffer.erase(it);
    }

    // Update CG1 registers
    for ( WriteBufferIt it = writeBuffer.begin(); it != writeBuffer.end(); it++ )
    {
        GPURegister reg = it->first.reg;
        U32 index = it->first.index;
        driver->_sendcgoMetaStream(new cgoMetaStream(reg, index, it->second.data, it->second.md));
    }
...
void RegisterWriteBuffer::dumpRegisterStatus (int frame, int batch)
{
    ...
    ofstream out;
    char fileName[128];
    sprintf(fileName, "HALDump_frame%d_batch%d.txt", frame, batch);
    out.open(fileName);
...
void RegisterWriteBuffer::initRegisterStatus (GPURegister reg, U32 index,
                                        const GPURegData& data, U32 md)
{
...
void RegisterWriteBuffer::dumpRegisterInfo(ofstream& out, GPURegister reg, U32 index,const GPURegData& data, U32 md)
{
    out << "Register: ";
    switch (reg)
    {
        ...
        case GPU_STENCIL_REFERENCE: out << "GPU_STENCIL_REFERENCE" << "(Index: " <<index<<", Data: "<<U08(data.uintVal)<<", MD:"<< md; break;
        case GPU_STENCIL_COMPARE_MASK: out << "GPU_STENCIL_COMPARE_MASK" << "(Index: " <<index<<", Data: "<<U08(data.uintVal)<<", MD:"<< md; break;
        case GPU_STENCIL_UPDATE_MASK: out << "GPU_STENCIL_UPDATE_MASK" << "(Index: " <<index<<", Data: "<<U08(data.uintVal)<<", MD:"<< md; break; 
