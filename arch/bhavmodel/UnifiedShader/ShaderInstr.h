...
enum cgeOperandType
{
    CG_OPERAND_TYPE_VGPR,
    CG_OPERAND_TYPE_SGPR,
    CG_OPERAND_TYPE_LOCAL,
    CG_OPERAND_TYPE_CONST,
    CG_OPERAND_TYPE_IMM,
};
...
//  To avoid double definitions.  Defined in cgoShaderInstr.cpp.  
...
class cgoShaderInstr : public DynamicMemoryOpt
{
public:
    //  Define constants.
...
private:
    ...
    struct cgsInstrFieldsLo64
    {
        U64 opcode      : 8,         //  Bits 00 - 07  =>  Instruction opcode.  
     ...
     *
     */
    struct cgsInstrFieldsHi64Operands
    {
        U64 op1reg     : 8,          //  Bits 00 - 07  =>  Instruction first operand register identifier.  
     ...
     *
     */
    struct cgsInstrFieldsHi64Immediate
    {
        U64 op1reg     : 8,          //  Bits 00 - 07  =>  Instruction first operand register identifier.  
               op1swizzle : 8,          //  Bits 08 - 15  =>  Instruction first operand swizzle.  
               resreg     : 8,          //  Bits 16 - 23  =>  Instruction result register identifier.  
     ...
     *
     */
    struct cgsInstrFieldsHi64
    {
        union
        {
            cgsInstrFieldsHi64Operands operands;
            cgsInstrFieldsHi64Immediate immediate;
        };
    };
     ...
     *
     */
    struct cgsInstrFields
    {
        cgsInstrFieldsLo64 lo64Bits;
        cgsInstrFieldsHi64 hi64Bits;
    };
    ...
    U32      op2;                //  Second operand.  Register Pointer.  
    Bank        bankOp2;            //  Register bank for Operand 2.  
    U32      op3;                //  Third operand.  Register Pointer.  
    Bank        bankOp3;            //  Register bank for Operand 3.  
    U32      res;                //  Result.  Register Pointer.  
    ...
    U32      predicateReg;       //  Predicate register.  
    ...
    U32      relModeAddrReg;     //  Address register used in relative access mode (constant registers).  
    U32      relModeAddrRegComp; //  Address register component used in relative access mode (constant registers).  
    S16      relModeOffset;      //  Offset in relative access mode -256 to +255 (constant registers).  
    bool        relativeModeFlag;   //  The instruction uses relative access mode to constant register.  
    U32      immediate;          //  Instruction 32-bit immediate value.  
    ...
    bool        isSOACompatibleB;   //  The instruction is compatible with a Scalar (scalar) ALU architecture.  
    ...
    //  Information for executing the instruction in bmoUnifiedShader.  
    ...
    static U32 setNumOperands(ShOpcode opcode);
    ...
    /**
     *
     *  Determines if the instruction is Scalar compatible (scalar ALU architecture) based on the opcode and the result write mask.
     *
     *  @param opcode The instruction opcode.
     *  @param writeMask The instruction result write mask.
     *
     *  @return Returns if the instruction is Scalar compatible.
     *
     */
    ...
    static bool parseRelAddr(char *line, U32 &i, U32 length,
    ...
    static bool parseSwizzle(char *line, U32 &i, U32 length, SwizzleMode &swz);
    ...
    static bool parseOperand(char *line, U32 &i, U32 length,
...
public:
    ...
    class cgoShaderInstrEncoding: public DynamicMemoryOpt
    {

    private:

        cgoShaderInstr   *shInstr;   //  Reference to the shader instruction for the decoded instruction.  

        //**  Pointer to the function that emulates the instruction in bmoUnifiedShader.   
        void (*emulFunc)(cgoShaderInstrEncoding &, bmoUnifiedShader &);

        U32      PC;                 //  Instruction PC when loaded in a Shader Behavior Model Unit.  
        U32      numThread;          //  Instruction Thread Number when loaded in a Shader Behavior Model.  

        U08       op1[16];            //  Local data array for first operand.  
        U08       op2[16];            //  Local data array for second operand.  
        U08       op3[16];            //  Local data array for third operand.  
        U08       res[16];            //  Local data array for result.  
        
        void        *shEmulOp1;         //  Pointer to the first operand register in a bmoUnifiedShader.  
        void        *shEmulOp2;         //  Pointer to the second operand register in a ShaderEmmulator.  
        void        *shEmulOp3;         //  Pointer to the third operand register in a bmoUnifiedShader.  
        void        *shEmulResult;      //  Pointer to the result register in a bmoUnifiedShader.  
        void        *shEmulPredicate;   //  Pointer to the predicate register in a bmoUnifiedShader.  
    ...
    public:
         ...
         *  @param thread The thread identifier for the cgoShaderInstr to be stored decoded.
         *
         *
         *  @return A new cgoShaderInstrEncoding instruction.
         *
         */

        cgoShaderInstrEncoding(cgoShaderInstr *shInstr, U32 pc, U32 thread);
        ...
        cgoShaderInstr *getShaderInstruction() const;

        /**
         *  Set the emulation function for the instruction.  This function
         *  should be one of the provided by bmoUnifiedShader.
         *
        ...
        void setEmulFunc(void (*emulFunc)(cgoShaderInstr::cgoShaderInstrEncoding &, bmoUnifiedShader &));

        /**
         *  Get the emulation function of the instruction.
         *
         *  \return Returns a pointer to the function in bmoUnifiedShader
         *  that emulates the instruction.
         *
         */

        void (*getEmulFunc())(cgoShaderInstr::cgoShaderInstrEncoding &, bmoUnifiedShader &) ;

        /**
         *  Set the Instruction PC inside a bmoUnifiedShader Instruction Memory.
         *
        ...
        /**
         *  Get the Instruction PC in a bmoUnifiedShader Instruction Memory.
         *
         *  \return Returns the PC of the instruction in a bmoUnifiedShader
         *  Instruction Memory.
         *
         */
        ...
        /**
         *  Set the Thread for the instruction in a bmoUnifiedShader.
         *
         *  \param numThread  Identifier of the thread in which this
         *  instruction is going to be executed inside a bmoUnifiedShader.
         *
         */

        void setNumThread(U32 numThread);

        /**
         *  Get the Thread of the instruction.
         *
         *  \return Returns the identifier of a thread in a bmoUnifiedShader
         *  where the instruction is going to be executed.
         *
         */

        U32 getNumThread() const;

        /**
         *  Set the pointer of the first operand register in a bmoUnifiedShader.
         *
         *  \param reg  Pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Get the pointer for the first operand register in a bmoUnifiedShader.
         *
         *  \return Returns a pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Set the pointer of the second operand register in a bmoUnifiedShader.
         *
         *  \param reg  Pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Get the pointer for the second operand register in a bmoUnifiedShader.
         *
         *  \return Returns a pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Set the pointer of the third operand register in a bmoUnifiedShader.
         *
         *  \param reg  Pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Get the pointer for the third operand register in a bmoUnifiedShader.
         *
         *  \return Returns a pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Set the pointer of the result register in a bmoUnifiedShader.
         *
         *  \param reg  Pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Get the pointer for the result register in a bmoUnifiedShader.
         *
         *  \return Returns a pointer to a bmoUnifiedShader register.
         *
         */
        ...
        /**
         *  Set the pointer to the predicate register in the bmoUnifiedShader.
         *
         *  @param reg Pointer to the bmoUnifiedShader register.
         *
         */
        ...
        /**
         *
         *  Get the pointer to the predicate register in a bmoUnifiedShader.
         *
         *  @return Returns a pointer to a bmoUnifiedShader register.
         *
         */
     ...
     *  \return Returns a cgoShaderInstr object with the decoded instruction.
     *
     */

    cgoShaderInstr(U08 *code);
     ...
     *
     *  @return Returns a cgoShaderInstr object and encodes the
     *  instruction binary representation.
     *
     */

    cgoShaderInstr(ShOpcode opc,                                     //  Instruction Opcode.
        ...
        Bank resB, U32 resR, bool resSat, MaskMode resM,             //  Result parameters.
    ...
    void disassemble(char* dis, int size);

    /**
     *  Converts a text line with an assembly shader instruction
     *  to a cgoShaderInstr object.
     *
     ...
     *
     *  @return A pointer to a cgoShaderInstr that represents the
     *  assembly instruction, a NULL if the assembly instruction could
     *  not be correctly assembled.
     *
     */

    static cgoShaderInstr *assemble(char *asmline, string &errorString);
    ...
    /**
     *
     *  Check fi the instruction is Scalar compatible.
     *
     *  @return A boolean value informing if the instruction is Scalar compatible (scalar ALU architecture).
     *
     */
    ...
    void getCode(U08 *code);


    U32 getOperandIndex(int opIdx);
    /**
