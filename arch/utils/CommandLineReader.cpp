...
//  CommandLineReader single instance.  
CommandLineReader *CommandLineReader::lineReaderInst = 0;


// This method is fully multiplatform
//  Returns a reference to the single CommandLineReader instance.  
CommandLineReader &CommandLineReader::instance()
{
    //  Create the CommandLineReader instance if it doesn't exist yet.  
    if (lineReaderInst == 0)
    {
        lineReaderInst = new CommandLineReader();
    }

    //  Return the single CommandLineReader instance.  
...
{
    CG_ASSERT("Line reader not supported under windows for now");
}

void CommandLineReader::initialize() {}
void CommandLineReader::restore() {}
void CommandLineReader::readLine(string prompt, string& line) {}
CommandLineReader::~CommandLineReader() {}
*/
...
const char CommandLineReader::UP_KEY[]       = "H";     //  Up key escape string.  
const char CommandLineReader::DOWN_KEY[]     = "P";     //  Down key escape string.  
const char CommandLineReader::RIGHT_KEY[]    = "M";     //  Right key escape string.  
const char CommandLineReader::LEFT_KEY[]     = "K";     //  Left key escape string.  
#else
const char CommandLineReader::UP_KEY[]       = "[A";     //  Up key escape string.  
const char CommandLineReader::DOWN_KEY[]     = "[B";     //  Down key escape string.  
const char CommandLineReader::RIGHT_KEY[]    = "[C";     //  Right key escape string.  
const char CommandLineReader::LEFT_KEY[]     = "[D";     //  Left key escape string.  
#endif

//  CommandLineReader constructor.  
CommandLineReader::CommandLineReader() :
    escapeMode(false), endLine(false), linePos(0), currentLine(0)
{
    ...
    if (inputHandle == INVALID_HANDLE_VALUE)
        CG_ASSERT("Error obtaining the console input handle.");
...
void CommandLineReader::setInputProcessing(bool enable)
{
    return;
    ...
    if (!result)
    {
        ...
        CG_ASSERT("Error getting the console input mode.");
    }
    ...
    {
        DWORD error = GetLastError();
        CG_ASSERT("Error setting the console input mode.");   
    }
}

char CommandLineReader::getChar()
{
    char resultChar;
    ...
    while (!keyPressed)
    {
        ...
        if (!result)
            CG_ASSERT("Error reading console input.");
        ...
        switch(inputRecord.EventType)
        {
            ...
            default:
                CG_ASSERT("Undefined console input event.");
                break;
        }        
    }
...
void CommandLineReader::escapeChar(string s)
{
    if (!s.compare(UP_KEY))
    {
        if (currentLine > 0)
...
void CommandLineReader::specialChar(unsigned char c)
{
    if (c == BACKSPACE)
    {
...
void CommandLineReader::printableChar(unsigned char c)
{
    ...
    {
        cout << "eror => CommandLineReader::printableChar(), Line position is out of bonds." << endl;
        exit(-1);
    }
}

/*void (*CommandLineReader::callBackTable[])(char) =
{
