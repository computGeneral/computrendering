...
void ShaderOptimizer::setCode(const gal_ubyte* code, gal_uint sizeInBytes)
{
    ...
    while (!programEnd && (binary_pointer < (_inputCode + _inputSizeInBytes)))
    {
        ...
        binary_pointer += arch::cgoShaderInstr::CG_ISA_INSTR_SIZE;
...
void ShaderOptimizer::optimize()
{
    ...
    _outputSizeInBytes = _inputInstrInfoVect.size() * arch::cgoShaderInstr::CG_ISA_INSTR_SIZE;
    ...
    {
        if (_inputInstrInfoVect[pointer]->operation.opcode != CG_ISA_OPCODE_NOP)
            found = true;
        else    
            pointer--;
    }
    ...
    while ( iter != _inputInstrInfoVect.end() )
    {
        ...
        delete shInstr;
        outputPointer += arch::cgoShaderInstr::CG_ISA_INSTR_SIZE;
        iter++;
        listCount++;
    }
    ...
    /*gal_ubyte* auxPointer = new gal_ubyte[_outputSizeInBytes + arch::cgoShaderInstr::CG_ISA_INSTR_SIZE * 16];
    memcpy((void *)auxPointer,(const void*)_outputCode, _outputSizeInBytes);
    
    for (gal_uint i=0; i < 16; i++)
    {
        cgoShaderInstr shInstr(NOP);
        shInstr.getCode((U08 *)(auxPointer + _outputSizeInBytes + i * arch::cgoShaderInstr::CG_ISA_INSTR_SIZE));
    }
    ...
    _outputSizeInBytes += arch::cgoShaderInstr::CG_ISA_INSTR_SIZE * 16;*/
}
