...
///////////////////////////////////////////////////////////////////
BankSelectionPolicy* BankSelectionPolicyFactory::create(const std::string& definition, U32 banks)
{
...
void BankSelectionPolicy::sortBanks(BankInfoArray& banksInfo)
{
    if ( bankCompareObject.ready() ) {
        ...
        CG_ASSERT("At least 1 bankCOmparator policy has to be added");
    }
}

void BankSelectionPolicy::debug_printBanks(const BankInfoArray& bia)
{
    for ( U32 i = 0; i < bia.size(); ++i ) {
...
{
    S32 result = 0;
    for ( U32 i = 0; i < policies.size() && result == 0; ++i )
...
RandomComparator::RandomComparator(U32 banks) : Banks(banks), randomWeights(banks)
{
    // empty
}
...
void RandomComparator::update()
{
    ...
    for ( U32 i = 0; i < Banks; ++i )
        randomWeights[i] = static_cast<U32>(rand());
}

S32 RandomComparator::compare(const BankSelectionPolicy::BankInfo* a, const BankSelectionPolicy::BankInfo* b)
{
...
RoundRobinComparator::RoundRobinComparator(U32 banks) : Banks(banks), nextRR(0)
{
    // empty
}
...
S32 RoundRobinComparator::compare(const BankSelectionPolicy::BankInfo* a, const BankSelectionPolicy::BankInfo* b)
{
    if ( a->bankID == b->bankID )
        return 0;

    U32 turnBankA = (a->bankID + (Banks - nextRR)) % Banks;
    U32 turnBankB = (b->bankID + (Banks - nextRR)) % Banks;
...
S32 AgeComparator::compare(const BankSelectionPolicy::BankInfo* a, const BankSelectionPolicy::BankInfo* b) 
{       
...
S32 ConsecutiveHitsComparator::compare(const BankSelectionPolicy::BankInfo* a, const BankSelectionPolicy::BankInfo* b)
{       
...
S32 QueueSizeComparator::compare(const BankSelectionPolicy::BankInfo* a, const BankSelectionPolicy::BankInfo* b)
{
