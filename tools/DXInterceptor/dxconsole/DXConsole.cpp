...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
  ...
  for (int i=1; i < argc; ++i)
  {
    ...
    {
      if (!options.ProfilingFilename)
      {
        options.ProfilingFilename = argv[i];
        continue;
      }
  ...
  params.TraceFilename = options.ProfilingFilename;
