...
MCTConsole::MCTConsole( const cgsArchConfig& arch, 
                        const char** tuPrefixes,
                        const char** suPrefixes,
                        const char** slPrefixes,
                        const char* name,
                        const MemoryControllerProxy& mcProxy,
                        cmoMduBase* parentBox ) : MemoryControllerTestBase(arch, tuPrefixes, suPrefixes, slPrefixes, name, mcProxy, parentBox),
                        currentCycle(0), startCycle(0), quitConsole(false)
{
    ...
    lastData[DISPLAYCONTROLLER].push_back(string());
    ...
    if ( arch.mem.memoryControllerV2 ) {
        ...
        const U32 splitterType = arch.mem.v2SplitterType;
        const U32 burstLength = arch.mem.burstLength;
        const U32 memoryChannels = arch.mem.v2MemoryChannels;
        const U32 banksPerChannel = arch.mem.v2BanksPerMemoryChannel;
        const U32 memoryRowSize = arch.mem.v2MemoryRowSize;
        const U32 gpuMemorySize = arch.mem.memSize;
        ...
        if ( splitterType == 0 ) {
            splitter = new MCSplitter
                       (
                           ...
                           memoryRowSize / 4, 
                           arch.mem.v2ChannelInterleaving,
                           arch.mem.v2BankInterleaving
                       );
        }
            ...
            splitter = new MCSplitter2
                       (
                           ...
                           gpuMemorySize / (memoryChannels * banksPerChannel * memoryRowSize),
                           memoryRowSize / 4, 
                           string(arch.mem.v2ChannelInterleavingMask),                    
                           string(arch.mem.v2BankInterleavingMask)
                       );
        }
        else {
            stringstream ss;
...
bool MCTConsole::processPrintCommand(const std::vector<std::string>& params)
{
    ...
    U32 unit = MC_INVALID_INTEGER;
    U32 subunit = MC_INVALID_INTEGER;
    ...
    U32 startByte = MC_INVALID_INTEGER;
    U32 byteCount = MC_INVALID_INTEGER;
...
bool MCTConsole::processSendCommand(const vector<string>& params)
{
    ...
    U32 address = parseAddress(params[2]);
    ...
    if ( params.size() > 4 )
        sendRequest(static_cast<GPUUnit>(unit), subunit, rt, bytes, address, (U08*)params[4].c_str());
    else
...
U32 MCTConsole::parseAddress(string addressStr)
{
    U32 dots = static_cast<U32>(std::count(addressStr.begin(), addressStr.end(), '.'));
    if ( dots == 0 ) {
        U32 hasx = static_cast<U32>(std::count(addressStr.begin(), addressStr.end(), 'x'));
        U32 hasX = static_cast<U32>(std::count(addressStr.begin(), addressStr.end(), 'X'));
        if ( hasx + hasX == 1 ) {
            // hex value expected
            ...
            U32 address = MC_INVALID_INTEGER;
        ...
        U32 channel = MC_INVALID_INTEGER;
        U32 bank = MC_INVALID_INTEGER;
        U32 row = MC_INVALID_INTEGER;
        U32 col = MC_INVALID_INTEGER;
...
{
    const MemParameters& mem = ArchConfig().mem;
    cout << "----------------------------------------------------------\n";
    cout << "GPU Global Architecture\n";
    cout << "   GPU clock (gclock): " << ArchConfig().gpu.gpuClock << " MHz\n";
    cout << "   Memory Controller clock (mclock): " << ArchConfig().gpu.memoryClock << " MHz\n";
...
string MCTConsole::parseUnitName(string unitName, U32& unit, U32& subunit)
{    
    ...
    if ( unitName == "CP" || unitName == "COMMANDPROCESSOR" )
        ...
        unit = COLORWRITE;
    else if ( unitName == "DisplayController" || unitName == "DISPLAYCONTROLLER" )
...
void MCTConsole::clock_test(U64 cycle)
{
    currentCycle = cycle;
...
void MCTConsole::handler_receiveTransaction(U64 cycle, MemoryTransaction* mt)
{
    ...
    switch ( unit ) 
    {
        ...
        case TEXTUREUNIT:
            break;
        default:
            CG_ASSERT("Unknown GPU unit identifier!");
    }

    U32 subunit = mt->getUnitID();
    if ( subunit >= lastData[unit].size() ) 
        CG_ASSERT("Sub-unit ID too high");
...
void MCTConsole::handler_sendTransaction(U64 cycle, MemoryTransaction* mt)
{
    if ( mt->isToSystemMemory() ) {
        switch ( mt->getCommand() ) {
            ...
            case MT_WRITE_DATA:
                ...
                break;
            default:
                CG_ASSERT("Unnexpected type of request sent!");
        }
    }
    else {
        switch ( mt->getCommand() ) {
