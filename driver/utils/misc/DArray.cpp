...
void DArray::init(size_t maxDArrays, size_t memoryAmount, size_t clusterSize)
{
    ...
    if ( memSet )
        CG_ASSERT("Can be called only once");
    memSet = true;
    ...
    if ( memoryAmount % clusterSize != 0 )
        CG_ASSERT("Memory size must be multiple of cluster size");
...
void* DArray::operator new(size_t size_)
{
    ...
    if ( !memSet )
        CG_ASSERT("setMemory must be called before any DArray object creation");
    ...
    CG_ASSERT("Memory for DArray objects exhaused, no more DArrays can be created");
    throw bad_alloc();
    return 0;
}
...
void DArray::operator delete(void* ptr)
{
    ...
    if ( id >= objectBitmap.size() )
        CG_ASSERT("trying to deleting a pointer not obtained using new");
...
void DArray::set(size_t bytes_)
{
    ...
    if ( start == ClusterNotFound )
        CG_ASSERT("Data Memory exhausted!");
...
DArray::DArray(const void* ptr_, size_t bytes_)
{
    ...
    if ( !memSet )
        CG_ASSERT("setMemory must be called before allocating any DArray object");
    ...
    if ( start == ClusterNotFound )
    {
        CG_ASSERT("bad_alloc (Memory exhausted)");
        throw bad_alloc();
    }
