...
ColorWrite::ColorWrite(U32 numStamps, U32 overWidth, U32 overHeight,
    U32 scanWidth, U32 scanHeight, U32 genWidth, U32 genHeight, U32 pixelBytes,
    ...
    U32 colorBlocks, U32 blocksPerCycle, U32 comprCycles,
    U32 decomprCycles, U32 blendQSize, U32 blendStartLat, U32 blendLat,
    U32 blockLatency, U32 fragMapMode, bmoFragmentOperator &fragEmu,
    ...
    blockStateLatency(blockLatency), fragmentMapMode(fragMapMode), frEmu(fragEmu), cmoMduBase(name, parent)

{
    ...
    GPU_ASSERT(
        ...
        if (bytesPixel != 4)
            CG_ASSERT("Only supported 32bit color values.");
        if (colorCacheWays < 1)
        ...
        if (blendQueueSize < 2)
            CG_ASSERT("Blend queue requires at least two entries.");
        if (blendRate == 0)
            CG_ASSERT("At least one cycle required between stamps being blended.");
        if (blendLatency < 1)
            CG_ASSERT("Blend ALU latency must be 1 cycle or greater.");
    ...
    //  Create signals with Z Test mdu.  
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(blendQueue == NULL), "Error allocating the blend queue.");
    //  Allocate the buffers for the stamp fragment color.  
    for(i = 0; i < blendQueueSize; i++)
    {
        //  Allocate stamp input colors (float).  
        blendQueue[i].inColor = new Vec4FP32[STAMP_FRAGMENTS];

        //  Check allocation.  
        CG_ASSERT_COND(!(blendQueue[i].inColor == NULL), "Error allocating stamp input color buffer in the blend queue.");
        //  Allocate stamp output colors (integer).  
        blendQueue[i].outColor = new U08[STAMP_FRAGMENTS * bytesPixel];

        //  Check allocation.  
        CG_ASSERT_COND(!(blendQueue[i].outColor == NULL), "Error allocating stamp output color buffer.");
        //  Allocate destination color buffer (integer).  
        blendQueue[i].destColor = new U08[STAMP_FRAGMENTS * bytesPixel];

        //  Check allocation.  
        CG_ASSERT_COND(!(blendQueue[i].destColor == NULL), "Error allocating stamp destination color buffer (integer).");
        //  Allocate destination color buffer (float).  
        blendQueue[i].destColorF = new Vec4FP32[STAMP_FRAGMENTS];

        //  Check allocation.  
        CG_ASSERT_COND(!(blendQueue[i].destColorF == NULL), "Error allocating stamp destination color buffer (float).");
        //  Allocate stamp write mask.  
        blendQueue[i].mask = new bool[STAMP_FRAGMENTS * bytesPixel];

        //  Check allocation.  
        CG_ASSERT_COND(!(blendQueue[i].mask == NULL), "Error allocating stamp write mask.");
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(stamp == NULL), "Error allocating the current stamp.");
...
void ColorWrite::clock(U64 cycle)
{
                    ...
                    switch(fragmentMapMode)
                    {
                        case COLOR_MAP:
                            ...
                            latencyMap[(blendQueue[writeBlend].y >> 1) * ((hRes >> 1) + (hRes & 0x01)) + (blendQueue[writeBlend].x >> 1)] =
                                U32(blendQueue[writeBlend].outColor[0] + (blendQueue[writeBlend].outColor[1] << 8) + (blendQueue[writeBlend].outColor[2] << 16));
                        ...
                        case FRAGMENT_LATENCY_MAP:
                            ...
                            latencyMap[(blendQueue[writeBlend].y >> 1) * ((hRes >> 1) + (hRes & 0x01)) + (blendQueue[writeBlend].x >> 1)] =
                                U32(cycle - blendQueue[writeBlend].startCycle);
...
printf("ColorWrite => End of swap.  Cycle %lld\n", cycle);
    ...
    //  Send current state to Z Test mdu.  
...
void ColorWrite::processRegisterWrite(GPURegister reg, U32 subreg,
    GPURegData data)
{
    //  Process register write.  
    switch(reg)
...
void ColorWrite::processMemoryTransaction(U64 cycle,
    MemoryTransaction *memTrans)
{
...
void ColorWrite::colorInt2Float(U08 *in, Vec4FP32 *out)
{
    U32 i;

    //  Check number of bytes per pixel.  
    CG_ASSERT_COND(!(bytesPixel != 4), "Unsupported pixel format.");
...
void ColorWrite::colorFloat2Int(Vec4FP32 *in, U08 *out)
{
    U32 i;
    ...
    for(i = 0; i < STAMP_FRAGMENTS; i++)
    {
        /*  Convert 32bit float point color components to 8 bit integer
            color components.  */
        out[i * 4] = U32(255.0f * GPU_CLAMP(in[i][0], 0.0f, 1.0f));
...
void oldClear()
{
        ...
        (clearAddress < endClearAddress))
    {
        ...
        CG_ASSERT_COND(!(freeTickets == 0), "No free memory tickets.");
        GPU_DEBUG(
...
U32 *ColorWrite::getLatencyMap(U32 &mapWidth, U32 &mapHeight)
{
