...
ShaderSetupData::ShaderSetupData(NativeShader* _translated, U32 _gpu_memory):
...
void CShaders9::on_add_node_child(StateDataNode* parent, StateDataNode* child) {
    ...
    if(child->get_id().name == NAME_VERTEX_SHADER_9) {
        ...
        U32 md = gpu->obtainMemory(translated->lenght);
        ...
        if(i_vsh != 0) {
            ...
            for(it_c = const_dec.begin(); it_c != const_dec.end(); it_c ++) {
                ...
                if(con.defined) {
                    ...
                    GPUProxy* gpu = GPUProxy::get_instance();
                    gpu->writeGPURegister(GPU_VERTEX_CONSTANT, U32(resource.index), data);
                }
            }
...
void CShaders9::load_default_pixel_shader() {
   D3D_DEBUG( cout << "CSHADERS9: Load default pixel shader" << endl; )
    ...
    // Create a buffer
    U32 program_size = instructions.size() * 16;
    U08 *memory = new U08[program_size];
	...
	if(i_psh != 0) {
		...
		for(it_c = const_dec.begin(); it_c != const_dec.end(); it_c ++) {
			...
			if(con.defined) {
				...
				D3D_DEBUG( cout << ", " << value.value.floatValue.z << ", " << value.value.floatValue.w << ")" << endl; )
				GPUProxy* gpu = GPUProxy::get_instance();
				gpu->writeGPURegister(GPU_FRAGMENT_CONSTANT, U32(resource.index), data);
			}
		}
