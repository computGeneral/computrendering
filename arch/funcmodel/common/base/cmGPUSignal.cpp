...
// pre bigbang set up !
bool Signal::setData( DynamicObject* initialData[], U64 firstCycleForReadOrWrite )
{
    if ( bandwidth <= 0 || maxLatency <= 0 )
        CG_ASSERT("Error. Signal is not well defined yet. It Cannot be initializated");

    U32 i = 0; // for indexing initialData array
    // note: Using global in
    in = static_cast<U32>(GPU_MOD( firstCycleForReadOrWrite, capacity )); // mapping of 'i' in signal contents
    for ( ; i < maxLatency; i++ ) {
        for ( nReads[in] = 0; nReads[in] < bandwidth; nReads[in]++ ) {
            U32 aux = i*bandwidth + nReads[in];
...
void Signal::clockFast(U64 cycle)
{
    //  Check for missed reads.  

    //  Move the signal read point by the number of passed cycles .  
    nextRead = (nextRead + U32(cycle - lastCycle)) & capacityMask;

    //  Move the signal write point by the number of passed cycles .  
    nextWrite = (nextWrite + U32(cycle - lastCycle)) & capacityMask;
...
bool Signal::writeGenFast(U64 cycle, DynamicObject* dataW)
{
    //  Update the signal clock.  
    if (cycle > lastCycle)
        clock(cycle);
...
bool Signal::writeGenFast(U64 cycle, DynamicObject* dataW, U32 latency)
{
    //  Update the signal clock.  
    if (cycle > lastCycle)
        clock(cycle);
...
bool Signal::readGenFast(U64 cycle, DynamicObject*& dataR)
{
...
bool Signal::writeGen( U64 cycle, DynamicObject* dataW )
{
    char buffer[200];
    ...
    U32 oldIn = in; // BTW: First time 'in' equals to 0

    in = static_cast<U32>(GPU_MOD( cycle, capacity ));

    U32 index = static_cast<U32>(GPU_MOD( ( in + maxLatency ), capacity )); // needed here for error checking
...
bool Signal::writeGen( U64 cycle, DynamicObject* dataW, U32 latency )
{
    char buffer[200];
    ...
    in = static_cast<U32>(GPU_MOD( cycle, capacity )); // new in

    U32 index = static_cast<U32>(GPU_MOD( ( in + latency ), capacity )); // needed here for error checking
...
bool Signal::readGen( U64 cycle, DynamicObject*& dataR )
{
    char buffer[200];
    U32 oldIn = in;
    in = static_cast<U32>(GPU_MOD( cycle, capacity )); // new in
...
bool Signal::checkReadsMissed( U64 cycle, U32 oldIn, U32 newIn )
{
    U32 i;
...
bool Signal::write( U64 cycle, DynamicObject* dataW )
{
    return writeGenFast( cycle, dataW );
}

bool Signal::write( U64 cycle, DynamicObject* dataW, U32 lat )
{
    return writeGenFast( cycle, dataW, lat );
}

bool Signal::read( U64 cycle, DynamicObject *&dataR )
{
    return readGenFast( cycle, dataR );
}

//  Dumps the signal trace for this cycle and signal.  
void Signal::traceSignal(ostream *ProfilingFile, U64 cycle)
{
    U32 sigPos;
    ...
    /*  Calculate position in the signal storage array for
        the cycle to dump.  */
    sigPos = static_cast<U32>(GPU_MOD( cycle, capacity ));
    ...
    for(i = 0; i < nReads[sigPos]; i++)
    {
        ...
        {
            //fprintf(ProfilingFile, "%d:", cookies[j]);
            sprintf(buffer, "%d:", cookies[j]);
            (*ProfilingFile) << buffer;
        }
        ...
        //fprintf(ProfilingFile, "%d", (U32) dynObj->getColor());
        sprintf(buffer, "%d", (U32) dynObj->getColor());
        (*ProfilingFile) << buffer;
...
// inline
U32 Signal::getBandwidth() const
{
    return bandwidth;
}

// inline
U32 Signal::getLatency() const
{
    return maxLatency;
}
...
// inline
bool Signal::setBandwidth( U32 newBandwidth )
{
    return ( setParameters( newBandwidth, maxLatency ) );
}

// inline
bool Signal::setLatency( U32 newMaxLatency )
{
    return ( setParameters( bandwidth, newMaxLatency ) );
}


bool Signal::setParameters( U32 newBandwidth, U32 newMaxLatency )
{
...
void Signal::create()
{
    ...
    capacity = 1 << U32(ceil(log(F64(maxLatency + 1))/log(2.0)));
    ...
    //    CG_ASSERT("Maximum latency excedes maximun capacity");
