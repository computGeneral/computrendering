...
void CSurfaces9::on_add_node_child(StateDataNode* parent, StateDataNode* child) {
    if(child->get_id().name == NAME_SURFACE_9) {
        ...
        // Calculate size
        U32 surface_size = get_surface_size(width, height, format, surf_usage);
        ...
        U32 md = GPUProxy::get_instance()->obtainMemory(surface_size);
        // Initialize memory
        /**@note this is necessary because during simulation accessing
           non initialized areas of the texture raises an error. */
        U08* initial_data = new U08[surface_size];
        for(U32 i = 0; i < surface_size; i ++) initial_data[i] = rand()%256;
...
void CSurfaces9::on_added_controller(StateDataNode* node) {
    if(node->get_id().name == NAME_DEVICE_9) {
        ...
        U32 gpuMemSz, sysMemSz, blocksz, sblocksz, fetchRate;
        bool doubleBuffer;
...
UINT CSurfaces9::get_surface_size(UINT width, UINT height, D3DFORMAT format, DWORD usage) {
    ...
    if(usage & D3DUSAGE_DEPTHSTENCIL) {
        size = U32(ceil(F32(height) / F32(scan_height * overscan_height))) *
        U32(ceil(F32(width) / F32(scan_width * overscan_width))) *
		...
		U32 width2 = ceiledLog2(width);
        U32 last = texel2MortonAddress(width, height, block_dim, s_block_dim, width2);
