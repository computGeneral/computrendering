...
U32 MemoryTransaction::instances = 0;

U32 MemoryTransaction::countInstances()
{
    return instances;
}

U32 MemoryTransaction::busWidth[LASTGPUBUS] =
{
...
{
    if ( unit >= LASTGPUBUS )
        CG_ASSERT("Unknown GPU Unit");
    return busNames[unit];
}

U32 MemoryTransaction::getBusWidth(GPUUnit unit)
{
    if ( unit >= LASTGPUBUS )
        CG_ASSERT("Unknown GPU Unit");
    return busWidth[unit];
}
void MemoryTransaction::setBusWidth(GPUUnit unit, U32 newBW)
{
    if ( unit >= LASTGPUBUS )
        CG_ASSERT("Unknown GPU Unit");
...
MemoryTransaction::MemoryTransaction(MemTransCom com, U32 addr,
    U32 sz, U08 *dataBuffer, GPUUnit source, U32 id) :
    ...
    state(MS_BOTH)
{
    ...
    (
       ...
       if ( (com == MT_PRELOAD_DATA) && (source != COMMANDPROCESSOR))
        ...
        if ( dataBuffer == 0 )
            CG_ASSERT("NULL pointer not allowed");
        if ( size == 0 )
            CG_ASSERT(
    ...
    switch(com)
    {
        ...
        case MT_READ_DATA:
            ...
            cycles = (U32) ceil((F32) size / (F32) busWidth[sourceUnit]);
            //  Set the transaction info field
 ...
 if ( com == MT_PRELOAD_DATA && id == 20 )
    {
        cout << "addr = " << addr << "\n";
        cout << "sz = " << sz << "\n";
        cout << "*dataBuffer = " << (U32)dataBuffer << "\n";
...
MemoryTransaction::MemoryTransaction(U32 addr, U32 sz, U08 *dataBuffer,
    U32 *writeMask, GPUUnit source, U32 id) :
    address(addr), size(sz), sourceUnit(source), unitID(0), ID(id), state(MS_BOTH)
{
    ...
    memcpy(mask, writeMask, WRITE_MASK_SIZE * sizeof(U32)); // Copy the write mask
    // Calculate transaction bus cycles
    cycles = (U32) ceil((F32) size / (F32) busWidth[sourceUnit]);
    // Set the transaction info field
...
MemoryTransaction::MemoryTransaction(MemTransCom com, U32 addr,
    U32 sz, U08 *dataBuffer, GPUUnit source, U32 sourceID, U32 id) :
...
//Memory transaction operation (for masked MT_WRITE_DATA)
MemoryTransaction::MemoryTransaction(U32 addr, U32 sz, U08 *dataBuffer,
    U32 *writeMask, GPUUnit source, U32 sourceID, U32 id) :
    unitID(sourceID),
    address(addr), size(sz), sourceUnit(source), ID(id), state(MS_BOTH)
{
    ...
    memcpy(writeData, dataBuffer, size);
    // Copy the write mask.  */
    memcpy(mask, writeMask, WRITE_MASK_SIZE * sizeof(U32));
    // Calculate transaction bus cycles
    cycles = (U32) ceil((F32) size / (F32) busWidth[sourceUnit]);
    //Set the transaction info field
    sprintf((char *) getInfo(), "WRITE_DATA(masked) @%x, %d", addr, size);
...
MemoryTransaction::MemoryTransaction(MemoryTransaction *request)
{
    ...
    (
        if (request->command != MT_READ_REQ)
            CG_ASSERT("Illegal source transaction.");
    )
...
U32 MemoryTransaction::getAddress() const
{
    return address;
}

U32 MemoryTransaction::getSize() const
{
    return size;
}

U08 *MemoryTransaction::getData() const
{
    switch(command)
    {
        case MT_READ_REQ:
...
U32 MemoryTransaction::getID() const
{
    return ID;
}

U32 MemoryTransaction::getBusCycles() const
{
    return cycles;
}
...
U32* MemoryTransaction::getMask() const
{
    return (U32*)mask;
}

U32 MemoryTransaction::getUnitID() const
{
    return unitID;
}

void MemoryTransaction::setRequestID(U32 id)
{
    requestID = id;
}

U32 MemoryTransaction::getRequestID() const
{
    return requestID;
}
...
string MemoryTransaction::getIdentificationStr(bool concatData) const
{
    ...
    if ( ptr && concatData)
    {
        for ( U32 i = 0; i < size; i++ )
        {
            ss << hex << (U32)ptr[i] << dec << ".";
        }
    }
    return ss.str();
}

U32 MemoryTransaction::getHashCode(bool useData) const
{
    U32 sum = 0;
    string s = getIdentificationStr(useData);
    string::const_iterator it = s.begin();
    for ( U32 i = 1; it != s.end(); it++, i++ )
        sum += (((U32)*it) * i) % 255;
    return sum;
}
