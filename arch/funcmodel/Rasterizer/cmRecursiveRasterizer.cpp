...
#include "RecursivecmRasterizer.h"
...
RecursiveRasterizer::RecursiveRasterizer(bmoRasterizer &rsEmu,
    U32 tsFIFOSize, U32 rdTrFIFO, U32 rdTileStack, U32 rdFrBuffer,
    U32 stampFr, U32 stampPerCycle, U32 tileEval, U32 tilesPerCycle,
    U32 inputBuffer, U32 outputBuffer, U32 tileLatency,
    char **tePrefixes, U32 numInterpolators, char *name, cmoMduBase *parent):

//  Initializations.  
bmRast(rsEmu), triangleSetupFIFOSize(tsFIFOSize), recDescTrFIFOSize(rdTrFIFO),
...
interpolators(numInterpolators), cmoMduBase(name, parent)

{
    ...
    //  Check allocation 
    CG_ASSERT_COND(!(tileEvalComm == NULL), "Error allocating command signal array to the Tile Evaluators.");
    //  Create array of state signals from the Tile Evaluators.  
    tileEvalState = new Signal*[numTileEvaluators];

    //  Check allocation 
    CG_ASSERT_COND(!(tileEvalState == NULL), "Error allocating state signal array to the Tile Evaluators.");
    ...
    triangleSetup = new TriangleSetup(bmRast, tsFIFOSize,
        "TriangleSetup", this);

    //  Create Recursive Descent mdu.  
    recDescent = new RecursiveDescent(bmRast, recDescTrFIFOSize,
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(tileEvaluator == NULL), "Error allocating the array of Tile Evaluators.");
    ...
    for(i = 0; i < numTileEvaluators; i++)
    {
        ...
        //  Create tile evaluator mdu.  
        tileEvaluator[i] = new TileEvaluator(bmRast, tilesCycle,
    ...
    interpolator = new Interpolator(bmRast, interpolators,
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(teState == NULL), "Error allocating Tile Evaluators state array.");
...
void RecursiveRasterizer::clock(U64 cycle)
{
    ...
    {
        CG_ASSERT("Missing signal from Recursive Descent.");
    }
    ...
    for(i = 0; i < numTileEvaluators; i++)
    {
        ...
        {
            CG_ASSERT("Missing signal from a Tile Evaluator.");
        }
    }
    ...
    U64 cycle)
{
    ...
    switch(rastComm->getCommand())
    {
        ...
        case RSCOM_REG_WRITE:
            ...
            CG_ASSERT_COND(!(state != RAST_READY), "Rasterizer register writes can only be received in READY state.");
        ...
        case RSCOM_DRAW:
            ...
            CG_ASSERT_COND(!(state != RAST_READY), "Rasterizer DRAW command can only be received in READY state.");
        ...
        case RSCOM_END:
            ...
            //  Check current state.  
            CG_ASSERT_COND(!(state != RAST_END), "Rasterizer END command can only be received in END state.");
        ...
        default:
            ...
            CG_ASSERT("Unsuppored Rasterizer Command.");
...
void RecursiveRasterizer::processRegisterWrite(GPURegister reg, U32 subreg,
    GPURegData data, U64 cycle)
{
    ...
    switch(reg)
    {
        ...
        default:
            ...
            CG_ASSERT("Unsupported Rasterizer register.");
            break;
    }
}
