...
// 64 muls
void mathlib::_mult_matrixf( F32 m[], const F32 a[], const F32 b[] )
{
...
void mathlib::_transpose_matrixf( F32 m[] )
{
    F32 aux[16];

    memcpy(aux, m, 16*sizeof(F32));
...
void mathlib::_translate_matrix( F32 m[], F32 x, F32 y, F32 z )
{
...
void mathlib::_scale_matrix(F32 mat[], F32 x, F32 y, F32 z)
{
     F32 m[16]; // scale matrix 

     memcpy(m, IDENTITY, sizeof(F32)*16);
 ...
 */
void mathlib::_rotate_matrix( F32 mat[], F32 angle, F32 x, F32 y, F32 z )
{
   F32 xx, yy, zz, xy, yz, zx, xs, ys, zs, one_c, s, c;
   F32 m[16]; // rotation matrix 
   bool optimized;

   s = (F32) sin( angle * DEG2RAD );
   c = (F32) cos( angle * DEG2RAD );
...
#define M(row,col)  m[col*4+row]
   ...
   if (!optimized) {
      const F32 mag = (F32) mathlib::sqrt(x * x + y * y + z * z);
 ...
 */
bool mathlib::_invert_matrixf( const F32* m, F32* out )
{
   F32 wtmp[4][8];
   F32 m0, m1, m2, m3, s;
   F32 *r0, *r1, *r2, *r3;
...
void mathlib::_mult_mat_vect( const F32* mat, F32* vect)
{
    F32 x = vect[0];
    F32 y = vect[1];
    F32 z = vect[2];
