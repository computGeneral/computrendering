...
void GLIStatsManager::beginBatch()
{
    ...
    {
        CG_ASSERT("Error. We are already in a batch");
    }
...
{    
    ...
    else
    {
        // pos != 1 
        if ( perBatchStats )
            CG_ASSERT("Error. We are not in a batch");
...
{
    if ( pos != 0 )
        CG_ASSERT("Error. We are not in a frame");
...
bool GLIStatsManager::dumpFrameStatsVertical( const char* file, int firstFrame, int lastFrame )
{
    ...
    for ( i = 0; i < APICall_UNDECLARED; i++ )
    {
        if ( frames[i].first != NULL )
        {
            ...
            if ( !callPlayable[i] )
                f << "*"; // Indicates call is not playable by GLTracePlayer 
...
bool GLIStatsManager::dumpFrameStatsHorizontal( const char* file, int firstFrame, int lastFrame )
{
    ...
    f.open(file);
    if ( !f.is_open() )
        CG_ASSERT("Error opening oputput file");
...
{    
    ...
    f.open(file);
    if ( !f.is_open() )
        CG_ASSERT("Error opening output file");
