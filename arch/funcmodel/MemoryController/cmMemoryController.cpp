 ...
 *
 *  Memory Controller implementation file.  The Memory Controller class defines the mdu
...
//  Memory Controller mdu constructor.  
MemoryController::MemoryController(U32 memSize, U32 clockMult, U32 memFreq, U32 memBusWidth, U32 numBuses,
    bool sharedBanks, U32 bankGranurality, U32 burstLength, U32 readLat, U32 writeLat, U32 writeToReadLat,
    U32 pageSize, U32 memOpenPages, U32 pageOpenLat,
    U32 consReads, U32 consWrites, U32 comProcBus, U32 streamerFetchBus, U32 streamerLoaderBus,
    U32 zStencilBus, U32 colorWriteBus, U32 dacBus, U32 textUnitBus,
    U32 systemMem, U32 readLines, U32 writeLines, U32 reqQSize, U32 servQSize,
    U32 stampUnits, const char **suPrefixArray, U32 numTxUnits, const char **tuPrefixArray,
    U32 streamLoadUnits, const char **slPrefixArray, char *name, cmoMduBase *parent):
...
//  Memory Controller mdu parameter initalization.  
...

{
    ...
    busNames[DISPLAYCONTROLLER] = MemoryTransaction::getBusName(DISPLAYCONTROLLER);
    ...
    CG_ASSERT_COND(!(gpuMemoryBuses == 0), "At least one memory bus required.");
    //  Check limit of buses per GPU unit.  
    GPU_ASSERT(
        if (baseClockMultiplier == 0)
            CG_ASSERT("Base clock multiplier must be at least 1.");
        if (gpuMemoryFrequency == 0)
            CG_ASSERT("Memory frequency (in reference clock cycles) must be at least 1.");
        if (gpuMemoryBuses == 0)
            CG_ASSERT("At least one memory bus required to work.");
        if (gpuMemReadLatency == 0)
            CG_ASSERT("GPU memory Read Latency must be at least 1 cycle.");
        if (gpuMemWriteLatency == 0)
            CG_ASSERT("GPU memory Write latency must be at least 1 cycle.");
        if (gpuMemWriteLatency > gpuMemReadLatency)
            CG_ASSERT("GPU memory Write latency must be smaller or equal than read latency.");
        if (gpuMemPageSize < MAX_TRANSACTION_SIZE)
            CG_ASSERT("A page must be larger than the maximum memory transaction.");
        if (gpuBankGranurality > gpuMemPageSize)
            CG_ASSERT("The bus interleaving granurality must be equal or smaller than the page size.");
        if (gpuMemPageOpenLat == 0)
            CG_ASSERT("The latency for opening a page must be at least 1 cycle.");
        if (readBufferSize == 0)
            CG_ASSERT("Minimum number of read buffer lines required is 1.");
        if (writeBufferSize == 0)
            CG_ASSERT("Minimum number of write buffer lines required is 1.");
        if (requestQueueSize < ((numTextUnits + numStampUnits + 4)*2))
            CG_ASSERT("Minimum number of request queue entries is: (numTextUnits + numStampsUnits + 4) * 2.");
        if (serviceQueueSize < 1)
            CG_ASSERT("Minimum number of service queue entries is: 1.");
        if (numTextUnits > MAX_GPU_UNIT_BUSES)
            CG_ASSERT("Number of Texture Unit excedes GPU unit buses limit.");
        if (numStampUnits > MAX_GPU_UNIT_BUSES)
            CG_ASSERT("Number of stamp units excedes GPU unit buses limit.");
        if (streamerLoaderUnits > MAX_GPU_UNIT_BUSES)
            CG_ASSERT("Number of cmoStreamController Loader units excedes GPU unit buses limit.");
    )

    //  Initialize the GPU memory buffer.  
    gpuMemory = new U08[memSize];

    CG_ASSERT_COND(!(gpuMemory == NULL), "Error allocation GPU memory.");
    //  Mark memory as empty.  
    for(i = 0; i < gpuMemorySize; i = i + 4)
        *((U32 *) &gpuMemory[i]) = 0xDEADCAFE;

    //  Initialize the mapped system memory buffer.  
    mappedMemory = new U08[mappedMemorySize];

    CG_ASSERT_COND(!(mappedMemory == NULL), "Error allocation mapped system memory.");
    //  Mark memory as empty.  
    for(i = 0; i < mappedMemorySize; i = i + 4)
        *((U32 *) &mappedMemory[i]) = 0xDEADCAFE;
    ...
    //  Check number of attached cmoStreamController Loader units and their prefixes.  
    GPU_ASSERT(
        if (streamerLoaderUnits == 0)
            CG_ASSERT("There should be at least one cmoStreamController Loader unit.");

        if (slPrefixArray == NULL)
            CG_ASSERT("The cmoStreamController Loader unit signal name prefix array can not be null.");
    ...
    //  Allocate signals with cmoStreamController Loader units.  
    ...
    GPU_ASSERT(
        if (streamLoadDataSignal == NULL)
            CG_ASSERT("Error allocating cmoStreamController Loader data signal array.");

        if (streamLoadReqSignal == NULL)
            CG_ASSERT("Error allocating cmoStreamController Loader request signal array.");
    )

    for (i = 0; i < streamerLoaderUnits; i++)
    {
        //  Read signal from the Memory Controller to the cmoStreamController Loader.  
        ...
        //  Command signal from the cmoStreamController Loader unit to the memory Controller.  
        streamLoadReqSignal[i] = newInputSignal("StreamerLoaderMemoryRequest", 1, 1, slPrefixArray[i]);
    }

    //  Data signal from the Memory Controller to the cmoStreamController Fetch.  
    ...
    //  Request signal from the cmoStreamController Fetch to the Memory Controller.  
    ...
    GPU_ASSERT(
        if (zStencilDataSignal == NULL)
            CG_ASSERT("Error allocating Z Stencil data signal array.");

        if (zStencilReqSignal == NULL)
            CG_ASSERT("Error allocating Z Stencil request signal array.");

        if (colorWriteDataSignal == NULL)
            CG_ASSERT("Error allocating Color Write data signal array.");

        if (colorWriteReqSignal == NULL)
            CG_ASSERT("Error allocating Color Write request signal array.");
    )
    ...
    //  Data signal from the Memory Controller to the DisplayController unit.  
    dacDataSignal = newOutputSignal("DisplayControllerMemoryData", 2, 1, NULL);
    ...
    //  Request signal from the DisplayController unit to the Memory Controller.  
    dacReqSignal = newInputSignal("DisplayControllerMemoryRequest", 1, 1, NULL);
    ...
    GPU_ASSERT(
        ...
        if (tuPrefixArray == NULL)
            CG_ASSERT("The Texture Unit signal name prefix array can not be null.");
    ...
    GPU_ASSERT(
        if (tuDataSignal == NULL)
            CG_ASSERT("Error allocating Texture Unit data signal array.");

        if (tuReqSignal == NULL)
            CG_ASSERT("Error allocating Texture Unit request signal array.");
    )
    ...
    GPU_ASSERT(
        if (memoryModuleRequestSignal == NULL)
            CG_ASSERT("Error allocating memory bus request signal.");
        if (memoryModuleDataSignal == NULL)
            CG_ASSERT("Error allocating memory bus data signal.");
    )
    ...
    GPU_ASSERT(
        if (mappedMemoryRequestSignal == NULL)
            CG_ASSERT("Error allocating system memory bus request signal.");
        if (mappedMemoryDataSignal == NULL)
            CG_ASSERT("Error allocating system memory bus data signal.");
    )
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(moduleTrans == NULL), "Error allocating array of current transactions in memory buses.");
    //  Allocate array storing the current commands in the memory module buses.  
    lastCommand = new MemTransCom[gpuMemoryBuses];

    //  Check allocation.  
    CG_ASSERT_COND(!(lastCommand == NULL), "Error allocating array of last issued commands in memory buses.");
    //  Allocate array storing the last cycle a transaction was issued in the memory module buses.  
    lastCycle = new U64[gpuMemoryBuses];

    //  Check allocation.  
    CG_ASSERT_COND(!(lastCycle == NULL), "Error allocating array of last cycle a transaction was issued in the memory buses.");
    //  Allocate array storing the pointer to the current request being processed in each memory bus.  
    currentBusRequest = new U32[gpuMemoryBuses];

    //  Check allocation.  
    CG_ASSERT_COND(!(currentBusRequest == NULL), "Error allocating array storing the pointer to the request being processed in the memory buses.");
    //  Allocate array storing the number of memory requests being processed in each memory bus.  
    activeBusRequests = new U32[gpuMemoryBuses];

    //  Check allocation.  
    CG_ASSERT_COND(!(activeBusRequests == NULL), "Error allocating array storing the number of memory requests request being processed in the memory buses.");
    //  Allocate array storing the identifiers for the pages open per bus.  
    currentPages = new U32*[gpuMemoryBuses];

    //  Check allocation.  
    CG_ASSERT_COND(!(currentPages == NULL), "Error allocating per bus array of open pages.");
    //  Allocating page identifier for each bus.  
    for(i = 0; i < gpuMemoryBuses; i++)
    {
        //  Allocate open page identifiers for a bus.  
        currentPages[i] = new U32[gpuMemOpenPages];

        //  Check allocation.  
        CG_ASSERT_COND(!(currentPages[i] == NULL), "Error allocating array of open pages for a bus.");    }
    ...
    busWidth[DISPLAYCONTROLLER] = dacBus;
    ...
    */
    ...
    MemoryTransaction::setBusWidth(DISPLAYCONTROLLER,dacBus);
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(requestBuffer == NULL), "Error allocating the memory request buffer.");
    //  Allocate the memory request buffer free entry queue.  
    freeRequestQ = new U32[requestQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(freeRequestQ == NULL), "Error allocating the request buffer free entry queue.");
    ...
    readRequestBusQ = new U32*[busQueues];
    writeRequestBusQ = new U32*[busQueues];
    numReadBusRequests = new U32[busQueues];
    numWriteBusRequests = new U32[busQueues];
    numBusRequests = new U32[busQueues];
    firstReadReq = new U32[busQueues];
    lastReadReq = new U32[busQueues];
    firstWriteReq = new U32[busQueues];
    lastWriteReq = new U32[busQueues];
    consecutiveReadReqs = new U32[busQueues];
    consecutiveWriteReqs = new U32[busQueues];

    //  Check allocation.  
    GPU_ASSERT(
        if (readRequestBusQ == NULL)
            CG_ASSERT("Error allocating per GPU memory bus read request queues.");
        if (readRequestBusQ == NULL)
            CG_ASSERT("Error allocating per GPU memory bus write request queues.");
        if (numReadBusRequests == NULL)
            CG_ASSERT("Error allocating per GPU memory bus read request counters.");
        if (numWriteBusRequests == NULL)
            CG_ASSERT("Error allocating per GPU memory bus write request counters.");
        if (numBusRequests == NULL)
            CG_ASSERT("Error allocating per GPU memory bus request counters.");
        if (firstReadReq == NULL)
            CG_ASSERT("Error allocating per GPU memory bus pointers to the first read request in the read request queues.");
        if (lastReadReq == NULL)
            CG_ASSERT("Error allocating per GPU memory bus pointers to the last read request in the read request queues.");
        if (firstWriteReq == NULL)
            CG_ASSERT("Error allocating per GPU memory bus pointers to the first write request in the write request queues.");
        if (lastWriteReq == NULL)
            CG_ASSERT("Error allocating per GPU memory bus pointers to the last write request in the write request queues.");
        if (consecutiveReadReqs == NULL)
            CG_ASSERT("Error allocating per GPU memory bus counters for consecutive read requests processed.");
        if (consecutiveWriteReqs == NULL)
            CG_ASSERT("Error allocating per GPU memory bus counters for consecutive write requests processed.");
    )
    ...
    for(i = 0; i < busQueues; i++)
    {
        ...
        readRequestBusQ[i] = new U32[requestQueueSize];
        writeRequestBusQ[i] = new U32[requestQueueSize];

        //  Check allocation.  
        GPU_ASSERT(
            if (readRequestBusQ[i] == NULL)
                CG_ASSERT("Error allocating the per GPU memory bus read request queue.");
            if (writeRequestBusQ[i] == NULL)
                CG_ASSERT("Error allocating the per GPU memory bus write request queue.");
        )
    }
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(systemTrans == NULL), "Error allocating array of current transactions in system memory buses.");
    //  Allocate the system memory request queue.  
    mappedQueue = new MemoryRequest[requestQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(mappedQueue == NULL), "Error allocating the system memory request queue.");
    //  Allocate the system memory request queue free list.  
    freeMapped = new U32[requestQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(freeMapped == NULL), "Error allocating the system memory request queue free entry list.");
    //  Allocate the system memory request ready list.  
    readyMapped = new U32[requestQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(readyMapped == NULL), "Error allocating the system memory request queue ready entries list.");

    //  Allocate service queues.  
    serviceQueue = new MemoryRequest[serviceQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(serviceQueue == NULL), "Error allocating the service queue.");
    ...
    for(i = 0; i < LASTGPUBUS; i++)
    {
        ...
        transactions[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"%sReadTransactions", busNames[i]);
        readTrans[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"%sWriteTransactions", busNames[i]);
        writeTrans[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"%sReadBytes", busNames[i]);
        readBytes[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"%sWriteBytes", busNames[i]);
        writeBytes[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");
    }

    preloadTrans = &getSM().getNumericStatistic("PreloadTransactions", U32(0), "MemoryController", "MC");
    unusedCycles = &getSM().getNumericStatistic("UnusedCycles", U32(0), "MemoryController", "MC");
    ...
    GPU_ASSERT(
        if (dataCycles == NULL)
            CG_ASSERT("Error allocating per gpu memory bus data cycles statistic.");
        if (rtowCycles == NULL)
            CG_ASSERT("Error allocating per gpu memory bus read to write penalty cycles statistic.");
        if (wtorCycles == NULL)
            CG_ASSERT("Error allocating per gpu memory bus write to read penalty cycles statistic.");
        if (openPageCycles == NULL)
            CG_ASSERT("Error allocating per gpu memory bus opening page penalty cycles statistic.");
        if (openPages == NULL)
            CG_ASSERT("Error allocating per gpu memory bus number of new open pages statistic.");
        if (gpuBusReadBytes == NULL)
            CG_ASSERT("Error allocating per gpu memory bus read bytes statistic.");
        if (gpuBusWriteBytes == NULL)
            CG_ASSERT("Error allocating per gpu memory bus write bytes statistic.");
    )

    for(i = 0; i < gpuMemoryBuses; i++)
    {
        sprintf(busName,"DataCycles%02d", i);
        dataCycles[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"ReadToWritePenalty%02d", i);
        rtowCycles[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"WriteToReadPenalty%02d", i);
        wtorCycles[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"OpenPagePenalty%02d", i);
        openPageCycles[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"OpenPages%02d", i);
        openPages[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"ReadBytesMemoryBus%02d", i);
        gpuBusReadBytes[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");

        sprintf(busName,"WriteBytesMemoryBus%02d", i);
        gpuBusWriteBytes[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");
    }

    sysBusReadBytes = &getSM().getNumericStatistic("ReadBytesSystemBus", U32(0), "MemoryController", "MC");;
    sysBusWriteBytes = &getSM().getNumericStatistic("WriteBytesSystemBus", U32(0), "MemoryController", "MC");;

    for(i = 0; i < MAPPED_MEMORY_BUSES; i++)
    {
        sprintf(busName,"SystemDataCycles%02d", i);
        sysDataCycles[i] = &getSM().getNumericStatistic(busName, U32(0), "MemoryController", "MC");
    }
    ...
    if ( mcCommSignal->read(cycle, dynObj) )
    {
        ...
        switch ( mcComm->getCommand() ) {
            ...
            case MCCOM_SAVE_MEMORY:
                ...
                break;
            default:
                CG_ASSERT("Unsupported command received by Legacy memory controller");
        }
    }
}
...
{
printf("MC %lld => gpu mem requests %d mapped mem requests %d free read buffers %d free write buffers %d\n",
...
printf("MC => pending services %d\n", pendingServices);
    ...
    //  Read new memory request from the cmoStreamController Loader Units.  
    for (i = 0; i < streamerLoaderUnits; i++)
    {
        if (streamLoadReqSignal[i]->read(cycle, (DynamicObject *&) memTrans))
        {
            GPU_DEBUG_BOX(
                printf("MemoryController => Request from the cmoStreamController Loader unit %d. \n", i);
            )
    ...
    //  Read new memory request from cmoStreamController Fetch.  
    if (streamFetchReqSignal->read(cycle, (DynamicObject *&) memTrans))
    {
        GPU_DEBUG_BOX(
            printf("MemoryController => Request from the cmoStreamController Fetch. \n");
        )
    ...
    //  Read new memory request from DisplayController unit.  
    if (dacReqSignal->read(cycle, (DynamicObject *&) memTrans))
    {
        GPU_DEBUG_BOX( printf("MemoryController %lld => Request from DisplayController unit. \n", cycle); )
    ...
    //  Send status signal to all the cmoStreamController Loader units.  
    sendBusState(cycle, STREAMERLOADER, state, streamLoadDataSignal, streamerLoaderUnits);

    //  Send status signal to cmoStreamController Fetch.  
    ...
    //  Send status signal to DisplayController.  
    sendBusState(cycle, DISPLAYCONTROLLER, state, dacDataSignal);
...
void MemoryController::sendBusState(U64 cycle, GPUUnit unit, MemState state, Signal *dataSignal)
{
    MemoryTransaction *memTrans;
    MemState unitState;
...
void MemoryController::sendBusState(U64 cycle, GPUUnit unit, MemState state, Signal **dataSignal, U32 units)
{
    MemoryTransaction *memTrans;
    MemState unitState;
    U32 i;
...
void MemoryController::receiveTransaction(MemoryTransaction *memTrans)
{
    ...
    switch(memTrans->getCommand())
    {
        ...
        case MT_WRITE_DATA:
            ...
            CG_ASSERT_COND(!(busCycles[memTrans->getRequestSource()][memTrans->getUnitID()] != 0), "Write not allowed, GPU unit bus is busy.");
            //  Check buffer is available.  
            CG_ASSERT_COND(!(freeWriteBuffers == 0), "No free write buffer available.");
        ...
        case MT_PRELOAD_DATA:
            ...
            CG_ASSERT_COND(!(data == NULL), "Undefined data buffer.");
            GPU_DEBUG_BOX(
            ...
            switch(memTrans->getAddress() & ADDRESS_SPACE_MASK)
            {
                case GPU_ADDRESS_SPACE:
                    ...
                    CG_ASSERT_COND(!(((address & SPACE_ADDRESS_MASK) >= gpuMemorySize) ||
                                   (((address & SPACE_ADDRESS_MASK) + size) > gpuMemorySize)),
                                  "GPU memory operation out of range.");
                    //  Copy data to GPU memory.  
                    memcpy(&gpuMemory[address & SPACE_ADDRESS_MASK], data, size);
                    break;
                case SYSTEM_ADDRESS_SPACE:
                    //  Check memory operation correctness.  
                    CG_ASSERT_COND(!(((address & SPACE_ADDRESS_MASK) >= mappedMemorySize) ||
        ...
        default:
            CG_ASSERT("Unsupported transaction type.");
            break;
    }
...
U32 MemoryController::addRequest(MemoryTransaction *memTrans)
{
    U32 nextRequest;
    U32 bus;
    U32 i;
    U32 searchReq;
    U32 searchRangeStart;
    U32 searchRangeEnd;
    U32 currentRangeStart;
    U32 currentRangeEnd;
    bool foundDep;

    //  Check if request buffer is full.  
    CG_ASSERT_COND(!(numFreeRequests == 0), "Request buffer full.");
    GPU_DEBUG_BOX(
...
//  Adds a new request to the request queue.  
U32 MemoryController::addMappedRequest(MemoryTransaction *memTrans)
{
    U32 nextFreeEntry;

    //  Check if queue is full.  
    CG_ASSERT_COND(!(numFreeMapped == 0), "Mapped memory request queue full.");
...
void MemoryController::wakeUpReadRequests(U32 bus, U32 wrTrans)
{
    U32 i;
    U32 wakeUpReq;
...
//printf("wake up read requests bus %d writeTrans %d\n", bus, writeTrans);
    ...
    for(i = 0; i < numReadBusRequests[bus]; i++)
    {
        ...
        if (requestBuffer[wakeUpReq].dependency && (requestBuffer[wakeUpReq].waitForTrans == wrTrans))
        {
...
void MemoryController::wakeUpWriteRequests(U32 bus, U32 rdTrans)
{
    U32 i;
    U32 wakeUpReq;
...
//printf("wake up write requests bus %d readTrans %d\n", bus, readTrans);
    ...
    for(i = 0; i < numWriteBusRequests[bus]; i++)
    {
        ...
        if (requestBuffer[wakeUpReq].dependency && (requestBuffer[wakeUpReq].waitForTrans == rdTrans))
        {
//printf("waking up write trans %d\n", wakeUpReq);
...
void MemoryController::removeRequest(U32 req)
{
...
void MemoryController::removeMappedRequest(U32 req)
{
...
U32 MemoryController::selectNextRequest(U32 bus)
{
    U32 req;
    bool readyReadReq;
    bool readyWriteReq;
    ...
    CG_ASSERT_COND(!(numBusRequests[bus] == 0), "No pending requests for the GPU memory bus.");
...
//printf("MC => selecting read request (%p) for bus %d at %d\n", requestBuffer[req].memTrans, bus, req);
    ...
    else if (readyWriteReq)
    {
        ...
        GPU_ASSERT(
            if (numWriteBusRequests[bus] == 0)
            {
                CG_ASSERT("No write or read pendeing request for the GPU memory bus.");
            }
        )
...
//printf("MC => selecting write request (%p) for bus %d at %d\n", requestBuffer[req].memTrans, bus, req);
    ...
    else
    {
        ...
        GPU_ASSERT(
            ...
            {
                CG_ASSERT("Deadlock, both request queues are blocked for the bus.");
            }
        )
...
void MemoryController::updateReadRequests(U32 bus)
{
...
void MemoryController::updateWriteRequests(U32 bus)
{
...
void MemoryController::issueTransaction(U64 cycle, U32 bus)
{
    U32 req;
    U32 address;
    U32 size;
    ...
    CG_ASSERT_COND(!(bus >= gpuMemoryBuses), "Memory bus identifier out of range.");
    ...
    GPU_ASSERT(
        if (!gpuSharedBanks && (bank != address2Bus(memTrans->getAddress())))
            CG_ASSERT("Memory transaction from a wrong bank.");
        if (!gpuSharedBanks && (((memTrans->getAddress() & busOffsetMask) + memTrans->getSize()) > gpuBankGranurality))
        {
printf("MC %lld > address %x size %d usOffset %x unit %s id %d\n",
    ...
    busNames[memTrans->getRequestSource()], memTrans->getUnitID());
            CG_ASSERT("Memory transaction crosses bank boundary.");
        }
    )
    ...
    switch(memTrans->getCommand())
    {
            ...
            ((cycle - lastCycle[bus]) >= (TRANSACTION_CYCLES + gpuMemWriteLatency + gpuMemWriteToReadLatency)))))
        {
            ...
            if  ((freeReadBuffers > 0) && (freeServices > 0))
            {
                ...
                GPU_DEBUG_BOX
                (
                    printf("MemoryController %lld => Issuing MT_READ_REQ (%x, %d) from unit %s id %d to bus %d.\n", cycle, address, size,
                ...
                //GPU_ASSERT(
                //    if (*((U32 *) &gpuMemory[address & SPACE_ADDRESS_MASK]) == 0xDEADCAFE)
                //    {
                //        printf("MC %lld => Warning.  Unallocated memory -> MT_READ_REQ : address %x size %d unit %s unitID %d to bus %d\n", cycle, address, size,
                //            busNames[memTrans->getRequestSource()], memTrans->getUnitID(), bus);
                //        //CG_ASSERT("Reading unallocated memory.");
                //    }
                //)
    ...
    case MT_WRITE_DATA:
        ...
        if (requestBuffer[req].state == MRS_READY)
        {
                ...
                ((lastCommand[bus] == MT_READ_REQ) && ((cycle - lastCycle[bus]) >= (TRANSACTION_CYCLES + gpuMemReadLatency)))))
            {
                ...
                CG_ASSERT_COND(!(data == NULL), "Undefined data buffer.");
                GPU_DEBUG_BOX(
                    printf("MemoryController %lld => Issuing MT_WRITE (%x, %d)"
                ...
                if (memTrans->isMasked())
                {
                    ...
                    {
                        //  Get source data.  
                        source = *((U32 *) &gpuMemory[(address & SPACE_ADDRESS_MASK) + i]);
                        ...
                        //  Write to memory.  
                        *((U32 *) &gpuMemory[(address & SPACE_ADDRESS_MASK) + i]) = source | dest;
                    }
                }
                else
                {
...
//if (cycle > 1769000)
            ...
            GPU_ASSERT(
                    ...
                    (((address & SPACE_ADDRESS_MASK) + size) > mappedMemorySize))
                {
                    ...
                    CG_ASSERT("Mapped memory operation out of range.");
                }
            )

            //  Check for unallocated memory.  
            //GPU_ASSERT(
                //if (*((U32 *) &mappedMemory[address & SPACE_ADDRESS_MASK]) == 0xDEADCAFE)
                //{
                //   printf("MC %lld =>  Warning.  Unallocated memory -> MT_READ_REQ : address %x size %d unit %s unitID %d\n", cycle, address, size,
                //        busNames[memTrans->getRequestSource()], memTrans->getUnitID());
                //   CG_ASSERT("Reading unallocated memory.");
                //}
            //)
        ...
        if (((cycle - systemBus[1]) >= MAPPED_TRANSACTION_CYCLES) && (mappedQueue[req].state == MRS_READY))
        {
            ...
            GPU_ASSERT(
                    ...
                    (((address & SPACE_ADDRESS_MASK) + size) > mappedMemorySize))
                {
                    ...
                    CG_ASSERT("Mapped system memory operation out of range.");
                }
            )
...
//if (cycle > 1769000)
            ...
            if (memTrans->isMasked())
            {
                ...
                {
                    //  Get source data.  
                    source = *((U32 *) &mappedMemory[(address & SPACE_ADDRESS_MASK) + i]);
                    ...
                    //  Write to memory.  
                    *((U32 *) &mappedMemory[(address & SPACE_ADDRESS_MASK) + i]) = source | dest;
                }
            }
            else
            {
...
void MemoryController::serveRequest(U64 cycle)
{
    MemoryTransaction *memTrans;
    GPUUnit unit;
    U32 unitID;
...
void MemoryController::moduleTransaction(U64 cycle, U32 bus)
{
    MemoryTransaction *memTrans;
    ...
    if (memoryModuleDataSignal[bus]->read(cycle, (DynamicObject *&) memTrans))
    {
        ...
        CG_ASSERT_COND(!(busCycles[MEMORYMODULE][bus] > 0), "Memory bus busy.");
        GPU_DEBUG_BOX(
...
void MemoryController::updateMemoryBus(U64 cycle, U32 bus)
{
    GPUUnit currentUnit;
    U32 bank;
        ...
        if (busCycles[MEMORYMODULE][bus] == 0)
        {
            ...
            switch(moduleTrans[bus]->getCommand())
            {
                case MT_READ_DATA:
                    ...
                    CG_ASSERT_COND(!(freeServices < 0), "No free service entry available.");
                    //  Get current unit.  
                    currentUnit = moduleTrans[bus]->getRequestSource();
...
//printf("MC %lld => remove MT_READ_DATA request %p bus %d\n", cycle, moduleTrans[bus], currentBusRequest[bus]);
...
//printf("MC %lld => remove MT_WRITE_DATA request %p bus %d\n", cycle, moduleTrans[bus], currentBusRequest[bus]);
...
void MemoryController::updateSystemBus(U64 cycle, U32 bus)
{
    GPUUnit currentUnit;

    CG_ASSERT_COND(!(bus >= MAPPED_MEMORY_BUSES), "System memory bus identifier out of range.");
    //  Check if there is a data transmission in the bus.  
...
//if (cycle > 1769740)
//printf("MC %lld => update -> memTrans %p\n", cycle, systemTrans[bus]);
...
void MemoryController::mappedTransaction(U64 cycle, U32 bus)
{
    MemoryTransaction *memTrans;
    GPUUnit currentUnit;

    CG_ASSERT_COND(!(bus >= MAPPED_MEMORY_BUSES), "Memory bus identifier out of range.");
    ...
    if (mappedMemoryDataSignal[bus]->read(cycle, (DynamicObject *&) memTrans))
    {
        ...
        CG_ASSERT_COND(!(busCycles[SYSTEM][bus] > 0), "System memory bus busy.");
        GPU_DEBUG_BOX(
//if (cycle > 1769000)
...
bool MemoryController::searchPage(U32 address, U32 bus)
{
    U32 pageID;
    U32 pageBankID;
    bool hit;
...
void MemoryController::openPage(U32 address, U32 bus)
{
    U32 pageID;
    U32 pageBankID;
...
U32 MemoryController::address2Bus(U32 address)
{
...
U32 arch::busWidth[] =
{
    ...
    16,     //  Data bus with the cmoStreamController Loader unit (attributes)
    32,     //  Data bus with the Z Stencil Test unit (pixels depth/stencil)
    32,     //  Data bus with the Color Write unit (pixels color)
    8,      //  Data bus with the DisplayController unit (pixels color)
...
void MemoryController::getDebugInfo(string &debugInfo) const
{
    ...
    for ( U32 i = 0; i < LASTGPUBUS; ++i ) 
    {
        ...
        for ( U32 j = 0; j < MAX_GPU_UNIT_BUSES; ++j ) 
        {
            if ( reserveBus[i][j] ) {
                if ( !busReserved ) {
