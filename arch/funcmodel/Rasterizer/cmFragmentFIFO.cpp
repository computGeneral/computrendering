 ...
 *
 *  The Fragment FIFO class implements the Fragment FIFO simulation mdu.
 *
...
FragmentFIFO::FragmentFIFO(U32 hzStampsC, U32 stampCycle, bool unified, bool shSetup, U32 trCycle,
    U32 trLat, U32 nVSh, U32 nFSh, U32 shInCycle, U32 shOutCycle, U32 thGroup,
    U32 maxShOutLat, U32 interp, U32 shInQSz, U32 shOutQSz, U32 shInBSz, bool shTileDistro,
    U32 vInQSz, U32 vOutQSz, U32 trInQSz, U32 trOutQSz, U32 rastQSz, U32 testQSz, U32 intQSz,
    U32 shaderQSz, char **vshPrefix, char **fshPrefix, U32 nStampUnits, char **suPrefixes, char *name, cmoMduBase *parent) :
    ...
    numStampUnits(nStampUnits), cmoMduBase(name, parent)

{
    ...
    GPU_ASSERT(
        if (hzStampsCycle == 0)
            CG_ASSERT("At least one stamp must be recieved from HZ per cycle.");
        if (stampsCycle == 0)
            CG_ASSERT("At least one stamp must be processed per cycle.");
        if (shInputsCycle == 0)
            CG_ASSERT("At least one shader input per cycle required.");
        if (shOutputsCycle == 0)
            CG_ASSERT("At least one shader output per cycle required.");
        if (threadGroup < STAMP_FRAGMENTS)
            CG_ASSERT("A thread group must be at least the size of a fragment quad.");
        if ((threadGroup > STAMP_FRAGMENTS) && (GPU_MOD(threadGroup, STAMP_FRAGMENTS) != 0))
            CG_ASSERT("A thread group must be multiple of a fragment quad.");
        if (fshOutLatency == 0)
            CG_ASSERT("Shader output signal latency must be at least 1.");
        if (interpolators == 0)
            CG_ASSERT("At least one attribute interpolator unit required.");
        if (shInputQSize < (numStampUnits * threadGroup * 2))
            CG_ASSERT("Required minimum size for shader input queue is 2 * SUs * thGroup.");
        if (shOutputQSize < (numStampUnits * threadGroup * 2))
            CG_ASSERT("Required minimum size for shader output queue is 2 * SUs * thGroup.");
        if (unifiedModel && (vInputQueueSz < (threadGroup + 2 * numVShaders)))
            CG_ASSERT("Vertex Input Queue requires as many entries as a wave plus the number of vertex inputs per cycle by the latency.");
        if (unifiedModel && (vShadedQueueSz < threadGroup))
            CG_ASSERT("Vertex Shaded Queue requires as many entries as a wave.");
        if (shadedSetup && !unifiedModel)
            CG_ASSERT("Triangle setup on shaders only supported for unified shader model.");
        if (shadedSetup && (trianglesCycle == 0))
            CG_ASSERT("At least one triangle must be received per cycle from Triangle Setup.");
        if (shadedSetup && (triangleLat == 0))
            CG_ASSERT("Triangle signals latency from Triangle Setup must be at least one cycle.");
        if (shadedSetup && (triangleInQueueSz < (threadGroup + (1 + triangleLat) * trianglesCycle)))
            CG_ASSERT("Triangle input queue requires at least the size of a shader thread group plus triangle input bw and latency entries.");
        if (shadedSetup && (triangleInQueueSz < ((1 + triangleLat) * trianglesCycle)))
            CG_ASSERT("Triangle input queue requires at least triangle signal latency by the number of triangles that can received per cycle entries.");
        if (shadedSetup && (triangleOutQueueSz < threadGroup))
            CG_ASSERT("Triangle output queue requires at least the size of a shader input group entries.");
        if (shadedSetup && (triangleOutQueueSz < ((1 + triangleLat) * trianglesCycle)))
            CG_ASSERT("Triangle output queue requires at least triangle signal latency by the number of triangles that can received per cycle entries.");
        if (rastQueueSz == 0)
            CG_ASSERT("Post HZ stamp queue requires at least 1 entry.");
        if (testQueueSz == 0)
            CG_ASSERT("Post Z test stamp queue requires at least 1 entry.");
        if (intQueueSz < (threadGroup/4))
            CG_ASSERT("Post interpolation stamp queue requires at a thread group equivalent of fragments.");
        if (shQueueSz == 0)
            CG_ASSERT("Post shading stamp queue requires at least 1 entry.");
        if (numStampUnits == 0)
            CG_ASSERT("FFIFO must be attached to at least one stamp unit.");
        if (GPU_MOD(shInputBatchSize, threadGroup) != 0)
            CG_ASSERT("Shader input work batch size must be a multiple of the size of a shader thread group.");
    )
...
GPU_TEX_TRACE(
    ...
    //  Create statistics.  
    inputs = &getSM().getNumericStatistic("InputFragments", U32(0), "FFIFOU", "FFU");
    outputs = &getSM().getNumericStatistic("OutputFragments", U32(0), "FFIFOU", "FFU");
    interpolated = &getSM().getNumericStatistic("InterpolatedFragments", U32(0), "FFIFOU", "FFU");
    shadedFrag = &getSM().getNumericStatistic("ShadedFragments", U32(0), "FFIFOU", "FFU");
    inVerts = &getSM().getNumericStatistic("InputVertices", U32(0), "FFIFOU", "FFU");
    outVerts = &getSM().getNumericStatistic("OutputVertices", U32(0), "FFIFOU", "FFU");
    shVerts = &getSM().getNumericStatistic("ShadedVertices", U32(0), "FFIFOU", "FFU");
    inTriangles = &getSM().getNumericStatistic("InputTriangles", U32(0), "FFIFOU", "FFU");
    outTriangles = &getSM().getNumericStatistic("OutputTriangles", U32(0), "FFIFOU", "FFU");
    shTriangles = &getSM().getNumericStatistic("ShadedTriangles", U32(0), "FFIFOU", "FFU");
    rastGLevel = &getSM().getNumericStatistic("RastStampQueuesOccupation", U32(0), "FFIFOU", "FFU");
    testGLevel = &getSM().getNumericStatistic("TestStampQueuesOccupation", U32(0), "FFIFOU", "FFU");
    intGLevel = &getSM().getNumericStatistic("IntStampQueuesOccupation", U32(0), "FFIFOU", "FFU");
    shadedGLevel = &getSM().getNumericStatistic("ShadedStampQueuesOccupation", U32(0), "FFIFOU", "FFU");
    vertInLevel = &getSM().getNumericStatistic("VertexInputQueueOccupation", U32(0), "FFIFOU", "FFU");
    vertOutLevel = &getSM().getNumericStatistic("VertexOutputQueueOccupation", U32(0), "FFIFOU", "FFU");
    trInLevel = &getSM().getNumericStatistic("TriangleInputQueueOccupation", U32(0), "FFIFOU", "FFU");
    trOutLevel = &getSM().getNumericStatistic("TriangleOutputQueueOccupation", U32(0), "FFIFOU", "FFU");

    //  Create signals with Hierarchical Z.  

    //  Stamp input signal from Hierarchical/Early Z mdu.  
    hzInput = newInputSignal("HZOutput", hzStampsCycle * STAMP_FRAGMENTS, 1, NULL);

    //  Create state signal to the Hierarchical/Early Z mdu.  
    ...
    //  Create signals with Interpolator mdu.  

    //  Create input stamp signal to Interpolator mdu.  
    interpolatorInput = newOutputSignal("InterpolatorInput", stampsCycle * STAMP_FRAGMENTS, 1, NULL);

    //  Create output stamp signal from the Interpolator mdu.  
    ...
    GPU_ASSERT(
        if (zStencilInput == NULL)
            CG_ASSERT("Error allocating z stencil input signal array.");

        if (zStencilOutput == NULL)
            CG_ASSERT("Error allocating z stencil output signal array.");

        if (zStencilState == NULL)
            CG_ASSERT("Error allocating z stencil state signal array.");

        if (fFIFOZSTState == NULL)
            CG_ASSERT("Error allocating to z stencil signal array.");

        if (cWriteInput == NULL)
            CG_ASSERT("Error allocating color write input signal array.");

        if (cWriteState == NULL)
            CG_ASSERT("Error allocating color write state signal array.");
    )

    //  Calculate the number of stamps sent to each unit.  
    stampsPerUnit = GPU_MAX(U32(1), stampsCycle / numStampUnits);
    ...
    if (unifiedModel)
    {
        ...
        GPU_ASSERT(
            if (numVShaders == 0)
                CG_ASSERT("No virtual vertex shader defined.");
            if (vshPrefix == NULL)
                CG_ASSERT("No prefixes for the Vertex Shader signals.");

            for(i = 0; i < numVShaders; i++)
                if (vshPrefix[i] == NULL)
                    CG_ASSERT("Undefined Vertex Shader prefix.");
        )
        ...
        GPU_ASSERT(
            if (vertexInput == NULL)
                CG_ASSERT("Error allocating array of vertex input signals from cmoStreamController Loader.");
            if (vertexOutput == NULL)
                CG_ASSERT("Error allocating array of vertex output signals to cmoStreamController Commit.");
            if (vertexState == NULL)
                CG_ASSERT("Error allocating array of state signals to cmoStreamController Loader.");
            if (vertexConsumer == NULL)
                CG_ASSERT("Error allocating array of state signals from cmoStreamController Commit.");
        )

        //  Create signals for with each shader.  
        for(i = 0; i < numVShaders; i++)
        {
            //  Create vertex input signal from cmoStreamController Loader.  
            vertexInput[i] = newInputSignal("ShaderCommand", 1, 1, vshPrefix[i]);

            //  Create vertex output signal to cmoStreamController Commit.  
            vertexOutput[i] = newOutputSignal("ShaderOutput", 1, fshOutLatency, vshPrefix[i]);

            //  Create state signal to cmoStreamController Loader.  
            ...
            //  Create state signal from cmoStreamController Commit.  
    ...
    GPU_ASSERT(
        if (numFShaders == 0)
            CG_ASSERT("No Fragment Shader attached.");
        if (fshPrefix == NULL)
            CG_ASSERT("No prefixes for the Fragment Shader signals.");

        for(i = 0; i < numFShaders; i++)
            if (fshPrefix[i] == NULL)
                CG_ASSERT("Undefined Fragment Shader prefix.");
    )
    ...
    GPU_ASSERT(
        if (shaderInput == NULL)
            CG_ASSERT("Error allocating array of stamp input signals to the Shader unit");
        if (shaderOutput == NULL)
            CG_ASSERT("Error allocating array of stamp output signals from the Shader unit");
        if (shaderState == NULL)
            CG_ASSERT("Error allocating array of state signals from the Shader unit");
        if (ffStateShader == NULL)
            CG_ASSERT("Error allocating array of Fragment FIFO state signals to the Shader unit");
    )
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(consumerState == NULL), "Error allocating array for the cmoStreamController Commit states.");
    //  Allocate space for the shader states.  
    shState = new ShaderState[numFShaders];

    //  Check allocation.  
    CG_ASSERT_COND(!(shState == NULL), "Error allocating array for the fragment shader states.");
    //  Allocate space for the Z Stencil states.  
    zstState = new ROPState[numStampUnits];

    //  Check allocation.  
    CG_ASSERT_COND(!(zstState == NULL), "Error allocating array for the Z Stencil Test states.");
    //  Allocate space for the Color Write states.  
    cwState = new ROPState[numStampUnits];

    //  Check allocation.  
    CG_ASSERT_COND(!(cwState == NULL), "Error allocating array for the ColorWrite states.");
    //  Check shader model.  
    if (unifiedModel)
    {
        ...
        GPU_ASSERT(
            if (inputVertexQueue == NULL)
                CG_ASSERT("Error allocating vertex input queue.");
            if (shadedVertexQueue == NULL)
                CG_ASSERT("Error allocating shaded vertex output queue.");
        )
        ...
        if (shadedSetup)
        {
            ...
            GPU_ASSERT(
                if (triangleInQueue == NULL)
                    CG_ASSERT("Error allocating triangle input queue.");
                if (triangleOutQueue == NULL)
                    CG_ASSERT("Error allocating triangle output queue.");
            )
        }
    }
    ...
    nextRast = new U32[numStampUnits];
    nextFreeRast = new U32[numStampUnits];
    rastStamps = new U32[numStampUnits];
    freeRast = new U32[numStampUnits];

    //  Check allocation.  
    GPU_ASSERT(
        if (rastQueue == NULL)
            CG_ASSERT("Error allocating rasterizer stamp queue.");
        if (nextRast == NULL)
            CG_ASSERT("Error allocating per unit next rasterizer stamp pointer array.");
        if (nextFreeRast == NULL)
            CG_ASSERT("Error allocating per unit next free rasterizer queue entry pointer array.");
        if (rastStamps == NULL)
            CG_ASSERT("Error allocating per unit rasterizer stamps counter array.");
        if (freeRast == NULL)
            CG_ASSERT("Error allocating per unit free rasterizer queue entry counter array.");
    )

    //  Allocate queue for stamps after early z test.  
    testQueue = new FragmentInput***[numStampUnits];
    nextTest = new U32[numStampUnits];
    nextFreeTest = new U32[numStampUnits];
    testStamps = new U32[numStampUnits];
    freeTest = new U32[numStampUnits];

    //  Check allocation.  
    GPU_ASSERT(
        if (testQueue == NULL)
            CG_ASSERT("Error allocating after early Z test stamp queue.");
        if (nextTest == NULL)
            CG_ASSERT("Error allocating per unit next tested stamp pointer array.");
        if (nextFreeTest == NULL)
            CG_ASSERT("Error allocating per unit next free z tested queue entry pointer array.");
        if (testStamps == NULL)
            CG_ASSERT("Error allocating per unit z tested stamps counter array.");
        if (freeTest == NULL)
            CG_ASSERT("Error allocating per unit free z tested queue entry counter array.");
    )

    //  Allocate queue for stamps interpolated in the Interpolator.  
    intQueue = new FragmentInput***[numStampUnits];
    nextInt = new U32[numStampUnits];
    nextFreeInt = new U32[numStampUnits];
    intStamps =  new U32[numStampUnits];
    freeInt = new U32[numStampUnits];
    lastStampInterpolated = new bool[numStampUnits];

    //  Check allocation.  
    GPU_ASSERT(
        if (intQueue == NULL)
            CG_ASSERT("Error allocating interpolator stamp queue.");
        if (nextInt == NULL)
            CG_ASSERT("Error allocating per unit next interpolated stamp pointer array.");
        if (nextFreeInt == NULL)
            CG_ASSERT("Error allocating per unit next free interpolated queue entry pointer array.");
        if (intStamps == NULL)
            CG_ASSERT("Error allocating per unit z interpolated stamps counter array.");
        if (freeInt == NULL)
            CG_ASSERT("Error allocating per unit free interpolated queue entry counter array.");
        if (lastStampInterpolated == NULL)
            CG_ASSERT("Error allocating per unit last stamp interpolated flag.");
    )
    ...
    nextShaded = new U32[numStampUnits];
    nextFreeSh = new U32[numStampUnits];
    shadedStamps = new U32[numStampUnits];
    freeShaded = new U32[numStampUnits];
    lastShaded = new bool[numStampUnits];

    //  Check allocation.  
    GPU_ASSERT(
        if (shQueue == NULL)
            CG_ASSERT("Error allocating stamp units shaded stamp queue.");
        if (shaded == NULL)
            CG_ASSERT("Error allocating stamp units shaded flag array.");
        if (nextShaded == NULL)
            CG_ASSERT("Error allocating stamp units next shaded stamp pointer array.");
        if (nextFreeSh == NULL)
            CG_ASSERT("Error allocating stamp units next free shaded queue pointer array.");
        if (shadedStamps == NULL)
            CG_ASSERT("Error allocating stamp units shaded stamps counter array.");
        if (freeShaded == NULL)
            CG_ASSERT("Error allocating stamp units free shaded queue counter array.");
        if (lastShaded == NULL)
            CG_ASSERT("Error allocating stamp unit shaded queues flag array marking last stamp enqueued.");
    )
    ...
    for(i = 0; i < numStampUnits; i++)
    {
        ...
        GPU_ASSERT(
            if (rastQueue[i] == NULL)
                CG_ASSERT("Error allocating rasterizer stamp queue for stamp unit.");
            if (testQueue[i] == NULL)
                CG_ASSERT("Error allocating z tested stamp queue for stamp unit.");
            if (intQueue[i] == NULL)
                CG_ASSERT("Error allocating interpolated stamp queue for stamp unit.");
            if (shQueue[i] == NULL)
                CG_ASSERT("Error allocating shader stamp queue for stamp unit.");
            if (shaded[i] == NULL)
                CG_ASSERT("Error allocating shaded flag array for shaded queue for stamp unit.");
        )
        ...
        for(j = 0; j < rastQueueSz; j++)
        {
            ...
            //  Check allocation.  
            CG_ASSERT_COND(!(rastQueue[i][j] == NULL), "Error allocating stamp for the rasterizer queue.");        }
        ...
        for(j = 0; j < testQueueSz; j++)
        {
            ...
            //  Check allocation.  
            CG_ASSERT_COND(!(testQueue[i][j] == NULL), "Error allocating stamp for the early z tested stamp queue.");        }
        ...
        for(j = 0; j < intQueueSz; j++)
        {
            ...
            //  Check allocation.  
            CG_ASSERT_COND(!(intQueue[i][j] == NULL), "Error allocating stamp for the interpolated queue.");        }
        ...
        for(j = 0; j < shQueueSz; j++)
        {
            ...
            //  Check allocation.  
            CG_ASSERT_COND(!(shQueue[i][j] == NULL), "Error allocating stamp for the shaded queue.");        }
    }

    //  Allocate shader batch queues.  
    shaderInputQ = new ShaderInput**[numFShaders];
    nextShaderInput = new U32[numFShaders];
    nextFreeShInput = new U32[numFShaders];
    numShaderInputs = new U32[numFShaders];
    numFreeShInputs = new U32[numFShaders];
    shaderOutputQ = new ShaderInput**[numFShaders];
    nextShaderOutput = new U32[numFShaders];
    nextFreeShOutput = new U32[numFShaders];
    numShaderOutputs = new U32[numFShaders];
    numFreeShOutputs = new U32[numFShaders];
    batchedShInputs = new U32[numFShaders];

    //  Check allocation.  
    GPU_ASSERT(
        if (shaderInputQ == NULL)
            CG_ASSERT("Error allocating shader input queue.");
        if (nextShaderInput == NULL)
            CG_ASSERT("Error allocating per shader next shader input pointers.");
        if (nextFreeShInput == NULL)
            CG_ASSERT("Error allocating per shader next free shader input queue entry pointers.");
        if (numShaderInputs == NULL)
            CG_ASSERT("Error allocating per shader shader inputs counters.");
        if (numFreeShInputs == NULL)
            CG_ASSERT("Error allocating per shader shader input queue free entries counters.");
        if (shaderOutputQ == NULL)
            CG_ASSERT("Error allocating shader output queue.");
        if (nextShaderOutput == NULL)
            CG_ASSERT("Error allocating per shader next shader output pointers.");
        if (nextFreeShOutput == NULL)
            CG_ASSERT("Error allocating per shader next free shader output queue entry pointers.");
        if (numShaderOutputs == NULL)
            CG_ASSERT("Error allocating per shader shader output counters.");
        if (numFreeShOutputs == NULL)
            CG_ASSERT("Error allocating per shader shader output queue free entries counters.");
        if (batchedShInputs == NULL)
            CG_ASSERT("Error allocating per shader batched shader inputs counters.");
    )
    ...
    for(i = 0; i < numFShaders; i++)
    {
        ...
        GPU_ASSERT(
            if (shaderInputQ[i] == NULL)
                CG_ASSERT("Error allocating per shader unit shader input queue.");
            if (shaderOutputQ[i] == NULL)
                CG_ASSERT("Error allocating per shader unit shader output queue.");
        )
    }
    
    //  Allocate shader stall detection state.
    lastCycleShInput = new U64[numFShaders];
    lastCycleShOutput = new U64[numFShaders];
    shaderElements = new U32[numFShaders];
    lastCycleZSTIn = new U64[numStampUnits];
    lastCycleCWIn = new U64[numStampUnits];
    
    //  Check allocation.
    GPU_ASSERT(
        if (lastCycleShInput == NULL)
            CG_ASSERT("Error allocating per shader last cycle shader input sent register.");
        if (lastCycleShOutput == NULL)
            CG_ASSERT("Error allocating per shader last cycle shader output received register.");
        if (shaderElements == NULL)
            CG_ASSERT("Error allocating per shader elements in the shader counter.");
        if (lastCycleZSTIn == NULL)
            CG_ASSERT("Error allocating per ROP unit last cycle an input was sent to ZST register.");
        if (lastCycleCWIn == NULL)
            CG_ASSERT("Error allocating per ROP unit last cycle an input was sent to CW register.");
    )
...
//  Simulation function.  
void FragmentFIFO::clock(U64 cycle)
{
    ...
    if (unifiedModel)
    {
        //  Receive state from cmoStreamController Commit.  
        for(i = 0; i < numVShaders; i++)
        {
            //  Read state from cmoStreamController Commit.  
            if (vertexConsumer[i]->read(cycle, (DynamicObject *&) consumerStateInfo))
            {
                //  Get state.  
                consumerState[i] = consumerStateInfo->getState();

                //  Process last vertex sent to primitive assembly from cmoStreamController Commit.  
...
//printf("FFIFO: First vertex input sent signal received in cycle %d\n", cycle);
            ...
            {
                sprintf(buffer, "State signal from cmoStreamController Commit consumer state signal %d lost data.", i);
                CG_ASSERT(buffer);
            }
        }
    }
    ...
    for (i = 0; i < numStampUnits; i++)
    {
        ...
        {
            CG_ASSERT("Missing state signal from Z Stencil Test.");
        }
    }
    ...
    for (i = 0; i < numStampUnits; i++)
    {
        ...
        {
            CG_ASSERT("Missing state signal from Color Write.");
        }
    }
...
switch(state)
{
    ...
    switch(state)
    {
        ...
        case RAST_READY:
            ...
            if (unifiedModel)
            {
                //  Send vertices to cmoStreamController Commit.  
                sendVertices(cycle);
                ...
                //  Receive input vertices from cmoStreamController Loader.  
        ...
        default:
            CG_ASSERT("Unsupported FragmentFIFO state.");
            break;
    }
    ...
    if (unifiedModel)
    {
        //  Send state to cmoStreamController Loader.  
        for(i = 0; i < numVShaders; i++)
        {
...
void FragmentFIFO::processCommand(RasterizerCommand *command, U64 cycle)
{
    U32 i;
    U32 j;
    ...
    switch(command->getCommand())
    {
        ...
        case RSCOM_DRAW:
            ...
            interpCycles = (U32) ceil((double) activeAttributes / (double) interpolators);
...
void FragmentFIFO::processRegisterWrite(GPURegister reg, U32 subreg, GPURegData data)
{
    //  Process register write.  
    switch(reg)
    {
...
void FragmentFIFO::receiveVertices(U64 cycle)
{
    U32 i;
    ShaderInput *shInput;
    U32 lastVertexReceived = false;

    //  Receive shader inputs from cmoStreamController Loader.  
    for(i = 0; i < numVShaders; i++)
    {
        //  Check if there is an input from cmoStreamController Loader.  
        if (vertexInput[i]->read(cycle, (DynamicObject *&) shInput))
        {
            //  Check there are free vertex input queue entries.  
            GPU_ASSERT(
                if (freeInputs == 0)
                    CG_ASSERT("No free vertex input queue entry.");

                if (lastVertexCommit)
                    CG_ASSERT("Vertex received after last vertex commit.");
            )
...
void FragmentFIFO::receiveTriangles(U64 cycle)
{
    ShaderInput *shInput;
    ...
    while (triangleInput->read(cycle, (DynamicObject *&) shInput))
    {
        ...
        CG_ASSERT_COND(!(freeInputTriangles == 0), "No free triangle input queue entry.");
        GPU_DEBUG_BOX(
...
void FragmentFIFO::receiveStamps(U64 cycle)
{
    ...
    for(i = 0; i < hzStampsCycle; i++)
    {
        ...
        if (receivedFragment)
        {
            CG_ASSERT_COND(!(lastFragment), "Stamp received after last stamp.");
            if (!lastStamp)
            {
                CG_ASSERT_COND(!(freeRast[unit] == 0), "Not enough room in the rasterizer queue for incoming stamp.");
                GPU_DEBUG_BOX(
            ...
            else
            {
                for(k = 0; k < numStampUnits; k++)
                {
                    CG_ASSERT_COND(!(freeRast[k] == 0), "Not enough room in the rasterizer queue for incoming stamp.");
                    GPU_DEBUG_BOX(
...
void FragmentFIFO::endInterpolation(U64 cycle)
{
    FragmentInput *frInput;
    bool receivedFragment;
    U32 i;
        ...
        for(j = 0; (j < STAMP_FRAGMENTS) && receivedFragment; j++)
        {
            ...
            if (receivedFragment)
            {
                ...
                else
                {
                    ...
                    CG_ASSERT_COND(!((j == 0) && (lastStampInterpolated[unit])), "Stamp received from interpolator after last stamp.");                }
...
void FragmentFIFO::startInterpolation(U64 cycle)
{
    bool sentStamp;
    U32 visited;
    U32 i;
...
void FragmentFIFO::startInterpolationEarlyZ(U64 cycle)
{
    bool sentStamp;
    U32 visited;
    U32 i;
...
void FragmentFIFO::startEarlyZ(U64 cycle)
{
    U32 unit;
    U32 i;
    U32 j;
...
void FragmentFIFO::endEarlyZ(U64 cycle)
{
    FragmentInput *frInput;
    bool receivedFragment;
    bool cullStamp;
    ...
    for(unit = 0; unit < numStampUnits; unit++)
    {
        ...
        for(i = 0;  i < stampsPerUnit; i++)
        {
            ...
            if (receivedFragment)
            {
                ...
                {
                    CG_ASSERT_COND(!(freeTest == 0), "Early Z tested stamp queue is full.");
                    GPU_DEBUG_BOX(
                ...
                else
                {
                    ...
                    {
                        CG_ASSERT_COND(!(freeTest[unit] == 0), "Early Z tested stamp queue is full.");
                        GPU_DEBUG_BOX(
...
//  Shades stamps.  
void FragmentFIFO::shadeStamps(U64 cycle)
{
    U32 i;
    U32 j;
    U32 k;
    ...
    bool lastStamp;
    bool addPadding;
    Vec4FP32 *lastStampAttribs[4] = { NULL, NULL, NULL, NULL };
    Vec4FP32 *paddingAttributes;
        ...
        (freeShaded[nextIntUnit] >= stampsPerGroup) && (numFreeShInputs[nextFreeShInQ] >= threadGroup);)
    {
        ...
        //  Check unit.  
        CG_ASSERT_COND(!(unit != nextIntUnit), "Trying to shade a stamp from a different unit.");
            ...
            ((!lastStamp) || addPadding); j++)
        {
            ...
            for(k = 0; k < STAMP_FRAGMENTS; k++)
            {
                ...
                if (addPadding)
                {
                    ...
                    paddingAttributes = new Vec4FP32[MAX_FRAGMENT_ATTRIBUTES];

                    if (lastStampAttribs[k] != NULL)
                    {
                        //  Copy attribute values from previous stamp.  
                        memcpy(paddingAttributes, lastStampAttribs[k], sizeof(Vec4FP32) * MAX_FRAGMENT_ATTRIBUTES);
                    }
                    else
                    {
                        memset(paddingAttributes, 0, sizeof(Vec4FP32) * MAX_FRAGMENT_ATTRIBUTES);
                    }
...
GPU_TEX_TRACE(
    for(int ii = 0; ii < 2; ii++)
        fragTrace << U32(paddingAttributes[POSITION_ATTRIBUTE][ii]) << " ";
...
GPU_TEX_TRACE(
    ...
    for(int ii = 0; ii < 2; ii++)
        fragTrace << U32(attr[POSITION_ATTRIBUTE][ii]) << " ";
    fragTrace << intQueue[nextIntUnit][nextInt[nextIntUnit]][k]->isCulled() << " " << nextFreeShInQ << endl;
                    ...
                    U32 fragTriangleID = intQueue[nextIntUnit][nextInt[nextIntUnit]][k]->getTriangleID();
                    U32 fragX = intQueue[nextIntUnit][nextInt[nextIntUnit]][k]->getFragment()->getX();
                    U32 fragY = intQueue[nextIntUnit][nextInt[nextIntUnit]][k]->getFragment()->getY();
        ...
        if (lastStamp)
        {
            ...
            CG_ASSERT_COND(!(lastShaded[nextIntUnit]), "Trying to queue again the last stamp into the shaded stamp queue.");
...
void FragmentFIFO::receiveShaderOutput(U64 cycle)
{
    U32 i;
    U32 j;
    ...
    for(i = 0; i < numFShaders; i++)
    {
        ...
        //{
            while(shaderOutput[i]->read(cycle, (DynamicObject *&) shOutput))
            {
                ...
                //        CG_ASSERT("Non fragment shader output for non unified shader architecture.");
                //    if ((j > 0) && (outputMode != shOutput->getInputMode()))
                //        CG_ASSERT("Shader output type changed.");
                //)
        ...
        //        else
        //        {
        //            CG_ASSERT("Missing shaded output in group from shader.");
        //        }
        //    }
        //}
...
printf("FF %lld => numShaderOutput[%d] = %d | shOutputQSz %d \n", cycle, i,   numShaderOutputs[i], shOutputQSize);
                    CG_ASSERT("Shader output queue overflow.");
                }
            )
...
void FragmentFIFO::distributeShaderOutput(U64 cycle)
{
    ShaderInput *shOutput;
    ShaderInputMode outputMode;
    U32 shadedEntry;
                            ...
                            GPU_ASSERT(
                                if (j == 0)
                                {
                                    ...
                                    if ((shadedEntry != shOutput->getEntry()) || (shadedUnit != shOutput->getUnit()))
                                        CG_ASSERT("Fragments outputs for the current stamp come from different stamps units.");
                                if (outputMode != SHIM_FRAGMENT)
                                    CG_ASSERT("Non fragment shader output in the middle of a fragment stamp. ");
                            )
...
{
Vec4FP32 *attrib = shOutput->getAttributes();
...
//}
                                ...
                                shQueue[shadedUnit][shadedEntry][j]->setShaderLatency(
                                    (U32) (cycle - shQueue[shadedUnit][shadedEntry][j]->getStartCycleShader())
                                    );
                            }
                ...
                case SHIM_TRIANGLE:
                    ...
                    {
                        //  Check for triangle outputs used for wave padding.  
                        if (shOutput->getKill())
                        {
                            GPU_DEBUG_BOX(
                ...
                default:
                    ...
                    CG_ASSERT("Undefined shader output mode.");
                    break;
            }
        }
    }
...
void FragmentFIFO::sendShaderInputs(U64 cycle)
{
    U32 i;
    U32 j;
    U32 k;
    ...
    for(i = 0; i < numFShaders; i++)
    {
        CG_ASSERT_COND(!(numShaderInputs[i] > shInputQSize), "Shader input queue overflow.");
...
//  Shades triangles.  
void FragmentFIFO::shadeTriangles(U64 cycle)
{
    U32 i;
    ShaderInput *shInput;
    ...
    if ((inputTriangles < threadGroup) && (freeInputTriangles >= (threadGroup - inputTriangles)))
    {
        ...
        CG_ASSERT_COND(!(!lastTriangle), "Only last group of triangle inputs can be uncomplete.");
        ...
        for(i = inputTriangles; i < threadGroup; i++)
        {
            ...
            shInput = new ShaderInput(0, 0, 0, new Vec4FP32[MAX_TRIANGLE_ATTRIBUTES], SHIM_TRIANGLE);
...
//  Shades vertices.  
void FragmentFIFO::shadeVertices(U64 cycle)
{
    U32 i;
    ShaderInput *shInput;
    ...
    {   
        ...
        for(i = vertexInputs; i < threadGroup; i++)
        {
            ...
            //  Create fake shader input.  
            shInput = new ShaderInput(0, 0, 0, new Vec4FP32[MAX_VERTEX_ATTRIBUTES]);
...
void FragmentFIFO::sendStampsZStencil(U64 cycle)
{
    U32 i;
    U32 j;
    U32 unit;
...
void FragmentFIFO::sendStampsColorWrite(U64 cycle)
{
    U32 i;
    U32 j;
    U32 unit;
...
void FragmentFIFO::sendTestedStampsColorWrite(U64 cycle)
{
    U32 i;
    U32 j;
    U32 unit;
...
//  Send triangles to Triangle Setup.  
void FragmentFIFO::sendTriangles(U64 cycle)
{
    U32 i;
...
//  Send vertices to cmoStreamController Commit.  
void FragmentFIFO::sendVertices(U64 cycle)
{
    U32 i;
...
void FragmentFIFO::detectStall(U64 cycle, bool &active, bool &stalled)
{
    ...
    for(U32 sh = 0; (sh < numFShaders) && shaderUnitsStalled; sh++)
    {
    ...
    for(U32 rop = 0; (rop < numStampUnits) && ropUnitStalled; rop++)
    {
...
void FragmentFIFO::stallReport(U64 cycle, string &stallReport)
{
    ...
    for(U32 sh = 0; sh < numFShaders; sh++)
    {
    ...
    for(U32 rop = 0; rop < numStampUnits; rop++)
    {
        if (earlyZ)
        {
