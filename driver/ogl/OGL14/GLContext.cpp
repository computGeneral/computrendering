...
GLContext::GLContext(HAL* driver, bool triangleSetupOnShader) :
    driver(driver),
...
void GLContext::setCurrentMatrix(GLenum mt)
{
    switch ( mt )
    {
        ...
        case GL_TEXTURE:
            ...
            break;
        default:
            CG_ASSERT("GLenum Matrix not supported");
    }
}
...
void GLContext::mpush(const Matrixf& newTop, MatrixType mt)
{
    ...
    switch ( mt )
    {
        ...
        default:
            CG_ASSERT("Invalid current matrix");
    }
...
void GLContext::mpop(MatrixType mt)
{
    ...
    if ( m.empty() )
        CG_ASSERT("Trying to pop an empty matrix stack");
...
const Matrixf& GLContext::mtop(MatrixType mt)
{
    std::stack<Matrixf>& m = mstack[mt];
    if ( m.empty() )
        CG_ASSERT("Trying to get the top of an empty matrix stack");
...
{
    if ( unit >= textureUnits.size() )
        CG_ASSERT("Texture unit not available");
...
void GLContext::clearBuffers()
{
    ...
    for ( U32 i = 0; i < countTextureUnits(); i++ )
        texbuf(i).clear();
...
{
    if ( unit >= textureUnits.size() )
        CG_ASSERT("Texture unit is not available");
...
void GLContext::setParamLight(GLenum light_, GLenum pname, const float* params)
{
    ...
    switch ( pname )
    {
        ...
        default:
            CG_ASSERT("Pname value not supported");
    }
}
...
void GLContext::setMaterial(GLenum face, GLenum pname, const float* params)
{
    ...
    if ( face != GL_FRONT && face != GL_BACK && face != GL_FRONT_AND_BACK )
        CG_ASSERT("GL_FRONT, GL_BACK or GL_FRONT_AND_BACK required to select a 'face'");
    ...
    switch ( pname ) // CHECK ERRORS 
    {
        ...
        default:
            CG_ASSERT("Unknown parameter name");
    }
...
void GLContext::setLightModel(GLenum pname, const float* params)
{
    ...
    else if ( pname == GL_LIGHT_MODEL_TWO_SIDE )
    {
        ...
        CG_ASSERT("Unsupported property value");
}
...
void GLContext::setLightModel(GLenum pname, const GLint* params)
{
    ...
    else if ( pname == GL_LIGHT_MODEL_COLOR_CONTROL )
    {
        if ( params[0] == GL_SEPARATE_SPECULAR_COLOR )
            ...
            CG_ASSERT("GL_LIGHT_MODEL_COLOR_CONTROL unsupported value");
    }
    else
        CG_ASSERT("Unsupported property value");
}
        ...
        case GL_OBJECT_PLANE:
            {
                ...
                if ( coord == GL_S )
                    ...
                    CG_ASSERT("Unexpected object plane coordinate");
            }
            break;
        case GL_EYE_PLANE:
            {
                ...
                if ( coord == GL_S )
                    ...
                    CG_ASSERT("Unexpected eye plane coordinate");
            }
            break;
        default:
            CG_ASSERT("Unsuported parameter name");
...
void GLContext::setFog(GLenum pname, const GLint* params)
{
    ...
    else if ( pname == GL_FOG_COORDINATE_SOURCE )
    {
        if ( value != GL_FRAGMENT_DEPTH && value != GL_FOG_COORD )
            CG_ASSERT("unexpected GL_FOG_COORD_SOURCE value");
...
void GLContext::setAlphaTest(GLenum func, GLclampf ref)
{
    switch ( func )
    {
        ...
        case GL_NOTEQUAL:
            ...
            break;
        default:
            CG_ASSERT("Unexpected alpha function");
    }
}
...
void GLContext::setCullFace(GLenum face)
{
    switch ( face )
    {
        ...
        case GL_FRONT_AND_BACK:
            ...
            break;
        default:
            CG_ASSERT("Unexpected culling face");
    }
}
...
ProgramObject& GLContext::getCurrentVertexProgram()
{
    ...
    if ( po == 0 )
        CG_ASSERT("Automatically generated vertex program not found in cache!");
    return *po;
}
...
ProgramObject& GLContext::getCurrentFragmentProgram()
{
    ...
    if ( po == 0 )
        CG_ASSERT("Automatically generated fragment program not found in cache!");
    return *po;
}
...
TLState::ColorMaterialMode TLSImp::colorMaterialMode()
{
    switch(ctx->colorMaterialMode)
    {
        ...
        case GL_SPECULAR:
        case GL_AMBIENT_AND_DIFFUSE:
            CG_ASSERT("Color material mode not implemented yet");
    }
    CG_ASSERT("Color material mode unknown");
...
FPState::TextureUnit FPSImp::getTextureUnit(GLuint unit)
{
    ...
    if ( !to )
        CG_ASSERT("Texture object not defined in this Texture unit!");
    ...
    switch ( to->getBaseInternalFormat() )
    {
        ...
        case GL_RGBA:
            ...
            break;
        default:
            CG_ASSERT("Unknown base internal format");
    }
    ...
    if ( tu.function == FPState::TextureUnit::COMBINE || tu.function == FPState::TextureUnit::COMBINE4_NV )
    {
        switch ( tuConf.getRGBCombinerFunction() )
        {
            ...
            default:
                CG_ASSERT("Unknown RGB Combiner function");
        }
        ...
        switch ( tuConf.getAlphaCombinerFunction() )
        {
            ...
            default:
                CG_ASSERT("Unknown Alpha Combiner function");
        }
