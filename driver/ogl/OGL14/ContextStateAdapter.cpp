...
ContextStateAdapter::ContextStateAdapter(HAL* driver)
...
#define PANIC_WITH_CALLER(cName , fName, Message) {\
                                                    char msg[80];\
                                                    sprintf(msg,"%s called by %s OGL14 module",fName, functionCallerName.c_str());\
...
#define CASE_BRANCH_FILTER(x) case GL_##x: data.txFilter = GPU_##x; break;
    ...
    data.uintVal = U32(maxAnisotropy);
...
void ContextStateAdapter::setDepthClearValue(GLclampd clearValue)
{
    ...
    data.uintVal = U32((F32(clearValue) * F32((1 << 24) - 1)));
...
void ContextStateAdapter::setCurrentColor( GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha )
{
    ...
    data.qfVal[2] = blue;
    data.qfVal[3] = alpha;
    driver->writeGPURegister(GPU_VERTEX_ATTRIBUTE_DEFAULT_VALUE, (U32)HAL::VS_COLOR, data); 
}
