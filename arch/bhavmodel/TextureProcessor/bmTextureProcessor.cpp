/**************************************************************************
 *
 * Texture Behavior Model class implementation file.
 *
 */

/**
 *
 *  @file bmoTextureProcessor.cpp
 *
 *  This file implements Texture Behavior Model Operation class.
 *
...
bmoTextureProcessor::bmoTextureProcessor(U32 stampFrags, U32 blockDim, U32 superBlockDim, U32 anisoAlg,
                    bool forceAniso, U32 maxAniso, U32 triPrecision, U32 briThreshold,
                    U32 anisoRoundPrec, U32 _anisoRoundThreshold, bool _anisoRatioMultOfTwo,
                    ...
                    U32 genWidth, U32 genHeight)
{
    ...
    GPU_ASSERT(
        if ((trilinearPrecision == 0) || (trilinearPrecision > 32))
            CG_ASSERT("Trilinear precision valid range is [1, 32].");
        if (brilinearThreshold > U32(1 << (trilinearPrecision - 1)))
            CG_ASSERT("Brilinear threshold must be less than the 2^(precision - 1).");
        if (maxAniso > 16)
            CG_ASSERT("Maximum anisotropy supported is 16.");            
    )
        
    trilinearRangeMin = F32(brilinearThreshold) / F32(U64(1) << trilinearPrecision );
    ...
    GPU_ASSERT(
        if ((anisoRoundPrecision == 0) || (anisoRoundPrecision > 32))
            CG_ASSERT("Aniso round precision valid range is [1, 32]");
        if (U64(anisoRoundThreshold) > (U64(1) << anisoRoundPrecision))
            CG_ASSERT("Aniso round threshold must be less than 2^prec.");
    )
    
    anisoRoundUp = F32(anisoRoundThreshold) / F32(U64(1) << anisoRoundPrecision);
    ...
    CG_ASSERT_COND(!(stampFragments != 4), "Only supported stamps with 4 fragments in a quad.");
}

//  Calculates the texel address for a stamp.  
TextureAccess *bmoTextureProcessor::textureOperation(U32 id, TextureOperation texOp, Vec4FP32 *stampCoord, F32 *stampParameter, U32 textUnit)
{
...
/*if (id == 488)
{
printf("bmoTextureProcessor::textureOperation => Texture Operation id = %d texOp = %d textUnit = %d\n", id, texOp, textUnit);
...
f, stampCoord[f][0], stampCoord[f][1], stampCoord[f][2], stampCoord[f][3], stampParameter[f]);
    ...
    Vec4FP32 origCoord[STAMP_FRAGMENTS];
...
//printf("frag %d > lod: %f  d1: %d  levelBase: %d  q: %d\n", frag, lod, d1, levelBase, q);
                ...
                case GPU_LINEAR_MIPMAP_LINEAR:
                    ...
                    //d1 = ((F32(levelBase) + lod) >= F32(q))?q:U32(GPU_FLOOR(F32(levelBase) + lod));
                    d1 = levelBase + U32(GPU_FLOOR(lod));
                    //d1 = levelBase + U32(lod);
                    d1 = (d1 <= q)?d1:q;

                    //d2 = ((F32(levelBase) + lod) >= F32(q))?q:d1 + 1;
                    d2 = (d1 < q)?d1 + 1:q;
...
//printf("frag %d > lod: %f  d1: %d d2: %d levelBase: %d  q: %d\n", frag, lod, d1, d2, levelBase, q);
                ...
                default:
                    CG_ASSERT("Unsupported filter mode.");
            }
        ...
        else
        {
            // Magnification.
            d1 = levelBase;
            d2 = levelBase; // gene, add for line 396 use uninitialized varible (TO BE CHECKED)
            //  Set filter mode.
        ...
        textAccess->texelSize[frag] = GPU_MAX(U32(4), U32(adjustToFormat(textUnit, 1)));
...
void bmoTextureProcessor::calculateAddress(TextureAccess *textAccess)
{
    Vec4FP32 *stampCoord;
    U32 textUnit;
    U32 d1, d2;
    ...
    CG_ASSERT_COND(!(textAccess->trilinear[trilinearAccess] != NULL), "Trilinear object should be NULL at this point.");
...
//printf("TU > N/N_M_N > d1 %d i %d j %d k %d\n", d1, i, j, k);
                        ...
                        width  = GPU_MAX(textureWidth[textUnit]  >> d1, U32(1));
                        height = GPU_MAX(textureHeight[textUnit] >> d1, U32(1));
                        depth  = GPU_MAX(textureDepth[textUnit]  >> d1, U32(1));
...
//printf("TU > L/L_M_N > d1 %d i %d j %d k %d\n", d1, i, j, k);
                    ...
                    case GPU_NEAREST_MIPMAP_LINEAR:
                        ...
                        fractionalLOD = textAccess->lod[frag] - static_cast<F32>(GPU_FLOOR(textAccess->lod[frag]));
...
//printf("TU > N_M_L > d2 %d i %d j %d k %d\n", d2, i, j, k);
                            ...
                            width  = GPU_MAX(textureWidth[textUnit]  >> d2, U32(1));
                            height = GPU_MAX(textureHeight[textUnit] >> d2, U32(1));
                            depth  = GPU_MAX(textureDepth[textUnit]  >> d2, U32(1));
...
//printf("TxEmu::calculateAddress => ATTRIBUTE_READ -> Reading index = %d\n", index);                    
            ...
            default:
                CG_ASSERT("Unsupported texture operation.");
                break;
        }                
    }
}
...
void bmoTextureProcessor::genBilinearTexels(U32 textUnit, TextureAccess &textAccess, U32 trilinearAccess,
    U32 frag, U32 level, U32 i, U32 j, U32 k, U32 l)
{
    ...
    switch(textureMode[textUnit])
    {
        case GPU_TEXTURE1D:
            ...
            width = GPU_MAX(textureWidth[textUnit] >> level, U32(1));
        ...
        case GPU_TEXTURECUBEMAP:
            ...
            width = GPU_MAX(textureWidth[textUnit] >> level, U32(1));
            height = GPU_MAX(textureHeight[textUnit] >> level, U32(1));
        ...
        case GPU_TEXTURE3D:
            ...
            width = GPU_MAX(textureWidth[textUnit] >> level, U32(1));
            height = GPU_MAX(textureHeight[textUnit] >> level, U32(1));
            depth = GPU_MAX(textureDepth[textUnit] >> level, U32(1));
...
/*  Selects the cubemap face accessed by the whole stamp and adjust the stamp texture coordinates to the
    selected image.  */
CubeMapFace bmoTextureProcessor::selectCubeMapFace(Vec4FP32 *coord)
{
    //CubeMapFace face[CUBEMAP_IMAGES];
    U32 face[CUBEMAP_IMAGES];
    //F32 sc, tc;
    //F32 ma;
    U32 i;
    U32 selectedFace;
    F32 rx[STAMP_FRAGMENTS];
    F32 ry[STAMP_FRAGMENTS];
    F32 rz[STAMP_FRAGMENTS];
    //U32 f;
    ...
    CG_ASSERT("Undefined cubemap face.");
...
void  bmoTextureProcessor::derivativesXY(Vec4FP32 *coordinates, U32 textUnit,
    F32 &dudx, F32 &dudy, F32 &dvdx, F32 &dvdy, F32 &dwdx, F32 &dwdy)
{
    F32 u0, v0, w0;
    F32 u1, v1, w1;
    ...
    */
    ...
    {
        //u0 = coordinates[0][0] * F32(1 << textureWidth2[textUnit]);
        //v0 = coordinates[0][1] * F32(1 << textureHeight2[textUnit]);
        //w0 = coordinates[0][2] * F32(1 << textureDepth2[textUnit]);
        u0 = coordinates[0][0] * F32(textureWidth[textUnit]);
        v0 = coordinates[0][1] * F32(textureHeight[textUnit]);
        w0 = coordinates[0][2] * F32(textureDepth[textUnit]);

        //u1 = coordinates[1][0] * F32(1 << textureWidth2[textUnit]);
        //v1 = coordinates[1][1] * F32(1 << textureHeight2[textUnit]);
        //w1 = coordinates[1][2] * F32(1 << textureDepth2[textUnit]);
        u1 = coordinates[1][0] * F32(textureWidth[textUnit]);
        v1 = coordinates[1][1] * F32(textureHeight[textUnit]);
        w1 = coordinates[1][2] * F32(textureDepth[textUnit]);
    }
    ...
    if (!textureNonNormalized[textUnit])
    {
        //u1 = coordinates[2][0] * F32(1 << textureWidth2[textUnit]);
        //v1 = coordinates[2][1] * F32(1 << textureHeight2[textUnit]);
        //w1 = coordinates[2][2] * F32(1 << textureDepth2[textUnit]);
        u1 = coordinates[2][0] * F32(textureWidth[textUnit]);
        v1 = coordinates[2][1] * F32(textureHeight[textUnit]);
        w1 = coordinates[2][2] * F32(textureDepth[textUnit]);
    }
...
F32 bmoTextureProcessor::calculateScale1D(F32 dudx, F32 dudy)
{
    F32 scale;
...
F32 bmoTextureProcessor::calculateScale2D(U32 textUnit, F32 dudx, F32 dudy, F32 dvdx, F32 dvdy, U32 maxAniso,
    U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 scale;
    ...
    if (maxAniso == 1)
    {
        ...
        //  Calculate the scale factor.  
        scale = static_cast<F32>(GPU_MAX(GPU_SQRT(dudx * dudx + dvdx * dvdx),
    ...
    else if (maxAniso > 1)
    {
        ...
        switch(anisoAlgorithm)
        {
            ...
            default:
                CG_ASSERT("Undefined anisotropy algorithm");
                break;
        }

        //  Normalize to s,t space : [0..1].  
        //dsOffset = dsOffset / F32(1 << textureWidth2[textUnit]);
        //dtOffset = dtOffset / F32(1 << textureHeight2[textUnit]);
        dsOffset = dsOffset / F32(textureWidth[textUnit]);
        dtOffset = dtOffset / F32(textureHeight[textUnit]);

//printf("Scale2D >> scale %f samples %d dsOffset %f dtOffset %f\n", scale, samples, dsOffset, dtOffset);
    }
    else
    {
        CG_ASSERT("Maximum anisotropy must be at least 1.");
    }
...
U32 bmoTextureProcessor::computeAnisoSamples(F32 anisoRatio)
{
    U32 samples;
    ...
    if (anisoRatioMultOfTwo)
    {
        ...
        F32 diffToMult2 = 2 - (F32(((U32(GPU_FLOOR(anisoRatio)) & 0xfffffffe) + 2)) - anisoRatio);
        ...
        if (diffToMult2 > anisoRoundUp)
            samples = (U32(GPU_FLOOR(anisoRatio)) & 0xfffffffe) + 2;
        else
            samples = (U32(GPU_FLOOR(anisoRatio)) & 0xfffffffe);
    }
    else
    {
        //  Compute the fractional part of the ratio.
        F32 fracRatio = anisoRatio - F32(GPU_FLOOR(anisoRatio));       
        ...
        if (fracRatio > anisoRoundUp)
            samples = U32(CG_CEIL(anisoRatio));
        else
            samples = U32(GPU_FLOOR(anisoRatio));        
    }

    return GPU_MAX(samples, U32(1));
}

//  Computes the anisotropy based on the screen aligned axis
F32 bmoTextureProcessor::anisoTwoAxis(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy,
    U32 maxAniso, U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 scale;
    F32 px;
    F32 py;
    ...
    px = (F32) GPU_SQRT(dudx * dudx + dvdx * dvdx);
    py = (F32) GPU_SQRT(dudy * dudy + dvdy * dvdy);
    ...
    N = GPU_MIN(pMax/pMin, F32(maxAniso));
    ...
    switch(axis)
    {
        case TextureAccess::X_AXIS:
            dsOffset = dudx / F32(samples + 1);
            dtOffset = dvdx / F32(samples + 1);
            break;
        case TextureAccess::Y_AXIS:
            dsOffset = dudy / F32(samples + 1);
            dtOffset = dvdy / F32(samples + 1);
            break;
    }
...
F32 bmoTextureProcessor::anisoFourAxis(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy,
    U32 maxAniso, U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 scale;
    F32 px;
...
//pMax * pMin, pMax2 * pMin2, N, N2);
    ...
    else
    {
        ...
        //  Calculate the number of samples required.  
        //samples = U32(CG_CEIL(N2));
        samples = computeAnisoSamples(N2);

        //  Calculate the texture scale for each sample.  
        //scale = pMax2 / F32(samples);
        scale = pMax2 / N2;
    }
...
//printf("XY ");
            dsOffset = ((dudx + dudy) / (F32) GPU_SQRT(2)) / F32(samples + 1);
            dtOffset = ((dvdx + dvdy) / (F32) GPU_SQRT(2)) / F32(samples + 1);
            break;
...
F32 bmoTextureProcessor::anisoRectangle(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy,
    U32 maxAniso, U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 scale;
    F32 pMinA;
    ...
    l1A = (F32) GPU_SQRT(dudx * dudx + dvdx * dvdx);
    l2A = (F32) GPU_SQRT(dudy * dudy + dvdy * dvdy);

    //  Calculate diagonals of the x and y vectors in texture space (45 degree rotation)
    diag1[0] = (dudx + dudy) / (F32) GPU_SQRT(2);
    diag1[1] = (dvdx + dvdy) / (F32) GPU_SQRT(2);
    diag2[0] = (dudx - dudy) / (F32) GPU_SQRT(2);
    diag2[1] = (dvdx - dvdy) / (F32) GPU_SQRT(2);

    //  Calculate lengths of the 45 degree rotated x and y vectors in texture space (diagonals)
    l1B = (F32) GPU_SQRT(diag1[0] * diag1[0] + diag1[1] * diag1[1]);
    l2B = (F32) GPU_SQRT(diag2[0] * diag2[0] + diag2[1] * diag2[1]);
    ...
    NA = GPU_MIN(pMaxA/pMinA, F32(maxAniso));

    //  Calculate ratio between the two anisotropy axis.  
    NB = GPU_MIN(pMaxB/pMinB, F32(maxAniso));
...
//pMaxA * pMinA, pMaxB * pMinB, NA, NB, ratioA, ratioB);
    ...
    if (ratioA >= ratioB)
    {
       //  Calculate the number of samples required.  
        ...
        //  Calculate the per anisotropic sample offsets in s,t space.  
        dsOffset = (axis1A[0] * lA) / F32(samples + 1);
        dtOffset = (axis1A[1] * lA) / F32(samples + 1);
    }
    else
    {
        //  Calculate the number of samples required.  
        ...
        //  Calculate the per anisotropic sample offsets in s,t space.  
        dsOffset = (axis1B[0] * lB) / F32(samples + 1);
        dtOffset = (axis1B[1] * lB) / F32(samples + 1);
    }
...
F32 bmoTextureProcessor::anisoEWA(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy,
    U32 maxAniso, U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 A;
    F32 B;
...
*/
    ...
    p = A - C;
    q = A + C;
    t = GPU_SIGN(p) * (F32) GPU_SQRT(p * p + B * B);
...
//printf("EWA >> p %f q %f t %f\n", p, q, t);
    ...
    if (t == 0.0f)
    {
        axis1[0] = 1.0f / (F32) GPU_SQRT(A);
        axis1[1] = 0.0f;
        axis2[0] = 0.0f;
        axis2[1] = 1.0f / (F32) GPU_SQRT(A);
    }
    else
    {
        //  Calculate the major and minor axis of the ellipse
        axis1[0] = (F32) GPU_SQRT((t + p) / (t * (q + t)));
        axis1[1] = GPU_SIGN(B * p) * (F32) GPU_SQRT((t - p) / (t * (q + t)));

        axis2[0] = -1.0f * GPU_SIGN(B * p) * (F32) GPU_SQRT((t - p) / (t * (q - t)));
        axis2[1] = (F32) GPU_SQRT((t + p) / (t * (q - t)));
    }
    ...
    /*GPU_ASSERT(
        if (isnan(axis1[0]) || isnan(axis1[1]) || isnan(axis2[0]) || isnan(axis2[1]))
        {
            ...
            CG_ASSERT("Computed axis coordinate is NaN");
        }
    )*/

    //  Compute the lenght of both vectors
    l1 = (F32) GPU_SQRT(axis1[0] * axis1[0] + axis1[1] * axis1[1]);
    l2 = (F32) GPU_SQRT(axis2[0] * axis2[0] + axis2[1] * axis2[1]);
...
//printf("EWA => Minor axis (%f, %f)\n", axis2[0], axis2[1]);
        ...
        //  Calculate the per anisotropic sample offsets in s,t space.
        dsOffset = axis1[0] / F32(samples + 1);
        dtOffset = axis1[1] / F32(samples + 1);
    }
    else
    {
//printf("EWA => Major axis (%f, %f)\n", axis2[0], axis2[1]);
//printf("EWA => Minor axis (%f, %f)\n", axis1[0], axis1[1]);
        ...
        //  Calculate the per anisotropic sample offsets in s,t space.
        dsOffset = axis2[0] / F32(samples + 1);
        dtOffset = axis2[1] / F32(samples + 1);
    }
...
F32 bmoTextureProcessor::anisoExperimentalAngle(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy, U32 maxAniso,
    U32 &samples, F32 &dsOffset, F32 &dtOffset)
{
    F32 scale;
    F32 scale1;
    ...
    F32 pMax2;
    F32 N2;
    F32 adjustedMajorLength1;
    F32 adjustedMinorLength1;
    F32 adjustedMajorLength2;
    F32 adjustedMinorLength2;
    F32 newN1;
    F32 newN2;
    F32 factor;
    F32 vectorA[2];
...
//printf("CG1_ISA_OPCODE_EXP-Angle >> (X/Y) adj Major %f adj Minor %f new ratio %f\n", adjustedMajorLength1, adjustedMinorLength1, N);
    ...
    vectorA[1] = (dvdx + dvdy) / (F32) GPU_SQRT(2.0f);
    vectorB[0] = (dudx - dudy) / (F32) GPU_SQRT(2.0f);
    vectorB[1] = (dvdx - dvdy) / (F32) GPU_SQRT(2.0f);
...
F32 bmoTextureProcessor::calculateScale3D(F32 dudx, F32 dudy, F32 dvdx, F32 dvdy, F32 dwdx, F32 dwdy)
{
    F32 scale;

    //  Calculate the scale factor.  
    scale = static_cast<F32>(GPU_MAX(GPU_SQRT(dudx * dudx + dvdx * dvdx + dwdx * dwdx),
...
F32 bmoTextureProcessor::calculateLOD(U32 textUnit, F32 fragmentBias, F32 scaleFactor)
{
    F32 lod;

    //  Calculate level of detail.  
    lod = static_cast<F32>(CG_LOG2(scaleFactor) + GPU_CLAMP(textureLODBias[textUnit] + textureUnitLODBias[textUnit]
...
void bmoTextureProcessor::texelCoord(U32 textUnit, U32 l, FilterMode filter, U32 currentSample,
    U32 numSamples, F32 dsOffset, F32 dtOffset, F32 s, F32 t, F32 r,
...
void bmoTextureProcessor::texelCoord1D(U32 textUnit, U32 level, FilterMode filter, F32 s, U32 &i, F32 &a)
{
    F32 u;
    U32 width;
    ...
    if (filter == GPU_LINEAR)
    {
        ...
        //  Calculate texel coordinates.
        i = (U32) GPU_FLOOR(u - 0.5f);

        //  Calculate weight factors for bilinear filtering.
        a = (u - 0.5f) - static_cast<F32>(GPU_FLOOR(u - 0.5f));
    }
}

//  Calculates texel cooordinates for a mipmap level of a GPU_TEXTURE2D texture.
void bmoTextureProcessor::texelCoord2D(U32 textUnit, U32 level, FilterMode filter,
    U32 currentSample, U32 numSamples, F32 dsOffset, F32 dtOffset, F32 s, F32 t,
    U32 &i, U32 &j, F32 &a, F32 &b)
{
    F32 u;
    F32 v;
    U32 width;
    ...
    if (maxAnisotropy[textUnit] > 1)
    {
        GPU_ASSERT(
            if (textureNonNormalized[textUnit])
                printf("bmoTextureProcessor", "texelCoord", "Anistropic filtering not implemented with non-normalized coordinates.");
        )
        
        //  Apply anisotropic sample offset.
        s = s + dsOffset * (currentSample - 0.5f * F32(numSamples + 1));
        t = t + dtOffset * (currentSample - 0.5f * F32(numSamples + 1));
    }
    ...
    switch(filter)
    {
        ...
        case GPU_LINEAR:
            ...
            //  Calculate texel coordinates.
            i = (U32) GPU_FLOOR(u - 0.5f);
            j = (U32) GPU_FLOOR(v - 0.5f);

            //  Calculate weight factors for bilinear filtering.
            a = (u - 0.5f) - static_cast<F32>(GPU_FLOOR(u - 0.5f));
            b = (v - 0.5f) - static_cast<F32>(GPU_FLOOR(v - 0.5f));
...
void bmoTextureProcessor::texelCoord3D(U32 textUnit, U32 level, FilterMode filter,
    F32 s, F32 t, F32 r, U32 &i, U32 &j, U32 &k, F32 &a, F32 &b, F32 &c)
{
    ...
    if (filter == GPU_LINEAR)
    {
        ...
        i = (U32) GPU_FLOOR(u - 0.5f);
        j = (U32) GPU_FLOOR(v - 0.5f);
        k = (U32) GPU_FLOOR(w - 0.5f);

        //  Calculate weight factors for bilinear filtering.
        a = (u - 0.5f) - static_cast<F32>(GPU_FLOOR(u - 0.5f));
        b = (v - 0.5f) - static_cast<F32>(GPU_FLOOR(v - 0.5f));
        c = (w - 0.5f) - static_cast<F32>(GPU_FLOOR(w - 0.5f));
    }
}

//  Applies a texture wrapping mode to a texture coordinate (texture normalized coordinate mode).
F32 bmoTextureProcessor::applyWrap(ClampMode wrapMode, F32 a, U32 size)
{
    F32 w;
    F32 min;
    F32 max;
    ...
    switch(wrapMode)
    {
        ...
        case GPU_TEXT_MIRRORED_REPEAT:
            ...
            w = (((U32) GPU_FLOOR(a)) & 0x01)?(1.0f - (a - (F32) GPU_FLOOR(a))):(a - (F32) GPU_FLOOR(a));
            w = GPU_CLAMP(w, min, max);
            break;

        default:

            CG_ASSERT("Unsupported texture wrap mode.");
            break;
    }
...
U32 bmoTextureProcessor::applyWrap(ClampMode wrapMode, U32 i, U32 size)
{
    U32 w;
...
void bmoTextureProcessor::filter(TextureAccess &textAccess, U32 nextTrilinearFilter)
{
    ...
    if (textAccess.texOperation == ATTRIBUTE_READ)
    {
        ...
        U32 stream = attributeMap[textAccess.textUnit];
        ...
        if (stream == ST_INACTIVE_ATTRIBUTE)
        {
            ...
            for(U32 frag = 0; frag < stampFragments; frag++)
            {
...
//printf("TU => combining filtered values\n");
        ...
        for(U32 frag = 0; frag < stampFragments; frag++)
        {
            ...
            for(U32 subSample = 0; subSample < textAccess.anisoSamples; subSample++)
            {
            ...
            //  Apply weight.
            textAccess.sample[frag][0] = textAccess.sample[frag][0] / F32(textAccess.anisoSamples);
            textAccess.sample[frag][1] = textAccess.sample[frag][1] / F32(textAccess.anisoSamples);
            textAccess.sample[frag][2] = textAccess.sample[frag][2] / F32(textAccess.anisoSamples);
            textAccess.sample[frag][3] = textAccess.sample[frag][3] / F32(textAccess.anisoSamples);
        }

/*if (textAccess.accessID == 488)
{
printf("bmoTextureProcessor::filter => AnisoSamples %d\n", textAccess.anisoSamples);
for(U32 f = 0; f < stampFragments; f++)
{
printf("Pixel %02d\n", f);
for(U32 s = 0; s < textAccess.anisoSamples; s++)
{
...
void bmoTextureProcessor::filterTrilinear(TextureAccess &textAccess, U32 trilinearAccess)
{
    U32 i;
    Vec4FP32 t1, t2;
    F32 w;

    CG_ASSERT_COND(!(textAccess.trilinearToFilter == 0), "Requesting to filter more trilinear samples than those required.");
        ...
        switch(textAccess.filter[i])
        {
            ...
            case GPU_NEAREST_MIPMAP_LINEAR:
                ...
                if (textAccess.trilinear[trilinearAccess]->sampleFromTwoMips[i])
                {
                    ...
                    w = textAccess.lod[i] - static_cast<F32>(GPU_FLOOR(textAccess.lod[i]));
...
Vec4FP32 bmoTextureProcessor::bilinearFilter(TextureAccess &textAccess, U32 trilinearAccess, U32 level, U32 frag)
{
    Vec4FP32 sample;
    F32 a, b, c;
...
U64 bmoTextureProcessor::texel2address(U32 textUnit, U32 level, U32 i)
{
    U64 address;

    //  Set black texel address for texels outside the texture.  
    //if (i >= U32(1) << mipmapSize(textureWidth2[textUnit], level))
    if (i >= U32(GPU_MAX(textureWidth[textUnit] >> level, U32(1))))
...
U64 bmoTextureProcessor::texel2address(U32 textUnit, U32 level, U32 cubemap, U32 i, U32 j)
{
    U64 address;
    U64 texelAddr;
    U64 blockAddr;
    ...
    U32 frameBufferTexelAddr;    
  
    //  Set black texel address for texels outside the texture.
    //if ((i >= U32(1) << mipmapSize(textureWidth2[textUnit], level))) ||
    //    (j >= U32(1) << mipmapSize(textureHeight2[textUnit], level))))
    if ((i >= U32(GPU_MAX(textureWidth[textUnit] >> level, U32(1)))) |
        (j >= U32(GPU_MAX(textureHeight[textUnit] >> level, U32(1)))))
        return BLACK_TEXEL_ADDRESS;
    ...
    switch(textureCompr[textUnit])
    {
        ...
        case GPU_S3TC_DXT1_RGB:
            ...
            address = (U64(textureAddress[textUnit][level][cubemap]) << DXT1_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_DXT1_RGB;

            break;
        case GPU_S3TC_DXT1_RGBA:

            //  Use compressed texture address space for ratio 1:8.
            address = (U64(textureAddress[textUnit][level][cubemap]) << DXT1_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_DXT1_RGBA;
        ...
        case GPU_S3TC_DXT3_RGBA:
            ...
            address = (U64(textureAddress[textUnit][level][cubemap]) << DXT3_DXT5_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_DXT3_RGBA;
        ...
        case GPU_S3TC_DXT5_RGBA:
            ...
            //  Use compressed texture address space for ratio 1:4.
            address = (U64(textureAddress[textUnit][level][cubemap]) << DXT3_DXT5_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_DXT5_RGBA;
        ...
        case GPU_LATC1:
            ...
            address = (U64(textureAddress[textUnit][level][cubemap]) << LATC1_LATC2_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_LATC1;
        ...
        case GPU_LATC1_SIGNED:
            ...
            //  Use compressed texture address space for ratio 1:2.
            address = (U64(textureAddress[textUnit][level][cubemap]) << LATC1_LATC2_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_LATC1_SIGNED;
        ...
        case GPU_LATC2:
            ...
            //  Use compressed texture address space for ratio 1:2.
            address = (U64(textureAddress[textUnit][level][cubemap]) << LATC1_LATC2_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_LATC2;
        ...
        case GPU_LATC2_SIGNED:
            ...
            //  Use compressed texture address space for ratio 1:2.
            address = (U64(textureAddress[textUnit][level][cubemap]) << LATC1_LATC2_SPACE_SHIFT) + COMPRESSED_TEXTURE_SPACE_LATC2_SIGNED;
            
            break;            
        
        default:

            CG_ASSERT("Undefined texture compression mode.");
            break;
    }
    ...
    switch(textureBlocking[textUnit])
    {
        case GPU_TXBLOCK_TEXTURE:
            ...
            mipmapWidthLog2 = U32(CG_CEIL(CG_LOG2(GPU_MAX(textureWidth[textUnit] >> level, U32(1)))));
            
            //  Calculate the number of 
            sBlocksPerLine = GPU_MAX(S32(mipmapWidthLog2) - S32((textCacheSBlockDim + textCacheBlockDim)), S32(0));

            //  Calculate the address of the superblock inside the cache.
            //sBlockAddr = ((j >> (textCacheSBlockDim + textCacheBlockDim)) *
            //    GPU_MAX(S32(GPU_MAX(textureWidth[textUnit] >> level, U32(1))) >> (textCacheSBlockDim + textCacheBlockDim), S32(0))) +
        ...
        case GPU_TXBLOCK_FRAMEBUFFER:
            ...
            if (!pixelMapperConfigured[textUnit])                                        
            {
                ...
                switch(textureFormat[textUnit])
                {
                    ...
                    default:
                        ...
                        CG_ASSERT("Unsupported format for framebuffer textures.");
                        break;
                }                
            ...
            texPixelMapper[textUnit].changeResolution(GPU_MAX(textureWidth[textUnit] >> level, U32(1)),
                                                      GPU_MAX(textureHeight[textUnit] >> level, U32(1)));
            frameBufferTexelAddr = texPixelMapper[textUnit].computeAddress(i, j);

            address += U64(frameBufferTexelAddr);
        
//printf(" bmoTextureProcessor => witdh = %d height = %d (i, j) = (%d, %d) -> texel address = %08x final address = %016llx\n",
//GPU_MAX(textureWidth[textUnit] >> level, U32(1)), GPU_MAX(textureHeight[textUnit] >> level, U32(1)), i, j, frameBufferTexelAddr, address);

            break;

        default:
        
            CG_ASSERT("Undefined texture blocking mode.");
            break;
    }
...
//GPU_MAX(S32(mipmapSize(textureWidth2[textUnit], level) - (textCacheSBlockDim + textCacheBlockDim)), S32(0)));

//U64 backupAddress = address;
...
//printf("TxEm > i,j %d,%d sblockAddr %lld blockAddr %lld texelAddr %lld\n", i, j, sBlockAddr, blockAddr, texelAddr);
    ...
    /*switch(textureCompr[textUnit])
    {
        case GPU_NO_TEXTURE_COMPRESSION:
            ...
            if (((U32) (address && 0xffff0000)) == 0x37270000)
            {
        ...
        case GPU_S3TC_DXT1_RGBA:
            ...
            if (((U32) ((address >> DXT1_SPACE_SHIFT) & 0xffff0000)) == 0x37270000)
            {
                printf("TxEm > texture Width %d level %d mipmapSize %d is %d\n",
                textureWidth2[textUnit], level, mipmapSize(textureWidth2[textUnit], level),
                GPU_MAX(S32(mipmapSize(textureWidth2[textUnit], level) - (textCacheSBlockDim + textCacheBlockDim)), S32(0)));
        ...
        case GPU_S3TC_DXT5_RGBA:
            ...
            if (((U32) ((address >> DXT3_DXT5_SPACE_SHIFT) & 0xffff0000)) == 0x37270000)
            {
                printf("TxEm > texture Width %d level %d mipmapSize %d is %d\n",
                textureWidth2[textUnit], level, mipmapSize(textureWidth2[textUnit], level),
                GPU_MAX(S32(mipmapSize(textureWidth2[textUnit], level) - (textCacheSBlockDim + textCacheBlockDim)), S32(0)));
...
U64 bmoTextureProcessor::texel2address(U32 textUnit, U32 level, U32 cubemap, U32 i, U32 j, U32 k)
{
    U64 address;
    U64 texelAddr;
    U64 blockAddr;
    ...
    switch(textureMode[textUnit])
    {
        ...
        case GPU_TEXTURE3D:
            ...
            //if ((i >= (U32(1) << mipmapSize(textureWidth2[textUnit], level))) ||
            //    (j >= (U32(1) << mipmapSize(textureHeight2[textUnit], level))) ||
            //    (k >= (U32(1) << mipmapSize(textureDepth2[textUnit], level))))
            if ((i >= U32(GPU_MAX(textureWidth[textUnit] >> level, U32(1)))) ||
                (j >= U32(GPU_MAX(textureHeight[textUnit] >> level, U32(1)))) ||
                (k >= U32(GPU_MAX(textureDepth[textUnit] >> level, U32(1)))))
                return BLACK_TEXEL_ADDRESS;
            ...
            //  Calculate the address of the superblock inside the cache.  
            //sBlockAddr = ((j >> (textCacheSBlockDim + textCacheBlockDim)) <<
                //GPU_MAX(mipmapSize(textureWidth2[textUnit], level) - (textCacheSBlockDim + textCacheBlockDim), (U32)0)) +
            //    GPU_MAX(GPU_MAX(textureWidth[textUnit] >> level, U32(1)) >> (textCacheSBlockDim + textCacheBlockDim), (U32)0)) +
            //    (i >> (textCacheSBlockDim + textCacheBlockDim));
            ...
            address = textureAddress[textUnit][level][0] + adjustToFormat(textUnit,
                ...
                (k * GPU_MAX(textureWidth[textUnit] >> level, U32(1)) *  GPU_MAX(textureHeight[textUnit] >> level, U32(1))) +
...
U32 bmoTextureProcessor::mipmapSize(U32 topSize, U32 level)
{
    return GPU_MAX((S32) topSize - (S32) level, (S32) 0);
}

//  Adjust a texel address to the size and type of the texture format.  
U64 bmoTextureProcessor::adjustToFormat(U32 textUnit, U64 texelAddress)
{
    U64 address;
...
void bmoTextureProcessor::convertFormat(TextureAccess &textAccess, U32 trilinearAccess, U32 frag, U32 tex, U08 *data)
{
    ...
    if (textAccess.texOperation == ATTRIBUTE_READ)
    {
        ...
        if (tex == 0)
        {
            ...
            U32 readOffset = textAccess.trilinear[trilinearAccess]->attrFirstOffset[frag];
            U32 readSize = textAccess.trilinear[trilinearAccess]->attrFirstSize[frag];
//printf("TxEmu::convertFormat => ATTRIBUTE_READ -> (1) copying %d bytes from %d\n", readSize, readOffset);
            for(U32 b = 0; b < readSize; b++)
        ...
        else if (tex == 1)
        {
            GPU_ASSERT(
                ...
                {
                    CG_ASSERT("Unexpected cache read for attribute read.");
                }
            )
            
            //  Copy the attribute data from the second read.
            U32 dataOffset = textAccess.trilinear[trilinearAccess]->attrFirstSize[frag];
            U32 readSize = textAccess.trilinear[trilinearAccess]->attrSecondSize[frag];
//printf("TxEmu::convertFormat => ATTRIBUTE_READ -> (2) copying %d bytes to %d\n", readSize, dataOffset);
        ...
        {
            CG_ASSERT("For attribute reads maximum two cache line splits expected.");
        }
    ...
    switch(textureFormat[textAccess.textUnit])
    {
        case GPU_ALPHA8:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[address & 0x03]) / 255.0f;
        ...
        case GPU_ALPHA12:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
        ...
        case GPU_ALPHA16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
        ...
        case GPU_LUMINANCE8:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[address & 0x03]) / 255.0f;
        ...
        case GPU_LUMINANCE8_SIGNED:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(((S08 *) data)[address & 0x03]) / 127.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_LUMINANCE12:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
        ...
        case GPU_LUMINANCE16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 0.0f;
        ...
        case GPU_LUMINANCE4_ALPHA4:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x03] >> 4) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[address & 0x03] & 0x0f) / 15.0f;
        ...
        case GPU_LUMINANCE6_ALPHA2:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x03] >> 2) / 63.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[address & 0x03] & 0x03) / 3.0f;
        ...
        case GPU_LUMINANCE8_ALPHA8:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[address & 0x02]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[(address & 0x02) + 1]) / 255.0f;
        ...
        case GPU_LUMINANCE8_ALPHA8_SIGNED:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(((S08 *) data)[address & 0x02]) / 127.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(((S08 *) data)[(address & 0x02) + 1]) / 127.0f;
        ...
        case GPU_LUMINANCE12_ALPHA4:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02]) >> 4) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02]) & 0x0f) / 15.0f;
        ...
        case GPU_LUMINANCE12_ALPHA12:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32((*((U32 *) &data[0]) >> 12) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U32 *) &data[0]) & 0x0fff) / 4095.0f;
        ...
        case GPU_LUMINANCE16_ALPHA16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[0])) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[2])) / 65535.0f;
        ...
        case GPU_INTENSITY8:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x03]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(data[address & 0x03]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[address & 0x03]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[address & 0x03]) / 255.0f;
        ...
        case GPU_INTENSITY12:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02]) & 0x0fff) / 4095.0f;
        ...
        case GPU_INTENSITY16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02])) / 65535.0f;
        ...
        case GPU_RGB332:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x03] >> 5) / 7.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((data[address & 0x03] >> 3) & 0x07) / 7.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[address & 0x03] & 0x03) / 3.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB444:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32((*((U16 *) &data[address & 0x02]) >> 8) & 0x0f) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U16 *) &data[address & 0x02]) >> 4) & 0x0f) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U16 *) &data[address & 0x02]) & 0x0f) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB555:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32((*((U16 *) &data[address & 0x02]) >> 10) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U16 *) &data[address & 0x02]) >> 5) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U16 *) &data[address & 0x02]) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB565:
            ...
            //  Five bits for each color component.  Alpha set to default 1.0f.  
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02]) >> 11) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U16 *) &data[address & 0x02]) >> 5) & 0x3f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U16 *) &data[address & 0x02]) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB888:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[0]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(data[1]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[2]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB101010:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32((*((U32 *) &data[0]) >> 20) & 0x03ff) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U32 *) &data[0]) >> 10) & 0x03ff) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U32 *) &data[0]) & 0x03ff) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGB121212:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U32 *) &data[0]) >> 24) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U32 *) &data[0]) >> 12) & 0xfff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U32 *) &data[0]) & 0x0fff) / 4095.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGBA2222:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x03] >> 6) / 3.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((data[address & 0x03] >> 4) & 0x03) / 3.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32((data[address & 0x03] >> 2) & 0x03) / 3.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[address & 0x03] & 0x03) / 3.0f;
        ...
        case GPU_RGBA4444:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(data[address & 0x02] >> 4) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(data[address & 0x02] & 0x0f) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[(address & 0x02) + 1] >> 4) / 15.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[(address & 0x02) + 1] & 0x0f) / 15.0f;
        ...
        case GPU_RGBA5551:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U16 *) &data[address & 0x02]) >> 11) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U16 *) &data[address & 0x02]) >> 6) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32((*((U16 *) &data[address & 0x02]) >> 1) & 0x1f) / 31.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U16 *) &data[address & 0x02]) & 0x01) / 1.0f;
        ...
        case GPU_RGBA8888:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(data[1]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(data[2]) / 255.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(data[3]) / 255.0f;
        ...
        case GPU_RGBA1010102:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(*((U32 *) &data[0]) >> 22) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32((*((U32 *) &data[0]) >> 12) & 0x3ff) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32((*((U32 *) &data[0]) >> 2) & 0x3ff) / 1023.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(*((U32 *) &data[0]) & 0x03) / 3.0f;
        ...
        case GPU_RG16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(((U16 *) &data[address & 0x02])[0]) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(((U16 *) &data[address & 0x02])[1]) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
        ...
        case GPU_RGBA16:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = F32(((U16 *) &data[address & 0x02])[0]) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = F32(((U16 *) &data[address & 0x02])[1]) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(((U16 *) &data[address & 0x02])[2]) / 65535.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = F32(((U16 *) &data[address & 0x02])[3]) / 65535.0f;
        ...
        case GPU_R32F:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = *((F32 *) data);
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
            break;
        ...
        case GPU_RG32F:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = ((F32 *) data)[0];
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = ((F32 *) data)[1];
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = 0.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
            break;
        ...
        case GPU_RGBA32F:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = ((F32 *) data)[0];
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = ((F32 *) data)[1];
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = ((F32 *) data)[2];
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = ((F32 *) data)[3];
            break;
        ...
        case GPU_DEPTH_COMPONENT24:
            ...
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][0] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][1] = 
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][2] = F32(*((U32 *) data) & 0x00FFFFFF) / 16777215.0f;
            textAccess.trilinear[trilinearAccess]->texel[frag][tex][3] = 1.0f;
            break;

        case GPU_DEPTH_COMPONENT32:
            CG_ASSERT("Format DEPTH_COMPONENT32 not implemented.");
            break;
    ...
    if (textD3D9ColorConv[textAccess.textUnit])
    {
        ...
        F32 red   = textAccess.trilinear[trilinearAccess]->texel[frag][tex][2];
        F32 green = textAccess.trilinear[trilinearAccess]->texel[frag][tex][1];
        F32 blue  = textAccess.trilinear[trilinearAccess]->texel[frag][tex][0];
        F32 alpha = textAccess.trilinear[trilinearAccess]->texel[frag][tex][3];
    ...
    {
        F32 red   = textAccess.trilinear[trilinearAccess]->texel[frag][tex][0];
        F32 green = textAccess.trilinear[trilinearAccess]->texel[frag][tex][1];
        F32 blue  = textAccess.trilinear[trilinearAccess]->texel[frag][tex][2];
        //F32 alpha = textAccess.trilinear[trilinearAccess]->texel[frag][tex][3];
...
F32 bmoTextureProcessor::comparisonFilter(CompareMode function, F32 reference, F32 texel)
{
    F32 result;
    ...
    switch(function)
    {
        ...
        default:
            CG_ASSERT("Undefined comparison function.");
            break;
    }
...
U32 bmoTextureProcessor::format(TextureFormat format, Vec4FP32 color)
{
    U32 output;
    ...
    switch(format)
    {
        case GPU_RGBA8888:
            ...
            output = U08(color[0] * 255.0f) + (U08(color[1] * 255.0f) << 8) +
                (U08(color[2] * 255.0f) << 16) + (U08(color[3] * 255.0f) << 24);

            break;

        case GPU_LUMINANCE8:
            output = U08(color[0] * 255.0f);
            break;

        case GPU_LUMINANCE8_SIGNED:
            output = S08(color[0] * 127.0f);
            break;

        case GPU_LUMINANCE8_ALPHA8:
            output = U08(color[0] * 255.0f) + (U08(color[3] * 255.0f) << 8);
            break;            
            
        case GPU_LUMINANCE8_ALPHA8_SIGNED:
            output = ((S08(color[0] * 255.0f)) & 0x00FF) + (S08(color[3] * 255.0f) << 8);
            break;            
...
void bmoTextureProcessor::decodeBlockDXT1RGB(U08 *inBuffer, U08 *outBuffer)
{
    U32 color0, color1;
    Vec4FP32 RGBA0, RGBA1;
    Vec4FP32 decodedColor;
    U32 code;
    U32 colorbits;
    U32 i, j;

    //  Patch to detect use of non initialized memory.  
    if (*((U32 *) inBuffer) == 0xDEADCAFE)
    {
        for(i = 0; i < 16; i++)
        {
            ((U32 *) outBuffer)[i] = 0xFFFFFFFF;
        }
    ...
    RGBA0[0] = F32(color0 >> 11) * (1.0f / 31.0f);
    RGBA0[1] = F32((color0 >> 5) & 0x3f) * (1.0f / 63.0f);
    RGBA0[2] = F32(color0 & 0x1f) * (1.0f / 31.0f);
    //RGBA0[3] = 1.0f;

    //  Convert second reference color of the compressed block to RGBA.  
    color1 = (inBuffer[3] << 8) + inBuffer[2];
    RGBA1[0] = F32(color1 >> 11) * (1.0f / 31.0f);
    RGBA1[1] = F32((color1 >> 5) & 0x3f) * (1.0f / 63.0f);
    RGBA1[2] = F32(color1 & 0x1f) * (1.0f / 31.0f);
    //RGBA1[3] = 1.0f;

    //  Get the code bits for the color components in the block.  
    colorbits = inBuffer[4] + U32(inBuffer[5] << 8) + U32(inBuffer[6] << 16) + U32(inBuffer[7] << 24);
    ...
    for(j = 0; j < 4; j++)
    {
        for(i = 0; i < 4; i++)
        {
            ...
            ((U32 *) outBuffer)[GPUMath::morton(2, i, j)] = format(GPU_RGBA8888, decodedColor);
        }
    }
}

//  Decode a S3TC compressed texture using DXT1 encoding for RGBA format.  
void bmoTextureProcessor::decodeBlockDXT1RGBA(U08 *inBuffer, U08 *outBuffer)
{
    U32 color0, color1;
    Vec4FP32 RGBA0, RGBA1;
    Vec4FP32 decodedColor;
...
void bmoTextureProcessor::decodeBlockDXT3RGBA(U08 *inBuffer, U08 *outBuffer)
{
    U32 color0, color1;
    Vec4FP32 RGBA0, RGBA1;
    Vec4FP32 decodedColor;
...
//printf("\n");
    ...
    alphabits = (U64) inBuffer[0] + (((U64) inBuffer[1]) << 8) + (((U64) inBuffer[2]) << 16) +
        (((U64) inBuffer[3]) << 24) + (((U64) inBuffer[4]) << 32) + (((U64) inBuffer[5]) << 40) +
        (((U64) inBuffer[6]) << 48) + (((U64) inBuffer[7]) << 56);
    ...
    for(j = 0; j < 4; j++)
    {
        for(i = 0; i < 4; i++)
        {
            ...
            //  Decode alpha.  
            decodedColor[3] = F32(alphabits & 0x0f) * (1.0f / 15.0f);
...
void bmoTextureProcessor::decodeBlockDXT5RGBA(U08 *inBuffer, U08 *outBuffer)
{
    U32 color0, color1;
    Vec4FP32 RGBA0, RGBA1;
    float alpha0, alpha1;
    ...
    alpha0 = F32(inBuffer[0]) * (1.0f / 255.0f);

    //  Convert second reference alpha from the compressed block.  
    alpha1 = F32(inBuffer[1]) * (1.0f / 255.0f);

    //  Get the code bits for the alpha components in the block.  
    alphabits = ((U64) inBuffer[2]) + (((U64) inBuffer[3]) << 8) + (((U64) inBuffer[4]) << 16) +
        (((U64) inBuffer[5]) << 24) + (((U64) inBuffer[6]) << 32) + (((U64) inBuffer[7]) << 40);
...
void bmoTextureProcessor::decodeBlockLATC1(U08 *inBuffer, U08 *outBuffer)
{
    F32 luminance0, luminance1;
    Vec4FP32 decodedColor;
    U64 luminanceCodes;

    //  Convert first reference luminance from the compressed block.
    luminance0 = F32(inBuffer[0]) * (1.0f / 255.0f);

    //  Convert second reference luminance from the compressed block.
    luminance1 = F32(inBuffer[1]) * (1.0f / 255.0f);

    //  Get the code bits for the luminance elements in the block.
    luminanceCodes = ((U64) inBuffer[2]) + (((U64) inBuffer[3]) << 8) + (((U64) inBuffer[4]) << 16) +
        (((U64) inBuffer[5]) << 24) + (((U64) inBuffer[6]) << 32) + (((U64) inBuffer[7]) << 40);
    ...
    for(U32 j = 0; j < 4; j++)
    {
        for(U32 i = 0; i < 4; i++)
        {
            ...
            U32 luminanceCode = U32(luminanceCodes & 0x07);
...
//printf("LATC1 => {%d, %d} decoded color { %f, %f, %f, %f }\n", i, j, decodedColor[0], decodedColor[1], decodedColor[2], decodedColor[3]);
            ...
            outBuffer[GPUMath::morton(2, i, j)] = U08(format(GPU_LUMINANCE8, decodedColor) & 0x00FF);
        }
    }
}

//  Decode a LATC1_SIGNED compressed texture.
void bmoTextureProcessor::decodeBlockLATC1Signed(U08 *inBuffer, U08 *outBuffer)
{
    F32 luminance0, luminance1;
    Vec4FP32 decodedColor;
    U64 luminanceCodes;
    ...
    //  Convert first reference luminance from the compressed block.
    luminance0 = F32(S08(inBuffer[0])) * (1.0f / 127.0f);

    //  Convert second reference luminance from the compressed block.
    luminance1 = F32(S08(inBuffer[1])) * (1.0f / 127.0f);
...
//printf("LATC1_SIGNED => {%d, %d} decoded color { %f, %f, %f, %f }\n", i, j, decodedColor[0], decodedColor[1], decodedColor[2], decodedColor[3]);
            ...
            outBuffer[GPUMath::morton(2, i, j)] = U08(format(GPU_LUMINANCE8_SIGNED, decodedColor) & 0x00FF);
        }
    }
}

//  Decode a LATC2 compressed texture.
void bmoTextureProcessor::decodeBlockLATC2(U08 *inBuffer, U08 *outBuffer)
{
    F32 luminance0, luminance1;
    F32 alpha0, alpha1;
    Vec4FP32 decodedColor;
    ...
    //  Convert first reference alpha from the compressed block.
    alpha0 = F32(inBuffer[8]) * (1.0f / 255.0f);

    //  Convert second reference alpha from the compressed block.
    alpha1 = F32(inBuffer[9]) * (1.0f / 255.0f);

    //  Get the code bits for the luminance elements in the block.
    alphaCodes = ((U64) inBuffer[10]) + (((U64) inBuffer[11]) << 8) + (((U64) inBuffer[12]) << 16) +
        (((U64) inBuffer[13]) << 24) + (((U64) inBuffer[14]) << 32) + (((U64) inBuffer[15]) << 40);
    ...
    for(U32 j = 0; j < 4; j++)
    {
        for(U32 i = 0; i < 4; i++)
        {
            ...
            U32 alphaCode = U32(alphaCodes & 0x07);
...
//printf("LATC2 => {%d, %d} decoded color { %f, %f, %f, %f }\n", i, j, decodedColor[0], decodedColor[1], decodedColor[2], decodedColor[3]);
            ...
            ((U16 *) outBuffer)[GPUMath::morton(2, i, j)] = U16(format(GPU_LUMINANCE8_ALPHA8, decodedColor) & 0x0000FFFF);
        }
    }
}

//  Decode a LATC2_SIGNED compressed texture.
void bmoTextureProcessor::decodeBlockLATC2Signed(U08 *inBuffer, U08 *outBuffer)
{
    F32 luminance0, luminance1;
    F32 alpha0, alpha1;
    Vec4FP32 decodedColor;
    ...
    //  Convert first reference luminance from the compressed block.
    luminance0 = F32(S32(inBuffer[0])) * (1.0f / 127.0f);

    //  Convert second reference luminance from the compressed block.
    luminance1 = F32(S32(inBuffer[1])) * (1.0f / 127.0f);
    ...
    //  Convert first reference alpha from the compressed block.
    alpha0 = F32(S32(inBuffer[8])) * (1.0f / 127.0f);

    //  Convert second reference alpha from the compressed block.
    alpha1 = F32(S32(inBuffer[9])) * (1.0f / 127.0f);
...
//printf("LATC2_SIGNED => {%d, %d} decoded color { %f, %f, %f, %f }\n", i, j, decodedColor[0], decodedColor[1], decodedColor[2], decodedColor[3]);
            ...
            ((U16 *) outBuffer)[GPUMath::morton(2, i, j)] = U16(format(GPU_LUMINANCE8_ALPHA8_SIGNED, decodedColor) & 0x0000FFFF);
        }
    }
}

//  Decodes and selects the proper alpha value for S3TC alpha encoding.  
F32 bmoTextureProcessor::decodeS3TCAlpha(U32 code, F32 alpha0, F32 alpha1)
{
    F32 output;
...
void bmoTextureProcessor::nonTransparentS3TCRGB(U32 code, Vec4FP32 RGB0, Vec4FP32 RGB1, Vec4FP32 &output)
{
...
void bmoTextureProcessor::transparentS3TCRGB(U32 code, Vec4FP32 RGB0, Vec4FP32 RGB1, Vec4FP32 &output)
{
    //  Select color for the texel and store.  
    switch(code)
    {
...
void bmoTextureProcessor::decompressDXT1RGB(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressDXT1RGBA(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressDXT3RGBA(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressDXT5RGBA(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressLATC1(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressLATC1Signed(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressLATC2(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::decompressLATC2Signed(U08 *input, U08 *output, U32 size)
{
    U32 i;
    U32 numBlocks;
...
void bmoTextureProcessor::reset()
{
    U32 i;
    ...
    for (i = 0; i < MAX_TEXTURES; i++)
    {
        ...
        if (forceMaxAnisotropy)
            ...
            maxAnisotropy[i] = GPU_MIN(U32(1), confMaxAniso);
...
void bmoTextureProcessor::writeRegister(GPURegister reg, U32 subReg, GPURegData data)
{
    U32 textUnit;
    U32 mipmap;
    U32 cubemap;
    ...
    switch(reg)
    {
        ...
        default:
            ...
            CG_ASSERT("Unsupported texture behaviorModel register.");
            break;
    }
}
...
void bmoTextureProcessor::index2address(U32 attribute, TextureAccess &texAccess, U32 trilinearAccess, U32 frag, U32 index)
{
    //  Get the stream associated with the attribute.
    U32 stream = attributeMap[attribute];
...
//texAccess.texelSize[frag]);
    ...
    U32 startAddress = streamAddress[stream] + streamStride[stream] * index;
    
    //  Compute the address of the last byte to read.
    U32 endAddress = startAddress + streamDataSize[stream] - 1;
...
//startAddress, endAddress, (textCacheBlockDim * 2), sameLine ? "Si" : "No");
    ...
    if (sameLine)
    {
        ...
        //  Set address to read.
        texAccess.trilinear[trilinearAccess]->address[frag][0] = U64(startAddress);
...
//printf("TxEmu::index2address => address0 = %x\n", texAccess.trilinear[trilinearAccess]->address[frag][0]);
    ...
    else
    {
        ...
        U32 firstLineSize = textCacheBlockSize - startAddress & (textCacheBlockSize - 1);
        U32 secondLineSize = streamDataSize[stream] - firstLineSize;
        U32 readSize = GPU_MAX(firstLineSize, secondLineSize);
        ...
        texAccess.trilinear[trilinearAccess]->address[frag][1] = U64(((startAddress >> (textCacheBlockDim * 2)) + 1) << (textCacheBlockDim * 2));
...
void bmoTextureProcessor::loadAttribute(TextureAccess &texAccess, U32 trilinearAccess, U32 frag)
{
    //  Get the attribute identifier for which to convert/fill data.
    U32 attr = texAccess.textUnit;
    
    //  Get the pointer to the data read for the attribute.
    U08 *data = texAccess.trilinear[trilinearAccess]->attrData[frag];
    
    //  Get the stream associated with the attribute.
    U32 stream = attributeMap[attr];

    //  Get attribute data format.
    StreamData format = streamData[stream];

    //  Get the size in bytes of an element.
    U32 elemSize = streamElementSize[stream];
...
//attr, stream, format, elemSize, streamElements[stream]);
    ...
    {
        case 0:
            CG_ASSERT("Attribute with zero elements can not request loads!.");
            break;
...
//for(U32 b = 0; b < (elemSize * streamElements[stream]); b++)
//printf(" %02x", data[b]);
//printf("\n");
...
//texAccess.sample[frag][3]);
    ...
    if (d3d9ColorStream[stream])
    {
        ...
        F32 red = texAccess.sample[frag][2];
        F32 green = texAccess.sample[frag][1];
        F32 blue= texAccess.sample[frag][0];
        F32 alpha = texAccess.sample[frag][3];
...
F32 bmoTextureProcessor::attributeDataConvert(StreamData format, U08 *data)
{
    F32 res;
    ...
    {
        case SD_UNORM8:
            res = F32(data[0]) * (1.0f / 255.0f);
            break;
        case SD_SNORM8:
            res = F32(*((S08 *) data)) * (1.0f / 127.0f);
            break;
        case SD_UNORM16:
            res = F32(*((U16 *) data)) * (1.0f / 65535.0f);
            break;
        case SD_SNORM16:
            res = F32(*((S16 *) data)) * (1.0f / 32767.0f);
            break;
        case SD_UNORM32:
            res = F32(*((U32 *) data)) * (1.0f / 4294967295.0f);
            break;
        case SD_SNORM32:
            res = F32(*((S32 *) data)) * (1.0f / 2147483647.0f);
            break;
        case SD_FLOAT16:
        ...
        case SD_SINT32:
            ...
            break;
        default:
            CG_ASSERT("Unsupported stream data format.");
            break;
    }
...
TextureAccess::TextureAccess(U32 id, TextureOperation texOp, Vec4FP32 *coord, F32 *param, U32 unit)
{
    U32 i;

    //  Set texture access identifier (entry in the shader behaviorModel queue).
    accessID = id;
