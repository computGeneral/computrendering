...
if(! -e "$gpu3d_path/BUILD/bin/CG1GpuSim") {
	die "cg1gpu binary not found in $gpu3d_path/bin/CG1GpuSim";
}
$binary_path = "$gpu3d_path/BUILD/bin/CG1GpuSim";
if (! -e "$gpu3d_path/arch/config/CG1GPU.ini"){
	die "cg1gpu configuration file not found in $gpu3d_path/arch/config/CG1GPU.ini";
}
$config_path = "$gpu3d_path/arch/config";
...
foreach $line (@lines) 
{
	...
	else
	{
		...
		print("Executing $test_dir...\n");
        	copy("$config_path/$configfile","./CG1GPU.ini") or die "ERROR: $configfile copy failed";
        	my $code;
