...
{
    // falta c�digo para comprobar si este programa ya ten�a un fuente previamente
...
unsigned int ProgramObject::numOperands(unsigned int Opcode) const
{
    ...
    CG_ASSERT("Unexpected binary Opcode");
    return 0;
}
...
void ProgramObject::_computeASMCode() const
{
    ...
    while(binary_pointer < binSize)
    {
        ...
        if (asmlength <= assemblerSz)
            ...
            CG_ASSERT("Not enough buffer length");
    }
...
bool ProgramObject::getBinary(GLubyte* program, GLsizei& size) const
{
    if ( binSize > size )
    {
        CG_ASSERT("buffer size is not enough");
        return false;
    }
...
{
    if ( portion != 0 )
        CG_ASSERT("A program object only contains 1 portion");

    //if ( binary == 0 )
    //    CG_ASSERT("Binary code not yet available");
