/**************************************************************************
 *
 * Texture Unit cmoMduBase.
 *
 */

/**
 *
 *  @file cmoTextureProcessor.cpp
 *
 *  This file implements the cmoTextureProcessor cmoMduBase Class.
 *
 *  This mdu simulates the texture unit attached to a shader for
...
cmoTextureProcessor::cmoTextureProcessor(
    cgsArchConfig* ArchConf,
    bmoTextureProcessor* BmTexProc,
    //U32 stFrags,
    //U32 reqCycle, U32 reqQueueSz,
    //U32 accQueueSz, U32 resQueueSz, U32 readWaitWSz, bool twoLevel, U32 cacheLineSzL0,
    //U32 cacheWaysL0, U32 cacheLinesL0, U32 portWidth, U32 reqQSz, U32 inputReqQSzL0,
    //U32 missesCycle, U32 decomprLat, U32 cacheLineSzL1, U32 cacheWaysL1, U32 cacheLinesL1,
    //U32 inputReqQSzL1, U32 addrALULat, U32 filterLat,
    char *name, char *prefix, cmoMduBase* parent):

    //  Pre-initializations.  
    cmoMduBase(name, parent), bmTexProc(BmTexProc), 
    stampFragments(STAMP_FRAGMENTS),
    requestsCycle(ArchConf->ush.textRequestRate),
    requestQueueSize(ArchConf->ush.textReqQSize),
    accessQueueSize(ArchConf->ush.textAccessQSize),
    resultQueueSize(ArchConf->ush.textResultQSize),
    waitReadWindowSize(ArchConf->ush.textWaitWSize),
    useTwoLevelCache(ArchConf->ush.twoLevelCache),
    textCacheLineSizeL0(ArchConf->ush.txCacheInQSz),
    textCacheWaysL0(ArchConf->ush.txCacheWays),
    textCacheLinesL0(ArchConf->ush.txCacheLines),
    textCachePortWidth(ArchConf->ush.txCachePortWidth),
    textCacheReqQSize(ArchConf->ush.txCacheReqQSz),
    textCacheInReqSizeL0(ArchConf->ush.txCacheInQSz),
    textCacheMaxMiss(ArchConf->ush.txCacheMissCycle),
    textCacheDecomprLatency(ArchConf->ush.txCacheDecomprLatency),
    textCacheLineSizeL1(ArchConf->ush.txCacheLineSzL1),
    textCacheWaysL1(ArchConf->ush.txCacheWaysL1),
    textCacheLinesL1(ArchConf->ush.txCacheLinesL1),
    textCacheInReqSizeL1(ArchConf->ush.txCacheInQSzL1),
    addressALULatency(ArchConf->ush.addressALULat),
    filterLatency(ArchConf->ush.filterLat)

{
    ...
    GPU_ASSERT(
        if (stampFragments != 4)
            CG_ASSERT("Only implemented support for 4 fragment stamps.");
        if (requestsCycle == 0)
            CG_ASSERT("Texture requests per cycle must be at least 1.");
        if (requestQueueSize < (2 * requestsCycle))
            CG_ASSERT("Request queue requires at least twice the request rate entries.");
        if (accessQueueSize < 1)
            CG_ASSERT("Texture access queue requires at least an entry.");
        if (waitReadWindowSize < 1)
            CG_ASSERT("Read wait window requires at least one entry.");
        if (resultQueueSize < 1)
            CG_ASSERT("Result queue requires at least an entry.");
        if (textCacheLineSizeL0 == 0)
            CG_ASSERT("Texture cache line size can't be zero.");
        if (textCacheWaysL0 == 0)
            CG_ASSERT("Texture cache requires at least a way.");
        if (textCacheLinesL0 == 0)
            CG_ASSERT("Texture cache requires at least a line.");
        if (textCachePortWidth == 0)
            CG_ASSERT("Texture cache port must be at least a byte wide.");
        if (textCacheReqQSize == 0)
            CG_ASSERT("Texture cache request queue requires at least an entry.");
        if (textCacheInReqSizeL0 == 0)
            CG_ASSERT("Texture cache input request queue requires at least an entry.");
        if (textCacheMaxMiss == 0)
            CG_ASSERT("Texture cache maximum misses per cycles can't be zero.");
        if (addressALULatency < 1)
            CG_ASSERT("Address ALU latency must be at least 1.");
        if (filterLatency < 1)
            CG_ASSERT("Filter ALU latency must be at least 1.");
        if (useTwoLevelCache && (textCacheLineSizeL1 == 0))
            CG_ASSERT("Texture cache L1 line size can't be zero.");
        if (useTwoLevelCache && (textCacheWaysL1 == 0))
            CG_ASSERT("Texture cache L1 requires at least a way.");
        if (useTwoLevelCache && (textCacheLinesL1 == 0))
            CG_ASSERT("Texture cache L1 requires at least a line.");
        if (useTwoLevelCache && (textCacheInReqSizeL1 == 0))
            CG_ASSERT("Texture cache L1 input request queue requires at least an entry.");
    )
    ...
    //  Create statistics.  
    txRequests = &getSM().getNumericStatistic("TextureRequests", U32(0), fullName, postfix);
    results = &getSM().getNumericStatistic("TextureResults", U32(0), fullName, postfix);
    fetchOK = &getSM().getNumericStatistic("FetchesOK", U32(0), fullName, postfix);
    fetchFail = &getSM().getNumericStatistic("FetchesFailed", U32(0), fullName, postfix);
    fetchSkip = &getSM().getNumericStatistic("FetchesSkiped", U32(0), fullName, postfix);
    fetchStallFetch = &getSM().getNumericStatistic("FetchStallFetch", U32(0), fullName, postfix);
    fetchStallAddress = &getSM().getNumericStatistic("FetchStallAddress", U32(0), fullName, postfix);
    fetchStallWaitRead = &getSM().getNumericStatistic("FetchStallWaitRead", U32(0), fullName, postfix);
    fetchStallReadyRead = &getSM().getNumericStatistic("FetchStallReadyRead", U32(0), fullName, postfix);
    readOK = &getSM().getNumericStatistic("ReadsOK", U32(0), fullName, postfix);
    readFail = &getSM().getNumericStatistic("ReadsFailed", U32(0), fullName, postfix);
    readBytes = &getSM().getNumericStatistic("ReadBytesMemory", U32(0), fullName, postfix);
    readTrans = &getSM().getNumericStatistic("ReadTransactions", U32(0), fullName, postfix);
    readyReadLevel = &getSM().getNumericStatistic("ReadyReadQueueOccupation", U32(0), fullName, postfix);
    waitReadLevel = &getSM().getNumericStatistic("WaitReadWindowOccupation", U32(0), fullName, postfix);
    resultLevel = &getSM().getNumericStatistic("ResultQueueOccupation", U32(0), fullName, postfix);
    accessLevel = &getSM().getNumericStatistic("AccessQueueOccupation", U32(0), fullName, postfix);
    requestLevel = &getSM().getNumericStatistic("RequestQueueOccupation", U32(0), fullName, postfix);
    addrALUBusy = &getSM().getNumericStatistic("AddressALUBusyCycles", U32(0), fullName, postfix);
    filterALUBusy = &getSM().getNumericStatistic("FilterALUBusyCycles", U32(0), fullName, postfix);
    textResLat = &getSM().getNumericStatistic("TextureResultLatency", U32(0), fullName, postfix);
    bilinearSamples = &getSM().getNumericStatistic("BilinearSamples", U32(0), fullName, postfix);
    anisoRatio = &getSM().getNumericStatistic("AnisotropyRatio", U32(0), fullName, postfix);
    addrCalcFinished  = &getSM().getNumericStatistic("AddressCalculationFinished", U32(0), fullName, postfix);

    //  Histogram for anisotropy samples taken per request.
    for(U32 aratio = 0; aratio <= MAX_ANISOTROPY; aratio++)
    {
        char statsName[256];
        if (aratio == 0)
        ...
        anisoRatioHisto[aratio] = &getSM().getNumericStatistic(statsName, U32(0), fullName, postfix);
    }
    
    //  Histogram for bilinear samples taken per request.
    for(U32 bl = 1; bl <= (MAX_ANISOTROPY * 2); bl++)
    {
        char statsName[256];
        sprintf(statsName, "HistoBilinears%02d", bl);
        bilinearsHisto[bl - 1] = &getSM().getNumericStatistic(statsName, U32(0), fullName, postfix);
    }

    magnifiedPixels = &getSM().getNumericStatistic("MagnifiedPixels", U32(0), fullName, postfix);

    mipsSampled[0] = &getSM().getNumericStatistic("MipsSampled1", U32(0), fullName, postfix);
    mipsSampled[1] = &getSM().getNumericStatistic("MipsSampled2", U32(0), fullName, postfix);

    pointSampled = &getSM().getNumericStatistic("PointSampled", U32(0), fullName, postfix);
    ...
    //  Texture request signal from the Shader Decoder/Execute cmoMduBase.  
    textRequestSignal = newInputSignal("TextureRequest", requestsCycle, 1, prefix);

    //  State signal to the Shader Decode Execute mdu.  
    ...
    //  Texture data signal to Shader Decode Execute mdu.  
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(textRequestsQ == NULL), "Unable to allocate texture request queue.");
    //  Allocate texture access queue.  
    texAccessQ = new TextureAccess*[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(texAccessQ == NULL), "Unable to allocate texture access queue.");
    //  Allocate texture access request queue.  
    texAccessReqQ = new TextureRequest*[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(texAccessReqQ == NULL), "Unable to allocate texture access request queue.");
    //  Allocate free texture access entry queue.  
    freeTexAccessQ = new U32[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(freeTexAccessQ == NULL), "Unable to allocate texture access free entries queue.");
    //  Allocate address calculation texture access entry queue.  
    calcTexAccessQ = new U32[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(calcTexAccessQ == NULL), "Unable to allocate address calculation queue.");
    //  Allocate fetch texture access entry queue.  
    fetchTexAccessQ = new U32[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(fetchTexAccessQ == NULL), "Unable to allocate texture access fetch entries queue.");    //  Allocate texture result queue.  
    textResultQ = new TextureResult*[resultQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(textResultQ == NULL), "Unable to allocate texture result queue.");
    //  Allocate ready read queue.  
    readyReadQ = new TrilinearElement[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(readyReadQ == NULL), "Unable to allocate ready read queue.");
    //  Allocate wait read window.  
    waitReadW = new TrilinearElement[waitReadWindowSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(waitReadW == NULL), "Unable to allocate wait read window.");
    //  Allocate free wait read entries queue.  
    freeWaitReadQ = new U32[waitReadWindowSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(freeWaitReadQ == NULL), "Unable to allocate wait read window free entries queue.");
    //  Allocate trilinear elements waiting in the wait read window queue.  
    waitReadQ = new U32[waitReadWindowSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(waitReadQ == NULL), "Unable to allocate wait read queue.");
    //  Allocate trilinear filter queue.  
    filterQ = new TrilinearElement[accessQueueSize];

    //  Check allocation.  
    CG_ASSERT_COND(!(filterQ == NULL), "Unable to allocate trilinear filter queue.");
...
//  Clock function for Texture Unit cmoMduBase.  Drives the time of the simulation.  
void cmoTextureProcessor::clock(U64 cycle)
{
...
cycle, numFreeFilters, numToFilter, numFiltered);
    ...
    //  Check texture requests from the Shader Decode Execute mdu.  
...
void cmoTextureProcessor::calculateAddress(U64 cycle)
{
    U32 frag;
    U32 filterLoops;
    ...
    if ((numCalcTexAcc > 0) && (addressALUCycles == 0))
    {
        ...
        bmTexProc->calculateAddress(texAccessQ[addressAccess]);
...
void cmoTextureProcessor::fetch(U64 cycle)
{
    U32 frag;
    U32 i;
    U32 start;
...
//frag, start + i, texAccessQ[fetchAccess]->trilinear[nextTrilinear]->address[frag][start + i]);
                    ...
                    {
GPU_TEX_TRACE(
    U64 address64 = texAccessQ[fetchAccess]->trilinear[nextTrilinear]->address[frag][start + i];
    U32 address32;
...
void cmoTextureProcessor::read(U64 cycle)
{
    U32 frag;
    U32 i;
    U32 start;
    U32 nextTrilinear;
    U08 data[MAX_TEXEL_DATA];
    bool allRead;
    bool lastLoop;
    U32 readAccess;
...
//texAccessQ[readAccess]->trilinear[nextTrilinear]->line[frag][start + i]);
                            ...
                            ))
                        {
                            ...
                            bmTexProc->convertFormat(*texAccessQ[readAccess], nextTrilinear, frag, start + i, data);
                        ...
                        {
                            //CG_ASSERT("Ready read address couldn't be read.");
                ...
                {
                    CG_ASSERT("All texels should be already fetched.");
                    //  Not all texels in the current loop could be read.  
                    //allRead = FALSE;
...
//  Filters a stamp.  
void cmoTextureProcessor::filter(U64 cycle)
{
    U32 frag;
    U32 filterLoops;
    ...
    if ((numToFilter > 0) && (filterCycles == 0))
    {
        ...
        bmTexProc->filter(*texAccessQ[filterAccess], nextTrilinear);
...
void cmoTextureProcessor::sendTextureSample(U64 cycle)
{
...
void cmoTextureProcessor::processTextureRequest(TextureRequest *txRequest)
{
    //  Check if the queue has free entries.  
    CG_ASSERT_COND(!(freeRequests == 0), "No free entry in the texture request queue.");
    GPU_DEBUG_BOX(
...
void cmoTextureProcessor::genTextureAccess()
{
    U32 freeAccess;
...
GPU_TEX_TRACE(
    ...
    {
        for(int j = 0; j < 4; j++)
            texTrace << *((U32 *) &(texAccess->originalCoord[i][j])) << " ";
    };
...
void cmoTextureProcessor::processShaderCommand(U64 cycle, ShaderCommand *command)
{
...
void cmoTextureProcessor::processRegisterWrite(U64 cycle, GPURegister reg, U32 subReg, GPURegData data)
{
    U32 textUnit;
    U32 mipmap;
    U32 cubemap;
    ...
    switch(reg)
    {
        case GPU_TEXTURE_ENABLE:
            ...
            CG_ASSERT_COND(!(subReg > MAX_TEXTURES), "Texture Unit index overflow.");
            ...
            bmTexProc->writeRegister(reg, subReg, data);
        ...
        case GPU_TEXTURE_ADDRESS:
            ...
            CG_ASSERT_COND(!(textUnit >= MAX_TEXTURES), "Texture Unit index overflow.");
            //  Check mipmap index.  
            CG_ASSERT_COND(!(mipmap >= MAX_TEXTURE_SIZE), "Texture Unit Mipmap index overflow.");
            //  Check cubemap index.  
            CG_ASSERT_COND(!(cubemap >= CUBEMAP_IMAGES), "Texture Unit cubemap index overflow.");
        ...
        case GPU_TEXTURE_BORDER_COLOR:
            ...
            GPU_TEX_TRACE(
                ...
                texTrace << *((U32 *) &data.qfVal[0]) << " ";
                texTrace << *((U32 *) &data.qfVal[1]) << " ";
                texTrace << *((U32 *) &data.qfVal[2]) << " ";
                texTrace << *((U32 *) &data.qfVal[3]);
                texTrace << endl;
            )
        ...
        case GPU_TEXTURE_MAG_FILTER:
            ...
            //  Check texture unit index.  
            CG_ASSERT_COND(!(subReg > MAX_TEXTURES), "Texture Unit index overflow.");
            CG_ASSERT_COND(!((data.txFilter != GPU_NEAREST) && (data.txFilter != GPU_LINEAR)), "Unsupported magnification filter.");
            ...
            GPU_DEBUG_BOX(
                ...
                switch(textureMagFilter[subReg])
                {
                    ...
                    default:
                        CG_ASSERT("Unsupported texture magnification filter mode.");
                        break;
                }
            )
        ...
        case GPU_TEXTURE_MIN_LOD:
            ...
            GPU_TEX_TRACE(texTrace << "UPD " << subReg << " TEX_MIN_LOD " << *((U32 *) &data.f32Val) << endl;)
        ...
        case GPU_TEXTURE_MAX_LOD:
            ...
            GPU_TEX_TRACE(texTrace << "UPD " << subReg << " TEX_MAX_LOD " << *((U32 *) &data.f32Val) << endl;)
        ...
        case GPU_TEXTURE_LOD_BIAS:
            ...
            GPU_TEX_TRACE(texTrace << "UPD " << subReg << " TEX_LOD_BIAS " << *((U32 *) &data.f32Val) << endl;)
        ...
        case GPU_TEXT_UNIT_LOD_BIAS:
            ...
            GPU_TEX_TRACE(texTrace << "UPD " << subReg << " TEX_UNIT_LOD_BIAS " << *((U32 *) &data.f32Val) << endl;)
        ...
        case GPU_TEXTURE_MAX_ANISOTROPY:
            ...
            GPU_ASSERT(
                if (data.uintVal == 0)
                    CG_ASSERT("Maximum anisotropy for a texture must be at least 1.");
                if (data.uintVal > MAX_ANISOTROPY)
                    CG_ASSERT("Maximum anisotropy for the texture too large.");
            )
        ...
        case GPU_VERTEX_ATTRIBUTE_DEFAULT_VALUE:
            ...
            CG_ASSERT_COND(!(subReg >= MAX_VERTEX_ATTRIBUTES), "Illegal vertex attribute ID.");
        ...
        case GPU_STREAM_ADDRESS:
            ...
            CG_ASSERT_COND(!(subReg >= MAX_STREAM_BUFFERS), "Illegal stream buffer ID.");
        ...
        case GPU_STREAM_ELEMENTS:
            ...
            CG_ASSERT_COND(!(data.uintVal > 4), "Maximum 4 elements per stream read allowed (SIMD4).");            
        ...
        default:
            ...
            CG_ASSERT("Unsupported texture unit register.");
            break;
    }
...
void cmoTextureProcessor::processMemoryTransaction(U64 cycle, MemoryTransaction *memTrans)
{
...
void cmoTextureProcessor::setDebugMode(bool mode)
{
    debugMode = mode;
    textCache->setDebug(mode);
}

void cmoTextureProcessor::getState(string &stateString)
{
...
void cmoTextureProcessor::stallReport(U64 cycle, string &stallReport)
{
    ...
    for(U32 w = 0; w < numWaitReads; w++)
    {
        U32 waitAccess = waitReadW[waitReadQ[w]].access;
        U32 waitTrilinear = waitReadW[waitReadQ[w]].trilinear;
        
        reportStream << "  Waiting request " << w << " access queue entry " << waitAccess << " trilinear " << waitTrilinear << endl;
        for(U32 f = 0; f < stampFragments; f++)
        {            
            U32 texelsToRequest = texAccessQ[waitAccess]->trilinear[waitTrilinear]->texelsLoop[f] * texAccessQ[waitAccess]->trilinear[waitTrilinear]->loops[f];
            reportStream << "    Fragment " << f << " texels to read " << texelsToRequest << endl;
            for(U32 t = 0; t < texelsToRequest; t++)
            {
...
void cmoTextureProcessor::printTextureAccessInfo(TextureAccess *texAccess)
{
