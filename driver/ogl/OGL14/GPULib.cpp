...
GLAPI void GLAPIENTRY glBufferDataARB(GLenum target, GLsizeiptr size, const GLvoid *data, GLenum usage)
{
    ...
    GET_BUFFER(target).setContents(size, (U08*)data, usage);
}
                                          ...
                                          GLsizeiptrARB size, const GLvoid *data)
{
    ...
    GET_BUFFER(target).setPartialContents(offset, size, (U08*)data);
}
...
GLAPI void GLAPIENTRY glBegin( GLenum mode )
{
    ...
    if ( GL_POLYGON < mode || mode < GL_POINTS )
        CG_ASSERT("GLenum is not a valid primitive mode");
    ...
    {
        case GL_POINTS:
            CG_ASSERT("Primitive mode GL_POINTS not supported.");
            break;
        case GL_LINES:
            CG_ASSERT("Primitive mode GL_LINES not supported.");
            break;
        case GL_LINE_STRIP:
            CG_ASSERT("Primitive mode GL_LINE_STRIP not supported.");
            break;
        case GL_LINE_LOOP:
            CG_ASSERT("Primitive mode GL_LINE_LOOP not supported.");
            break;
        case GL_TRIANGLES:
            break;
        ...
        case GL_QUAD_STRIP:
            break;
        case GL_POLYGON:
            CG_ASSERT("Primitive mode GL_POLYGON not supported.");
            break;
    }
...
*/
    ...
    if (clearZ)
    {
        ...
        data.uintVal = U32((F32(depth) * F32((1 << 24) - 1)));
                                 ...
                                 GLdouble near_, GLdouble far_ )
{
    ...
    if ( near_ == far_  || near_ == far_ || top == bottom )
        CG_ASSERT("Invalid GL parameter values");

    Matrixf mat;

    mat[0][0] = 2.0F / static_cast<F32>(right-left);
    mat[1][0] = 0.0F;
    mat[2][0] = 0.0F;
    mat[3][0] = 0.0F;

    mat[0][1] = 0.0F;
    mat[1][1] = 2.0F / static_cast<F32>(top-bottom);
    mat[2][1] = 0.0F;
    mat[3][1] = 0.0F;

    mat[0][2] = 0.0F;
    mat[1][2] = 0.0F;
    mat[2][2] = -2.0F / static_cast<F32>(far_-near_);
    mat[3][2] = 0.0F;

    mat[0][3] = static_cast<F32>(-(right+left) / (right-left));
    mat[1][3] = static_cast<F32>(-(top+bottom) / (top-bottom));
    mat[2][3] = static_cast<F32>(-(far_+near_) / (far_-near_));
    mat[3][3] = 1.0F;
                                   ...
                                   GLdouble near_, GLdouble far_ )
{
    ...
    mat[0][0] = (2.0F*static_cast<F32>(near_)) / static_cast<F32>(right-left); // M(0,0)
    mat[1][0] = 0.0F; // M(1,0)
    mat[2][0] = 0.0F; // M(2,0)
    mat[3][0] = 0.0F; // M(3,0)

    mat[0][1] = 0.0F; // M(0,1)
    mat[1][1] = (2.0F*static_cast<F32>(near_)) / static_cast<F32>(top-bottom); // M(1,1)
    mat[2][1] = 0.0F; // M(2,1)
    mat[3][1] = 0.0F; // M(3,1)

    mat[0][2] = static_cast<F32>((right+left) / (right-left)); // M(0,2)
    mat[1][2] = static_cast<F32>((top+bottom) / (top-bottom)); // M(1,2)
    mat[2][2] = static_cast<F32>(-(far_+near_) / (far_-near_)); // M(2,2)
    mat[3][2] = -1.0F; // M(3,2)

    mat[0][3] = 0.0F; // M(0,3)
    mat[1][3] = 0.0F; // M(1,3)
    mat[2][3] = static_cast<F32>(-(2*far_*near_) / (far_-near_)); // M(2,3)
    mat[3][3] = 0.0F; // M(3,3)
...
#define CONFIG_ATTRIB(stream_,md_,attrib_,compos_)\
    ...
    if ( useTextures )
    {
        ...
        {
            HAL::ShAttrib tu = HAL::ShAttrib(U32(HAL::VS_TEXTURE_0) + texUnit[i]);
            U32 md = mdTextures[i];
...
#undef CONFIG_ATTRIB
    ...
    if ( useTextures )
    {
        vector<U32>::iterator it = mdTextures.begin();
                                                ...
                                                GLboolean normalized, GLsizei stride, const GLvoid *pointer)
{
    ...
    //if ( normalized )
        //CG_ASSERT("Normalization not supported yet");
...
GLAPI void GLAPIENTRY glDisable( GLenum cap )
{
    ...
    switch ( cap )
    {
        ...
        default:
            ...
            //CG_ASSERT("Unsupported constant");
            popup("GPULib", msg);
    }
}
...
GLAPI void GLAPIENTRY glMaterialf( GLenum face, GLenum pname, GLfloat param )
{
    ...
    switch ( pname )
    {
        case GL_SHININESS:
            ...
            break;
        default:
            CG_ASSERT("This call can be used to set the only single-valued material property : shininess");
    }
}
...
GLAPI void GLAPIENTRY glLightModelf( GLenum pname, GLfloat param )
{
    ...
    switch ( pname )
    {
        ...
        case GL_LIGHT_MODEL_TWO_SIDE:
            ...
            break;
        default:
            CG_ASSERT("This call can be used to set only single-valued lightmodel characteristics");
    }
}
...
GLAPI void GLAPIENTRY glLightf( GLenum light, GLenum pname, GLfloat param )
{
    ...
    switch ( pname )
    {
        ...
        case GL_QUADRATIC_ATTENUATION:
            ...
            break;
        default:
            CG_ASSERT("This call can be used to set only single-valued light characteristics");
    }
}
...
GLAPI void GLAPIENTRY glDepthFunc( GLenum func )
{
    ...
    switch ( func )
    {
        ...
        default:
            CG_ASSERT("Unknown depth function");
    }
