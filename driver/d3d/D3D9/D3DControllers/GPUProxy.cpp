...
GPUProxyAddress::GPUProxyAddress(U32 _md, U32 _offset) {
    md = _md;
    offset = _offset;
};
...
GPUProxy::GPUProxy()
{
    ...
    for(U32 i = 0; i < MAX_VERTEX_CONSTANTS; i ++) {
    ...
    for(U32 i = 0; i < MAX_USER_CLIP_PLANES; i++) {
    ...
    for(U32 i = 0; i < MAX_FRAGMENT_CONSTANTS; i ++) {
    ...
    for(U32 i = 0; i < MAX_TEXTURES; i ++) {
        ...
        for(U32 j = 0; j < MAX_TEXTURE_SIZE; j ++)
            for(U32 k = 0; k < CUBEMAP_IMAGES; k++)
...
void GPUProxy::initBuffers(U32* _mdCWFront, U32* _mdCWBack, U32* _mdZS) {
   U32 mdCWFront;
   U32 mdCWBack;
   U32 mdZS;
   HAL::getHAL()->initBuffers(&mdCWFront, &mdCWBack, &mdZS);

   /* HAL initBuffers assigns privately some registers.
...
U32 GPUProxy::obtainMemory( U32 sizeBytes) {
    U32 md = HAL::getHAL()->obtainMemory(sizeBytes);
    D3D_DEBUG( cout << "GPUPROXY: Allocating " << (int)sizeBytes << " for md " << (int)md << endl; )
    D3D_DEBUG( HAL::getHAL()->printMemoryUsage(); )
    return md;
}

void GPUProxy::releaseMemory( U32 md ) {
    HAL::getHAL()->releaseMemory(md);
...
bool GPUProxy::writeMemory( U32 md, const U08* data, U32 dataSize, bool isLocked) {
    return HAL::getHAL()->writeMemory(md, data, dataSize, isLocked);
}

bool GPUProxy::writeMemory( U32 md, U32 offset, const U08* data, U32 dataSize, bool isLocked) {
    return HAL::getHAL()->writeMemory(md, offset, data, dataSize, isLocked);
}

void GPUProxy::setResolution(U32 width, U32 height) {
     HAL::getHAL()->setResolution(width, height);
}

bool GPUProxy::commitVertexProgram( U32 memDesc, U32 programSize, U32 startPC ) {
     return HAL::getHAL()->commitVertexProgram(memDesc, programSize, startPC);
}

bool GPUProxy::commitFragmentProgram( U32 memDesc, U32 programSize, U32 startPC ) {
     return HAL::getHAL()->commitFragmentProgram(memDesc, programSize, startPC);
}
...
void GPUProxy::getGPUParameters(U32& gpuMemSz, U32& sysMemSz, U32& blocksz, U32& sblocksz,
U32& scanWidth, U32& scanHeight, U32& overScanWidth, U32& overScanHeight,
bool& doubleBuffer, U32& fetchRate) const {
    HAL::getHAL()->getGPUParameters(gpuMemSz, sysMemSz, blocksz, sblocksz,
