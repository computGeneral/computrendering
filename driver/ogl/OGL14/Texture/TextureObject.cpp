...
{
    CG_ASSERT("Texture object with 3D dimensionality are not supported for now");
}
...
{
    if ( format_ != GL_RGBA )
        CG_ASSERT("only GL_RGBA format supported for now");
    if ( type_ != GL_UNSIGNED_BYTE )
        CG_ASSERT("only GL_UNSIGNED_BYTE type supported for now");
    if ( border_ != 0 )
                                  ...
                                  GLenum type_, const GLvoid* data_, GLint compressedSize_)
{
    ...
    if ( size != 0 ) // Compressed texture (glCompressedTexImage2D)
    {
        ...
        switch ( ifmt )
        {
            ...
            default:
                CG_ASSERT("Unexpected compressed internal format");
        }
        ...
        w2 = (U32) mathlib::ceil2(mathlib::log2(width_ >> 2));
        ...
        for(i = 0; i < (height_ >> 2); i++)
        {
            for(j = 0; j < (width_ >> 2); j++)
            {
                ...
                for(k = 0; k < (s3tcBlockSz >> 2); k ++)
                    ((U32 *) data)[(a * (s3tcBlockSz >> 2)) + k] = ((U32 *) data_)[(i * (width_ >> 2) + j) * (s3tcBlockSz >> 2) + k];
            }
        }*/
    ...
    w2 = (U32) mathlib::ceil2(mathlib::log2(width_));
    ...
    if ( texelsSize == 4 )
    {
        for(i = 0; i < height; i++)
        {
            for(j = 0; j < width; j++)
            {
                ...
                ((U32 *) data)[a] = ((U32 *) data_)[i * width_ + j];
            }
        }
    }
    else if ( texelsSize == 1 )
    {
        for(i = 0; i < height; i++)
        {
            ...
            {
                a = texel2address(w2, tileLevel1Sz, tileLevel2Sz, j, i);
                ((U08 *) data)[a] = ((U08 *) data_)[i * width_ + j];
            }
        }
    }*/
...
{
    if ( fmt_ != format )
        CG_ASSERT("format previously defined was diferent (maybe is not a panic)");
    ...
    if (compressedSize_ != 0)
        CG_ASSERT("set partial contents of compressed textures not supported yet");
...
/*
    ...
    w2 = (U32) mathlib::ceil2(mathlib::log2(width));
    ...
    if ( texelsSize == 4 )
    {
        for(i = yoffset; i < yoffset + height_; i++)
        {
            ...
            {
                a = texel2address(w2, tileLevel1Sz, tileLevel2Sz, j, i);
                ((U32 *) data)[a] = ((U32 *) subData)[(i - yoffset) * width_ + (j - xoffset)];
            }
        }
    }
    else if ( texelsSize == 1 )
    {
        for(i = yoffset; i < yoffset + height_; i++)
        {
            ...
            {
                a = texel2address(w2, tileLevel1Sz, tileLevel2Sz, j, i);
                ((U08 *) data)[a] = ((U08 *) subData)[(i - yoffset) * width_ + (j - xoffset)];
            }
        }
    }*/
    ...
    switch ( targetFace )
    {
        ...
        case GL_TEXTURE_CUBE_MAP_POSITIVE_X:
            if ( getTargetName() != GL_TEXTURE_CUBE_MAP )
                CG_ASSERT("Only cube maps have faces");
            return mips[0][level];
        ...
        )
{
    ...
    if ( targetFace== GL_TEXTURE_1D )
        CG_ASSERT("Textures 1D not implemented");
    else if ( targetFace == GL_TEXTURE_3D )
        CG_ASSERT("Textures 3D not implemented");
...
void TextureObject::getMipmapPosition(GLuint portion, GLuint& face, GLuint& pos)
{
    ...
    if ( offset > getMaxActiveMipmap() )
        CG_ASSERT("Mipmap selected greater than maxLevel");

    if ( tn == GL_TEXTURE_1D || tn == GL_TEXTURE_2D || tn == GL_TEXTURE_3D )
    {
        if ( set != 0 )
            CG_ASSERT("(NOT a CubeMap) Mipmap out of range [baseLevel..maxLevel]");
    }
    else // CUBE_MAP
    {
        if ( set >= 6 )
            CG_ASSERT("CubeMap: Mipmap out of range [baseLevel..maxLevel]");
    }
...
void TextureObject::getMipmapPortion(GLuint face, GLuint pos, GLuint& portion)
{
    ...
    {
        CG_ASSERT("NOT a CubeMap and face different from face 0");
    }
    else if ( (tn == GL_TEXTURE_CUBE_MAP) && face >= 6 )
    {
        CG_ASSERT("CubeMap and face greater than 6");
    }
    
    if ( pos > getMaxActiveMipmap())
    {
        CG_ASSERT("mipmap position greater than maximum active mipmap");
    }
...
GLuint TextureObject::getMaxActiveMipmap() const
{
    ...
    if ( isCompressed() )
    {
        ...
        if (  targetName == GL_TEXTURE_2D || targetName == GL_TEXTURE_CUBE_MAP )
            ...
            CG_ASSERT("GL_TEXTURE_2D or CUBE_MAP expected");
    }

    if (  p < 0 )
        CG_ASSERT("Max active mipmap cannot be less than 0");
...
void TextureObject::sync()
{
    ...
    if ( min > max )
        CG_ASSERT("min mipmap is greater than max mipmap");
