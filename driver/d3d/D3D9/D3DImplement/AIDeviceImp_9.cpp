...
i_parent(_i_parent)
{
    ...
    D3D9State::instance().initialize(this, max(1, pp->BackBufferWidth >> 1), max(1, pp->BackBufferHeight >> 1));
#else
    D3D9State::instance().initialize(this, pp->BackBufferWidth, pp->BackBufferHeight);
#endif
    
    refs = 0;

    i_surface_childs.insert(D3D9State::instance().getZStencilBuffer());
...
ULONG D3D_CALL AIDeviceImp9 :: Release ( ) 
{
    ...
    if (refs == 0)
        D3D9State::instance().destroy(); // TODO: Destroy everything...
...
HRESULT D3D_CALL AIDeviceImp9 :: Present (  CONST RECT* pSourceRect , CONST RECT* pDestRect , HWND hDestWindowOverride , CONST RGNDATA* pDirtyRegion ) 
{
    ...
    D3D9State::instance().swapBuffers();
...
HRESULT D3D_CALL AIDeviceImp9 :: GetBackBuffer (  UINT iSwapChain , UINT iBackBuffer , D3DBACKBUFFER_TYPE Type , IDirect3DSurface9** ppBackBuffer ) 
{
    ...
    //*ppBackBuffer = (IDirect3DSurface9 *) D3D9State::instance().getRenderTarget(iBackBuffer);
    *ppBackBuffer = (IDirect3DSurface9 *) D3D9State::instance().getDefaultRenderSurface();
...
HRESULT D3D_CALL AIDeviceImp9 :: CreateRenderTarget (  UINT Width , UINT Height , D3DFORMAT Format , D3DMULTISAMPLE_TYPE MultiSample , DWORD MultisampleQuality , BOOL Lockable , IDirect3DSurface9** ppSurface , HANDLE* pSharedHandle ) 
{
    ...
    D3D9State::instance().addRenderTarget(rt, Width, Height, Format);
...
HRESULT D3D_CALL AIDeviceImp9 :: CreateDepthStencilSurface (  UINT Width , UINT Height , D3DFORMAT Format , D3DMULTISAMPLE_TYPE MultiSample , DWORD MultisampleQuality , BOOL Discard , IDirect3DSurface9** ppSurface , HANDLE* pSharedHandle ) 
{
    ...
    D3D9State::instance().addRenderTarget(zstencil, Width, Height, Format);
...
HRESULT D3D_CALL AIDeviceImp9 :: UpdateSurface (  IDirect3DSurface9* pSourceSurface , CONST RECT* pSourceRect , IDirect3DSurface9* pDestinationSurface , CONST POINT* pDestPoint ) 
{
    ...
    //D3D9State::instance().getDataSurface(this);
    ...
    D3D9State::instance().copySurface((AISurfaceImp9 *)pSourceSurface, &srcRect, (AISurfaceImp9 *)pDestinationSurface, &dstRect, D3DTEXF_POINT);
...
HRESULT D3D_CALL AIDeviceImp9 :: StretchRect (  IDirect3DSurface9* pSourceSurface , CONST RECT* pSourceRect , IDirect3DSurface9* pDestSurface , CONST RECT* pDestRect , D3DTEXTUREFILTERTYPE Filter )
{
    ...
    if (pDestRect != NULL) {
        cout << "AIDeviceImp9: srcRect: " << pDestRect->top << ", " << pDestRect->left << ", " << pDestRect->bottom << ", " << pDestRect->right << endl;
            //CG_ASSERT("Position diferent than (0,0) not supported.");
    }*/
    ...
    D3D9State::instance().copySurface((AISurfaceImp9 *)pSourceSurface, &srcRect, (AISurfaceImp9 *)pDestSurface, &dstRect, Filter);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetRenderTarget (  DWORD RenderTargetIndex , IDirect3DSurface9* pRenderTarget ) 
{
    ...
    { 
        if (pRenderTarget != 0)
            CG_ASSERT("Render Target index out of range.");
        
        return D3D_OK;
    }
    
    D3D9State::instance().setRenderTarget(RenderTargetIndex, (AISurfaceImp9 *) pRenderTarget);
...
HRESULT D3D_CALL AIDeviceImp9 :: GetRenderTarget (  DWORD RenderTargetIndex , IDirect3DSurface9** ppRenderTarget ) 
{
    ...
    if (RenderTargetIndex != 0)
        CG_ASSERT("Multiple render target support not yet implemented.");
        
    *ppRenderTarget = (IDirect3DSurface9 *) D3D9State::instance().getRenderTarget(RenderTargetIndex);
    (*ppRenderTarget)->AddRef();
...
HRESULT D3D_CALL AIDeviceImp9 :: SetDepthStencilSurface (  IDirect3DSurface9* pNewZStencil ) 
{
    ...
    D3D9State::instance().setZStencilBuffer((AISurfaceImp9 *) pNewZStencil);
...
HRESULT D3D_CALL AIDeviceImp9 :: GetDepthStencilSurface (  IDirect3DSurface9** ppZStencilSurface ) 
{
    ...
    *ppZStencilSurface = (IDirect3DSurface9 *) D3D9State::instance().getZStencilBuffer();    
...
HRESULT D3D_CALL AIDeviceImp9::Clear(DWORD Count, CONST D3DRECT* pRects, DWORD Flags, D3DCOLOR Color, float Z, DWORD Stencil) 
{
    ...
    D3D9State::instance().clear(Count, pRects, Flags, Color, Z, Stencil);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetTransform (  D3DTRANSFORMSTATETYPE State , CONST D3DMATRIX* pMatrix ) 
{
    ...
    D3D9State::instance().setTransform(State, pMatrix);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetViewport (  CONST D3DVIEWPORT9* pViewport ) 
{
    ...
    S32 x = pViewport->X >> 1;
    S32 y = pViewport->Y >> 1;
    U32 width = max(1, pViewport->Width >> 1);
    U32 height = max(1, pViewport->Height >> 1);
    D3D9State::instance().setViewport(x, y, width, height, pViewport->MinZ, pViewport->MaxZ);
#else    
    D3D9State::instance().setViewport(pViewport->X, pViewport->Y, pViewport->Width, pViewport->Height, pViewport->MinZ, pViewport->MaxZ);
#endif
...
HRESULT D3D_CALL AIDeviceImp9 :: SetMaterial (  CONST D3DMATERIAL9* pMaterial ) 
{
    ...
    D3D9State::instance().setMaterial(pMaterial);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetLight (  DWORD Index , CONST D3DLIGHT9* pLight ) 
{
    ...
    D3D9State::instance().setLight(Index, pLight);
...
HRESULT D3D_CALL AIDeviceImp9 :: LightEnable (  DWORD Index , BOOL Enable ) 
{
    ...
    D3D9State::instance().enableLight(Index, Enable);
...
HRESULT D3D_CALL AIDeviceImp9::SetRenderState (D3DRENDERSTATETYPE State , DWORD Value) 
{
    ...
    switch(State)
    {
        ...
        case D3DRS_TEXTUREFACTOR: 
            ...
            D3D9State::instance().setFixedFunctionState(State, Value);
            break;
        
        //case D3DRS_FOGENABLE:
        //case D3DRS_FOGCOLOR:
        //    D3D9State::instance().setFogState(State, Value);
        //    break;

        case D3DRS_CULLMODE:
            D3D9State::instance().setCullMode(Value);
            break;

        case D3DRS_ZENABLE:
        case D3DRS_ZFUNC:
        case D3DRS_ZWRITEENABLE:
            D3D9State::instance().setZState(State, Value);
            break;

        case D3DRS_ALPHATESTENABLE:
        case D3DRS_ALPHAREF:
        case D3DRS_ALPHAFUNC:
            D3D9State::instance().setAlphaTestState(State, Value);
            break;
        ...
        case D3DRS_BLENDFACTOR:
            D3D9State::instance().setBlendingState(State, Value);
            break;

        case D3DRS_SCISSORTESTENABLE:
            D3D9State::instance().enableScissorTest(Value);
            break;
        ...
        case D3DRS_CCW_STENCILFUNC:
            D3D9State::instance().setStencilState(State, Value);
            break;

        case D3DRS_COLORWRITEENABLE:
            D3D9State::instance().setColorWriteEnable(0, Value);
            break;

        case D3DRS_COLORWRITEENABLE1:
            D3D9State::instance().setColorWriteEnable(1, Value);
            break;

        case D3DRS_COLORWRITEENABLE2:
            D3D9State::instance().setColorWriteEnable(2, Value);
            break;

        case D3DRS_COLORWRITEENABLE3:
            D3D9State::instance().setColorWriteEnable(3, Value);
            break;

        case D3DRS_SRGBWRITEENABLE:
            D3D9State::instance().setColorSRGBWrite(Value);
            break;
...
HRESULT D3D_CALL AIDeviceImp9 :: SetTexture (  DWORD Stage , IDirect3DBaseTexture9* pTexture ) 
{
    ...
    D3D9State::instance().setTexture((AIBaseTextureImp9*)pTexture, Stage);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetTextureStageState (  DWORD Stage , D3DTEXTURESTAGESTATETYPE Type , DWORD Value ) 
{
    ...
    D3D9State::instance().setTextureStage(Stage, Type, Value);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetScissorRect (  CONST RECT* pRect ) 
{
    ...
    D3D9State::instance().scissorRect(pRect->left, pRect->top, pRect->right, pRect->bottom);
...
HRESULT D3D_CALL AIDeviceImp9::DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount) 
{
    ...
    D3D9State::instance().draw(PrimitiveType, StartVertex, PrimitiveCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: DrawPrimitiveUP (  D3DPRIMITIVETYPE PrimitiveType , UINT PrimitiveCount , CONST void* pVertexStreamZeroData , UINT VertexStreamZeroStride ) 
{
    ...
    UINT originalOffset;
    UINT originalStride;
    AIVertexBufferImp9* originalStream0 = D3D9State::instance().getVertexBuffer(0, originalOffset, originalStride);
    D3D9State::instance().setVertexBuffer(vertexBuffer, 0, 0, VertexStreamZeroStride);

    D3D9State::instance().draw(PrimitiveType, 0, PrimitiveCount);

    D3D9State::instance().setVertexBuffer(originalStream0, 0, originalOffset, originalStride);
...
HRESULT D3D_CALL AIDeviceImp9 :: DrawIndexedPrimitive (  D3DPRIMITIVETYPE Type , INT BaseVertexIndex , UINT MinVertexIndex , UINT NumVertices , UINT startIndex , UINT primCount ) 
{
    ...
    D3D9State::instance().drawIndexed(Type, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: DrawIndexedPrimitiveUP (  D3DPRIMITIVETYPE PrimitiveType , UINT MinVertexIndex , UINT NumVertices , UINT PrimitiveCount , CONST void* pIndexData , D3DFORMAT IndexDataFormat , CONST void* pVertexStreamZeroData , UINT VertexStreamZeroStride ) 
{
    ...
    AIIndexBufferImp9* originalIndexBuffer = D3D9State::instance().getIndexBuffer();
    D3D9State::instance().setIndexBuffer(indexBuffer);
    ...
    D3D9State::instance().drawIndexed(PrimitiveType, 0, MinVertexIndex, NumVertices, 0, PrimitiveCount);

    // Return indexBuffer and stream0 to the original state
    D3D9State::instance().setIndexBuffer(originalIndexBuffer);
    D3D9State::instance().setVertexBuffer(originalStream0, 0, originalOffset, originalStride);
...
HRESULT D3D_CALL AIDeviceImp9::SetVertexDeclaration(IDirect3DVertexDeclaration9* pDecl) 
{
    ...
    D3D9State::instance().setVertexDeclaration((AIVertexDeclarationImp9*)pDecl);
...
HRESULT D3D_CALL AIDeviceImp9 :: GetVertexDeclaration (  IDirect3DVertexDeclaration9** ppDecl ) 
{
    ...
    (*ppDecl) = D3D9State::instance().getVertexDeclaration();
...
HRESULT D3D_CALL AIDeviceImp9 :: SetVertexShader (  IDirect3DVertexShader9* pShader ) 
{
    ...
    D3D9State::instance().setVertexShader((AIVertexShaderImp9*)pShader);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetVertexShaderConstantF (  UINT StartRegister , CONST float* pConstantData , UINT Vector4fCount ) 
{
    ...
    D3D9State::instance().setVertexShaderConstant(StartRegister, pConstantData, Vector4fCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetVertexShaderConstantI (  UINT StartRegister , CONST int* pConstantData , UINT Vector4iCount ) 
{
    ...
    D3D9State::instance().setVertexShaderConstant(StartRegister, pConstantData, Vector4iCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetVertexShaderConstantB (  UINT StartRegister , CONST BOOL* pConstantData , UINT BoolCount ) 
{
    ...
    D3D9State::instance().setVertexShaderConstantBool(StartRegister, pConstantData, BoolCount);
...
HRESULT D3D_CALL AIDeviceImp9::SetStreamSource(UINT StreamNumber, IDirect3DVertexBuffer9* pStreamData, UINT OffsetInBytes, UINT Stride) 
{
    ...
    D3D9State::instance().setVertexBuffer((AIVertexBufferImp9*)pStreamData, StreamNumber, OffsetInBytes, Stride);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetStreamSourceFreq (  UINT StreamNumber , UINT Setting ) 
{
    ...
    if (Setting & D3DSTREAMSOURCE_INDEXEDDATA) {
        ...
        D3D9State::instance().setFreqType(StreamNumber, D3DSTREAMSOURCE_INDEXEDDATA);
        D3D9State::instance().setFreqValue(StreamNumber, (Setting & ~D3DSTREAMSOURCE_INDEXEDDATA));
        ...
        D3D9State::instance().setFreqType(StreamNumber, D3DSTREAMSOURCE_INSTANCEDATA);
        D3D9State::instance().setFreqValue(StreamNumber, (Setting & ~D3DSTREAMSOURCE_INSTANCEDATA));

    }
    else {

        D3D9State::instance().setFreqType(StreamNumber, 0);
        D3D9State::instance().setFreqValue(StreamNumber, Setting);
...
HRESULT D3D_CALL AIDeviceImp9::SetIndices(IDirect3DIndexBuffer9* pIndexData) 
{
    ...
    D3D9State::instance().setIndexBuffer((AIIndexBufferImp9*)pIndexData);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetPixelShader (  IDirect3DPixelShader9* pShader ) 
{
    ...
    D3D9State::instance().setPixelShader((AIPixelShaderImp9*)pShader);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetPixelShaderConstantF (  UINT StartRegister , CONST float* pConstantData , UINT Vector4fCount ) 
{
    ...
    D3D9State::instance().setPixelShaderConstant(StartRegister, pConstantData, Vector4fCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetPixelShaderConstantI (  UINT StartRegister , CONST int* pConstantData , UINT Vector4iCount ) 
{
    ...
    D3D9State::instance().setPixelShaderConstant(StartRegister, pConstantData, Vector4iCount);
...
HRESULT D3D_CALL AIDeviceImp9 :: SetPixelShaderConstantB (  UINT StartRegister , CONST BOOL* pConstantData , UINT BoolCount ) 
{
    ...
    D3D9State::instance().setPixelShaderConstantBool(StartRegister, pConstantData, BoolCount);
