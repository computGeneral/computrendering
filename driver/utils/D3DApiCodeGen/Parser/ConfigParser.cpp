...
ConfigParser::ConfigParser(ParserConfiguration& config, bool verbose) :
    m_config(config),
    m_verbose(verbose)
{
...
void ConfigParser::ParseApiHeaders()
{    
    try
    {
...
void ConfigParser::ParseApiHeader(const string& filename)
{
  if (!Utilities::FileSystem::FilenameExists(filename))
  {
    Exception e("could'nt open file '" + filename + "'");
    throw e;
  } 
  ...
  MacroExpander* eMacro = new MacroExpander(m_config.GetMacros(), true);
...
void ConfigParser::ReadFileData(const string& filename, string& cadena)
{
...
void ConfigParser::AddEnums(vector<EnumDescription>& enums)
{
...
void ConfigParser::AddStructs(vector<StructDescription>& structs)
{
...
void ConfigParser::AddClasses(vector<ClassDescription>& classes)
{
...
vector<ClassDescription>& ConfigParser::GetClasses()
{
  return *m_lstClasses;
}

vector<EnumDescription>& ConfigParser::GetEnums()
{
  return *m_lstEnums;
}

vector<StructDescription>& ConfigParser::GetStructs()
{
  return *m_lstStructs;
}
