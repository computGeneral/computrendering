...
AICubeTextureImp9::AICubeTextureImp9(AIDeviceImp9* _i_parent, UINT _EdgeLength,
    ...
    //D3D9State::instance().addCubeTexture(this);



    // Create a new GAL Texture Cube Map
    cubeTexture = D3D9State::instance().createCubeMap();
    ...
    D3D9State::instance().addTypeTexture((AIBaseTextureImp9*)this, D3D9_CUBETEXTURE);

    //  Set layout to render buffer.
    if ((Usage & D3DUSAGE_DEPTHSTENCIL) || (Usage & D3DUSAGE_RENDERTARGET))
    {
        cubeTexture->setMemoryLayout(libGAL::GAL_LAYOUT_RENDERBUFFER);
    }
    ...
    for (UINT i = 0; i < 6; i++) {
        ...
        for (UINT j = 0; j < numLevels; j++) {
            ...
            //D3D9State::instance().addCubeTextureMipLevel(this, i_mipmap, (D3DCUBEMAP_FACES)i, j, mip_width, Format);
            ...
            //cubeTextures[tex]->setData(getGALCubeMapFace(face), mipLevel, width, width, getGALFormat(format), //getSurfacePitch(width, format) 0, (libGAL::gal_ubyte*)data, size);
            cubeTexture->setData(D3D9State::instance().getGALCubeMapFace((D3DCUBEMAP_FACES)i), j, mip_width, mip_width, D3D9State::instance().getGALFormat(Format), 0, NULL, size);
...
libGAL::GALTextureCubeMap* AICubeTextureImp9::getGALCubeTexture() 
{
    return cubeTexture;
}
...
DWORD D3D_CALL AICubeTextureImp9 :: SetLOD (  DWORD LODNew ) 
{
    ...
    D3D9State::instance().setLOD((AIBaseTextureImp9*)this, LODNew);
