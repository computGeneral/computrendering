...
void GALxGLState::normalizeQuad(Quadf& qr)
{ 
...
GALxGLState::GALxGLState()
{
    memset(dirtyMat,0,sizeof(dirtyMat));
    
    // Init all matrices to identity  
    setMatrix(M_MODELVIEW, 0, GALxMatrixf::identity());
    setMatrix(M_PROJECTION, 0, GALxMatrixf::identity());

    int i;
    for ( i = 0;  i < MAX_TEXTURE_UNITS_ARB; i++ )
           setMatrix(M_TEXTURE, i, GALxMatrixf::identity());
...
bool GALxGLState::isMatrix(GLuint id)
{
    return (id >= VECTORS_SIZE);
}


void GALxGLState::setVector(const Quadf& data, VectorId id)
{
    if ( GLuint(id) < VECTORS_SIZE )
        vectorState[id] = data;
    else
        CG_ASSERT("State ID is not a Vector State");
}

void GALxGLState::setVector(const Quadf& data, VectorId id, GLubyte mask)
{
    if ( GLuint(id) >= VECTORS_SIZE )
        CG_ASSERT("State ID is not a Vector State");
...
void GALxGLState::setVectorGroup(const Quadf& data, VectorId id, GLuint unit)
{
    GLuint i;
    switch ( id )
    {
        ...
        case V_LIGHT_SPOT_DIRECTION:
            if ( unit >= MAX_LIGHTS_ARB )
                CG_ASSERT("Light unit does not exist (too high)");
            i = 7*unit + GLuint(id);
            setVector(data, VectorId(i));
            break; // OK
        case V_LIGHT_HALF:
            CG_ASSERT("HALF is derivative, cannot be set directly");
            break;
        // TEXTURE GENERATION 
        case V_TEXGEN_EYE_S:
        case V_TEXGEN_EYE_T:
        ...
        case V_TEXGEN_OBJECT_Q:
            if ( unit >= MAX_TEXTURE_UNITS_ARB )
                CG_ASSERT("Texture unit does not exist (too high)");
            i = 8*unit + GLuint(id);
            setVector(data, VectorId(i));
            break;
        case V_TEXENV_COLOR:
            ...
            break;
        default:
            CG_ASSERT("This vector does not pertain to a unit or cannot be set directly (derivative)");            
    }       
}

void GALxGLState::setVectorGroup(const Quadf& data, VectorId id, GLuint unit,GLubyte mask)
{    
    GLuint i;
    switch ( id )
    {
        ...
        case V_LIGHT_SPOT_DIRECTION:
            if ( unit >= MAX_LIGHTS_ARB )
                CG_ASSERT("Unit does not exist (too high)");
            i  = 7*unit + GLuint(id);
...
void GALxGLState::setMatrix(const GALxMatrixf& data, GLuint id)
{
    // do not allow to modify derivative matrices directly 
    if ( id % 4 != 0 )
        CG_ASSERT("It is not possible to modify a derivative matrix");

    if ( id == STATE_MATRIX_MVP_OFFSET )
        // MVP is fully derivative Modelview*projection 
        CG_ASSERT("MVP matrix can not be updated, it derives from Modelview & Projection");   
...
void GALxGLState::checkMatrix(GLuint id) const
{
    if ( dirtyMat[id] )
    {
        GALxMatrixf& m = matrixState[id]; // get a reference to the target matrix 
        ...
        if ( generative == M_MVP )
        {
            ...
            // Compute MVP 
            GALxMatrixf proj = getMatrix(M_PROJECTION, MT_NONE, 0);
            GALxMatrixf mview = getMatrix(M_MODELVIEW, MT_NONE, 0);
            m = proj * mview;
            dirtyMat[id] = true;
        }
        else 
        ...
        switch ( id % 4 )
        {
            ...
            case MT_NONE:
                if ( generative != M_MVP )
                    CG_ASSERT("Matrix that cannot be dirtyt!");
                break;
            case MT_INVERSE:
                m.inverseMe();
                break;
            ...
            case MT_INVTRANS:
                ...
                break;
            default:
                CG_ASSERT("Implementation error: see 'switch(id % 4)' line");
        }

    }
}

const GALxMatrixf& GALxGLState::getMatrix(GLuint id) const

{
    if ( id >= VECTORS_SIZE + MATRICES_SIZE )
        CG_ASSERT("Access out of range");
...
const GALxMatrixf& GALxGLState::getMatrix(MatrixId mid, MatrixType mtype, GLuint unit) const
{
    if ( M_MODELVIEW > mid || mid > M_PROGRAM )
    {
        CG_ASSERT("Matrix identifier unknown");
    }
    ...
    switch ( mid )
    {
        ...
        case M_MODELVIEW:
            if ( unit >= MAX_VERTEX_UNITS_ARB )
                CG_ASSERT("VERTEX_UNIT not available (M_MODELVIEW)");
            break;
        case M_TEXTURE:
            if ( unit >= MAX_TEXTURE_UNITS_ARB )
                CG_ASSERT("TEXTURE_UNIT not available (M_TEXTURE)");
            break;
        case M_PALETTE:
            if ( unit >= MAX_PALETTE_MATRICES_ARB )
                CG_ASSERT("PALETTE_UNIT not available (M_PALETTE)");
            break;
        case M_PROGRAM:
            if ( unit >= MAX_PROGRAM_MATRICES_ARB )
                CG_ASSERT("PROGRAM_UNIT not available (M_PROGRAM)");
            break;
        default:
            CG_ASSERT("Unknown matrix identifier");
    }
...
void GALxGLState::setMatrix(MatrixId mid, GLuint unit, const GALxMatrixf& mat)
{
    if ( M_MODELVIEW > mid || mid > M_PROGRAM )
        CG_ASSERT("Matrix identifier unknown");
    ...
    switch ( mid )
    {
        ...
        case M_PALETTE:
            if ( unit >= MAX_PALETTE_MATRICES_ARB )
                CG_ASSERT("PALETTE_MATRIX not available (M_PALETTE)");
            break;
        case M_PROGRAM:
            if ( unit >= MAX_PROGRAM_MATRICES_ARB )
                CG_ASSERT("PROGRAM_MATRIX not available (M_PROGRAM)");
            break;
        default:
            CG_ASSERT("Matrix identifier unknow (switch(mid) )");
    }
...
const Quadf& GALxGLState::getVector(GLuint id) const
{
    if ( id >= VECTORS_SIZE )
        CG_ASSERT("Trying to get a vector out of range");
        ...
        if ( dirtyVec[id] ) // recompute.... 
        {
            ...
            switch ( state )
            {
                ...
                case 5: // LIGHTPROD.unit.back.specular
                    ...
                    break;
                default:
                    CG_ASSERT("Should not happen ever (error programming?)");
            }
...
void GALxGLState::dump(ostream& os) const
{   
    int i;
...
void GALxGLState::printMatrix(ostream& os, MatrixId matId_) const
{
