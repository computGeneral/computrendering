/**************************************************************************
 *
 * cmoStreamController Loader class implementation file.
 *
 */
...
StreamerLoader::StreamerLoader(U32 unitId, U32 idxCycle, U32 irqSize, U32 attrCycle,
    U32 lines, U32 lineSz, U32 portWidth, U32 cacheRequests, U32 cacheInputReqs,
    bool forceAttrLoadBypass, U32 shNumber, char **shPrefixes,
    ...
    unitID(unitId), cmoMduBase(name, parent)

{
    ...
    GPU_ASSERT(
        ...
        if (indicesCycle == 0)
            CG_ASSERT("At least an index must be received per cycle.");

        if (attributesCycle == 0)
            CG_ASSERT("At least an attribute must be filled per cycle.");

        if (indicesCycle > IRQSize)
            CG_ASSERT("The input request queue must be at least indices per cycle large.");
    ...
    CG_ASSERT_COND(!(shaderState == NULL), "Error allocating the shader state array.");
    ...
    //  Create command signal from the main cmoStreamController mdu.  
	...
	atoi((const char *)&prefix[2]), //  The StreamController loader unit identifier.  
    ...
    //  Check memory allocation.  
    CG_ASSERT_COND(!(inputRequestQ == NULL), "Error allocating memory for the Input Request Queue.");

    //  Create statistics.  
    indices = &getSM().getNumericStatistic("Indices", U32(0), "StreamerLoader", prefix);
    transactions = &getSM().getNumericStatistic("MemTransactions", U32(0), "StreamerLoader", prefix);
    bytesRead = &getSM().getNumericStatistic("ReadBytes", U32(0), "StreamerLoader", prefix);
    fetches = &getSM().getNumericStatistic("Fetches", U32(0), "StreamerLoader", prefix);
    noFetches = &getSM().getNumericStatistic("NoFetches", U32(0), "StreamerLoader", prefix);
    reads = &getSM().getNumericStatistic("Reads", U32(0), "StreamerLoader", prefix);
    noReads = &getSM().getNumericStatistic("NoReads", U32(0), "StreamerLoader", prefix);
    inputs = &getSM().getNumericStatistic("Inputs", U32(0), "StreamerLoader", prefix);
    splitted = &getSM().getNumericStatistic("SplittedAttributes", U32(0), "StreamerLoader", prefix);
    mapAttributes = &getSM().getNumericStatistic("MappedAttributes", U32(0), "StreamerLoader", prefix);
    avgVtxSentThisCycle = &getSM().getNumericStatistic("AvgVtxSentThisCycle", U32(0), "StreamerLoader", prefix);
    stallsShaderBusy = &getSM().getNumericStatistic("StallsShaderBusy", U32(0), "StreamerLoader", prefix);
...
//  cmoStreamController Loader simulation function.  
void StreamerLoader::clock(U64 cycle)
{
    ...
    for (i = 0; i < numShaders; i++)
    {
        ...
        {
            //  Something got lost.  
            CG_ASSERT("Missing state signal from a Shader.");
        }
...
//    getName(),cycle, freeInputs, loadInputs, readInputs, fetchInputs);
    ...
    switch(state)
    {
        ...
        case ST_READY:
            //  Ready state.  Wait for command from the cmoStreamController main mdu.  

            GPU_DEBUG_BOX( printf("%s => READY state.\n", getName()); )

            //  Process commands from the cmoStreamController main mdu.  
        ...
        case ST_STREAMING:
            /*  Streaming state.  Wait for new indexes to load
                from the StreamerOutputCache mdu.  */
            ...
            for(i = 0, visited = 0; (i < indicesCycle) && (loadInputs > 0); i++)
            {
                ...
                if (visited != numShaders)
                {
                    ...
                    iaux = new Vec4FP32[MAX_VERTEX_ATTRIBUTES];

                    //  Check memory allocation.  
                    CG_ASSERT_COND(!(iaux == NULL), "Input buffer could not be allocated.");
                    ...
                    {
                        CG_ASSERT_COND(!(!inputRequestQ[nextLoadInput].stCCom->isLast()), "Sending a hit to the shader that wasn't the last input.");
                    ...
                    //  Copy cookies from the new input StreamController control command that produced the current input/index.  
                    ...
                    //  Set the StreamController loader unit identifier.  
                    ...
                    //  Send deallocation signal to the cmoStreamController Fetch.  
                ...
                if (readInputs > 0)
                {
                    ...
                    if (nextAttribute == MAX_VERTEX_ATTRIBUTES)
                    {
                        ...
                        if (attributeLoadBypass)
                        {
                            ...
                            *((U32 *) &indexAux) = inputRequestQ[nextReadInput].index;
                            *((U32 *) &instanceAux) = inputRequestQ[nextReadInput].instance;
            ...
            for(i = 0; i < attributesCycle; i++)
            {
                ...
                if (fetchInputs > 0)
                {
                    ...
                    if (nextAttribute == MAX_VERTEX_ATTRIBUTES)
                    {
                        ...
                        {
                            CG_ASSERT_COND(!(!inputRequestQ[nextFetchInput].stCCom->isLast()), "Only last index/input can be a hit.");
            ...
            //  Read new indexes from the cmoStreamController Output Cache.  
            while(streamerNewIndex->read(cycle, (DynamicObject *&) streamCCom))
            {
                //  Check the input request queue is not full.  
                CG_ASSERT_COND(!(freeInputs == 0), "No empty input request queue entries.");
                //  Check that the new index was sent to the proper unit  
                CG_ASSERT_COND(!(streamCCom->getUnitID() != unitID), "Received input index of a different StreamController loader unit.");
                ...
                if (streamCCom->isAHit() && (!streamCCom->isLast()))
                {
                    ...
                    //  Delete StreamController command.  
                    delete streamCCom;
                }
                else
                {
                    ...
                    //  Store the StreamController control command that carries the new index..  
                    ...
                    {
                        CG_ASSERT_COND(!(!streamCCom->isLast()), "Only last index is queued when there is a hit.");
        ...
        default:
            CG_ASSERT("Unsupported StreamController state.");
            break;
    }
...
void StreamerLoader::processStreamerCommand(StreamerCommand *streamCom)
{
    ...
    switch(streamCom->getCommand())
    {
        ...
        case STCOM_END:
            ...
            //  Check cmoStreamController state.  
            CG_ASSERT_COND(!(state != ST_STREAMING), "Command not allowed in this state.");
...
void StreamerLoader::processGPURegisterWrite(GPURegister gpuReg, U32 gpuSubReg,
     GPURegData gpuData)
{
...
F32 StreamerLoader::attributeDataConvert(StreamData format, U08 *data)
{
    F32 res;
...
void StreamerLoader::configureAttributes(U64 cycle, InputRequest *input)
{
    U32 stBuff;
    U32 dataSize;
    U32 offset;
    ...
    for(i = 0; i < MAX_VERTEX_ATTRIBUTES; i++)
    {
        ...
        if (attributeMap[i] != ST_INACTIVE_ATTRIBUTE)
        {
            ...
            CG_ASSERT_COND(!(streamElements[stBuff] == 0), "Zero active elements are not allowed for defined attributes.");
            U32 readIndex;
            bool needsRead;
            ...
            if (needsRead)
            {
                ...
                dataSize = U32(ceil(F32(streamElements[stBuff] * streamDataSize[streamData[stBuff]]) / 4.0f)) * 4;
...
void StreamerLoader::loadAttribute(InputRequest *input, U32 attr, U08 *data)
{
    StreamData format;
    U32 elemSize;
    U32 offset;
    ...
    if (streamFrequency[attributeMap[attr]] != 0)
    {
        ...
        if (input->size[attr][0] != 0)
        {
            ...
            for(U32 b = 0; b < 8; b++)
                *((U32 *) &instanceAttribData[attr][b * 4]) = *((U32 *) &data[b * 4]);                           
        }
        else
        {
            //  Get the data from the per instance attribute cache.
            for(U32 b = 0; b < 8; b++)
                *((U32 *) &data[b * 4]) = *((U32 *) &instanceAttribData[attr][b * 4]);
        }
    }
    ...
    if (d3d9ColorStream[attributeMap[attr]])
    {
        ...
        F32 red = input->attributes[attr][2];
        F32 green = input->attributes[attr][1];
        F32 blue= input->attributes[attr][0];
        F32 alpha = input->attributes[attr][3];
