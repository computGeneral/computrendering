 ...
 * accessing the ROP data buffer from a Generic ROP mdu.
 *
 */
...
ROPCache::ROPCache(U32 ways, U32 lines, U32 lineSz,
    ...
    U32 outReqs, bool comprDisabled, U32 numStampUnits_, U32 stampUnitStride_,
    U32 maxColorBlocks, U32 blocksPerCycle,
    U32 compCycles, U32 decompCycles, GPUUnit gpuUnit, char *nameCache, char *postfix) :
...

{
    ...
    GPU_ASSERT(
        ...
        if (numStampUnits == 0)
            CG_ASSERT("The number of stamp units must be at least 1.");
        if (stampUnitStride == 0)
            CG_ASSERT("The cache block stride per stamp unit must be at least 1.");
        //if (maxBlocks < ((MAX_DISPLAY_RES_X * MAX_DISPLAY_RES_Y) / (lineSize >> 2)))
        //    CG_ASSERT("Block state memory too small for the highest supported resolution.");
        if (blocksCycle == 0)
    ...
    //  Create statistics.
    noRequests = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("NoNewRequests", U32(0), "RopCache", postfix);
    writeQFull = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("WriteReqQFull", U32(0), "RopCache", postfix);
    readQFull = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ReadReqQFull", U32(0), "RopCache", postfix);
    rwQFull = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("RWReqFull", U32(0), "RopCache", postfix);
    waitWriteStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("WaitWriteStall", U32(0), "RopCache", postfix);
    writeReqQueued = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("WriteReqAccepted", U32(0), "RopCache", postfix);
    readReqQueued = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ReadReqAccepted", U32(0), "RopCache", postfix);
    readInQEmpty = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ReadInputQEmpty", U32(0), "RopCache", postfix);
    decomprBusy = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("DecompressorBusy", U32(0), "RopCache", postfix);
    waitReadStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ReadInputWaitReadStall", U32(0), "RopCache", postfix);
    writeOutQEmpty = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("WriteOutQEmpty", U32(0), "RopCache", postfix);
    comprBusy = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("CompressorBusy", U32(0), "RopCache", postfix);
    memReqStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("MemReqStall", U32(0), "RopCache", postfix);
    warStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("CacheLineWARStall", U32(0), "RopCache", postfix);
    dataBusBusy = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("DataBusBusy", U32(0), "RopCache", postfix);
    writePortStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("WriteLinePortStall", U32(0), "RopCache", postfix);
    readPortStall = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ReadLinePortStall", U32(0), "RopCache", postfix);

    comprBlocks64 = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ComprBlock64", U32(0), "RopCache", postfix);
    comprBlocks128 = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ComprBlock128", U32(0), "RopCache", postfix);
    comprBlocks192 = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ComprBlock192", U32(0), "RopCache", postfix);
    comprBlocks256 = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("ComprBlock256", U32(0), "RopCache", postfix);
    ...
    blockStateBuffer = new U08[maxBlocks >> 1];

    //  Check allocation.
    CG_ASSERT_COND(!(blockStateBuffer == NULL), "Error allocating the block state buffer (encoded data).");
...
bool ROPCache::fetch(U32 address, U32 &way, U32 &line, DynamicObject *source, U32 reserves)
{
...
bool ROPCache::allocate(U32 address, U32 &way, U32 &line, DynamicObject *source, U32 reserves)
{
    U32 i;
    U32 e;
    U32 block;
...
bool ROPCache::read(U32 address, U32 way, U32 line, U32 size, U08 *data)
{
...
bool ROPCache::write(U32 address, U32 way, U32 line, U32 size, U08 *data, bool *mask)
{
    //  Search for the next free write port.
    ...
    if ((nextWritePort < writePorts) && (writeCycles[nextWritePort] == 0))
    {
        ...
        if (cache->write(address, way, line, size, data, mask))
        {
            ...
            writeCycles[nextWritePort] += (U32) ceil((F32) size / (F32) portWidth);
...
bool ROPCache::write(U32 address, U32 way, U32 line, U32 size, U08 *data)
{
    //  Search for the next free write port.
    for(; (nextWritePort < writePorts) && (writeCycles[nextWritePort] > 0); nextWritePort++);
...
void ROPCache::unreserve(U32 way, U32 line)
{
...
//U08 *p = memTrans->getData();
//for(U32 dw = 0; dw < (lastSize >> 2); dw++)
//printf("%08x ", ((U32 *) p)[dw]);
//printf("\n");
//}
...
MemoryTransaction *ROPCache::update(U64 cycle, MemState memState)
{
...
bool ROPCache::resetState()
{
    ...
    if (!resetStateRequest)
    {
        ...
        resetStateCycles = (U32) ceil((F32) maxBlocks / (F32) blocksCycle);
    }
    else
    {
...
void ROPCache::swap(U32 bufferAddress)
{
...
void ROPCache::setBytesPerPixel(U32 bytes)
{
...
void ROPCache::setMSAASamples(U32 samples)
{
    msaaSamples = samples;
}
...
void ROPCache::setStateAddress(U32 address)
{
...
//    ((block.state == ROPBlockState::COMPRESSED) ? "COMPR" : "UNCOM")));
            ...
            switch(block.state)
            {
                ...
                default:
                    ...
                    CG_ASSERT("Unsupported ROP data block state mode.");
            }
        }
    }
                ...
                case ROPBlockState::CLEAR:
                    ...
                    for(i = 0; i < lineSize; i += bytesPixel)
                        for(U32 j = 0; j < (bytesPixel >> 2); j++)
                            *((U32 *) &inputBuffer[nextRead][i + j * 4]) = ((U32 *) clearROPValue)[j];
...
printf("%s%02d => Block marked as CLEAR and masked.\n", ropCacheName, cacheID);
            ...
            for(i = 0; i < lineSize; i += bytesPixel)
            {
                for(U32 j = 0; j < (bytesPixel >> 2); j++)
                {
                    ...
                    *((U32 *) &outputBuffer[nextWrite][i + j * 4]) =
                        ((*((U32 *) &outputBuffer[nextWrite][i + j * 4])) & maskBuffer[nextWrite][(i >> 2) + j]) |
                        (((U32 *) clearROPValue)[j] & ~maskBuffer[nextWrite][(i >> 2) + j]);
...
//(nextWriteBlock.state == ROPBlockState::CLEAR) ? "CLEAR" : (nextWriteBlock.state == ROPBlockState::COMPRESSED ? "COMPR" : "UNCOMPR"));
        ...
        default: CG_ASSERT("Unsupported compressor block size."); break;
        }
...
MemoryTransaction *ROPCache::requestBlock(U64 cycle, U32 readReq)
{
    U32 size;
    U32 offset;
    MemoryTransaction *memTrans;
...
//  Requests a compressed block/line of data.
MemoryTransaction *ROPCache::writeBlock(U64 cycle, U32 writeReq)
{
    U32 size;
    U32 offset;
    MemoryTransaction *memTrans;
        ...
        && ((memoryState & MS_WRITE_ACCEPT) != 0))
    {
        ...
        if (writeQueue[writeReq].request->masked)
        {
            ...
            for (U32 dw = 0; (dw < (size >> 2)) && wholeBlockMasked; dw ++)
...
printf("%s%02d (%lld) => MT_WRITE_DATA (masked) addr %x size %d.\n", ropCacheName, cacheID, cycle,
    writeQueue[writeReq].address + offset, size);
printf("data = ");
for(U32 dw = 0; dw < (size >> 2); dw++)
printf("%08x ", ((U32 *) &outputBuffer[writeReq][offset])[dw]);
printf("\n");
printf("mask = ");
for(U32 dw = 0; dw < (size >> 2); dw++)
...
U32 ROPCache::addressToBlockSU(U32 address)
{
    //  Check if compression is enabled.
    if (compression)
    {
...
U32 ROPCache::addressToBlock(U32 address)
{
    //  Check if compression is enabled.
    if (compression)
    {
...
void ROPCache::saveBlockStateMemory()
{
    ...
    if (!out.is_open())
    {
        CG_ASSERT("Error creating cache block state memory snapshot file.");
    }
...
void ROPCache::loadBlockStateMemory()
{
    ...
    if (!in.is_open())
    {
        CG_ASSERT("Error opening cache block state memory snapshot file.");
    }
...
void ROPCache::encodeBlocks(U08 *data, U32 blocks)
{
    U32 encodedBlockState[8];
    U32 encodedBlocks;
    ...
    if ((blocks & 0x07) != 0)
    {
        ...
        //  Store the packed block state data.
        ((U32 *) data)[(blocks >> 3) + 1] = encodedBlocks;
    }    
}


//  Binary decode and fill the block state data.
void ROPCache::decodeAndFillBlocks(U08 *data, U32 blocks)
{
...
/*printf("   block %06x - %06x => (%s %d) (%s %d) (%s %d) (%s %d) (%s %d) (%s %d) (%s %d) (%s %d)\n",
    ...
    if ((blocks & 0x07) != 0)
    {
        ...
        packedBlockData = ((U32 *) data)[(blocks >> 3) + 1];
...
ROPBlockState::State ROPCache::decodeState(U32 state)
{
    switch(state)
    {
        ...
        default:
            CG_ASSERT("Undefined block state.");
...
//  Update save state machine.
void ROPCache::updateSaveState(U64 cycle)
{
    U32 size;
    U32 address;
    MemoryTransaction *memTrans;
    ...
    if (savedBlocks != maxBlocks)
    {
        ...
        if ((!memoryWrite) && (!memoryRead) && (freeTickets > 0) && ((memoryState & MS_WRITE_ACCEPT) != 0))
        {
            ...
            U32 padding = MAX_TRANSACTION_SIZE - GPU_MOD((maxBlocks >> 1), MAX_TRANSACTION_SIZE);
...
//  Update restore state machine.
void ROPCache::updateRestoreState(U64 cycle)
{
    U32 size;
    U32 address;
    MemoryTransaction *memTrans;
...
void ROPCache::updateResetState(U64 cycle)
{
...
void ROPCache::stallReport(U64 cycle, string &stallReport)
{
    ...
    for(U32 port = 0; port < writePorts; port ++)
