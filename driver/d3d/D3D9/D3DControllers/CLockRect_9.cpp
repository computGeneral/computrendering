...
void CLockRect9::on_write_node_data(StateDataNode* node, size_t size, unsigned int offset)  {
    ...
    if(node->get_id().name == NAME_DATA) {
        if(node->get_parent()->get_id().name == NAME_SURFACE_9) {
            ...
            GPUProxy::get_instance()->writeMemory(md, offset, (U08*)(data), size);
            node->unmap_data();
            ...
            U32 width2 = ceiledLog2(surf_width);

            /*for(UINT i = 0; i < lock_height; i ++) {
                for(UINT j = 0; j < lock_width; j ++) {
                    U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
            ...
            switch(pixel_size)
            {
                case 1:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            ((U08 *) surf_data)[m] = ((U08 *) lock_data)[(i * lock_width + j)];
                        }
                    }
                    break;
                ...
                case 2:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            ((U16 *) surf_data)[m] = ((U16 *) lock_data)[(i * lock_width + j)];
                        }
                    }
                    break;
                ...
                case 4:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            ((U32 *) surf_data)[m] = ((U32 *) lock_data)[(i * lock_width + j)];
                        }
                    }
                    break;
                ...
                case 8:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            ((U64 *) surf_data)[m] = ((U64 *) lock_data)[(i * lock_width + j)];
                        }
                    }
                    break;
                ...
                case 16:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            ((U64 *) surf_data)[m * 2] = ((U64 *) lock_data)[(i * lock_width + j) * 2];
                            ((U64 *) surf_data)[m * 2 + 1] = ((U64 *) lock_data)[(i * lock_width + j) * 2 + 1];
                        }
                    }
                    break;
                ...
                default:
                    for(UINT i = 0; i < lock_height; i ++)
                    {
                        ...
                        {
                            U32 m = texel2MortonAddress(lock_left + j ,lock_top + i , block_dim, s_block_dim, width2);
                            for(U32 b = 0; b < pixel_size; b++)
                                ((U08 *) surf_data)[m * pixel_size + b] = ((U08 *) lock_data)[(i * lock_width + j) * pixel_size + b];
                         }
                    }
                    break;
            }
