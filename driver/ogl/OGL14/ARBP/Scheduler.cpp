...
vector<InstructionInfo*>* Scheduler::extractInstructionInfo(const list<cgoShaderInstr*>& originalCode)
{
    vector<InstructionInfo*>* returnedList = new vector<InstructionInfo*>;

    ShaderArchParam *shArchParams = ShaderArchParam::getShaderArchParam();

    unsigned int instructionCounter = 0;

    list<cgoShaderInstr*>::const_iterator iter = originalCode.begin();
    ...
    while( iter != originalCode.end() )
    {
            ...
            char instr[256];
            (*iter)->disassemble(instr, MAX_INFO_SIZE);
            ii->str = string(instr);
    ...
    while( iter != code.end() )
    {
        ...
        if ((*iter)->operation.writesAnyRegister)
        {
            ...
            else if ((*iter)->writeReg.isAddressRegister())
            {
                ...
                CG_ASSERT("Instruction writes a register that is not a temporal, and output or an address register");
        }
       ...
       // Check for true dependencies of instruction read operands with last register writer 
        ...
        /* Check for true dependencies with relative address registers writers, namely, ARL�s. */
        ...
        /* Push back current instruction as last register writer. It�s necessary to do it here because we
...
list<cgoShaderInstr*> Scheduler::createNewCode(const vector<InstructionInfo*>& newcode) const
{
    list<cgoShaderInstr*> returned;
    ...
    while( iter != newcode.end() )
    {
        ...
        returned.push_back(new cgoShaderInstr((ShOpcode)(*iter)->operation.opcode,
...
list<cgoShaderInstr*> Scheduler::optimizeCode(list<cgoShaderInstr*>& originalCode, unsigned int nway, unsigned int temporaries, unsigned int outputRegs, unsigned int addrRegs, bool _reorderCode)
{   
    //************************************************************
    ...
    //******************************
    list<cgoShaderInstr*> returnedCode = createNewCode(*code);
...
U32 Scheduler::getMaxAliveTemps() const
{
    if ( maxAliveTempsComputed )
        return maxAliveTemps;
    CG_ASSERT("Optimize code must be called before call this method");
    return 0;
}
