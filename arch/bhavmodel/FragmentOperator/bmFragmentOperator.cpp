/**************************************************************************
 *
 * Fragment Operation Behavior Model class implementation file.
 *
 */

/**
 *
 *  @file bmoFragmentOperator.cpp
 *
 *  This file implements Fragment Behavior Model Operation class.
 *
...
bmoFragmentOperator::bmoFragmentOperator(U32 stampFrags)
{
    ...
    stencil = new U08[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(stencil == NULL), "Error allocating stamp stencil buffer.");
    //  Allocate stamp z buffer values buffer.  
    depth = new U32[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(depth == NULL), "Error allocating the stamp z buffer values buffer.");
    //  Allocate the stamp stencil test result array.  
    stencilResult = new bool[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(stencilResult == NULL), "Error allocating the stamp stencil test result buffer.");
    //  Allocate the stamp z test result buffer.  
    zResult = new bool[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(zResult == NULL), "Error allocating stamp z test result buffer.");
    //  Allocate the blend factor arrays.  
    sFactor = new Vec4FP32[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(sFactor == NULL), "Error allocating source factor array.");
    dFactor = new Vec4FP32[stampFragments];

    //  Check allocation.  
    CG_ASSERT_COND(!(dFactor == NULL), "Error allocating destination factor array.");
    //  Set initial state.  
    depthTest = FALSE;
    stencilTest = FALSE;
...
void bmoFragmentOperator::setStencilTest(bool enable)
{
    //  Set stencil test.  
    stencilTest = enable;
}
...
void bmoFragmentOperator::setZTest(bool enable)
{
    //  Set z test.  
    depthTest = enable;
}
...
void bmoFragmentOperator::configureStencilTest(CompareMode stencilFunc, U08 ref,
        U08 testMask, U08 updateMask, StencilUpdateFunction stnclFail,
...
void bmoFragmentOperator::configureZTest(CompareMode zFunc, bool zMask)
{
...
void bmoFragmentOperator::setBlending(U32 rt, BlendEquation eq, BlendFunction sRGB,
    BlendFunction sA, BlendFunction dRGB, BlendFunction dA, Vec4FP32 color)
{
...
void bmoFragmentOperator::setLogicOpMode(LogicOpMode mode)
{
    logicOpMode = mode;
}

//  Perform Stencil and Z test for a stamp.  
void bmoFragmentOperator::stencilZTest(U32 *stampZ, U32 *bufferZ,
    bool *stampCull)
{
    U32 i;
...
void bmoFragmentOperator::blend(U32 rt, Vec4FP32 *color, Vec4FP32 *source,
    Vec4FP32 *destination)
{
    U32 i;
...
void bmoFragmentOperator::logicOp(U08 *color, U08 *source, U08 *destination)
{
    U32 i;
    ...
    switch(logicOpMode)
    {
        ...
        default:
            CG_ASSERT("Unsupported logical operation mode.");
            break;
    }
}
...
void bmoFragmentOperator::factorRGB(BlendFunction bf, Vec4FP32 constColor, Vec4FP32 *source,
    Vec4FP32 *destination, Vec4FP32 *factor)
{
    ...
    switch(bf)
    {
        ...
        default:
            CG_ASSERT("Unsupported blend factor function.");
            break;
    }
...
void bmoFragmentOperator::factorAlpha(BlendFunction bf, Vec4FP32 constColor, Vec4FP32 *source,
    Vec4FP32 *destination, Vec4FP32 *factor)
{
    U32 i;
    switch(bf)
...
void bmoFragmentOperator::updateStencil(StencilUpdateFunction func,
    U08 stencilVal, U32 &bufferVal)
{
    U08 stencilAux;
...
void bmoFragmentOperator::compare(CompareMode func, U08 *value,
    U08 ref, bool *result)
{
    U32 i;

//printf("bmoFragmentOperator::compare -> func %d value %p ref %x result %p stencilTest %s\n", func, value, ref, result,
//    stencilTest?"T":"F");
...
void bmoFragmentOperator::compare(CompareMode func, U32 *value,
    U32 *ref, bool *result)
{
    U32 i;
...
bool bmoFragmentOperator::compare(CompareMode func, U08 value, U08 ref)
{
    switch(func)
    {
        case GPU_NEVER:
...
//printf("compare(stencil) => mask (%x) | ref(%x) > value(%x) -> %s\n", stencilTestMask, ref, value, ((ref & stencilTestMask) > (value & stencilTestMask))?"T":"F");
        ...
        default:
            CG_ASSERT("Undefined compare mode.");
            break;
    }
    return false;
}
...
bool bmoFragmentOperator::compare(CompareMode func, U32 value, U32 ref)
{
    switch(func)
    {
        case GPU_NEVER:
...
void bmoFragmentOperator::blockMaxZ(U32 *input, U32 size, U32 &maxZ)
{
    ...
    for(U32 i = 0; i < size; i++)
    {
        ...
        U32 d = input[i] & 0x00ffffff;
        maxZ = d > maxZ ? d : maxZ;
    }
}

//  Calculates the z and z-stencil minimum and maximum.  
void bmoFragmentOperator::blockMinMaxZ(U32 *input, U32 size,
    U32 &minZ, U32 &maxZ, U32 &min, U32 &max)
{
    U32 i;
...
void bmoFragmentOperator::blockMinMax(U32 *input, U32 size,
    U32 &min, U32 &max)
{
    U32 i;
...
bmoFragmentOperator::CompressionMode bmoFragmentOperator::hiloCompress(
    U32 *input, U08 *output,
    ...
    U32 loMaskL1, U32 min, U32 max)
{
    ...
    U32 byteOff;
    U32 bitOff;
    U16 loBitsL0[MAX_COMPR_FRAGMENTS];
    U08 loBitsL1[MAX_COMPR_FRAGMENTS];
    bool level0;
    bool level1;
    U32 aux;
...
bool bmoFragmentOperator::testCompression(U32 a, U32 b, U32 min,
    U32 max, U32 hiMask, U32 value, bmoFragmentOperator::ReferenceValue &ref)
{
    bool compressable;
...
void bmoFragmentOperator::hiloUncompress(U08 *input, U32 *output,
    U32 size, CompressionMode level, U32 hiMaskL0, U32 hiMaskL1,
    U32 loShiftL0, U32 loShiftL1)
{
    U32 ref[4];
    U32 min;
    U32 max;
...
//printf("===>> Low bits %x\n", lowBits);
                ...
                if ((bitOff > 7) && ((i + 1) < size))
                {
                    ...
                    aux = aux | (((U32) input[byteOff]) << bitOff);
...
//printf("===>> Low bits %x\n", lowBits);
                ...
                if ((bitOff > 15) && ((i + 1) < size))
                {
                    ...
                    aux = aux | (((U32) input[byteOff]) << (8 + bitOff)) | (((U32) input[byteOff + 1]) << bitOff);
...
//printf("===> Output %d : %x\n", i, output[i]);
        ...
        default:
            CG_ASSERT("Undefined compression level.");
            break;
    }
}
...
void bmoFragmentOperator::compressionSwizzle(U32 *input, U32 *output, U32 size, U32 regions,
