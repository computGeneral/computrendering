    ...
    
{
    ...
    //D3D9State::instance().addTexture2D(this, Usage);


    // Create a new GAL Texture 2D
    texture2d = D3D9State::instance().createTexture2D();
    ...
    {
        texture2d->setMemoryLayout(libGAL::GAL_LAYOUT_RENDERBUFFER);
    ...
    D3D9State::instance().addTypeTexture((AIBaseTextureImp9*)this, D3D9_TEXTURE2D);
    ...
    for (UINT i = 0; i < numLevels; i++)
    {
        ...
        //D3D9State::instance().addMipLevel(this, i_mipmap, i, mip_width, mip_height, Format);
        ...
        //textures2D[tex]->setData(mipLevel, width, height, getGALFormat(format), //getSurfacePitch(width, format) 0, (libGAL::gal_ubyte*)data, size);
        texture2d->setData(i, mip_width, mip_height, D3D9State::instance().getGALFormat(Format), 0, NULL, size);
...
libGAL::GALTexture2D* AITextureImp9::getGALTexture2D() 
{
    return texture2d;
}
