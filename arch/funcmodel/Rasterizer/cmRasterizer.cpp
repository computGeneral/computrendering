/**************************************************************************
 *
 * cmoRasterizer class implementation file (unified version).
 *  This file implements the cmoRasterizer class (unified shaders version).
 *  The cmoRasterizer class implements the main cmoRasterizer mdu.
 *
 */
...
cmoRasterizer::cmoRasterizer(bmoRasterizer &rsEmu, U32 trCycle, U32 tsFIFOSize,
    ...
    bool shSetup, U32 trShQSz, U32 stampsPerCycle, U32 depthSamplesCycle, U32 overWidth, U32 overHeight,
    U32 scanWidth, U32 scanHeight, U32 genWidth, U32 genHeight, U32 trBatchSz, U32 trBatchQSz,
    bool recursive, bool hzDisabled, U32 stampsBlock, U32 hzBufferSz, U32 hzCacheLins,
    U32 hzCacheLineSz, U32 hzQueueSz, U32 hzBufferLat, U32 hzUpLat, U32 clearBCycle,
    U32 numInterpolators, U32 shInQSz, U32 shOutQSz, U32 shInBSz, bool shTileDistro,
    U32 nVShaders, char **vshPrefix, U32 nFShaders, char **fshPrefix, U32 shInCycle,
    U32 shOutCycle, U32 thGroup, U32 fshOutLat, U32 vInQSz, U32 vOutQSz,
    U32 trInQSz, U32 trOutQSz, U32 rastQSz, U32 testQSz, U32 intQSz, U32 shQSz,
    U32 nStampUnits, char **suPrefixes, char *name, cmoMduBase *parent):

    //  Initializations.  
    bmRaster(rsEmu), triangleSetupFIFOSize(tsFIFOSize), trianglesCycle(trCycle), triangleSetupUnits(trSetupUnits),
    ...
    unifiedModel(true), numVShaders(nVShaders), numFShaders(nFShaders),
...

{
    ...
    GPU_ASSERT(
        if (stampsCycle < numStampUnits)
            CG_ASSERT("One stamp generated per cycle and stamp unit required.");
        if (shadedSetup && !unifiedModel)
            CG_ASSERT("Triangle setup in the shaders only supported for unified shader model.");
    )
    ...
    //  Create command signal to Interpolator mdu.  
    interpolatorComm = newOutputSignal("InterpolatorCommand", 1, 1, NULL);

    //  Create state signal from Interpolator mdu.  
    ...
    //  Create Triangle Setup mdu.  
    triangleSetup = new TriangleSetup(
        bmRaster,                    //  Reference to the rasterizer behaviorModel object to use.  
        ...
        triangleSetupUnits,         //  Number of setup units in the mdu.  
    ...
    //  Create Triangle Traversal mdu.  
    triangleTraversal = new TriangleTraversal(
        bmRaster,                //  Reference to the rasterizer behaviorModel object to be used.  
    ...
    hierarchicalZ = new HierarchicalZ(
        ...
        true,                   //  Process microtriangle fragments (enabled in the Micropolygon cmoRasterizer).  
        0,                      //  Microtriangle mode (enabled in the Micropolygon cmoRasterizer).  

        "HierarchicalZ", this);

    //  Create Interpolator mdu.  
    interpolator = new Interpolator(
        bmRaster,                //  Reference to the rasterizer behaviorModel to use for interpolation.  
...
//  cmoRasterizer simulation function.  
void cmoRasterizer::clock(U64 cycle)
{
    ...
    GPU_DEBUG_BOX( printf("cmoRasterizer => Clock %lld\n", cycle); )
    ...
    {
        CG_ASSERT("Missing signal from Triangle Setup.");
    }
    ...
    {
        CG_ASSERT("Missing signal from Triangle Traversal.");
    }
    ...
    {
        CG_ASSERT("Missing signal from Hierarchical Z.");
    }
    ...
    {
        CG_ASSERT("Missing signal from Interpolator.");
    }
    ...
    {
        CG_ASSERT("Missing signal from Fragment FIFO.");
    }
    ...
    switch(state)
    {
        case RAST_RESET:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => State RESET.\n"); )

            //  Reset cmoRasterizer registers.  
            hRes = 400;
            vRes = 400;
            ...
            for(i = 0; i < MAX_USER_CLIP_PLANES; i++)
                userClip[i] = Vec4FP32(0.0, 0.0, 0.0, 0.0);
        ...
        case RAST_READY:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => State READY.\n"); )
        ...
        case RAST_DRAWING:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => State DRAWING.\n"); )
        ...
        case RAST_END:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => State RAST_END.\n"); )
        ...
        case RAST_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => State RAST_CLEAR.\n");
            )
        ...
        case RAST_CLEAR_END:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => State RAST_CLEAR_END.\n"); )
...
void cmoRasterizer::processRasterizerCommand(RasterizerCommand *rastComm,
    U64 cycle)
{
    ...
    switch(rastComm->getCommand())
    {
        case RSCOM_RESET:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => RESET Command.\n"); )
        ...
        case RSCOM_REG_WRITE:
            //  Write to a cmoRasterizer register.  

            //  Check that current state is RAST_READY.  
            CG_ASSERT_COND(!(state != RAST_READY), "cmoRasterizer register writes can only be received in READY state.");
            GPU_DEBUG_BOX( printf("cmoRasterizer => REG_WRITE Command.\n"); )
        ...
        case RSCOM_DRAW:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => DRAW Command.\n"); )

            //  Check the current state.  
            CG_ASSERT_COND(!(state != RAST_READY), "cmoRasterizer DRAW command can only be received in READY state.");
        ...
        case RSCOM_END:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => END Command.\n"); )
            ...
            {
                CG_ASSERT("cmoRasterizer END command can only be received in END state.");
            }
        ...
        default:
            ...
            CG_ASSERT("Unsuppored cmoRasterizer Command.");
...
void cmoRasterizer::processRegisterWrite(GPURegister reg, U32 subreg,
    GPURegData data, U64 cycle)
{
    RasterizerCommand *rastComm;
    ...
    switch(reg)
    {
        case GPU_DISPLAY_X_RES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_DISPLAY_X_RES = %d.\n", hRes);
            )
        ...
        case GPU_DISPLAY_Y_RES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_DISPLAY_Y_RES = %d.\n", vRes);
            )
        ...
        case GPU_D3D9_PIXEL_COORDINATES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_D3D9_PIXEL_COORDINATES = %s\n", d3d9PixelCoordinates ? "TRUE" : "FALSE");
            )
        ...
        case GPU_VIEWPORT_INI_X:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_VIEWPORT_INI_X = %d.\n", data.intVal); )
        ...
        case GPU_VIEWPORT_INI_Y:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_VIEWPORT_INI_Y = %d.\n", data.intVal); )
        ...
        case GPU_VIEWPORT_HEIGHT:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_VIEWPORT_HEIGHT = %d.\n", data.uintVal); )
        ...
        case GPU_VIEWPORT_WIDTH:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_VIEWPORT_WIDTH = %d.\n", data.uintVal); )
        ...
        case GPU_SCISSOR_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_SCISSOR_TEST = %s.\n", scissorTest?"TRUE":"FALSE");
            )
        ...
        case GPU_SCISSOR_INI_X:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_SCISSOR_INIT_X = %d.\n", data.intVal); )
        ...
        case GPU_SCISSOR_INI_Y:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_SCISSOR_INIT_Y = %d.\n", data.intVal); )
        ...
        case GPU_SCISSOR_WIDTH:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_SCISSOR_WIDTH = %d.\n", data.uintVal); )
        ...
        case GPU_SCISSOR_HEIGHT:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_SCISSOR_HEIGHT = %d.\n", data.uintVal); )
        ...
        case GPU_DEPTH_RANGE_NEAR:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_DEPTH_RANGE_NEAR = %f.\n", data.f32Val); )
        ...
        case GPU_DEPTH_RANGE_FAR:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_DEPTH_RANGE_FAR = %f.\n", data.f32Val); )
        ...
        case GPU_D3D9_DEPTH_RANGE:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_D3D9_DEPTH_RANGE = %s.\n", data.booleanVal ? "T" : "F"); )
        ...
        case GPU_DEPTH_SLOPE_FACTOR:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_DEPTH_SLOPE_FACTOR = %f.\n", data.f32Val); )
        ...
        case GPU_DEPTH_UNIT_OFFSET:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_DEPTH_UNIT_OFFSET = %f.\n", data.f32Val); )
        ...
        case GPU_Z_BUFFER_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_Z_BUFFER_CLEAR = %x.\n", clearDepth);
            )
        ...
        case GPU_Z_BUFFER_BIT_PRECISSION:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_Z_BUFFER_BIT_PRECISSION = %d.\n", data.uintVal); )
        ...
        case GPU_HIERARCHICALZ:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Writing register GPU_HIERARCHICALZ = %s.\n",
                    hzEnable?"TRUE":"FALSE");
            )
        ...
        case GPU_EARLYZ:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Writing register GPU_EARLYZ = %s.\n",
                    earlyZ?"TRUE":"FALSE");
            )
        ...
        case GPU_USER_CLIP:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_USER_CLIP(%d) = (%f, %f, %f, %f).\n",
        ...
        case GPU_USER_CLIP_PLANE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_USER_CLIP_PLANE = %s.\n",
        ...
        case GPU_FACEMODE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_FACEMODE = ");
                switch(data.culling)
                {
                    case GPU_CW:
                        printf("CW.\n");
        ...
        case GPU_CULLING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_CULLING = ");
                switch(data.culling)
                {
                    case NONE:
                        printf("NONE.\n");
        ...
        case GPU_D3D9_RASTERIZATION_RULES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_D3D9_RASTERIZATION_RULES = %s\n", data.booleanVal?"ENABLED":"DISABLED");
            )
        ...
        case GPU_TWOSIDED_LIGHTING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_TWOSIDED_LIGHTING = %s\n", data.booleanVal?"ENABLED":"DISABLED");
            )
        ...
        case GPU_MULTISAMPLING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_MULTISAMPLING = %s\n", multisampling?"TRUE":"FALSE");
            )
        ...
        case GPU_MSAA_SAMPLES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_MSAA_SAMPLES = %d\n", msaaSamples);
            )
        ...
        case GPU_INTERPOLATION:
            ...
            CG_ASSERT_COND(!(subreg >= MAX_FRAGMENT_ATTRIBUTES), "Out of range fragment attribute identifier.");
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_INTERPOLATION = %s.\n",
                    data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_MODIFY_FRAGMENT_DEPTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_MODIFY_FRAGMENT_DEPTH = %s\n",
                    data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_FRUSTUM_CLIPPING:
            ...
            GPU_DEBUG_BOX( printf("cmoRasterizer => GPU_FRUSTUM_CLIPPING = %s.\n",
        ...
        case GPU_STENCIL_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_STENCIL_TEST = %s.\n", data.booleanVal?"ENABLED":"DISABLED");
            )
        ...
        case GPU_DEPTH_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_DEPTH_TEST = %s.\n", data.booleanVal?"ENABLED":"DISABLED");
            )
        ...
        case GPU_DEPTH_FUNCTION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => Write GPU_DEPTH_FUNCTION = ");
        ...
        case GPU_RENDER_TARGET_ENABLE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_RENDER_TARGET_ENABLE[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_COLOR_MASK_R:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_COLOR_MASK_R[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_COLOR_MASK_G:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_COLOR_MASK_G[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_COLOR_MASK_B:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_COLOR_MASK_B[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
        ...
        case GPU_COLOR_MASK_A:
            ...
            GPU_DEBUG_BOX(
                printf("cmoRasterizer => GPU_COLOR_MASK_A[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
        ...
        default:
            ...
            CG_ASSERT("Unsupported cmoRasterizer register.");
            break;
    }
}

void cmoRasterizer::getState(string &stateString)
{
...
void cmoRasterizer::saveHZBuffer()
{
    hierarchicalZ->saveHZBuffer();
}

void cmoRasterizer::loadHZBuffer()
{
    hierarchicalZ->loadHZBuffer();
}

void cmoRasterizer::detectStall(U64 cycle, bool &active, bool &stalled)
{
    ...
    //  Detect stalls on all the boxes instantiated inside cmoRasterizer.
...
void cmoRasterizer::stallReport(U64 cycle, string &stallReport)
{
    stringstream reportStream;

    string report;
        
    //  Obtain the stall report from all the boxes instantiated inside cmoRasterizer.
