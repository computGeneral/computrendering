...
DXPlayerStatisticViewerDlg::DXPlayerStatisticViewerDlg(const string& ProfilingFilePath, CWnd* pParent //=NULL) :
CDialog(IDD_STATISTICVIEWER, pParent),
m_ProfilingFilePath(ProfilingFilePath),
...
{
  m_dataman.LoadTraceStatistics(m_ProfilingFilePath);
}
...
bool DXPlayerStatisticViewerDlg::LoadTraceStatisticsOnTheFly(dxplugin::DXIntPluginManager& plugman)
{
  ...
  if (m_dataman.LoadTraceStatisticsOnTheFly(m_ProfilingFilePath, plugman))
  {
    m_dlgCurveOptions.Refresh();
    m_dlgGraphOptions.Refresh();
    m_dlgExportOptions.Refresh();
