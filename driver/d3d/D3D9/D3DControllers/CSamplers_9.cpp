    ...
    if(node->get_id() == NAME_TEXTURE) {
        ...
        if(i_tex != 0) {
            ...
            if(is_sampler_assigned(sampler)) {
                U32 texture_unit = get_assigned_texture_unit(sampler);
...
void CSamplers9::on_added_controller(StateDataNode* node) {
    if(node->get_id().name == NAME_DEVICE_9) {
        ...
        U32 default_texels[] = {0xFF808080, 0xFFFFFFFF, 0xFFFFFFFF, 0xFF808080};
        default_texture = GPUProxy::get_instance()->obtainMemory(sizeof(default_texels));
        GPUProxy::get_instance()->writeMemory(default_texture, (U08*)&default_texels,
...
void CSamplers9::reset_texture(U32 texture_unit) {
    ...
    for(U32 face = 0; face < CUBEMAP_IMAGES; face ++) {
        U32 offset = texture_unit * MAX_TEXTURE_SIZE * CUBEMAP_IMAGES + face;
        gpu->writeGPUAddrRegister(GPU_TEXTURE_ADDRESS, offset, default_texture);
    }
}

void CSamplers9::update_texture(StateDataNode* s_texture, U32 texture_unit) {
    ...
    for(UINT level = 0; level < level_count; level ++) {
        if(s_texture->get_id().name == NAME_TEXTURE_9) {
            ...
            for(UINT face = 0; face < 6; face++) {
                ...
                U32 md = (U32)res.index;
                // Write address
                U32 offset = texture_unit * MAX_TEXTURE_SIZE * CUBEMAP_IMAGES + level * CUBEMAP_IMAGES + face;
...
void CSamplers9::update_addressing(StateDataNode* s_sampler, U32 texture_unit) {
     DWORD address;
...
void CSamplers9::update_filtering(StateDataNode* s_sampler, U32 texture_unit) {
...
U32 CSamplers9::get_assigned_texture_unit(UINT sampler) {
    ///@todo Find a way to avoid polling for sampler type
    ResourceAssignationTable* a_ps_texture = AssignationsRegistry::get_table(NAME_PS_TEXTURE);
    ResourceId r;
