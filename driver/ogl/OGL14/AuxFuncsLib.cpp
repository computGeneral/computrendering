...
GLuint afl::computeHighestIndex(GLenum type, const GLvoid* indices, GLsizei count)
{
    switch ( type )
    {
        ...
        default:
            CG_ASSERT("Type index unsupported");
    }
...
void afl::drawElements(GLContext* ctx, ::HAL* driver, GLenum mode, GLsizei count, GLenum type, const GLvoid* indices)
{
    using namespace std;
    ...
    if ( GL_POINTS <= mode && mode <= GL_POLYGON )
    {
        ...
        CG_ASSERT("Unknown primitive type");
    ...
    GLubyte filename[30];
    sprintf((char*)filename, "OGL14DumpStreams.txt");
...
#ifdef GLLIB_DUMP_STREAMS
        ofstream out;
        out.open("OGL14DumpStreams.txt",ios::app);
    ...
    if ( IS_BUFFER_BOUND(GL_ELEMENT_ARRAY_BUFFER) )
        driver->setIndexedStreamingMode(nextFreeStream, count, 0, mIndices, U64(indices), afl::getStreamDataType(type));
    else
                                ...
                                GLsizei height, GLint border, GLsizei imageSize, const GLvoid *data )
{
    ...
    switch ( target )
    {
        ...
        case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z:
            ...
            break;
        default:
            CG_ASSERT("only TEXTURE_2D or CUBE_MAP faces are allowed as target");
    }
...
U32 afl::sendDB(DataBuffer<float>& db, HAL* drv)
{
    U32 md = drv->obtainMemory(db.bytes());
...
#define CONF_ATTRIB(arrayInfo_, attrib_, stream_, bo_)\
    {\
        HAL::VertexAttribute va;\
        va.stream = stream_;\
        va.attrib = attrib_;\
        va.md = ctx->gpumem().md(bo_);\
        va.stride = arrayInfo_.stride;\
        va.offset = U32(arrayInfo_.userBuffer); // pointer used as an offset \
...
GLuint afl::configureStream(GLContext* ctx, ::HAL* driver,
                            ...
                            std::set<GLuint>& iSet)
{
    ...
    if ( info.bufferID != 0 )
    {
        ...
        va.offset = U64(info.userBuffer); // pointer used as offset
        auxData = bo.getData();
    }
    else
    {
...
std::vector<GLuint> afl::setVertexArraysBuffers(GLContext* ctx, ::HAL* driver,
                                                GLuint start, GLuint count,
                                                std::set<GLuint>& iSet, GLuint& nextFreeStream)
{
    ...
    if ( ctx->testFlags(GLContext::flagVaPos) )
    {
        ...
        md = configureStream(ctx, driver, info, HAL::VS_VERTEX, stream, false, start, count, iSet);
        if ( md != 0 )
    ...
    if ( ctx->testFlags(GLContext::flagVaCol) )
    {
        ...
        md = configureStream(ctx, driver, info, HAL::VS_COLOR, stream, true, start, count, iSet);
        if ( md != 0 )
            clientDescriptors.push_back(md);
        stream++;
    }
    ...
    if ( ctx->testFlags(GLContext::flagVaNor) )
    {
        ...
        md = configureStream(ctx, driver, info, HAL::VS_NORMAL, stream, false, start, count, iSet);
        if ( md != 0 )
            clientDescriptors.push_back(md);
        stream++;
    }
    ...
    for ( GLuint i = 0; i < ctx->countTextureUnits(); i++ )
    {
        if ( ctx->isEnabledClientTextureUnit(i) )
        {
            ...
            md = configureStream(ctx, driver, info, HAL::ShAttrib(HAL::VS_TEXTURE_0 + i),
    ...
    if ( ctx->testFlags(GLContext::flagVS) )
    {
        for ( GLuint i = 0; i < ctx->countGenericVArrays(); i++ )
        {
            if ( ctx->isGenericAttribArrayEnabled(i) )
            {
                ...
                md = configureStream(ctx, driver, info, (HAL::ShAttrib)i, stream, false, start, count, iSet);
                if ( md != 0 )
                    clientDescriptors.push_back(md);
                stream++;
            }
...
void afl::setupShaders(GLContext* ctx, ::HAL* driver)
{
    if ( ctx->testFlags(GLContext::flagStencilTest) && ctx->checkClearStencilWarning() )
    {
        ...
        //CG_ASSERT("Stencil buffer not clear previously and now is enabled");
    }
    ...
    else
    {
        if (ctx->testFlags(GLContext::flagAlphaTest))
            CG_ASSERT("Alpha Test and User Fragment program not supported yet");
...
void afl::setupHZTest(GLContext* ctx, ::HAL* driver)
{
 ...
 */
void afl::setFragmentAttributes(HAL* driver, GLContext* ctx)
{
...
void afl::synchronizeTextureImages(GLContext* ctx, ::HAL* driver, bool& disableColorMask)
{
    if ( ctx->countTextureUnits() != ProgramObject::MaxTextureUnits )
    {
        ...
        CG_ASSERT("Inconsistent definitions for MaxTextureUnits");
    }
    ...
    if ( ctx->testFlags(GLContext::flagFS) )
    {
        ...
        for ( GLuint i = 0; i < ProgramObject::MaxTextureUnits; i++ )
        {
            if ( pr.textureTargets[i] != 0 )
            {
                ...
                if ( !to )
                    CG_ASSERT("Texture object not attached to this texture unit");
...
void afl::setupTexture(GLContext* ctx, HAL* driver, TextureObject& to, GLuint texUnit, GLenum kindOfAccess)
{
    GPURegData data;
    ...
    switch ( kindOfAccess )
    {
        ...
        case GL_TEXTURE_CUBE_MAP:
            ...
            break;
        default:
            CG_ASSERT("Unknown texture target");
    }
    ...
    if ( to.getTargetName() == GL_TEXTURE_CUBE_MAP )
    {
        if ( numberOfPortions % 6 != 0 )
            CG_ASSERT("Cubemap without same number of mipmaps per face");

        if ( numberOfPortions != ((end-start+1)*6) )
            CG_ASSERT("Cubemap portions/mipmaps inconsistent");


        for ( U32 i = 0; i < 6; i++ ) // loop through all 6 face
        {
            level = start;
            for ( ; level <= end; level ++, portion++)
            {
                U32 md = ctx->gpumem().md(to, portion);
                U32 pos = skipUnits + 6*level + i; // i == CUBEMAP
    ...
    else // 1D, 2D and 3D
    {
        ...
        if ( numberOfPortions != (end-start+1) )
            CG_ASSERT("Texture portions/mipmaps inconsistent");
        for ( ; level <= end; level++, portion++ )
        {
            // configure mipmaps from base to max
            U32 md = ctx->gpumem().md(to,portion);
            U32 pos = skipUnits + 6*level;
            driver->writeGPUAddrRegister(GPU_TEXTURE_ADDRESS, pos, md);
        }
    }
    ...
    data.uintVal = (U32) mathlib::ceil2(mathlib::log2(data.uintVal));
    ...
    switch ( to.getWrapS() )
    {
        ...
        default:
            CG_ASSERT("Unsupported WRAP_S mode");
    }
    ...
    switch ( to.getWrapT() )
    {
        ...
        CASE_BRANCH_CLAMP(MIRRORED_REPEAT)
        default:
            CG_ASSERT("Unsupported WRAP_T mode");
    }
    ...
    switch ( to.getMinFilter() )
    {
        ...
        default:
            CG_ASSERT("Unsupported MIN_FILTER");
    }
    ...
    switch ( to.getMagFilter() )
    {
        ...
        CASE_BRANCH_FILTER(LINEAR)
        default:
            CG_ASSERT("Unsupported MAG_FILTER");
    }
    driver->writeGPURegister(GPU_TEXTURE_MAG_FILTER, texUnit, data);

    data.uintVal = U32(to.getMaxAnisotropy());
...
int afl::sendRawIndices(HAL* driver, const GLvoid* indices, GLenum type, GLsizei count)
{
...
int afl::sendIndices(HAL* driver, const GLvoid* indices, GLenum indicesType, GLsizei count, const std::set<GLuint>& usedIndices)
{
    using namespace std;
...
int afl::sendIndices(HAL* driver, const GLvoid* indices, GLenum indicesType, GLsizei count, GLint offset)
{
    GLsizei dataSize = afl::getSize(indicesType)*count;
...
void afl::clearStencilOnly(GLContext *ctx)
{
    ...
    data.uintVal = (U32)ctx->getStencilBufferClearValue();
    ...
    U32 mdColor = 0;
    U32 mdNormal = 0;
    vector<U32> mdTextures; // Texture unit memory descriptor mdTextures[i] for texUnit[i]
    vector<U32> texUnit; // Texture unit id's
...
#define CONFIG_ATTRIB(stream_,md_,attrib_,compos_)\
    ...
    if ( vpres.inputAttribsRead[HAL::VS_VERTEX] )
    {
        mdVertex = afl::sendDB(ctx->posbuf(), driver);
        CONFIG_ATTRIB(stream, mdVertex, HAL::VS_VERTEX, ctx->posbuf().numberOfComponents());
        stream++;
    }

    if ( vpres.inputAttribsRead[HAL::VS_COLOR] )
    {
        mdColor = afl::sendDB(ctx->colbuf(), driver);
        CONFIG_ATTRIB(stream, mdColor, HAL::VS_COLOR, ctx->colbuf().numberOfComponents());
        stream++;
    }

    if ( vpres.inputAttribsRead[HAL::VS_NORMAL] )
    {
        mdNormal = afl::sendDB(ctx->norbuf(), driver);
        CONFIG_ATTRIB(stream, mdNormal, HAL::VS_NORMAL, ctx->norbuf().numberOfComponents());
        stream++;
    }
...
#undef CONFIG_ATTRIB
    ...
    driver->writeGPURegister(GPU_STENCIL_COMPARE_MASK, data);
    data.uintVal = (U32)saved_stencilBufferReferenceValue;
    driver->writeGPURegister(GPU_STENCIL_REFERENCE, data);
...
#undef CONFIG_ATTRIB
    ...
    switch ( saved_stencilBufferFunc )
    {
        ...
        BRANCH_CASE(NOTEQUAL)
        default:
            CG_ASSERT("Unexpected stencil function");
    }
    ...
    switch ( savedDepthFunc )
    {
        ...
        BRANCH_CASE(NOTEQUAL)
        default:
            CG_ASSERT("Unexpected depth function");
    }
...
void afl::dumpStencilBuffer(GLContext *ctx)
{
    ...
    while (currentMaskBit != 0x00)
    {
        ...
        driver->writeGPURegister(GPU_STENCIL_FUNCTION, data);
        data.uintVal = (U32)currentMaskBit;
        driver->writeGPURegister(GPU_STENCIL_REFERENCE, data);
        driver->writeGPURegister(GPU_STENCIL_COMPARE_MASK, data);
...
void afl::performBlitOperation(GLint xoffset, GLint yoffset, GLsizei texWidth, GLint x, GLint y, GLsizei width, GLsizei height, U32 md, GLenum ifmt, GLsizei &size)
{
    GPURegData data;
    ...
    switch ( ifmt )
    {
        ...
        default:
            CG_ASSERT("Unsupported Blit operation texture format");
    }
