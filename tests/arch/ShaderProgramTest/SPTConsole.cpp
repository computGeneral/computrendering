...
SPTConsole::SPTConsole( const cgsArchConfig& arch, const char* name) 
    : ShaderProgramTestBase(arch), quitConsole(false)
{}
...
bool SPTConsole::processCompileProgramCommand(const vector<string>& params)
{
    ...
    compileModel coFspModel = OGL_ARB_1_0;
    ...
    while (it != params.end())
    {
        ...
        else if (!(*it).compare(0, 7, "-target")) // compilation target
        {
            ...
            {
                coFspModel = OGL_ARB_1_0;
            }
            else if (!(*it).compare(0, 6, "SM_2_0"))
            {  
                coFspModel = D3D_SHADER_MODEL_2_0;
            }
            else if (!(*it).compare(0, 6, "SM_3_0"))
            {
                coFspModel = D3D_SHADER_MODEL_3_0;
            }
            else
            {
    ...
    compileProgram(programString, optimize, transform, coFspModel);
