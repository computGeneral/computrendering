...
{
    set<pair<gal_uint,gal_uint> > ranges;

    map<gal_uint, gal_uint> defs;
    map<gal_uint, gal_uint>::iterator defIter;
    ...
    for ( ; it != code.end(); it++ )
    {
        ...
        {
            gal_uint idReg = (*it)->writeReg.reg;
    ...
    */
    ...
    {
        gal_uint nOperands = (*itRev)->operation.numOperands;
        if ( nOperands >= 1 )
        {
            // check first operand
    ...
    gal_uint max = 0;
    for ( gal_uint i = 0; i < code.size(); i++ )
    {
        gal_uint temp = _countMatchingRanges(ranges, i);
...
gal_uint MaxLiveTempsAnalysis::_countMatchingRanges(std::set<std::pair<gal_uint,gal_uint> >& ranges, gal_uint instructionPos)
{
    typedef set<pair<gal_uint, gal_uint> > Ranges;
...
void RegisterUsageAnalysis::optimize()
{
    ...
    for (gal_uint i=0; i < MAX_SHADER_ATTRIBUTES; i++)
    {
    ...
    for ( ; it != code.end(); it++ )
    {
        ...
        gal_uint nOperands = (*it)->operation.numOperands;
        if ( nOperands >= 1 )
        {
            // check first operand
...
void StaticInstructionScheduling::_reorderCode(vector<InstructionInfo*>& originalCode, const vector<IssueInfo>& orderList) const
{
    ...
    {
        InstructionInfo *nop = new InstructionInfo(arch::CG_ISA_OPCODE_NOP, originalCode.size() + i);
