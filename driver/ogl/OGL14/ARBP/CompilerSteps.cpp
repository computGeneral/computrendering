...
U32 GenerationCode::computeMaxLiveTemps(vector<InstructionInfo*>& code)
{
    set<pair<U32,U32> > ranges;

    map<U32, U32> defs;
    map<U32, U32>::iterator defIter;
    ...
    */
    ...
    {
        U32 nOperands = (*itRev)->operation.numOperands;
        if ( nOperands >= 1 )
        {
            // check first operand
    ...
    {
        U32 temp = countMatchingRanges(ranges, i);
...
U32 GenerationCode::countMatchingRanges(std::set<std::pair<U32,U32> >& ranges, U32 instructionPos)
{
    typedef set<pair<U32,U32> > Ranges;
