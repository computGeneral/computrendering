/**
 *
 *  @file cmoCommandProcessor.cpp
 *  This class implements the Command Processor mdu.
...
//    char **tuPrefix,
//    char **suPrefix,
    char *name, cgeModelAbstractLevel MdlTyp, cmoMduBase *parent):

regWrites(getSM().getNumericStatistic("RegisterWrites", U32(0), "cmoCommandProcessor", "CP")),
bytesWritten(getSM().getNumericStatistic("BytesWritten", U32(0), "cmoCommandProcessor", "CP")),
bytesRead(getSM().getNumericStatistic("BytesRead", U32(0), "cmoCommandProcessor", "CP")),
writeTrans(getSM().getNumericStatistic("WriteTransactions", U32(0), "cmoCommandProcessor", "CP")),
batches(getSM().getNumericStatistic("Batches", U32(0), "cmoCommandProcessor", "CP")),
frames(getSM().getNumericStatistic("Frames", U32(0), "cmoCommandProcessor", "CP")),
bitBlits(getSM().getNumericStatistic("Bit Blits", U32(0), "cmoCommandProcessor", "CP")),
readyCycles(getSM().getNumericStatistic("Ready", U32(0), "cmoCommandProcessor", "CP")),
drawCycles(getSM().getNumericStatistic("Draw", U32(0), "cmoCommandProcessor", "CP")),
endGeomCycles(getSM().getNumericStatistic("EndGeometry", U32(0), "cmoCommandProcessor", "CP")),
endFragCycles(getSM().getNumericStatistic("EndFragment", U32(0), "cmoCommandProcessor", "CP")),
clearCycles(getSM().getNumericStatistic("Clear", U32(0), "cmoCommandProcessor", "CP")),
swapCycles(getSM().getNumericStatistic("Swap", U32(0), "cmoCommandProcessor", "CP")),
flushCycles(getSM().getNumericStatistic("Flush", U32(0), "cmoCommandProcessor", "CP")),
saveRestoreStateCycles(getSM().getNumericStatistic("SaveRestoreState", U32(0), "cmoCommandProcessor", "CP")),
memWriteCycles(getSM().getNumericStatistic("MemoryWrite", U32(0), "cmoCommandProcessor", "CP")),
memReadCycles(getSM().getNumericStatistic("MemoryRead", U32(0), "cmoCommandProcessor", "CP")),
memPreLoadCycles(getSM().getNumericStatistic("MemoryPreload", U32(0), "cmoCommandProcessor", "CP")),
bitBlitCycles(getSM().getNumericStatistic("bitBlitCycles", U32(0), "cmoCommandProcessor", "CP")),

cmoMduBase(name, parent), 
numVShaders(ArchConf->gpu.numFShaders),
numFShaders(ArchConf->gpu.numFShaders),
TotalTextureUnits(ArchConf->gpu.numFShaders * ArchConf->ush.textureUnits),
numPerShaderTextureUnits(ArchConf->ush.textureUnits),
numStampUnits(ArchConf->gpu.numStampUnits),
memorySize(ArchConf->mem.memSize),
pipelinedBatches(ArchConf->com.pipelinedBatches),
dumpShaderPrograms(ArchConf->com.dumpShaderPrograms),
CurMdlTyp(MdlTyp),
skipDraw(false),
skipFrames(false),
...
{
    CG_ASSERT_COND((numVShaders > 0), "Invalid number of Vertex Shaders (must be >0).");
    ...
    driver = tDriver;    //  Initialize the TraceDriver.  
    driver->startTrace();    //  Start TraceDriver.  

    //  Set current MetaStream to NULL.  
    lastMetaStreamTrans = NULL;

    //  Set program code buffer pointer to null.  
    programCode = NULL;

    //  Set cycles for current MetaStream end to 0.  
    transCycles = 0;
...
void cmoCommandProcessor::PortBinding()
{
    vshFCommSignal = new Signal*[numVShaders];//  Create the signal arrays for the vertex shaders.  
    vshDCommSignal = new Signal*[numVShaders];
    for(U32 i = 0; i < numVShaders; i++)
    {
        char vshPrefix[6]; //  Allocate prefix.
        sprintf(vshPrefix, "VS%d", i); //  Write prefix.
        vshFCommSignal[i] = newOutputSignal("CommShaderCommand", 1, 1, vshPrefix);//  Create the signals with each vertex shader unit.  
        vshDCommSignal[i] = newOutputSignal("ShaderDecodeExecuteCommand", 1, 1, vshPrefix);
    }
    streamCtrlSignal = newOutputSignal("StreamerControl", 1, 1, NULL);//  Create control signal to the cmoStreamController.  
    streamStateSignal = newInputSignal("StreamerState", 1, 1, NULL);//  Create state signal from the cmoStreamController.  
    paCommSignal = newOutputSignal("PrimitiveAssemblyCommand", 1, 1, NULL);//  Create command signal to Primitive Assembly.  
    paStateSignal = newInputSignal("PrimitiveAssemblyCommandState", 1, 1, NULL);//  Create state signal from Primitive Assembly.  
    clipCommSignal = newOutputSignal("ClipperCommand", 1, 1, NULL);//  Create command signal to the Clipper.  
    clipStateSignal = newInputSignal("ClipperCommandState", 1, 1, NULL);//  Create state signal from the Clipper.  
    rastCommSignal = newOutputSignal("RasterizerCommand", 1, 1, NULL);//  Create signal to the rasterizer.  
    rastStateSignal = newInputSignal("RasterizerState", 1, 1, NULL);//  Create a state signal from the rasterizer.  
    ...
    for(U32 i = 0; i < numFShaders; i++)
    {
        char fshPrefix[6];
        sprintf(fshPrefix, "FS%d", i); //  Write prefix.
        fshFCommandSignal[i] = newOutputSignal("CommShaderCommand", 1, 1, fshPrefix);
        fshDCommandSignal[i] = newOutputSignal("ShaderDecodeExecuteCommand", 1, 1, fshPrefix);
    }

    //  Allocate pointer array for the texture unit prefixes.
    //  Create prefixes for the fragment shaders.
    tuCommandSignal = new Signal*[TotalTextureUnits];
    for(U32 i = 0; i < numFShaders; i++)
    {
        for(U32 j = 0; j < numPerShaderTextureUnits; j++)
        {
            char tuPrefix[10];
            sprintf(tuPrefix, "FS%dTU%d", i, j); //  Write prefix.
            tuCommandSignal[i * numPerShaderTextureUnits + j] = newOutputSignal("TextureUnitCommand", 1, 1, tuPrefix);
        }
    }
    ...
    {
        char suPrefix[6];
        sprintf(suPrefix, "SU%d", i); //  Write prefix.
        zStencilCommSignal[i] = newOutputSignal("ZStencilTestCommand", 1, 1, suPrefix); //  Create command signal to Z Stencil Test.  
        zStencilStateSignal[i] = newInputSignal("ZStencilTestRasterizerState", 1, 1, suPrefix); //  Create state signal from Z Stencil Test.  
        colorWriteCommSignal[i] = newOutputSignal("ColorWriteCommand", 1, 1, suPrefix);//  Create command signal to color write.  
        colorWriteStateSignal[i] = newInputSignal("ColorWriteRasterizerState", 1, 1, suPrefix);//  Create state signal from color write.  
    }
    dacCommSignal = newOutputSignal("DACCommand", 1, 1);//  Create command signal to DisplayController.  
    dacStateSignal = newInputSignal("DACState", 1, 1);//  Create state signal from DisplayController.  
    writeMemorySignal = newOutputSignal("CommProcMemoryWrite", 1, 1, NULL);//  Create signal to the GPU memory interface.  
    readMemorySignal = newInputSignal("CommProcMemoryRead", 2, 1, NULL);//  Create signal from the GPU memory interface.  
    mcCommSignal = newOutputSignal("MemoryControllerCommand", 1, 1, NULL);// Create memory controller command signal
}

//  Command Processor clock rutine.  
void cmoCommandProcessor::clock(U64 cycle)
{
    U32 i;
    ...
    cgoMetaStream *auxMetaStreamTrans;
    ...
    CG_DEBUG("cmoCommandProcessor => Clock %lld\n", cycle);

    GPU_DEBUG_BOX(
        printf("cmoCommandProcessor => State: ");
    ...
    //  Get cmoStreamController State.  
    if (streamStateSignal->read(cycle, (DynamicObject *&) streamStateInfo))
    {
        //  Store StreamController state.  
        streamState = streamStateInfo->getState();

        //  Delete the StreamController state info.  
        delete streamStateInfo;

    }
    else
    {
        CG_ASSERT("Missing state signal from the cmoStreamController.");
    }
    ...
    {
        CG_ASSERT("Missing state signal from Primitive Assembly.");
    }
    ...
    {
        CG_ASSERT("Missing state signal from the Clipper.");
    }
    ...
    {
        CG_ASSERT("Missing state signal from the Rasterizer.");
    }
    ...
    for(i = 0, endAllZST = TRUE; i < numStampUnits; i++)
    {
        ...
        {
            CG_ASSERT("Missing state signal from the Color Write unit.");
        }
    }
    ...
    {
        CG_ASSERT("Missing state signal from the DisplayController unit.");
    }
...
//printf("CP %lld => Change to READY state\n", cycle);
        ...
        case GPU_READY:
            //  The GPU is ready to process a new MetaStream.  
            ...
            else
            {
                if (processNewTransaction)
                {
                    ...
                    if (!forceTransaction)
                    {
                        //  Try to read an MetaStream from the trace driver.
                        auxMetaStreamTrans = driver->nxtMetaStream();
//printf("CP (%lld) => Next MetaStream %p\n", cycle, auxMetaStreamTrans);                        
                        
                    }
                    else
                    {
                        //  Force the external transaction.
                        auxMetaStreamTrans = forcedTransaction;

                        //  Unset force transaction flag.
                        forceTransaction = false;
                    }

                    if (auxMetaStreamTrans != NULL)
                    {
                        //auxMetaStreamTrans->dump();

                        //  Delete old MetaStream.  
                        if (lastMetaStreamTrans != NULL)
                            delete lastMetaStreamTrans;

                        //  Set new MetaStream as last MetaStream.  
                        lastMetaStreamTrans = auxMetaStreamTrans;

                        //  Check if validation mode is enabled.
                        if (enableValidation)
                        {
                            //  Clone the MetaStream.
                            cgoMetaStream *clonedMetaStreamTrans = new cgoMetaStream(auxMetaStreamTrans);

                            //  Add the MetaStream to the log.
                            agpTransactionLog[nextWriteLog].push_back(clonedMetaStreamTrans);
                        }
                    }
                    else
                    {
                        //  No MetaStream means that the trace has finished with the current model.  
                        traceEnd = TRUE;
                    }
                }
                ...
                if (!traceEnd)
                {
                    //  Start the processing of this MetaStream.  
                    processNewcgoMetaStream(cycle);
                }
            }
        ...
        case GPU_DRAWING:
            ...
            /*  Wait for end messages from cmoStreamController, Vertex Shader and
                Fragment processor.  */
            ...
            {
                //  Create END command for the StreamController.  
                streamComm = new StreamerCommand(STCOM_END);

                //  Copy cookies from original MetaStream and add new cookie.  
                streamComm->copyParentCookies(*lastMetaStreamTrans);
                streamComm->addCookie();

                //  Send command to the StreamController.  
                ...
                /*  Copy cookies from original MetaStream and add a
                    new cookie.  */
                paCommand->copyParentCookies(*lastMetaStreamTrans);
                paCommand->addCookie();
                ...
                /*  Copy cookies from original MetaStream and add
                    a new cookie.  */
                clipperCommand->copyParentCookies(*lastMetaStreamTrans);
...
//printf("CP %lld => Updating buffered geometry state\n", cycle);
            ...
            if ((state.statusRegister == GPU_END_GEOMETRY) && endAllCW)
            {
                ...
                //  Copy cookies from original MetaStream and add new cookie.  
                rastComm->copyParentCookies(*lastMetaStreamTrans);
                rastComm->addCookie();
...
//printf("CP %lld => Updating buffered fragment state\n", cycle);
            ...
            else
            {
                ...
                if (storedLoadFragProgram)
                {
                    ...
                    if (lastMetaStreamTrans == loadFragProgram)
                    {
                        //  Delete buffered transaction.  
                        delete lastMetaStreamTrans;
                        loadFragProgram = NULL;

                        //  Restore original last MetaStream and state.  
                        lastMetaStreamTrans = backupMetaStreamTrans;
                    ...
                    {
                        //  Keep current last MetaStream and state.  
                        backupMetaStreamTrans = lastMetaStreamTrans;
                        backupProcNewTrans = processNewTransaction;

                        //  Set buffered transaction as last.  
                        lastMetaStreamTrans = loadFragProgram;
                ...
                else
                {
                    ...
                    if (geometryStarted)
                    {
                        ...
                        //  Copy cookies from original MetaStream and a new cookie.  
                        rastComm->copyParentCookies(*lastMetaStreamTrans);
                        rastComm->addCookie();
...
//printf("CP %lld => Change to READY state.\n", cycle);
        ...
        case GPU_MEMORY_WRITE:
            ...
            if (transCycles > 0)
            {
                ...
                GPU_DEBUG_BOX( printf("cmoCommandProcessor => Writing to memory.  Remaining cycles: %d.\n", transCycles); )
            ...
            if (lastMetaStreamTrans->GetMetaStreamType() == META_STREAM_WRITE)
            {
                //  Check if there are more data to write.  
                if (sent < lastMetaStreamTrans->getSize())
                {
                    ...
                    if ((transCycles == 0) && ((memoryState & MS_WRITE_ACCEPT) != 0) && (freeTickets > 0))
                    {
                        ...
                        size = GPU_MIN(lastMetaStreamTrans->getSize() - sent, MAX_TRANSACTION_SIZE);

                        //  Create a new memory transaction.  
                        memTrans = new MemoryTransaction(
                            MT_WRITE_DATA,
                            lastMetaStreamTrans->getAddress() + sent,
                            size, &(lastMetaStreamTrans->getData()[sent]),
                            COMMANDPROCESSOR,
                            currentTicket++);

                        //  Copy original MetaStream cookie and add new cookie.  
                        memTrans->copyParentCookies(*lastMetaStreamTrans);
                        memTrans->addCookie();

                        //  Send the memory transaction to the Memory Controller.  
                        writeMemorySignal->write(cycle, memTrans, 1);

                        /*  NOTE:  DOESN'T TAKES INTO ACCOUNT THE WIDHT OF THE MetaStream BUS.  IT IS ASSUMED
                            THAT THE MetaStream BUS HAS THE SAME WIDTH THAT THE BUS FROM THE COMMAND PROCESSOR
                            TO THE MEMORY CONTROLLER.  THERE ARE ALSO NO ADDITIONAL PENALTIES OR TRANSACTIONS
                            REQUIRED AFTER THE INITIAL ONE.  */

                        //  Set MetaStream transmission counter.  
                ...
                if ((transCycles == 0) && (sent == lastMetaStreamTrans->getSize()))
                {
//printf("CP %lld => Change to stacked state (%d)\n", cycle, stateStack);
            ...
            {
                CG_ASSERT("Unsupported MetaStream command writing to memory.");
            }
...
//printf("CP %lld => Change to stacked state (%d)\n", cycle, stateStack);
        ...
        case GPU_MEMORY_READ:
            ...
            if (transCycles > 0)
            {
                ...
                GPU_DEBUG_BOX( printf("cmoCommandProcessor => Reading data from memory.  Remaining cycles: %d.\n", transCycles); )
                ...
                if (transCycles == 0)
                {
                    ...
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => End of data transmision.\n");
                    )
            ...
            if (lastMetaStreamTrans->GetMetaStreamType() == META_STREAM_COMMAND)
            {
                //  Check load vertex program command.  
                if (lastMetaStreamTrans->getGPUCommand() == GPU_LOAD_VERTEX_PROGRAM)
                {
                    ...
                    if (received == state.vertexProgramSize)
                    {
                        ...
                        for (i = 0; i < numVShaders; i++)
                        {
                            ...
                            GPU_DEBUG_BOX(
                                printf("cmoCommandProcessor => Sending vertex program to vertex shader %d.\n",
                                    i);
                            )
                            ...
                            //  Copy cookies from original MetaStream and a new cookie.  
                            shComm->copyParentCookies(*lastMetaStreamTrans);
                            shComm->addCookie();
...
//printf("CP %lld => Change to stacked state (%d)\n", cycle, stateStack);
                ...
                else if (lastMetaStreamTrans->getGPUCommand() == GPU_LOAD_FRAGMENT_PROGRAM)
                {
                    //  Check if there is more data to request.  
                    ...
                    if (received == state.fragProgramSize)
                    {
                        ...
                        for (i = 0; i < numFShaders; i++)
                        {
                            ...
                            GPU_DEBUG_BOX(
                                printf("cmoCommandProcessor => Sending fragment program to fragment shader %d.\n",
                                    i);
                            )
...
//printf("CP %lld => Change to stacked state (%d)\n", cycle, stateStack);
                ...
                else if (lastMetaStreamTrans->getGPUCommand() == GPU_LOAD_SHADER_PROGRAM)
                {
                    //  Check if there is more data to request.
                    ...
                    if (received == state.programSize)
                    {
                        ...
                        for (i = 0; i < numFShaders; i++)
                        {
                            ...
                            GPU_DEBUG_BOX(
                                printf("cmoCommandProcessor => Sending shader program to fragment shader %d.\n",
                                    i);
                            )
...
//printf("CP %lld => Change to stacked state (%d)\n", cycle, stateStack);
                ...
                {
                    CG_ASSERT("Unsupported GPU command reading from memory.");
                }
            }
            else
            {
                    CG_ASSERT("Unsupported MetaStream reading from memory.");
            }
        ...
        case GPU_SWAP:
            ...
            if (endAllCW && (!colorWriteEnd))
            {
                ...
                //  Send SWAP command to the DisplayController unit.  
                ...
                //  Send the command to the DisplayController unit.  
            ...
            /**  NOTE:  THE COMMAND PROCESSOR SHOULD NOT WAIT FOR THE
                 DisplayController, AS IT SHOULD JUST HAVE TO SIMULATE THE SCREEN/DISPLAY
                 REFRESH, BUT AS THERE IS NO VERTICAL SYNC IMPLEMENTED
                 THIS A WAY TO DO IT FOR NOW.  **/

            //  Check if cmDisplayController.has finished dumping the framebuffer.  
            if (dacState == RAST_END)
            {
                //  Send END command to the DisplayController unit.  
                rastComm = new RasterizerCommand(RSCOM_END);
...
//printf("CP %lld => Change to READY state.\n", cycle);
            ...
            switch(dumpBufferCommand)
            {
                ...
                case RSCOM_DUMP_STENCIL:
                    ...
                    if (endAllZST && (!zStencilTestEnd))
                    {
                        ...
                        //  Send DUMP_BUFFER command to the DisplayController unit.
                ...
                default:
                    ...
                    CG_ASSERT("Expected dump buffer rasterizer command.");
                    break;
            }

            //  Check if cmDisplayController.has finished dumping the buffer.
            if (dacState == RAST_END)
            {
                //  Send END command to the DisplayController unit.
                rastComm = new RasterizerCommand(RSCOM_END);
        ...
        case GPU_BLITTING:
            ...
            if (endAllCW && (!colorWriteEnd))
            {
                ...
                //  Send BLIT command to the DisplayController unit.  
            ...
            //  Check if cmDisplayController.has finished performing bit blit operation.  
            if (dacState == RAST_END)
            {
                //  Send END command to the DisplayController unit.  
                rastComm = new RasterizerCommand(RSCOM_END);
...
//  Starts to process a new MetaStream.  
void cmoCommandProcessor::processNewcgoMetaStream(U64 cycle)
{
    MemoryTransaction *memTrans;
    GPURegData data;
    U32 regGroup;

    switch(lastMetaStreamTrans->GetMetaStreamType())
    {
        case META_STREAM_WRITE:
            ...
            if ((state.statusRegister != GPU_READY) && (((state.statusRegister != GPU_DRAWING) &&
                ((state.statusRegister != GPU_END_GEOMETRY) || geometryStarted)) || lastMetaStreamTrans->getLocked()))
            {
...
//    lastMetaStreamTrans->getLocked());

                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => Processing New MetaStream: META_STREAM_WRITE.\n");
                    printf("cmoCommandProcessor => META_STREAM_WRITE addr %x size %d\n", lastMetaStreamTrans->getAddress(),
                        lastMetaStreamTrans->getSize());
                )
                 ...
                 */
                if (((lastMetaStreamTrans->getAddress() & ADDRESS_SPACE_MASK) == SYSTEM_ADDRESS_SPACE) ||
                    skipDraw)
                {
//printf("CP %lld > Sending MT_PRELOAD_DATA @%x size %d data %p\n", cycle, lastMetaStreamTrans->getAddress(),
//lastMetaStreamTrans->getSize(), lastMetaStreamTrans->getData());
                    //  Create a new memory transaction.  
                    memTrans = new MemoryTransaction(
                        MT_PRELOAD_DATA,
                        lastMetaStreamTrans->getAddress(),
                        lastMetaStreamTrans->getSize(),
                        lastMetaStreamTrans->getData(),
                        COMMANDPROCESSOR,
                        currentTicket++);
...
//printf("CP %lld => Change to MEMORY_PRELOAD state.\n", cycle);
                    ...
                    /*  Set GPU state to preload to memory.  This gives time (1 cycle) to the Memory Controller to
                        copy the data from the MetaStream buffer.  */
                ...
                else
                {
                    ...
                    if ((memoryState & MS_WRITE_ACCEPT) != 0)
                    {
                        ...
                        CG_ASSERT_COND(!(freeTickets == 0), "No available memory tickets.");
                        //  Update free tickets counter.  
                        freeTickets--;

                        //  Set first write transaction bytes to sent.  
                        sent = GPU_MIN(lastMetaStreamTrans->getSize(), MAX_TRANSACTION_SIZE);
...
//printf("CP %lld => Change to MEMORY_WRITE state.\n", cycle);
        ...
        case META_STREAM_PRELOAD:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => Processing New MetaStream: META_STREAM_PRELOAD.\n");
                    printf("cmoCommandProcessor => META_STREAM_PRELOAD addr %x size %d\n", lastMetaStreamTrans->getAddress(),
                        lastMetaStreamTrans->getSize());
                )
...
//printf("CP %lld => Change to MEMORY_PRELOAD state.\n", cycle);
        ...
        case META_STREAM_READ:
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => Procesing MetaStream: META_STREAM_READ.\n"); )
            ...
            //  !!! META_STREAM_READ UNSOPPORTED RIGHT NOW !!!!  
            CG_ASSERT("META_STREAM_READ transaction unimplemented.");
        ...
        case META_STREAM_REG_WRITE:
            ...
            //regGroup = (lastMetaStreamTrans->getGPURegister() < GPU_LAST_GEOMETRY_REGISTER)?GEOM_REG:FRAG_REG;
            if (lastMetaStreamTrans->getGPURegister() < GPU_LAST_GEOMETRY_REGISTER)
                regGroup = GEOM_REG;
            else
                regGroup = FRAG_REG;
                ...
                else
                {
                    ...
                    if (freeUpdates[regGroup] > 0)
                    {
                        /*  NOTE:  IN CURRENT IMPLEMENTATION SUPPORT FOR REGISTER WRITE COOKIES IS
                            BROKEN.  THE MetaStream TRANSACTION OR THE COOKIES SHOULD BE KEPT UNTIL THE
                            UPDATE IS SENT TO THE UNIT.  */

                        GPU_DEBUG_BOX(
                            printf("cmoCommandProcessor => Procesing MetaStream: META_STREAM_REG_WRITE.\n");
                        )

//printf("CP => Buffering register change for phase %s\n", regGroup == GEOM_REG?"GEOMETRY":"FRAGMENT");

                        //  Store register update.  
                        updateBuffer[regGroup][nextFreeUpdate[regGroup]].reg = lastMetaStreamTrans->getGPURegister();
                        updateBuffer[regGroup][nextFreeUpdate[regGroup]].subReg = lastMetaStreamTrans->getGPUSubRegister();
                        updateBuffer[regGroup][nextFreeUpdate[regGroup]].data = lastMetaStreamTrans->getGPURegData();
...
// CG_ASSERT("JUST TO KNOW ABOUT IT!!");                        //  The transaction couldn't be processed, so wait until it can be.  
                        processNewTransaction = FALSE;
                    }
                }
            }
            else
            {
                ...
                processGPURegisterWrite(cycle, lastMetaStreamTrans->getGPURegister(),
                    lastMetaStreamTrans->getGPUSubRegister(), lastMetaStreamTrans->getGPURegData());
        ...
        case META_STREAM_REG_READ:
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => Procesing MetaStream: META_STREAM_REG_READ.\n"); )

            //  Process a register read request.  
            processGPURegisterRead(lastMetaStreamTrans->getGPURegister(), lastMetaStreamTrans->getGPUSubRegister(),
                data);

            //  Create an MetaStream to return the read value.  
            //readMetaStreamTrans = new cgoMetaStream(META_STREAM_REG_READ, lastMetaStreamTrans->getGPURegister(),
            //    lastMetaStreamTrans->getSubRegister(), data);

            //  Send the transaction to the driver.  
            //driver->sendcgoMetaStream(readMetaStreamTrans);
        ...
        case META_STREAM_COMMAND:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Procesing MetaStream: META_STREAM_COMMAND.\n");
            )

            //  Process GPU command.  
            processGPUCommand(cycle, lastMetaStreamTrans->getGPUCommand());
        ...
        case META_STREAM_INIT_END:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Processing MetaStream : META_STREAM_INIT_END.\n");
            )
        ...
        case META_STREAM_EVENT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Processign MetaStream : META_STREAM_EVENT.\n");
            )

            //  Process the event.
            processGPUEvent(cycle, lastMetaStreamTrans->getGPUEvent(), lastMetaStreamTrans->getGPUEventMsg());

            break;

        default:
            //  Unknown MetaStream command.  

            CG_ASSERT("Undefined MetaStream type.");
            break;
    }
...
void cmoCommandProcessor::processGPURegisterWrite(U64 cycle, GPURegister gpuReg, U32 gpuSubReg,
    GPURegData gpuData)
{
    StreamerCommand *streamComm;
    ShaderCommand *shComm;
    ...
    switch (gpuReg)
    {
        ...
        case GPU_DISPLAY_X_RES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DISPLAY_X_RES.\n");
            )

            //  Check x resolution range.  
            CG_ASSERT_COND(!(gpuData.uintVal > MAX_DISPLAY_RES_X), "Horizontal display resolution not supported.");
            ...
            //  Copy cookies from original MetaStream and a new cookie.  
            rastCom->copyParentCookies(*lastMetaStreamTrans);
            rastCom->addCookie();
            ...
            //  Send state to DisplayController.  
            ...
            //  Send state change to DisplayController.  
        ...
        case GPU_DISPLAY_Y_RES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DISPLAY_Y_RES.\n");
            )

            //  Check y resolution range.  
            CG_ASSERT_COND(!(gpuData.uintVal > MAX_DISPLAY_RES_Y), "Vertical display resolution not supported.");
        ...
        case GPU_D3D9_PIXEL_COORDINATES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_D3D9_PIXEL_COORDINATES.\n");
            )
        ...
        case GPU_VIEWPORT_INI_X:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VIEWPORT_INI_X.\n");
            )
        ...
        case GPU_VIEWPORT_INI_Y:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VIEWPORT_INI_Y.\n");
            )
        ...
        case GPU_VIEWPORT_HEIGHT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VIEWPORT_HEIGHT.\n");
            )
        ...
        case GPU_VIEWPORT_WIDTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VIEWPORT_WIDTH.\n");
            )
        ...
        case GPU_DEPTH_RANGE_NEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTHRANGE_NEAR.\n");
            )
        ...
        case GPU_DEPTH_RANGE_FAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTHRANGE_FAR.\n");
            )
        ...
        case GPU_COLOR_BUFFER_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_COLOR_BUFFER_CLEAR = {%f, %f, %f, %f}.\n",
        ...
        case GPU_BLIT_INI_X:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_INI_X = %x.\n", gpuData.uintVal);
            )

            //  Set GPU blit initial x coordinate register.  
            state.blitIniX = gpuData.uintVal;

            //  Send state to DisplayController/Blitter.  
        ...
        case GPU_BLIT_INI_Y:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_INI_Y = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_X_OFFSET:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_X_OFFSET = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_Y_OFFSET:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_Y_OFFSET = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_WIDTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_WIDTH = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_HEIGHT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_HEIGHT = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_DST_ADDRESS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_DST_ADDRESS = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BLIT_DST_TX_WIDTH2:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_DST_TX_WIDTH2 = %x.\n", gpuData.txFormat);
            )
        ...
        case GPU_BLIT_DST_TX_FORMAT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_DST_TX_FORMAT = %x.\n", gpuData.txFormat);
            )
        ...
        case GPU_BLIT_DST_TX_BLOCK:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLIT_DST_TX_BLOCK = %x.\n", gpuData.txFormat);
            )
        ...
        case GPU_Z_BUFFER_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_Z_BUFFER_CLEAR.\n");
            )
        ...
        case GPU_Z_BUFFER_BIT_PRECISSION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_Z_BUFFER_BIT_PRECISSION.\n");
            )
        ...
        case GPU_STENCIL_BUFFER_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_BUFFER_CLEAR.\n");
            )
        ...
        case GPU_FRONTBUFFER_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_FRONTBUFFER_ADDR = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_BACKBUFFER_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_BACKBUFFER_ADDR = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_ZSTENCILBUFFER_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_ZSTENCILBUFFER_ADDR.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_ZSTENCILBUFFER_ADDR = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_COLOR_STATE_BUFFER_MEM_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_COLOR_STATE_BUFFER_MEM_ADDR = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR.\n");
            )
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR = %x.\n", gpuData.uintVal); )
        ...
        case GPU_TEXTURE_MEM_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MEM_ADDR.\n");
            )
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => GPU_TEXTURE_MEM_ADDR = %x.\n", gpuData.uintVal); )
        ...
        case GPU_PROGRAM_MEM_ADDR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_PROGRAM_MEM_ADDR.\n");
            )
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => GPU_FRONTBUFFER_ADDR = %x.\n", gpuData.uintVal); )
        ...
        case GPU_VERTEX_PROGRAM:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_PROGRAM.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_VERTEX_PROGRAM = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_VERTEX_PROGRAM_PC:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_PROGRAM_PC.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_VERTEX_PROGRAM_PC = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_VERTEX_PROGRAM_SIZE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_PROGRAM_SIZE.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_VERTEX_PROGRAM_SIZE = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_VERTEX_THREAD_RESOURCES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_THREAD_RESOURCES.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_THREAD_RESOURCES = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_VERTEX_CONSTANT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_CONSTANT.\n");
            )
        ...
        case GPU_VERTEX_OUTPUT_ATTRIBUTE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_OUTPUT_ATTRIBUTE[%d] = %s.\n",
            ...
            //  Create StreamController command.  
            ...
            //  Send state change to the StreamController unit.  
            streamCtrlSignal->write(cycle, streamComm);
            ...
            //  Copy cookies from original MetaStream and add new cookie.  
            paComm->copyParentCookies(*lastMetaStreamTrans);
            paComm->addCookie();
        ...
        case GPU_VERTEX_ATTRIBUTE_MAP:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_ATTRIBUTE_MAP.\n");
            )
        ...
        case GPU_VERTEX_ATTRIBUTE_DEFAULT_VALUE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_VERTEX_ATTRIBUTE_DEFAULT_VALUE.\n");
            )
        ...
        case GPU_STREAM_ADDRESS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_ADDRESS.\n");
            )
        ...
        case GPU_STREAM_STRIDE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_STRIDE.\n");
            )
        ...
        case GPU_STREAM_DATA:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_DATA.\n");
            )
        ...
        case GPU_STREAM_ELEMENTS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_ELEMENTS.\n");
            )
        ...
        case GPU_STREAM_FREQUENCY:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_FREQUENCY.\n");
            )
        ...
        case GPU_STREAM_START:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_START.\n");
            )
        ...
        case GPU_STREAM_COUNT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_COUNT.\n");
            )
        ...
        case GPU_STREAM_INSTANCES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STREAM_INSTANCES.\n");
            )
        ...
        case GPU_INDEX_MODE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_INDEX_MODE.\n");
            )
        ...
        case GPU_INDEX_STREAM:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_INDEX_STREAM.\n");
            )
        ...
        case GPU_D3D9_COLOR_STREAM:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_D3D9_COLOR_STREAM.\n");
            )
        ...
        case GPU_ATTRIBUTE_LOAD_BYPASS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_ATTRIBUTE_LOAD_BYPASS.\n");
            )
        ...
        case GPU_PRIMITIVE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_PRIMITIVE.\n");
            )
        ...
        case GPU_FRUSTUM_CLIPPING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_FRUSTUM_CLIPPING.\n");
            )
            ...
            /*  Copy cookies from original MetaStream and add a new
                cookie.  */
            clipperCommand->copyParentCookies(*lastMetaStreamTrans);
            clipperCommand->addCookie();
        ...
        case GPU_USER_CLIP:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_USER_CLIP.\n");
            )
        ...
        case GPU_USER_CLIP_PLANE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_USER_CLIP_PLANE.\n");
            )
        ...
        case GPU_FACEMODE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_FACEMODE.\n");
            )
        ...
        case GPU_CULLING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_CULLING.\n");
            )
        ...
        case GPU_HIERARCHICALZ:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_HIERARCHICALZ.\n");
            )
        ...
        case GPU_EARLYZ:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_EARLYZ.\n");
            )
        ...
        case GPU_SCISSOR_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_SCISSOR_TEST.\n");
            )
        ...
        case GPU_SCISSOR_INI_X:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_SCISSOR_INI_X.\n");
            )
        ...
        case GPU_SCISSOR_INI_Y:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_SCISSOR_INI_Y.\n");
            )
        ...
        case GPU_SCISSOR_WIDTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_SCISSOR_WIDTH.\n");
            )
        ...
        case GPU_SCISSOR_HEIGHT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_SCISSOR_HEIGHT.\n");
            )
        ...
        case GPU_DEPTH_SLOPE_FACTOR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_SLOPE_FACTOR.\n");
            )
        ...
        case GPU_DEPTH_UNIT_OFFSET:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_UNIT_OFFSET.\n");
            )
        ...
        case GPU_D3D9_DEPTH_RANGE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_D3D9_DEPTH_RANGE = %s.\n", gpuData.booleanVal ? "T" : "F");
            )
            ...
            //  Copy cookies from original MetaStream and add a new cookie.
            clipperCommand->copyParentCookies(*lastMetaStreamTrans);
            clipperCommand->addCookie();
        ...
        case GPU_D3D9_RASTERIZATION_RULES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_D3D9_RASTERIZATION_RULES = %s.\n", gpuData.booleanVal ? "T" : "F");
            )
        ...
        case GPU_TWOSIDED_LIGHTING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TWOSIDED_LIGHTING.\n");
            )
        ...
        case GPU_MULTISAMPLING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_MULTISAMPLING.\n");
            )
        ...
        case GPU_MSAA_SAMPLES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_MSAA_SAMPLES.\n");
            )
        ...
        case GPU_INTERPOLATION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_INTERPOLATION.\n");
            )
        ...
        case GPU_FRAGMENT_INPUT_ATTRIBUTES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_FRAGMENT_INPUT_ATTRIBUTES[%d] = %s.\n",
                gpuSubReg, gpuData.booleanVal?"ENABLED":"DISABLED");
            )
        ...
        case GPU_FRAGMENT_PROGRAM:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_FRAGMENT_PROGRAM = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_FRAGMENT_PROGRAM_PC:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_FRAGMENT_PROGRAM_PC = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_FRAGMENT_PROGRAM_SIZE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_FRAGMENT_PROGRAM_SIZE = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_FRAGMENT_THREAD_RESOURCES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_FRAGMENT_THREAD_RESOURCES.\n");
            )
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_FRAGMENT_THREAD_RESOURCE = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_FRAGMENT_CONSTANT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_FRAGMENT_CONSTANT.\n");
            )
        ...
        case GPU_MODIFY_FRAGMENT_DEPTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_MODIFY_FRAGMENT_DEPTH.\n");
            )
        ...
        case GPU_SHADER_PROGRAM_ADDRESS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_SHADER_PROGRAM_ADDDRESS = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_SHADER_PROGRAM_SIZE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_SHADER_PROGRAM_SIZE = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_SHADER_PROGRAM_LOAD_PC:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => GPU_SHADER_PROGRAM_LOAD_PC = %x.\n", gpuData.uintVal);
            )
        ...
        case GPU_SHADER_PROGRAM_PC:

            {
                ...
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => GPU_SHADER_PROGRAM_LOAD_PC[");
        ...
        case GPU_SHADER_THREAD_RESOURCES:

            {
                ...
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => GPU_SHADER_THREAD_RESOURCES[");
        ...
        case GPU_TEXTURE_ENABLE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_ENABLE.\n");
            )
        ...
        case GPU_TEXTURE_MODE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MODE.\n");
            )
        ...
        case GPU_TEXTURE_ADDRESS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_ADDRESS.\n");
            )
        ...
        case GPU_TEXTURE_WIDTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_WIDTH.\n");
            )
        ...
        case GPU_TEXTURE_HEIGHT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_HEIGHT.\n");
            )
        ...
        case GPU_TEXTURE_DEPTH:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_DEPTH.\n");
            )
        ...
        case GPU_TEXTURE_WIDTH2:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_WIDTH2.\n");
            )
        ...
        case GPU_TEXTURE_HEIGHT2:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_HEIGHT2.\n");
            )
        ...
        case GPU_TEXTURE_DEPTH2:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_DEPTH2.\n");
            )
        ...
        case GPU_TEXTURE_BORDER:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_BORDER.\n");
            )
        ...
        case GPU_TEXTURE_FORMAT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_FORMAT.\n");
            )
        ...
        case GPU_TEXTURE_REVERSE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_REVERSE.\n");
            )
        ...
        case GPU_TEXTURE_D3D9_COLOR_CONV:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_D3D9_COLOR_CONV.\n");
            )
        ...
        case GPU_TEXTURE_D3D9_V_INV:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_D3D9_V_INV.\n");
            )
        ...
        case GPU_TEXTURE_COMPRESSION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_COMPRESSION.\n");
            )
        ...
        case GPU_TEXTURE_BLOCKING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_BLOCKING.\n");
            )
        ...
        case GPU_TEXTURE_BORDER_COLOR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_COLOR.\n");
            )
        ...
        case GPU_TEXTURE_WRAP_S:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_WRAP_S.\n");
            )
        ...
        case GPU_TEXTURE_WRAP_T:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_WRAP_T.\n");
            )
        ...
        case GPU_TEXTURE_WRAP_R:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_WRAP_R.\n");
            )
        ...
        case GPU_TEXTURE_NON_NORMALIZED:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_NON_NORMALIZED.\n");
            )
        ...
        case GPU_TEXTURE_MIN_FILTER:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MIN_FILTER.\n");
            )
        ...
        case GPU_TEXTURE_MAG_FILTER:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MAG_FILTER.\n");
            )
        ...
        case GPU_TEXTURE_ENABLE_COMPARISON:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_ENABLE_COMPARISON = %s.\n", gpuData.booleanVal ? "T" : "F");
            )
        ...
        case GPU_TEXTURE_COMPARISON_FUNCTION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_COMPARISON_FUNCTION = ");
        ...
        case GPU_TEXTURE_SRGB:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_SRGB = %s.\n", gpuData.booleanVal ? "T" : "F");
            )
        ...
        case GPU_TEXTURE_MIN_LOD:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MIN_LOD.\n");
            )
        ...
        case GPU_TEXTURE_MAX_LOD:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MAX_LOD.\n");
            )
        ...
        case GPU_TEXTURE_LOD_BIAS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_LOD_BIAS.\n");
            )
        ...
        case GPU_TEXTURE_MIN_LEVEL:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MIN_LEVEL.\n");
            )
        ...
        case GPU_TEXTURE_MAX_LEVEL:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MAX_LEVEL.\n");
            )
        ...
        case GPU_TEXT_UNIT_LOD_BIAS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXT_UNIT_LOD_BIAS.\n");
            )
        ...
        case GPU_TEXTURE_MAX_ANISOTROPY:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_TEXTURE_MAX_ANISOTROPY.\n");
            )
        ...
        case GPU_STENCIL_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_TEST.\n");
            )
        ...
        case GPU_STENCIL_FUNCTION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_FUNCTION.\n");
            )
        ...
        case GPU_STENCIL_REFERENCE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_REFERENCE.\n");
            )
        ...
        case GPU_STENCIL_COMPARE_MASK:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_COMPARE_MASK.\n");
            )
        ...
        case GPU_STENCIL_UPDATE_MASK:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_UPDATE_MASK.\n");
            )
        ...
        case GPU_STENCIL_FAIL_UPDATE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_STENCIL_FAIL_UPDATE.\n");
            )
        ...
        case GPU_DEPTH_FAIL_UPDATE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_FAIL_UPDATE.\n");
            )
        ...
        case GPU_DEPTH_PASS_UPDATE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_PASS_UPDATE.\n");
            )
        ...
        case GPU_DEPTH_TEST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_TEST.\n");
            )
        ...
        case GPU_DEPTH_FUNCTION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_FUNCION.\n");
            )
        ...
        case GPU_DEPTH_MASK:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_DEPTH_MASK.\n");
            )
        ...
        case GPU_ZSTENCIL_COMPRESSION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_ZSTENCIL_COMPRESSION.\n");
            )
        ...
        case GPU_COLOR_BUFFER_FORMAT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_COLOR_BUFFER_FORMAT.\n");
            )
        ...
        case GPU_COLOR_COMPRESSION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_COLOR_COMPRESSION = %s.\n", gpuData.booleanVal ? "T" : "F");
            )
        ...
        case GPU_COLOR_SRGB_WRITE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_COLOR_WRITE_SRGB.\n");
            )
        ...
        case GPU_RENDER_TARGET_ENABLE:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_RENDER_TARGET_ENABLE.\n");
            )
        ...
        case GPU_RENDER_TARGET_FORMAT:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_RENDER_TARGET_FORMAT.\n");
            )
        ...
        case GPU_RENDER_TARGET_ADDRESS:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_RENDER_TARGET_ADDRESS[%d] = %08x.\n", gpuSubReg, gpuData.uintVal);
            )
        ...
        case GPU_COLOR_BLEND:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_COLOR_BLEND.\n");
            )
        ...
        case GPU_BLEND_EQUATION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_EQUATION.\n");
            )
        ...
        case GPU_BLEND_SRC_RGB:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_SRC_RGB.\n");
            )
        ...
        case GPU_BLEND_DST_RGB:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_DST_RGB.\n");
            )
        ...
        case GPU_BLEND_SRC_ALPHA:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_SRC_ALPHA.\n");
            )
        ...
        case GPU_BLEND_DST_ALPHA:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_DST_ALPHA.\n");
            )
        ...
        case GPU_BLEND_COLOR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_COLOR.\n");
            )
        ...
        case GPU_COLOR_MASK_R:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_MASK_R.\n");
            )
        ...
        case GPU_COLOR_MASK_G:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_MASK_G.\n");
            )
        ...
        case GPU_COLOR_MASK_B:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_MASK_B.\n");
            )
        ...
        case GPU_COLOR_MASK_A:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_BLEND_MASK_A.\n");
            )
        ...
        case GPU_LOGICAL_OPERATION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_LOGICAL_OPERATION.\n");
            )
        ...
        case GPU_LOGICOP_FUNCTION:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_LOGICOP_FUNCTION.\n");
            )
        ...
        case GPU_MCV2_2ND_INTERLEAVING_START_ADDR:
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => Write GPU_MCV2_2ND_INTERLEAVING_START_ADDR.\n");
            )
            state.mcSecondInterleavingStartAddr = gpuData.uintVal;
            {
                ...
                mccom->copyParentCookies(*lastMetaStreamTrans);
                mccom->addCookie();
...
void cmoCommandProcessor::processGPURegisterRead(GPURegister gpuReg, U32 gpuSubReg,
    GPURegData &gpuData)
{
    ...
    //  Send back the read value through the MetaStream port.  
...
void cmoCommandProcessor::processGPUCommand(U64 cycle, GPUCommand gpuComm)
{
    ...
    switch (gpuComm)
    {
        case GPU_RESET:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => RESET command.\n");
            )

            //  Create a reset command to the cmoStreamController unit.  
            ...
            //  Send reset signal to the cmoStreamController Unit.  
            streamCtrlSignal->write(cycle, streamComm);
            ...
            /*  Copy cookies from original MetaStream and add a new
                cookie.  */
            clipComm->copyParentCookies(*lastMetaStreamTrans);
            clipComm->addCookie();
            ...
            for(U32 e = 0; e < GPU_NUM_EVENTS; e++)
                lastEventCycle[e] = 0;

            //  The MetaStream finished.  Process a new one.  
            processNewTransaction = TRUE;
...
//printf("CP %lld => Change to RESET state.\n", cycle);
        ...
        case GPU_DRAW:
            ...
            if (((state.statusRegister != GPU_READY) && (state.statusRegister != GPU_END_GEOMETRY)) || (streamState != ST_READY) ||
                (drawCommandDelay > 0))
            {
                //  MetaStream couldn't be processed so wait.  
                processNewTransaction = FALSE;
            }
            else
            {
                ...
                //        CG_ASSERT("Draw command while still drawing not supported.");
                //)

                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => DRAW command.\n");
                )
...
//printf("CP => Starting batch %d\n", batch);
                ...
                if (!skipDraw)
                {
                    //  Create a start drawing signal to the cmoStreamController unit.  
                    ...
                    //  Send the command to the cmoStreamController.  
                    streamCtrlSignal->write(cycle, streamComm);
...
//printf("CP %lld => Change to DRAWING state.\n", cycle);
                ...
                //  Send DRAW command to the DisplayController unit.
                rastComm = new RasterizerCommand(RSCOM_DRAW);
        ...
        case GPU_SWAPBUFFERS:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => SWAPBUFFERS command.\n");
                )
...
//printf("CP %lld => Change to SWAP state.\n", cycle);
                ...
                {
                    //  Send FRAME CHANGE command to the DisplayController unit.
        ...
        case GPU_DUMPCOLOR:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => DUMPCOLOR command.\n");
                )

                //  Set comamnd for the DisplayController unit.
...
//printf("CP %lld => Change to DUMPBUFFER state.\n", cycle);
        ...
        case GPU_DUMPDEPTH:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => DUMPDEPTH command.\n");
                )
...
//printf("CP %lld => Change to DUMPBUFFER state.\n", cycle);
        ...
        case GPU_DUMPSTENCIL:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => DUMPSTENCIL command.\n");
                )
...
//printf("CP %lld => Change to DUMPBUFFER state.\n", cycle);
        ...
        case GPU_BLIT:
            ...
            //  lines state. Then, signals the DisplayController unit to transfer  
            //  and convert data.                                    
            ...
            else
            {
                ...
                {
                    GPU_DEBUG(
                        printf("cmoCommandProcessor => BLIT command.\n");
                    )
...
//printf("CP %lld => Change to BLITTING state.\n", cycle);
        ...
        case GPU_CLEARBUFFERS:
            ...
            GPU_DEBUG_BOX(printf("cmoCommandProcessor => CLEARBUFFERS command.\n"); )

            CG_ASSERT("CLEARBUFFERS command not implemented.");
        ...
        case GPU_CLEARZBUFFER:
            ...
            GPU_DEBUG_BOX(
                printf("cmoCommandProcessor => CLEARZBUFFER command.\n");
            )

            //CG_ASSERT("CLEARZBUFFER command not supported.");
            ...
            CG_ASSERT("CLEARZSTENCILBUFFERS command not implemented.");
        ...
        case GPU_CLEARZSTENCILBUFFER:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => CLEARZSTENCILBUFFERS command.\n");
                )
...
//printf("CP %lld => Change to CLEAR_Z state.\n", cycle);
        ...
        case GPU_CLEARCOLORBUFFER:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => CLEARCOLORBUFFER command.\n");
                )
...
//printf("CP %lld => Change to CLEAR_COLOR state.\n", cycle);
        ...
        case GPU_FLUSHZSTENCIL:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => FLUSHZSTENCIL command.\n");
                    )
...
//printf("CP %lld => Change to FLUSH_Z state.\n", cycle);
        ...
        case GPU_FLUSHCOLOR:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => FLUSHCOLOR command.\n");
                    )
...
//printf("CP %lld => Change to FLUSH_COLOR state.\n", cycle);
        ...
        case GPU_LOAD_VERTEX_PROGRAM:
            ...
            {
                GPU_DEBUG_BOX(
                    printf("cmoCommandProcessor => LOAD_VERTEX_PROGRAM command.\n");
                )
                ...
                programCode = new U08[state.vertexProgramSize];

                //  Check memory allocation.  
                CG_ASSERT_COND(!(programCode == NULL), "Allocation of the vertex program code buffer failed.");
                //  Check if memory controller is available.  
        ...
        case GPU_LOAD_FRAGMENT_PROGRAM:
            ...
            else
            {
                ...
                if (state.statusRegister == GPU_END_GEOMETRY)
                {
                    ...
                    loadFragProgram = new cgoMetaStream(GPU_LOAD_FRAGMENT_PROGRAM);
                    loadFragProgram->copyParentCookies(*lastMetaStreamTrans);
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => LOAD_FRAGMENT_PROGRAM command.\n");
                    )
                    ...
                    programCode = new U08[state.fragProgramSize];

                    //  Check memory allocation.  
                    CG_ASSERT_COND(!(programCode == NULL), "Allocation of the fragment program code buffer failed.");
                    //  Check if memory controller is available.  
                    if ((memoryState & MS_READ_ACCEPT) != 0)
                    {
...
//printf("CP %lld => Change to MEMORY_READ state.\n", cycle);
        ...
        case GPU_LOAD_SHADER_PROGRAM:
            ...
            else
            {
                ...
                //  Check memory allocation.
                CG_ASSERT_COND(!(programCode == NULL), "Allocation of the shader program code buffer failed.");
                //  Check if memory controller is available.
                if ((memoryState & MS_READ_ACCEPT) != 0)
                {
...
//printf("CP %lld => Change to MEMORY_READ state.\n", cycle);
        ...
        case GPU_SAVE_COLOR_STATE:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => SAVE_COLOR_STATE command.\n");
                    )
...
//printf("CP %lld => Change to SAVE_COLOR_STATE state.\n", cycle);
        ...
        case GPU_RESTORE_COLOR_STATE:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => RESTORE_COLOR_STATE command.\n");
                    )
...
//printf("CP %lld => Change to RESTORE_COLOR_STATE state.\n", cycle);
        ...
        case GPU_SAVE_ZSTENCIL_STATE:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => SAVE_ZSTENCIL_STATE command.\n");
                    )
...
//printf("CP %lld => Change to RESTORE_STATE_Z state.\n", cycle);
        ...
        case GPU_RESET_ZSTENCIL_STATE:
            ...
            {            
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => GPU_RESET_ZSTENCIL_STATE command.\n");
                    )
        ...
        case GPU_RESET_COLOR_STATE:
            ...
            else
            {
                ...
                {
                    GPU_DEBUG_BOX(
                        printf("cmoCommandProcessor => GPU_RESET_COLOR_STATE command.\n");
                    )
        ...
        default:
            ...
            CG_ASSERT("Undefined GPU Command.");
...
void cmoCommandProcessor::processGPUEvent(U64 cycle, GPUEvent gpuEvent, string eventMsg)
{
    //  Process event.
    switch(gpuEvent)
    {
...
void cmoCommandProcessor::processMemoryTransaction(MemoryTransaction *memTrans)
{
    ...
    switch(memTrans->getCommand())
    {
        case MT_STATE:
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => Memory transaction: MT_STATE.\n"); )
        ...
        case MT_READ_DATA:
            ...
            GPU_DEBUG_BOX( printf("cmoCommandProcessor => Memory transaction: MT_READ_DATA.  Trans. cycles: %d\n", memTrans->getBusCycles()); )

            //  Check memory transmissions.  
            CG_ASSERT_COND(!(transCycles != 0), "Received memory transaction while bus was still busy.");
...
void cmoCommandProcessor::reset()
{
...
bool cmoCommandProcessor::isEndOfTrace()
{
    return (traceEnd && (state.statusRegister == GPU_READY));
}

//  Get if swap was received.  
bool cmoCommandProcessor::isSwap()
{
    return swapReceived;
}

//  Get end of batch flag.  
bool cmoCommandProcessor::endOfBatch()
{
    return batchEnd;
}

//  Returns a string with state information about the mdu.
void cmoCommandProcessor::getState(string &stateString)
{
    stateString.clear();
...
void cmoCommandProcessor::setSkipDraw(bool skip)
{
    skipDraw = skip;
}

//  Set the skip frames command flag.
void cmoCommandProcessor::setSkipFrames(bool skip)
{
    skipFrames = skip;
    skipDraw = skip;
}
...
void cmoCommandProcessor::getCommandList(std::string &commandList)
{
    commandList.append("forceswap       - Forces a SWAP command after the current MetaStream command\n");
    commandList.append("flushzst        - Forces a FLUSHZST command after the current MetaStream command\n");
    commandList.append("flushcolor      - Forces a FLUSHCOLOR command after the current MetaStream command\n");
...
void cmoCommandProcessor::execCommand(stringstream &commandStream)
{
    string command;
    ...
    if (!command.compare("forceswap"))
    {
        ...
        else
        {
            ...
            //  Check if no MetaStream is pending from being finished.
            if (processNewTransaction)
            {
                //  Delete finished MetaStream.
                if (lastMetaStreamTrans != NULL)
                    delete lastMetaStreamTrans;

                //  Insert the forced swap command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_SWAPBUFFERS);

                //  Unset new transaction flag as we have a pending MetaStream now.
                processNewTransaction = false;
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_SWAPBUFFERS);
    ...
    else if(!command.compare("flushzst"))
    {
        ...
        else
        {
            ...
            if (processNewTransaction)
            {
                ...
                //  Insert the forced flush z and stencil caches command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_FLUSHZSTENCIL);
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_FLUSHZSTENCIL);
    ...
    else if(!command.compare("flushcolor"))
    {
        ...
        else
        {
            ...
            if (processNewTransaction)
            {
                ...
                //  Insert the forced flush color caches command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_FLUSHCOLOR);
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_FLUSHCOLOR);
    ...
    else if(!command.compare("dumpcolor"))
    {
        ...
        else
        {
            ...
            if (processNewTransaction)
            {
                ...
                //  Insert the forced dump color command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_DUMPCOLOR);
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_DUMPCOLOR);
    ...
    else if(!command.compare("dumpdepth"))
    {
        ...
        else
        {
            ...
            if (processNewTransaction)
            {
                ...
                //  Insert the forced dump depth command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_DUMPDEPTH);
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_DUMPDEPTH);
    ...
    else if(!command.compare("dumpstencil"))
    {
        ...
        else
        {
            ...
            if (processNewTransaction)
            {
                ...
                //  Insert the forced dump stencil command as next MetaStream command to execute.
                lastMetaStreamTrans = new cgoMetaStream(GPU_DUMPSTENCIL);
            ...
            else
            {
                ...
                forcedTransaction = new cgoMetaStream(GPU_DUMPSTENCIL);
...
bool cmoCommandProcessor::endOfInitialization()
{
    return initEnd;
}

//  Return if the last command finished in this cycle.
bool cmoCommandProcessor::endOfCommand()
{
    return commandEnd;
}

//  Return if the forced command finished in this cycle.
bool cmoCommandProcessor::endOfForcedCommand()
{
...
void cmoCommandProcessor::saveRegisters()
{
    ofstream out;
...
vector<cgoMetaStream*> &cmoCommandProcessor::getcgoMetaStreamLog()
{
...
void cmoCommandProcessor::setValidationMode(bool enable)
{
