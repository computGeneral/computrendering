...
BufferDescriptor* BufferManager::find(uint id) const
{
    if ( workingMode == NotSelected )
    {
        ...
        logfile().popInfo();
        CG_ASSERT("Working mode not selected yet");
    }
...
void BufferManager::setMemory(vector<BufferDescriptor*> bds)
{
    ...
    if ( groups.size() != maxAddressInGroup.size() )
    {
        ...
        logfile().popInfo();
        CG_ASSERT("Programming error. Shouldn't happen ever");
    }
...
BufferDescriptor* BufferManager::create(const BPtr ptr, uint bytes)
{
    if ( workingMode != Storing )
    {
        ...
        else // Working mode == Loading
        {
            ...
            logfile().popInfo();
            CG_ASSERT("Method call not allowed in \"Loading\" mode");
        }
    }
    ...
    if ( bytes == 0 && ptr == 0 )
    {
        ...
        logfile().popInfo();
        CG_ASSERT("Data pointer & size cannot be both zero");
    }
                         ...
                         WorkingMode mode)
{
    ...
    {        
        ...
        if ( !bdv.empty() )
        {
            ...
            logfile().popInfo();
            CG_ASSERT("Previous buffer descriptors loaded");
        }
    ...
    bm(manager), mr(0), size(bytes), id(0), offset(0)
{
   //  Check pointer size.
   if (sizeof(void *) == 8)
      CG_ASSERT("Error: only 32-bit pointers supported");
