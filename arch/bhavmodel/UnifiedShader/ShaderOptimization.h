...
private:
     ...
     *  a CG_ISA_OPCODE_MOV instruction) of such values. Used by the copy propagation optimization pass.
     */
    struct ValueCopy
    {
     ...
     *  As the only three operand instruction is CG_ISA_OPCODE_MAD and the component usage is the same for all three operands
...
public:
    ...
    /**
     *
     *  Shader program optimization that removes redundant CG_ISA_OPCODE_MOV instructions (copies from the input operand to the same output
     *  result).
    ...
    /**
     *
     *  Shader program optimization that removes copies of values (CG_ISA_OPCODE_MOV instructions) by propagating the original copy
    ...
    /**
     *
     *  Assembles a shader program written in Shader Assembler Language.
     *
     *  @param program Pointer to a byte array with the shader program written in Shader Assembler Language.
    ...
    /**
     *
     *  Disassembles an encoded shader program to Shader Assembler Language.
     *
     *  @param code Pointer to a byte array with shader program code.
     *  @param codeSize Size of the encoded shader program in bytes.
     *  @param program Pointer to a byte array where to store the disassembled shader program in Shader Assembler Language.
