...
------------------------
    ...
    driver/                          API related source code.
        ...
        (metaStream traceTranslator has been removed — depended on removed TraceReader)
        ...
        utils/                       Miscellaneous classes used by the API source.
            ApitraceParser/          Apitrace binary format parser and call dispatchers (OGL & D3D9).
            TraceDriver/             Trace driver implementations (Apitrace OGL, Apitrace D3D9, MetaStream).
            OGLApiCodeGen/           Automatized code generation for the OpenGL API tools.
     ...
     tests/                          CG1 simulator mini-regression/simulator(mdl) tests.
        ...
        d3d/                         D3D9 apitrace traces for the CG1 simulator mini-regression test.
        ogl/                         OpenGL apitrace traces for the CG1 simulator mini-regression test.
...
B) OpenGL tools
    ...
    -> Select the configuration
        ...
        - Profile   : profiling build
    -> Build the selected project
        - GLInterceptor : OpenGL trace capturer, creates opengl32.dll (legacy)
        - GLTracePlayer      : OpenGL trace player (legacy)
    -> The compiled library or binaries can be found on the "BUILD/bin/" directory

C) Tools in the CG1 solution
...
# Run with an OpenGL apitrace (glxgears example)
...
### General Usage
    ...
    2. Execute the simulator
        ...
        When parameters are used:
            ...
            filename        Apitrace trace file (.trace) for OpenGL or D3D9
                            (auto-detected from trace header)
                            or
                            MetaStream trace file (.tracefile.gz)
...
#How to run the CG1 simulator or behaviorModel with MetaStream traces
---------------------------------------------------------------------------

> **Note:** The `traceTranslator` tool that generated MetaStream traces from GLInterceptor traces has been removed (it depended on the removed `TraceReader`). Pre-existing `.tracefile.gz` files can still be replayed directly.

    1. Copy the simulator (CG1SIM), a configuration file (CG1GPU.ini), and the MetaStream trace file
       (`.tracefile.gz`) into the same directory.
...
- **OGL**: Verified byte-identical output against reference PPMs
- **D3D9**: Functional playback of D3D9 game traces

### MetaStream Format (Optimized)

- **Generation**: Legacy `traceTranslator` tool has been removed; existing `.tracefile.gz` files can still be replayed
...
| Trace            | Path                                                   |
|------------------|--------------------------------------------------------|
| glxgears         | `tests/ogl/trace/glxgears/glxgears.trace`              |
...
diff _BUILD_/arch/frame0000.cm.ppm tests/ogl/trace/glxgears/glxgears.ppm

# If using cmp (silent on match):
cmp _BUILD_/arch/frame0000.cm.ppm tests/ogl/trace/glxgears/glxgears.ppm && echo "PASS" || echo "FAIL"
```
...
```bash
# Decompress and diff
zdiff _BUILD_/arch/stats.frames.csv.gz tests/ogl/trace/glxgears/stats.frames.csv.gz
```
...
Regression scripts are provided for both platforms:
...
| Item                     | Notes                                                         |
|--------------------------|---------------------------------------------------------------|
| D3D9 traces              | Supported via apitrace on all platforms                       |
