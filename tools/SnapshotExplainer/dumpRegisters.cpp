...
// Macro for fast typing in HDPTransaction::dump() method */
...
void dumpRegisters(cgsGpuRegister &state)
{
    ...
    for (U32 c = 0; c < MAX_USER_CLIP_PLANES; c++)
    ...
    for(U32 t = 0; t < MAX_TEXTURES; t++)
        for(U32 m = 0; m < MAX_TEXTURE_SIZE; m++)
            for(U32 c = 0; c < CUBEMAP_IMAGES; c++)
    ...
    for(U32 t = 0; t < MAX_RENDER_TARGETS; t++)
