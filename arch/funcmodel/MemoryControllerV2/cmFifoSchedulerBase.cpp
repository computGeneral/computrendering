...
cSched_IdleCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_IdleCycles")),
cSched_PrevCmdWaitCycles(getSM().getNumericStatistic(prefix, U32(0), "ChannelScheduler", "ChSched_PrevCmdWaitCycles")),
cSched_PrevPageCloseCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_PrevPageCloseCycles")),
cSched_OpeningPageCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_OpeningPageCycles")),
cSched_AccessingReadDelayCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_AccessingReadDelayCycles")),
cSched_AccessingReadDataCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_AccessingReadDataCycles")),
cSched_AccessingWriteDelayCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_AccessingWriteDelayCycles")),
cSched_AccessingWriteDataCycles(getSM().getNumericStatistic(prefix, U32(0),"ChannelScheduler", "ChSched_AccessingWriteDataCycles")),

ctrlIdleCyclesStat(getSM().getNumericStatistic("ctrlIdleCycles", U32(0),"FifoSchedulerBase", prefix)),

ctrlUsedCyclesStat(getSM().getNumericStatistic("ctrlUsedCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),

atorStat(getSM().getNumericStatistic("act2readCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
atowStat(getSM().getNumericStatistic("act2writeCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
atoaStat(getSM().getNumericStatistic("act2actCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
ptoaStat(getSM().getNumericStatistic("pre2actCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
wtorStat(getSM().getNumericStatistic("write2readCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
rtowStat(getSM().getNumericStatistic("read2writeCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
wtopStat(getSM().getNumericStatistic("write2preCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
atopStat(getSM().getNumericStatistic("act2preCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
rtopStat(getSM().getNumericStatistic("read2preCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
rtorStat(getSM().getNumericStatistic("read2readCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
wtowStat(getSM().getNumericStatistic("write2writeCycles", U32(0),
                                              "FifoSchedulerBase", prefix)),
switchModeCount(getSM().getNumericStatistic("SwitchModeCount", U32(0), "FifoSchedulerBase", prefix)),
lastTransactionWasRead(false),
cycleLastTransactionSelected(0),
waitingForFirstTransactionAccess(false),
sumPreAct2AccessCycles(getSM().getNumericStatistic("sumPreAct2AccessCycles", U32(0), "FifoSchedulerBase", prefix))
//commandBufferTransaction(0)
{
    if ( config.memSpecs.memtype() != GPUMemorySpecs::GDDR3 ) {
        CG_ASSERT("Only GDDR3 memory supported now");
    }
...
void FifoSchedulerBase::schedulerClock(U64 cycle)
{
    DynamicObject* dummy;
    ...
    if ( commandBuffer.empty() ) // check if the command buffer has pendent ddr commands
    {
        ...
        if ( currentTrans )
        {
            ...
            U32 burstsCount = fillCommandBuffer(currentTrans); //, getPagePolicy());

            GPU_ASSERT( 
                if ( commandBuffer.empty() ) {
                    CG_ASSERT("Current transaction has not generated DDR commands!");
                }
            )
    ...
    if ( !ongoingAccessesQueue.empty() ) {
        ...
        U64& startCycle = ongoingAccessesQueue.head().startCycle; // create an alias for startCycle
        U32& remainingCycles = ongoingAccessesQueue.head().remainingTransferCycles; // create an alias for remaning burst cycles
        ...
        switch ( cSchedState ) {
            ...
            case CSS_AccessingData:
                CG_ASSERT("Inconsistency between 'cSchedState' AND 'ongoingAccessesQueue'");
                break;
        }
    }
...
void FifoSchedulerBase::receiveData(U64 cycle, DDRBurst* data)
{
...
void FifoSchedulerBase::processReadReply(U64 cycle)
{
    if ( replyQ.empty() )
        return ;
...
void FifoSchedulerBase::processNextDDRCommand(U64 cycle)
{
    if ( commandBuffer.empty() )
    {
...
void FifoSchedulerBase::updateUnusedCyclesStats(DDRModuleState::IssueConstraint ic)
{
    switch ( ic )
    {
        ...
        default:
            CG_ASSERT("Unexpected CONSTRAINT");            
    }
}
