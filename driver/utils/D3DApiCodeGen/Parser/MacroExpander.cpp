...
void MacroExpander::MatchMacro(CppMacro& macro, string& cadena)
{
  ...
  if (br1.matched)
  {
    ...
    // Si no detectem cap par�metre en l'esquerra de la macro pero si veiem que
    // aquesta disposa de parentesis, llavors significa que hem detectat de
    // manera incorrecta els par�metres de la macro i que no l'aplicarem
    // correctament. [
    ...
    if (macroApplied)
    {
      // Contruim din�micament una expressi� regular per buscar en la cadena
      ...
      if (macroApplied)
      {
        // Construim la cadena de substituci� que s'aplicar� en cada aparici� de
        // la cadena final. [
  ...
  {
    Exception e("macro '" + macro.Left() + "' could'nt be parsed");
    throw e;
  }
}
