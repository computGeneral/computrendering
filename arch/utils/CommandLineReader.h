...
/**
 *
 *  The CommandLineReader class provides a basic command line reader features.
 *
 *  The CommandLineReader classes implement a basic command line reader with a
...
private:
    ...
    static CommandLineReader* lineReaderInst;          //  Singleton instance.  

    CommandLineReader();                               //  Constructors are private.  
    CommandLineReader(const CommandLineReader&);              //  Copy constructor is private.  
    CommandLineReader& operator=(const CommandLineReader&);   //  Copy constructor is private.  

    std::vector<std::string> lineBuffer;        //  Stores the current line.  
    U32 linePos;                             //  Current line position.  
    U32 currentLine;                         //  Current line in the line buffer.  
    ...
    U64 focusEvents;                         //  Focus events counter.  
    U64 menuEvents;                          //  Menu events counter.  
    U64 mouseEvents;                         //  Mouse events counter.  
    U64 windowBufferSizeEvents;              //  Window buffer size events counter.     
    U64 keyEvents;                           //  Key events counter.  
...
public:
    ...
    /**
     * Deallocates dynamic memory allocated by CommandLineReader object
     */
    ~CommandLineReader();
};
