...
U32 TextureCacheL2::cacheCounter = 0;


//  Texture cache constructor.  
TextureCacheL2::TextureCacheL2(U32 waysL0, U32 linesL0, U32 bytesLineL0,
        U32 pWidth, U32 reqQSize, U32 inReqsL0, U32 numBanks,
        U32 maxAccess, U32 bWidth, U32 maxMiss, U32 decompLat,
        U32 waysL1, U32 linesL1, U32 bytesLineL1, U32 inReqsL1,
        char *postfix) :
...

{
    ...
    pendingRequests = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("PendingMemoryRequests", U32(0), "TextureCache", postfix);
    pendingRequestsAvg = &gpuStatistics::StatisticsManager::instance().getNumericStatistic("PendingMemoryRequestsAvg", U32(0), "TextureCache", postfix);

    //  Create the L0 input buffer.  
    inputBufferL0 = new U08*[inputRequestsL0];

    //  Check allocation.  
    CG_ASSERT_COND(!(inputBufferL0 == NULL), "Error allocating L0 input buffers.");
    //  Create the L1 input buffer.  
    inputBufferL1 = new U08*[inputRequestsL1];

    //  Check allocation.  
    CG_ASSERT_COND(!(inputBufferL1 == NULL), "Error allocating L1 input buffers.");
    //  Create L0 read request queue.  
    readQueueL0 = new TextureCacheL2ReadRequest[inputRequestsL0];

    //  Check allocation.  
    CG_ASSERT_COND(!(readQueueL0 == NULL), "Error allocating the L0 read queue.");
    //  Create L1 read request queue.  
    readQueueL1 = new TextureCacheL2ReadRequest[inputRequestsL1];

    //  Check allocation.  
    CG_ASSERT_COND(!(readQueueL1 == NULL), "Error allocating the L1 read queue.");
    //  Allocate individual buffers.  
    for(i = 0; i < inputRequestsL0; i++)
    {
        //  Allocate L0 input buffer.  
        inputBufferL0[i] = new U08[lineSizeL0];

        //  Check allocation.  
        CG_ASSERT_COND(!(inputBufferL0[i] == NULL), "Error allocating L0 input buffer.");    }

    //  Allocate individual buffers.  
    for(i = 0; i < inputRequestsL1; i++)
    {
        //  Allocate L1 input buffer.  
        inputBufferL1[i] = new U08[lineSizeL1];

        //  Check allocation.  
        CG_ASSERT_COND(!(inputBufferL1[i] == NULL), "Error allocating L0 input buffer.");    }
    ...
    comprBuffer = new U08[lineSizeL0];
...
bool TextureCacheL2::fetch(U64 address, U32 &way, U32 &line, U64 &tag, bool &ready, DynamicObject *source)
{
    bool isMiss;
    bool fetchResult;
    U32 i;
...
//  Reads vertex data.  
bool TextureCacheL2::read(U64 address, U32 way, U32 line, U32 size, U08 *data)
{
    U32 port;
    U32 i;
    U64 bank;
...
void TextureCacheL2::unreserve(U32 way, U32 line)
{
...
MemoryTransaction *TextureCacheL2::update(U64 cycle, MemState memState, bool &filled, U64 &tag)
{
...
void TextureCacheL2::clock(U64 cycle)
{
    U32 i;
    U32 readEndReq;
        ...
        if (readQueueL0[nextFetchInputL0].size == 0)
        {
            ...
            switch(readQueueL0[nextFetchInputL0].address & TEXTURE_ADDRESS_SPACE_MASK)
            {
                case UNCOMPRESSED_TEXTURE_SPACE:
                    ...
                    readQueueL0[nextFetchInputL0].memAddress = (U32) (readQueueL0[nextFetchInputL0].address & 0xffffffff);
//if (cycle > 4913000)
//{
...
//}
                ...
                case COMPRESSED_TEXTURE_SPACE_DXT1_RGBA:
                    ...
                    //  Convert texture space address to a GPU memory address.  
                    readQueueL0[nextFetchInputL0].memAddress = (U32) ((readQueueL0[nextFetchInputL0].address >> DXT1_SPACE_SHIFT) &
                        0xffffffff);
...
//}
                ...
                case COMPRESSED_TEXTURE_SPACE_DXT5_RGBA:
                    ...
                    //  Convert texture space address to a GPU memory address.  
                    readQueueL0[nextFetchInputL0].memAddress = (U32) ((readQueueL0[nextFetchInputL0].address >> DXT3_DXT5_SPACE_SHIFT) &
                         0xffffffff);
...
//}
                ...
                case COMPRESSED_TEXTURE_SPACE_LATC2_SIGNED:
                    ...
                    //  Convert texture space address to a GPU memory address.
                    readQueueL0[nextFetchInputL0].memAddress = (U32) ((readQueueL0[nextFetchInputL0].address >> LATC1_LATC2_SPACE_SHIFT) &
                         0xffffffff);
...
//for(U32 w = 0; w < (lineSizeL0 >> 2); w++)
//printf("%08x ", ((U32*) inputBufferL0[nextReadL0])[w]);
//printf("\n");
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT1_RGB:
                ...
                bmoTextureProcessor::decompressDXT1RGB(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> DXT1_SPACE_SHIFT);
...
//printf("\n");
                //  Uncompress the block.  
                bmoTextureProcessor::decompressDXT1RGBA(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> DXT1_SPACE_SHIFT);
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT3_RGBA:
                ...
                //  Uncompress the block.  
                bmoTextureProcessor::decompressDXT3RGBA(comprBuffer, inputBufferL0[nextReadL0],
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT5_RGBA:
                ...
                //  Uncompress the block.  
                bmoTextureProcessor::decompressDXT5RGBA(comprBuffer, inputBufferL0[nextReadL0],
                    lineSizeL0 >> DXT3_DXT5_SPACE_SHIFT);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC1:
                ...
                //  Uncompress the block.
                bmoTextureProcessor::decompressLATC1(comprBuffer, inputBufferL0[nextReadL0],
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC1_SIGNED:
                ...
                //  Uncompress the block.
                bmoTextureProcessor::decompressLATC1Signed(comprBuffer, inputBufferL0[nextReadL0],
                    lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC2:
                ...
                //  Uncompress the block.
                bmoTextureProcessor::decompressLATC2(comprBuffer, inputBufferL0[nextReadL0],
                    lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC2_SIGNED:
                ...
                //  Uncompress the block.
                bmoTextureProcessor::decompressLATC2Signed(comprBuffer, inputBufferL0[nextReadL0],
                    lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT);
    ...
    if ((uncompressedL0 > 0) && (writeCycles == 0))
    {
            ...
            inputBufferL0[nextUncompressedL0], lastFilledLineTag))
        {
            ...
            writeCycles += (U32) ceil((F32) lineSizeL0 / ((F32) portWidth * readPorts));
...
MemoryTransaction *TextureCacheL2::requestBlock(U32 readReq)
{
    U32 size;
    U32 offset;
    MemoryTransaction *memTrans;
...
void TextureCacheL2::stallReport(U64 cycle, string &stallReport)
{
