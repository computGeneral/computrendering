...
#define CREATE_STRING(str_ptr,TEXT)                { (str_ptr) = new string(TEXT); GALxFp1ASTStringCollector.push_back(str_ptr); }
...
{
    CG_ASSERT("Sintactic error in Fragment Program");
}
...
list<string*> GALxFp1LexSymbolCollector;

list<string*> GALxFp1ASTStringCollector;
...
#include "GALxFP1Flex.gen" 

int GALxFp1StartParse(void *ptr)
{
    yyparse(ptr);
    
    list<string*>::iterator iter = GALxFp1LexSymbolCollector.begin();
    
    while(iter != GALxFp1LexSymbolCollector.end())
    {
        delete (*iter);
        iter++;
    }
    
    GALxFp1LexSymbolCollector.clear();

    iter = GALxFp1ASTStringCollector.begin();
    
    while(iter != GALxFp1ASTStringCollector.end())
    {   
        delete (*iter);
        iter++;
    }
    
    GALxFp1ASTStringCollector.clear();
