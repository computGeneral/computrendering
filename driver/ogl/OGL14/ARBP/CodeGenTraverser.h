...
private:
    ...
    RBank<F32> fragmentInBank;   ///< Bank for vertex in attributes. Not used
    RBank<F32> fragmentOutBank;      ///< Bank for vertex out attributes. Not used

    RBank<F32> parametersBank;   ///< Bank for constants, local parameters,
                                    ///< environment parameters and GL states.
    RBank<F32> temporariesBank;   ///< Bank for temporaries in code.
    RBank<F32> addressBank;      ///< Bank for address registers in code.
...
public:
    ...
    RBank<F32>& getParametersBank() { return parametersBank; }
    RBank<F32>& getTemporariesBank() { return temporariesBank;   }
