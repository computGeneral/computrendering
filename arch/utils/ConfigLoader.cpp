...
ConfigLoader::ConfigLoader(char *filename) :
    Parser(NULL, 0)
{
    configFile = fopen(filename, "r");    //  Tries to open the config file.  
    CG_ASSERT_COND((configFile != NULL), "[FATAL] Arch param load failed: %s", filename);    //  Check if file was correctly opened.  
}

//  Config Loader destructor.  
ConfigLoader::~ConfigLoader()
{
    fclose(configFile);    //  Close the config file.  
}

/*  Loads, parses and stores the simulator parameters from the
    configuration file.  */
void ConfigLoader::getParameters(cgsArchConfig *arch)
{
    bool end;
    ...
    if (getline(&str, &len, configFile) <= 0)
    {
            ...
            if (feof(configFile))
                ...
                CG_ASSERT("Error while reading the configuration file.");
    ...
    while (!end)
    {
        ...
        {
            //  Parse a section.  
            if (!parseSection(arch))
            {
                CG_ASSERT("Error parsing configuration file section.");
            }
        }
        else
        {
    ...
    paramsTracker.checkParamsIntegrity(*arch);
}

//  Parse a configuration section.  
bool ConfigLoader::parseSection(cgsArchConfig *arch)
{
    ...
    if (!strcmp("UNIFIEDSHADER", sectionName))
    {
        section = SEC_VSH;
    }
    ...
    if (!strcmp("DISPLAYCONTROLLER", sectionName))
    {
        section = SEC_DAC;
    }
    ...
    return parseSectionParameters(section, arch);
}

//  Parse all the section parameters.  
bool ConfigLoader::parseSectionParameters(Section section, cgsArchConfig *arch)
{
    bool end;
    ...
    if (section == SEC_SIG)
    {
        ...
        GPU_ASSERT(
            ...
            if (signalBuffer == NULL)
                CG_ASSERT("Error allocating buffer for the signal section.");
        )
    ...
    while (!end)
    {
                ...
                if (str[pos] != '[')
                {
                    ...
                    switch(section)
                    {
                        case SEC_SIM:
                            ...
                            if (!parseSimSectionParameter(arch))
                                return FALSE;
                        ...
                        case SEC_GPU:
                            ...
                            if (!parseGPUSectionParameter(&arch->gpu))
                                return FALSE;
                        ...
                        case SEC_COM:
                            ...
                            if (!parseComSectionParameter(&arch->com))
                                return FALSE;
                            break;
                        ...
                        case SEC_MEM:
                            ...
                            if (!parseMemSectionParameter(&arch->mem))
                                return FALSE;
                            break;

                        case SEC_STR:

                            //  Try to parse a cmoStreamController parameter.  
                            if (!parseStrSectionParameter(&arch->str))
                                return FALSE;
                            break;
                        ...
                        case SEC_VSH:
                            ...
                            if (!parseFShSectionParameter(&arch->ush))
                                return FALSE;
                            break;
                        ...
                        case SEC_PAS:
                            ...
                            if (!parsePAsSectionParameter(&arch->pas))
                                return FALSE;
                            break;
                        ...
                        case SEC_CLP:
                            ...
                            if (!parseClpSectionParameter(&arch->clp))
                                return FALSE;
                            break;
                        ...
                        case SEC_RAS:
                            ...
                            if (!parseRasSectionParameter(&arch->ras))
                                return FALSE;
                            break;
                        ...
                        case SEC_ZST:
                            ...
                            if (!parseZSTSectionParameter(&arch->zst))
                                return FALSE;
                            break;
                        ...
                        case SEC_CWR:
                            ...
                            if (!parseCWRSectionParameter(&arch->cwr))
                                return FALSE;
                            break;

                        case SEC_DAC:

                            //  Try to parse a DisplayController parameter.  
                            if (!parseDACSectionParameter(&arch->dac))
                                return FALSE;
                            break;
                        ...
                        case SEC_SIG:
                            ...
                            if (!parseSigSectionParameter(&signalBuffer[arch->numSignals]))
                                return FALSE;
                            else
                                arch->numSignals++;
        ...
        else
        {
            ...
            if (feof(configFile))
                ...
                CG_ASSERT("Error while reading config file.");
        }
    }
...
bool ConfigLoader::parseSimSectionParameter(cgsArchConfig *arch)
{
    char id[100];
    ...
    if (!parseDecimalParameter("SimCycles", id, arch->sim.simCycles))
        return FALSE;

    if (!parseDecimalParameter("SimFrames", id, arch->sim.simFrames))
        return FALSE;

    if (!parseStringParameter("InputFile", id, arch->sim.inputFile))
        return FALSE;

    if (!parseStringParameter("SignalDumpFile", id, arch->sim.signalDumpFile))
        return FALSE;

    if (!parseStringParameter("StatsFile", id, arch->sim.statsFile))
        return FALSE;

    if (!parseStringParameter("StatsFilePerFrame", id, arch->sim.statsFilePerFrame))
        return FALSE;

    if (!parseStringParameter("StatsFilePerBatch", id, arch->sim.statsFilePerBatch))
        return FALSE;

    if (!parseDecimalParameter("StartFrame", id, arch->sim.startFrame))
        return FALSE;

    if (!parseDecimalParameter("StartSignalDump", id, arch->sim.startDump))
        return FALSE;

    if (!parseDecimalParameter("SignalDumpCycles", id, arch->sim.dumpCycles))
        return FALSE;

    if (!parseDecimalParameter("StatisticsRate", id, arch->sim.statsRate))
        return FALSE;

    if (!parseBooleanParameter("DumpSignalTrace", id, arch->sim.dumpSignalTrace))
        return FALSE;

    if (!parseBooleanParameter("Statistics", id, arch->sim.statistics))
        return FALSE;

    if (!parseBooleanParameter("PerFrameStatistics", id, arch->sim.perFrameStatistics))
        return FALSE;

    if (!parseBooleanParameter("PerBatchStatistics", id, arch->sim.perBatchStatistics))
        return FALSE;

    if (!parseBooleanParameter("PerCycleStatistics", id, arch->sim.perCycleStatistics))
        return FALSE;

    if (!parseBooleanParameter("DetectStalls", id, arch->sim.detectStalls))
        return FALSE;

    if (!parseBooleanParameter("GenerateFragmentMap", id, arch->sim.fragmentMap))
        return FALSE;

    if (!parseDecimalParameter("FragmentMapMode", id, arch->sim.fragmentMapMode))
        return FALSE;

    if (!parseBooleanParameter("DoubleBuffer", id, arch->sim.colorDoubleBuffer))
        return FALSE;

    if (!parseBooleanParameter("ForceMSAA", id, arch->sim.forceMSAA))
        return FALSE;

    if (!parseDecimalParameter("MSAASamples", id, arch->sim.msaaSamples))
        return FALSE;

    if (!parseBooleanParameter("ForceFP16ColorBuffer", id, arch->sim.forceFP16ColorBuffer))
        return FALSE;

    if (!parseBooleanParameter("EnableDriverShaderTranslation", id, arch->sim.enableDriverShaderTranslation))
        return FALSE;

    if (!parseDecimalParameter("ObjectSize0", id, arch->sim.objectSize0))
        return FALSE;

    if (!parseDecimalParameter("BucketSize0", id, arch->sim.bucketSize0))
        return FALSE;

    if (!parseDecimalParameter("ObjectSize1", id, arch->sim.objectSize1))
        return FALSE;

    if (!parseDecimalParameter("BucketSize1", id, arch->sim.bucketSize1))
        return FALSE;

    if (!parseDecimalParameter("ObjectSize2", id, arch->sim.objectSize2))
        return FALSE;
...
bool ConfigLoader::parseMemSectionParameter(MemParameters *memP)
{
    ...
    if (!parseDecimalParameter("DisplayControllerBusWidth", id, memP->dacBus))
        return FALSE;
...
//  Parse cmoStreamController parameters.  
...
bool ConfigLoader::parseFShSectionParameter(cgsUnifiedShaderParam *fshP)
{
    char id[100];
    ...
    paramsTracker.startParamSectionDefinition("UNIFIEDSHADER");
    ...
    if (!parseDecimalParameter("TextureBlockDimension", id, fshP->texBlockDim))
        return FALSE;

    if (!parseDecimalParameter("TextureSuperBlockDimension", id, fshP->texSuperBlockDim))
        return FALSE;
    ...
    if ( !paramsTracker.wasAnyParamSectionDefined() ) {
        stringstream ss;
        ss << "Parameter '" << id << "' in section [UNIFIEDSHADER] is not supported";
        CG_ASSERT(ss.str().c_str());
    }
...
//  Parse DisplayController parameters.  
bool ConfigLoader::parseDACSectionParameter(cgsDisplayCtrlParam *dacP)
{
    char id[100];
    ...
    paramsTracker.startParamSectionDefinition("DisplayController");
    ...
    if ( !paramsTracker.wasAnyParamSectionDefined() ) {
        stringstream ss;
        ss << "Parameter '" << id << "' in section [DisplayController] is not supported";
        CG_ASSERT(ss.str().c_str());
    }
...
bool ConfigLoader::parseDecimalParameter(char *paramName, char *id, U32 &val)
{
    S32 aux;
...
bool ConfigLoader::parseDecimalParameter(char *paramName, char *id, U64 &val)
{
    S64 aux;
...
bool ConfigLoader::parseFloatParameter(char *paramName, char *id, F32 &val)
{
    F32 aux;
...
void ConfigLoader::ParamsTracker::checkParamsIntegrity(const cgsArchConfig& arch)
{   
