...
#define CREATE_STRING(str_ptr,TEXT)                { (str_ptr) = new string(TEXT); GALxVp1ASTStringCollector.push_back(str_ptr); }
...
{
    CG_ASSERT("Sintactic error in Vertex Program");
}
...
list<string*> GALxVp1LexSymbolCollector;

list<string*> GALxVp1ASTStringCollector;
...
#include "GALxVP1Flex.gen" 

int GALxVp1StartParse(void *ptr)
{  
    yyparse(ptr);
    
    list<string*>::iterator iter = GALxVp1LexSymbolCollector.begin();
    
    while(iter != GALxVp1LexSymbolCollector.end())
    {   
        //delete (*iter);
        iter++;
    }
    
    GALxVp1LexSymbolCollector.clear();

    iter = GALxVp1ASTStringCollector.begin();
    
    while(iter != GALxVp1ASTStringCollector.end())
    {   
        //delete (*iter);
        iter++;
    }
    
    GALxVp1ASTStringCollector.clear();
