...
{    
    ...
    CG_ASSERT("glShaderSource not implemented yet");
...
{    
    ...
    if ( bos.currentBind != bos.dataBind )
    {
        ...
        if ( it == bos.binds.end() )
        {
            ...
            logfile().popInfo();
            CG_ASSERT("bind -> map : without previous data associated");
            return 0;
        }
        ...
        //CG_ASSERT("Unsupported: Bind between glBufferData and this call");
        //return 0;
    }
...
GLboolean GLAPIENTRY glUnmapBufferARB_SPECIFIC(GLenum target)
{
    ...
    if ( bos.currentBind != bos.dataBind )
    {
        ...
        if ( it == bos.binds.end() )
        {
            ...
            logfile().popInfo();
            CG_ASSERT("bind -> unmap : without previous data associated");
            return false;
        }
...
void GLAPIENTRY glBufferDataARB_SPECIFIC(GLenum target, GLsizeiptrARB size, const GLvoid *data, GLenum usage)
{
    ...
    {        
        ...
        else
        {
            ...
            logfile().popInfo();
            CG_ASSERT("Previous bind expected");
        }
    }
    else
    {
                                            ...
                                            GLsizeiptrARB size, const GLvoid * data)
{
    ...
    if ( data == 0 )
    {
        ...
        logfile().popInfo();
        CG_ASSERT("Null pointer not expected");
    }
    ...
    if ( boElementArray.isBound() ) // Elements bound && data bound
    {
        ...
        for ( i = 0; i < MAX_POINTERS; i++ )
        {
            if ( arrayPointer[i].enabled && !arrayPointer[i].bufferBound )
            {
                ...
                logfile().popInfo();
                CG_ASSERT("GL_ELEMENT_ARRAY bound & GL_ARRAY unbound not supported");
                return ;
            }
        }
        for ( i = 0; i < MAX_GENERIC_POINTERS; i++ )
        {
            if ( genericPointer[i].enabled && !genericPointer[i].bufferBound )
            {
                ...
                logfile().popInfo();
                CG_ASSERT("GL_ELEMENT_ARRAY bound & GL_ARRAY unbound not supported (2)");
                return ;
            }
        }
        //logfile().pushInfo(__FILE__,__FUNCTION__);
