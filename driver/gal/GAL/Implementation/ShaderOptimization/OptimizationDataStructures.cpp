...
bool  libGAL::libGAL_opt::operator< (
...
OperationInfo::OperationInfo(unsigned int opcode, unsigned int numOperands,
                             ...
                             bool predicated, bool negPredicate, U32 predReg,                   
...
InstructionInfo::InstructionInfo(cgoShaderInstr& shInstr, gal_uint instrIndex, gal_uint execLat, const string& instrAsm)
{
    line = instrIndex;
    str = string(instrAsm);
...
cgoShaderInstr* InstructionInfo::getShaderInstructionCopy(gal_bool lastInstruction) const
{
    ...
    {
        return new cgoShaderInstr((ShOpcode)operation.opcode,
                                      (Bank)readReg0.bank,
                                      readReg0.reg,
...
#define CHECK_NODE_RANGE(n,funcName) if ( n >= numNodes ) CG_ASSERT("Unexpected number of node");
