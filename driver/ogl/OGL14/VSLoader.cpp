...
VSLoader::VSLoader(GLContext* ctx, ::HAL* driver) : driver(driver), ctx(ctx)
{
...
void VSLoader::initShader(ProgramObject& po, GLenum target, GLContext* ctx)
{
    ...
    if ( po.getTargetName() != target )
        CG_ASSERT("Program object target does not match");
    ...
    if (!po.isTranslated())
    {
        ...
        U32 translatedCodeSize = sourceCodeSize * 10;
        
        //  Allocate buffer for the input source code and the transformed source code.
        U08 *inCode = new U08[sourceCodeSize];
        U08 *outCode = new U08[translatedCodeSize];
...
void VSLoader::resolveBank(ProgramObject& po, RBank<float>& b)
{
    ...
    for ( U32 i = 0; i < b.size(); i++ )
    {
        ...
        switch ( regType )
        {
            ...
            default:
                ;
                //CG_ASSERT("QR_UNKNOWN not allowed here");
        }
    }
}
