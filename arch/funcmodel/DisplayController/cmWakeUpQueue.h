...
#include "GPUType.h"
#include "cmROPCache.h" // For ROPBlockState definition
#include <vector>
#include <ostream>
#include <set>
#include <list>
...
struct QueueEntry
{
    ...
    U32 addedEvents;      //  The number of added events up to now.  
    U32 totalEvents;      //  The total number of events to wait for.  
    U32 totalEventsLeft;  //  The countdown for the number of events.  

    QueueEntry(): 
        entry(0), events(std::set<Event>()), addedEvents(0), totalEvents(0), totalEventsLeft(0) {}
        
    QueueEntry(Entry* entry, const std::set<Event>& events, U32 totalEvents):
...
private:
    ...
    U32 maxSize;                 //  How many entries the queue can hold as maximum.  
    U32 maxEvents;       //  How many events each entry can wait as maximum.  
...
public:
     ...
     */
    WakeUpQueue(U32 size, U32 maxEvents);
     ...
     */
    void add(Entry* entry, const std::set<Event>& events, U32 totalEvents);
...
template<class Entry, class Event>
WakeUpQueue<Entry, Event>::WakeUpQueue(U32 size, U32 maxEvents)
    : maxSize(size), maxEvents(maxEvents), name("Default Queue name")
{
}

template<class Entry, class Event>
void WakeUpQueue<Entry, Event>::resize(U32 newSize)
{
    maxSize = newSize;
}
...
template<class Entry, class Event>
void WakeUpQueue<Entry, Event>::add(Entry* entry, const std::set<Event>& events, U32 totalEvents)
{
    if (size() == maxSize)
        CG_ASSERT("Queue is full");
    ...
    if (events.size() > totalEvents)
        CG_ASSERT("The totalEvents number is lower than the specified event list size");
        
    if (exists(entry))
        CG_ASSERT("Entry already exists");
    ...
    while( it != q.end() && !found)
    {
        if (it->entry == entry)
        {
            if ((it->addedEvents + events.size()) > it->totalEvents)
                CG_ASSERT("New added events and previously added sum more than totalEvents specified");
...
void WakeUpQueue<Entry, Event>::wakeup(const Event& event)
{
    ...
    if (!listenerFound)
    {
        CG_ASSERT("Any entry waits for the event.");
    }
}
...
void WakeUpQueue<Entry, Event>::remove(const Entry* entry)
{
    ...
    {
        if (position >= maxSize)
            CG_ASSERT("Entry not found");
        
        if (it->entry == entry)
        {
            if (it->totalEventsLeft)
                CG_ASSERT("Entry isn�t ready/selectable");
...
template<class Entry, class Event>
U32 WakeUpQueue<Entry, Event>::size() const
{
    return q.size();
}
