/**************************************************************************
 *
 * Shader Unit Fetch cmoMduBase (unified shader version).
 *
 */

/**
 *
 *  @file cmoShaderFetch.cpp
 *
 *  This file implements the cmoShaderFetch cmoMduBase Class (unified shader version).
 *  This mdu implements the simulation of the Fetch stage
...
cmoShaderFetch::cmoShaderFetch(bmoUnifiedShader &bmShader, U32 nThreads,
    U32 nInputBuffers, U32 resources, U32 threadsPerCycle, U32 instrPerCycle, U32 threadsPerGroup,
    bool lockStepMode, bool scalar, bool thWindow, U32 fDelay, bool swpOnBlck, U32 texUnits, U32 inCycle,
    U32 outCycle, U32 outputLatency, bool microTrisAsFrag, char *name, char *shPrefix, char *sh2Prefix, 
    cmoMduBase* parent):

    //  Pre-initializations.  
    cmoMduBase(name, parent), bmUnifiedShader(bmShader), unifiedModel(true), numBuffers(nThreads + nInputBuffers),
...

{
    ...
    GPU_ASSERT(
        if (threadGroup == 0)
            CG_ASSERT("At least a shader thread required per thread group.");
        if (numThreads < threadGroup)
            CG_ASSERT("At least a group of shader threads required.");
        if (numInputBuffers < threadGroup)
            CG_ASSERT("At least a group of threads/input buffers required.");
        if (numResources < numThreads)
            CG_ASSERT("At least one resource per shader thread required.");
        if (GPU_MOD(numBuffers, threadGroup) != 0)
            CG_ASSERT("The number of thread/buffers must be a multiple of the thread group.");
        if (threadsCycle == 0)
            CG_ASSERT("At least one shader thread must be fetched per cycle.");
        if (instrCycle == 0)
        ...
        if (shPrefix == NULL)
            CG_ASSERT("Shader prefix string required.");
        if (unifiedModel && (shPrefix == NULL))
            CG_ASSERT("Secondary shader prefix string required for unified shader model.");
        //if (lockStep && ((threadGroup > outputCycle) && (GPU_MOD(threadGroup, outputCycle) != 0)))
        //    CG_ASSERT("Outputs per cycle must be a divisor of the thread groups in lock step mode when the outputs per cycle is smaller than the thread group.");
        //if (lockStep && (threadGroup < outputCycle))
        //    CG_ASSERT("Outputs per cycle must be as large or smaller than a thread group.");
        //if (lockStep && ((threadGroup > threadsCycle) && (GPU_MOD(threadGroup, threadsCycle) != 0)))
        //    CG_ASSERT("Thread rate must be a divisor of thread groups in lock step mode when the thread rate is smaller than the thread group.");
        //if (lockStep && ((threadGroup < threadsCycle) && (GPU_MOD(threadsCycle, threadGroup) != 0)))
        //    CG_ASSERT("Thread rate must be multiple of thread groups in lock step mode when the thread rate is greater than the thread group.");
        if (scalarALU && (instrCycle != 2))
            CG_ASSERT("SIMD + scalar fetch mode requires a fetch rate of two instructions per cycle.");
        if (!threadWindow && (fetchDelay == 0))
            printf("cmoShaderFetch => WARNING!! to avoid desynchronization in small batches due to dependences you should set a large enough delay between fetch iterations.\n");
    )
...
//printf("ShFetch %s -> %p \n", fullName, this);
    ...
    instructionSignal = newOutputSignal("cgoShaderInstr", threadsCycle * instrCycle, 1, shPrefix);
    ...
    freeBufferList = new U32[numBuffers];
    ...
    finThreadList = new U32[numThreads];
    ...
    for(U32 target = 0; target < 3; target++)
    {
        ...
        for(U32 o = 0; o < MAX_VERTEX_ATTRIBUTES; o++)        
    ...
    //  Check allocation.  
    CG_ASSERT_COND(!(fetchGroup == NULL), "Error allocating thread group fetch buffer.");
    for(i = 0; i < threadGroup; i++)
    {
        ...
        //  Check allocation.  
        CG_ASSERT_COND(!(fetchGroup[i] == NULL), "Error allocating thread fetch buffer.");    }
    ...
    fetchedGroups = new U32[numGroups];

    //  Allocate the shader input batch queues.  
    batchGroupQueue[VERTEX_PARTITION][0] = new U32[numGroups];
    batchGroupQueue[VERTEX_PARTITION][1] = new U32[numGroups];
    batchGroupQueue[VERTEX_PARTITION][2] = new U32[numGroups];
    batchGroupQueue[VERTEX_PARTITION][3] = new U32[numGroups];
    batchGroupQueue[FRAGMENT_PARTITION][0] = new U32[numGroups];
    batchGroupQueue[FRAGMENT_PARTITION][1] = new U32[numGroups];
    batchGroupQueue[FRAGMENT_PARTITION][2] = new U32[numGroups];
    batchGroupQueue[FRAGMENT_PARTITION][3] = new U32[numGroups];
    batchGroupQueue[TRIANGLE_PARTITION][0] = new U32[numGroups];
    batchGroupQueue[TRIANGLE_PARTITION][1] = new U32[numGroups];
    batchGroupQueue[TRIANGLE_PARTITION][2] = new U32[numGroups];
    batchGroupQueue[TRIANGLE_PARTITION][3] = new U32[numGroups];

    //  Check allocation.  
    GPU_ASSERT(
        if (readyThreads == NULL)
            CG_ASSERT("Error allocating the per group ready thread counter list.");
        if (finThreads == NULL)
            CG_ASSERT("Error allocating the per group finished thread counter list.");
        if (readyGroups == NULL)
            CG_ASSERT("Error allocating the ready thread group queue.");
        if (groupBlocked == NULL)
            CG_ASSERT("Error allocating blocked group flag array.");
        if (fetchedGroups == NULL)
            CG_ASSERT("Error allocating the fetched thread group queue.");
        if ((batchGroupQueue[VERTEX_PARTITION][0] == NULL) || (batchGroupQueue[VERTEX_PARTITION][1] == NULL)
            ...
            CG_ASSERT("Error allocating the batch group queues.");
    )
...
/*  Clock instruction for cmoShaderFetch cmoMduBase.  Drives the time
 *  of the simulation.
 *
 */

void cmoShaderFetch::clock(U64 cycle)
{
    ...
    GPU_DEBUG_BOX( printf("cmoShaderFetch >> Clock %lld.\n", cycle); )
...
//printf("ShF %lld => adding ready group %d at %d\n", cycle, fetchedGroups[nextFetchedGroup], nextFreeReady);
            ...
            CG_ASSERT_COND(!(numReadyGroups == numGroups), "Ready group queue full.");
                ...
                if (nextThreadInGroup == 0)
                {
                    ...
                    if (threadWindow)
                    {
                        ...
                        while ((!readyThreadGroup) && (numReadyGroups > 0))
                        {
                            ...
                            CG_ASSERT_COND(!(threadTable[nextThread].nextFetchCycle > cycle), "Fetching from a thread with active fetch delay.");
                            ...
                            {
                                //  Check that all the threads in the group are ready.  
                                CG_ASSERT_COND(!(readyThreads[nextGroup] < threadGroup), "Non ready threads in the current thread group.");
...
//    this, cycle, nextGroup, readyThreadGroup);
                            ...
                            if (readyThreadGroup)
                            {
                                ...
                                else
                                {
                                    ...
                                    GPU_ASSERT(
                                        if (batchPC[currentBatchPartition] != groupPC)
                                        {
                                            ...
                                            CG_ASSERT("Desynchronized PC inside a batch.");
                                        }
                                    )
                                }
                                ...
                                CG_ASSERT_COND(!(nextFetchInBatch[currentBatchPartition] > numGroups), "Pointer to next fetch group in batch queue overflowed.");                            }
                            else
                            {
...
//printf("ShF(%p) %lld => B start fetching new partition %d batch %d\n", this, cycle, currentBatchPartition, fetchBatch[currentBatchPartition]);
                ...
                if (readyThreadGroup)
                {
                    ...
                    if (threadWindow && (nextThreadInGroup == 0))
                    {
                        ...
                        CG_ASSERT_COND(!(numFetchedGroups == numGroups), "Fetched groups queue is full.");
    ...
    {
        GPU_DEBUG_BOX(
            printf("cmoShaderFetch => Transmission in progress Thread = %d Remaining cycles = %d.\n",
        ...
        {
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Loading new inputs.\n");
            )
    ...
    {
        GPU_DEBUG_BOX(
            printf("cmoShaderFetch >> Sending Busy.\n");
        )
...
//printf("ShF(%p) %lld => closing (resources) partition %d batch %d\n", this, cycle, currentBatchPartition, loadBatch[currentBatchPartition]);
    ...
    {
        GPU_DEBUG_BOX(
            printf("cmoShaderFetch >> Sending Empty.\n");
        )
    ...
    {
        GPU_DEBUG_BOX(
            printf("cmoShaderFetch >> Sending Ready (NumFreeThreads: %d).\n", numFreeThreads);
        )
    ...
    nThReadyMax->maximum(numReadyThreads);
    nThReadyMin->minimum(numReadyThreads);
...
S32 cmoShaderFetch::getFreeThread(U32 partition)
{
    U32 i;
...
//  Frees a thread.  
void cmoShaderFetch::addFreeThread(U32 numThread, U32 partition)
{
    U32 requiredResources;
    ...
    (
        if (numThread > numBuffers)
            CG_ASSERT("Invalid thread number.");
    )
    ...
    GPU_ASSERT(
        ...
        {
            CG_ASSERT("Free Buffer List overflow.");
        }
        if (freeResources >= numResources)
        {
            CG_ASSERT("Free resources overflow.");
        }
    )
...
void cmoShaderFetch::reloadThread(U64 cycle, U32 nThread)
{
    U32 newThread;
    U32 newGroup;
    U32 partition;
...
//printf("ShF (reload) => adding ready group %d at %d\n", newGroup, nextFreeReady);
        ...
        else
        {
            ...
            if (readyThreads[newGroup] == threadGroup)
            {
                ...
                CG_ASSERT_COND(!(lastInBatch[partition][loadBatch[partition]] == numGroups), "Batch group queue is full.");
                //  Check that the batch is not closed.  
                CG_ASSERT_COND(!((fetchBatch[partition] == loadBatch[partition]) && closedBatch[partition][loadBatch[partition]]), "Load batch is already closed.");
//if (cycle > 2500000)
...
void cmoShaderFetch::finishThread(U32 numThread)
{
    U32 lastThread;
    U32 partition;
    ...
    if (!threadTable[numThread].end)
    {
        ...
        if (threadWindow)
        {
            U32 group = numThread / threadGroup;
...
void cmoShaderFetch::processShaderCommand(ShaderCommand *command, U32 partition)
{
    U32 i, firstAddr, lastAddr;
    Vec4FP32 *values;
    ...
    switch(command->getCommandType())
    {
        case LOAD_PROGRAM:
            ...
            CG_ASSERT_COND(!((!unifiedModel) && (numFreeThreads != numBuffers)), "Shader is still processing inputs.");
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch >> LOAD_PROGRAM Size %d PC %x\n",
        ...
        case PARAM_WRITE:
            ...
            GPU_DEBUG_BOX(
                for (U32 i = firstAddr; i < lastAddr; i ++)
                    printf("cmoShaderFetch >> PARAM_WRITE %d = {%f, %f, %f, %f}\n",
        ...
        case SET_INIT_PC:
            ...
            GPU_ASSERT(
                if ((!unifiedModel) && (numFreeThreads != numBuffers))
                    CG_ASSERT("Shader is still processing inputs.");
                    
                if ((partition == VERTEX_PARTITION) && (command->getShaderTarget() != VERTEX_TARGET))
                    CG_ASSERT("Commands to vertex shader can only be for the vertex shader target.\n");
                
            )

            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => SET_INIT_PC[%d] = %x.\n", command->getShaderTarget(), command->getInitPC());
            )
        ...
        case SET_THREAD_RES:
            ...
            GPU_DEBUG_BOX(
                ...
                printf("cmoShaderFetch => threadResources[%s] = %d, maxResources =  %d\n",
        ...
        case SET_IN_ATTR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Setting input attribute %d as %s for partition %s\n", command->getAttribute(),
...
//    activeInputs[partition]);
        ...
        case SET_OUT_ATTR:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Setting output attribute %d as %s for partition %s\n", command->getAttribute(),
                    command->isAttributeActive()?"ACTIVE":"INACTIVE",
                    (partition == VERTEX_PARTITION)?"VERTEX":((partition == FRAGMENT_PARTITION)?"FRAGMENT":"TRIANGLE"));
            )
...
//printf("ShFetch => activeOutputs[%d] = %d\n", partition, activeOutputs[partition]);
        ...
        case SET_MULTISAMPLING:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Setting multisampling register: %s\n", command->multisamplingEnabled()?"ENABLED":"DISABLED");
            )
        ...
        case SET_MSAA_SAMPLES:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Setting MSAA samples register: %d\n", command->samplesMSAA()); 
            )
...
void cmoShaderFetch::processShaderInput(U64 cycle, ShaderInput *input)
{
    U32 newThread;
    U32 newGroup;
    U32 partition;
    ...
    CG_ASSERT_COND(!(newThread == -1), "No free buffer/threads.");
    //  Reset the state of the thread.  
    bmUnifiedShader.resetShaderState(newThread);
    ...
    switch(input->getInputMode())
    {
        case SHIM_VERTEX:
            ...
            bmUnifiedShader.loadShaderState(newThread, IN, input->getAttributes(), 0, MAX_VERTEX_ATTRIBUTES);
            break;

        case SHIM_TRIANGLE:
            //  Load the input attributes for the shader thread element in the shader behaviorModel.
            bmUnifiedShader.loadShaderState(newThread, IN, input->getAttributes(), 0, MAX_TRIANGLE_ATTRIBUTES);
            break;

        case SHIM_FRAGMENT:
            //  Load the input attributes for the shader thread element in the shader behaviorModel.
            bmUnifiedShader.loadShaderState(newThread, IN, input->getAttributes(), 0, MAX_FRAGMENT_ATTRIBUTES);
            break;

        case SHIM_MICROTRIANGLE_FRAGMENT:
            //  Load the input attributes for the shader thread element in the shader behaviorModel.
            bmUnifiedShader.loadShaderState(newThread, IN, input->getAttributes(), 0, MAX_MICROFRAGMENT_ATTRIBUTES);
            break;
    ...
    //  Set thread PC in the shader behaviorModel.  
    bmUnifiedShader.setThreadPC(newThread, threadTable[newThread].PC);
...
//printf("ShF => (shInput) numReadyThreads %d\n", numReadyThreads);
    ...
    {
        inputOutAttribs->inc(GPU_MAX(U32(1), activeOutputs[PRIMARY_PARTITION]));
    }

    GPU_DEBUG_BOX(
        if (unifiedModel)
            printf("cmoShaderFetch >> New Shader Input Thread = %d for partition %s\n", newThread,
                partition==VERTEX_PARTITION?"VERTEX":(partition==FRAGMENT_PARTITION)?"FRAGMENT":"TRIANGLE");
        else
            printf("cmoShaderFetch >> New Shader Input Thread = %d\n", newThread);
    )
}

//  Processes a control command from Decode stage.  
void cmoShaderFetch::processDecodeCommand(ShaderDecodeCommand *decodeCommand)
{
    U32 numThread;
    U32 group;
    U32 pc;
    ...
    CG_ASSERT_COND(!(numThread > numBuffers), "Illegal thread number.");
    switch(decodeCommand->getCommandType())
    {
        case UNBLOCK_THREAD:

            GPU_DEBUG_BOX(
                printf("cmoShaderFetch(%p) => UNBLOCK_THREAD Thread = %d PC = %x\n",
                    this, numThread, pc);
            )
...
//printf("ShF => (unblock) numReadyThreads %d\n", numReadyThreads);
        ...
        case BLOCK_THREAD:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch(%p) => BLOCK_THREAD Thread = %d PC = %x\n", this, numThread, pc);
            )
...
//printf("ShF => (block) numReadyThreads %d\n", numReadyThreads);
        ...
        case END_THREAD:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch(%p) => END_THREAD Thread = %d PC = %x\n", this, numThread, pc);
            )
        ...
        case REPEAT_LAST:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch(%p) => REPEAT_LAST Thread = %d PC = %x\n", this, numThread, pc);
            )
            ...
            if (lockStep)
            {
                ...
                for(i = 0; i < threadGroup; i++)
                {
                    ...
                    //  Set thread PC in the shader behaviorModel.  
                    bmUnifiedShader.setThreadPC(GPU_MOD(numThread + i, numBuffers), pc);
            ...
            else
            {
                ...
                //  Set thread PC in the shader behaviorModel.  
                bmUnifiedShader.setThreadPC(numThread, threadTable[numThread].PC);
        ...
        case NEW_PC:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => NEW_PC Thread = %d PC = %x\n", numThread, pc);
            )
        ...
        case ZEXPORT_THREAD:
            ...
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => ZEXPORT_THREAD Thread = %d PC = %x\n", numThread, pc);
            )

            //  Check if the thread was ready.  
            CG_ASSERT_COND(!(!threadTable[numThread].ready), "Thread wasn't in runnable (ready) state.");
...
void cmoShaderFetch::fetchInstruction(U64 cycle, U32 threadID, ShaderExecInstruction *&shExecInstr)
{
    ...
    //shInstrDec = bmUnifiedShader.FetchInstr(threadID, threadTable[threadID].PC);
    shInstrDec = bmUnifiedShader.fetchShaderInstructionLight(threadID, threadTable[threadID].PC, partition);
    ...
    if (scalarALU)
    {
        ...
        {
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Ignoring second SIMD instruction fetch.\n");
            )
        ...
        {
            GPU_DEBUG_BOX(
                printf("cmoShaderFetch => Ignoring second scalar instruction fetch.\n");
            )
...
void cmoShaderFetch::fetchInstruction(U64 cycle, U32 threadID)
{
    ShaderExecInstruction *shExecInstr;
...
void cmoShaderFetch::fetchInstructionGroup(U64 cycle, U32 threadID)
{
    ShaderExecInstruction *shExecInstr;
    U32 i;
    U32 j;
...
void cmoShaderFetch::sendInstruction(U64 cycle, U32 threadID, ShaderExecInstruction *shExecInstr)
{
    char dis[256];

    //  Send instruction to decode/execute mdu.  
    instructionSignal->write(cycle, shExecInstr);

    GPU_DEBUG_BOX(
        shExecInstr->getShaderInstruction().getShaderInstruction()->disassemble(&dis[0], 256);

        printf("cmoShaderFetch (%p) => Instruction Fetched Thread = %d PC = %x : %s\n",
...
void cmoShaderFetch::sendOutput(U64 cycle, U32 outputThread)
{
    ...
    switch(shOutput->getInputMode())
    {
        ...
        case SHIM_MICROTRIANGLE_FRAGMENT:
            ...
            break;
        default:
            CG_ASSERT("Undefined shader input mode.");
            break;
    }
    
    //  Calculate the number of cycles for output transmission (only vertices).  */
    transCycles = (U32) ceil(((double) activeOutputs[partition]) * OUTPUT_TRANSMISSION_LATENCY);

    //  Read the result output registers and write them into the shader output array.  
    bmUnifiedShader.readShaderState(outputThread, OUT, shOutput->getAttributes());

    //  Check if the only sample for the shader thread was aborted/killed.  
    if (bmUnifiedShader.threadKill(outputThread, 0))
        shOutput->setKilled();
    ...
    GPU_DEBUG_BOX(
        printf("cmoShaderFetch => Output sent\n");
        printf("vo[0] = {%f %f %f %f}\n",
...
U32 cmoShaderFetch::computeMaxThreadResources()
{
    U32 maxResources = 1;
    ...
    else
    {
        maxResources = GPU_MAX(threadResources[PRIMARY_PARTITION], GPU_MAX(GPU_MAX(activeInputs[PRIMARY_PARTITION], threadResources[PRIMARY_PARTITION]),
            GPU_MAX(U32(1), activeOutputs[PRIMARY_PARTITION])));
    }

    return maxResources;
}

void cmoShaderFetch::getState(string &stateString)
{
