/**************************************************************************
 *
 * Triangle Setup mdu implementation file.
 *
 */
 ...
 *
 *  This file implements the Triangle Setup mdu.
 *
 *
 */
...
TriangleSetup::TriangleSetup(bmoRasterizer &rasEmu, U32 cycleTriangles, U32 tsFIFOSize,
    U32 setupUnits, U32 latency, U32 startLat, U32 paLat, U32 tbLat, U32 fgLat, 
    bool shadedSetup, bool preTriangleBound, bool microTrisAsFrag, U32 triShQSz, char *name, 
    cmoMduBase *parent) :
    ...
    triangleShaderQSz(triShQSz), bmRaster(rasEmu), cmoMduBase(name, parent)

{
    ...
    GPU_ASSERT(
        if (trianglesCycle != numSetupUnits)
            CG_ASSERT("Number of setup units must be the same that the triangles per cycle in current implementation.");
        if (setupFIFOSize <= ((paLatency + 1) * trianglesCycle))
            CG_ASSERT("Triangle setup fifo too small for triangles input througput and latency.");
        if (shaderSetup && (triangleShaderQSz == 0))
            CG_ASSERT("Triangle shader queue requires at least 1 entry.");
    )

    //  Create statistics.  
    inputs = &getSM().getNumericStatistic("InputTriangles", U32(0), "TriangleSetup", "TS");
    outputs = &getSM().getNumericStatistic("OutputTriangles", U32(0), "TriangleSetup", "TS");
    requests = &getSM().getNumericStatistic("RequestedTriangles", U32(0), "TriangleSetup", "TS");
    culled = &getSM().getNumericStatistic("CulledTriangles", U32(0), "TriangleSetup", "TS");
    front = &getSM().getNumericStatistic("FrontFacingTriangles", U32(0), "TriangleSetup", "TS");
    back = &getSM().getNumericStatistic("BackFacingTriangles", U32(0), "TriangleSetup", "TS");

    //  Create triangle size related statistics.  
    count_less_1 = &getSM().getNumericStatistic("TrianglesLess_1", U32(0), "TriangleSetup", "TS");
    count_1_to_4 = &getSM().getNumericStatistic("Triangles1_to_4", U32(0), "TriangleSetup", "TS");
    count_4_to_16 = &getSM().getNumericStatistic("Triangles4_to_16", U32(0), "TriangleSetup", "TS");
    count_16_to_100 = &getSM().getNumericStatistic("Triangles16_to_100", U32(0), "TriangleSetup", "TS");
    count_100_to_1000 = &getSM().getNumericStatistic("Triangles100_to_1000", U32(0), "TriangleSetup", "TS");
    count_1000_to_screen = &getSM().getNumericStatistic("Triangles1000_to_Screen", U32(0), "TriangleSetup", "TS");
    count_greater_screen = &getSM().getNumericStatistic("TrianglesGreaterScreen", U32(0), "TriangleSetup", "TS");
    count_overlap_1x1 = &getSM().getNumericStatistic("TrianglesOverlap_1x1", U32(0), "TriangleSetup", "TS");
    count_overlap_2x2 = &getSM().getNumericStatistic("TrianglesOverlap_2x2", U32(0), "TriangleSetup", "TS");
    ...
    {
        //  Create setup start signal to the Triangle Setup mdu.  
        setupStart = newOutputSignal("TriangleSetup", numSetupUnits, setupLatency, NULL);

        //  Create setup ed signal from the Triangle Setup mdu.  
    ...
    if (shaderSetup)
    {
        ...
        CG_ASSERT_COND(!(triangleShaderQueue == NULL), "Error allocating triangle shader reorder queue.");    }

    //  Allocate memory for the setup FIFO.  
    setupFIFO = new TriangleSetupOutput*[setupFIFOSize];

    //  Check memory allocation.  
    CG_ASSERT_COND(!(setupFIFO == NULL), "Error allocating memory for the setup FIFO.");
...
void TriangleSetup::clock(U64 cycle)
{
    ...
    ShaderState shState;
    Vec4FP32 *shAttributes;
    Vec4FP32 *vertAttr1;
    Vec4FP32 *vertAttr2;
    Vec4FP32 *vertAttr3;
    U32 triangleID;
    U32 triRequest;
    U32 shOutputTriangle;
    U32 i;
        ...
        case RAST_DRAWING:
            ...
            else
            {
                ...
                while (inputTriangle->read(cycle, (DynamicObject *&) tsInput))
                {
                    ...
                    CG_ASSERT_COND(!(setupTriangles == setupFIFOSize), "No free setup FIFO entries.");
                    GPU_DEBUG_BOX(
                    ...
                    if (shaderSetup)
                    {
                        ...
                        CG_ASSERT_COND(!(freeShTriangles == 0), "No free entries in the triangle shader queue.");
            ...
            if (shaderSetup)
            {
                ...
                for(i = 0; (i < numSetupUnits) && (sendShTriangles > 0) && (shState != SH_BUSY); i++)
                {
                    ...
                    shAttributes = new Vec4FP32[MAX_TRIANGLE_ATTRIBUTES];

                    //  Check allocation.  
                    CG_ASSERT_COND(!(shAttributes == NULL), "Error allocating triangle attributes.");
                ...
                for(i = 0; (i < trianglesCycle) && (shaderTriangles > 0); i++)
                {
                    ...
                    if (triangleShaderQueue[nextShaderTriangle].shaded)
                    {
                        ...
                        if (!triangleShaderQueue[nextShaderTriangle].tsInput->isLast())
                        {
                            ...
                            if (preTriangleBound)
                            {
                                CG_ASSERT_COND(!(!triangleShaderQueue[nextShaderTriangle].tsInput->isPreBound()), "Triangle processed at shader was not pre-bound.");
                                //  Get the Rasterizer behaviorModel ID of a previously bound triangle.  
                                triangleID = triangleShaderQueue[nextShaderTriangle].tsInput->getSetupID();
 
                                //  Ask the rasterizer behaviorModel to setup a triangle previously bound.  
                                bmRaster.setupEdgeEquations(triangleID,
                            ...
                            {
                                //  Ask the rasterizer behaviorModel to setup the triangle.  
                                triangleID = bmRaster.setup(vertAttr1, vertAttr2, vertAttr3,
                                    triangleShaderQueue[nextShaderTriangle].A,
                                    triangleShaderQueue[nextShaderTriangle].B,
                                    triangleShaderQueue[nextShaderTriangle].C,
                                    triangleShaderQueue[nextShaderTriangle].tArea);
            ...
            else
            {
                ...
                while (setupEnd->read(cycle, (DynamicObject *&) tsInput))
                {
                    ...
                    if (!tsInput->isLast())
                    {
                        ...
                        if (preTriangleBound)
                        {
                            CG_ASSERT_COND(!(!tsInput->isPreBound()), "Triangle was not pre-bound.");
                            //  Get the Rasterizer behaviorModel ID of a previously bound triangle.  
    ...
    //  Send current state to the rasterizer main mdu.  
...
void TriangleSetup::processCommand(RasterizerCommand *command)
{
    ...
    switch(command->getCommand())
    {
        ...
        case RSCOM_DRAW:
            ...
            //  Set rasterizer behaviorModel viewport.  
            bmRaster.setViewport(d3d9PixelCoordinates, iniX, iniY, width, height);

            //  Set rasterizer behaviorModel scissor rectangle.  
            bmRaster.setScissor(hRes, vRes, scissorTest, scissorIniX, scissorIniY, scissorWidth, scissorHeight);

            //  Set raterizer behaviorModel depth range.  
            bmRaster.setDepthRange(d3d9DepthRange, nearDepth, farDepth);

            //  Set rasterizer behaviorModel polygon offset.  
            bmRaster.setPolygonOffset(slopeFactor, unitOffset);

            //  Set rasterizer behaviorModel front face selection mode.  
            bmRaster.setFaceMode(faceMode);
            
            //  Set rasterizer behaviorModel d3d9 rasterization rules.
            bmRaster.setD3D9RasterizationRules(d3d9RasterizationRules);
            ...
            bmRaster.setDepthPrecission(zBitPrecission);
            ...
            */
            ...
            //minTriangleArea = (F64) .04 / ((F64) width * height);
...
void TriangleSetup::processRegisterWrite(GPURegister reg, U32 subreg,
    GPURegData data)
{
    //  Process register write.  
    switch(reg)
...
bool TriangleSetup::cullFaceTest(U32 triangleID, U32 setupID, F64 &tArea)
{
    bool dropTriangle;
    S32 minX, minY, maxX, maxY;

    //  Ask for the triangle signed area.  
    tArea = bmRaster.triangleArea(setupID);
    ...
    switch(culling)
    {
        case NONE:
            ...
            if (tArea < 0)
            {
                ...
                bmRaster.invertTriangleFacing(setupID);
        ...
        case FRONT:
            ...
            if (tArea > 0)
            {
                ...
                //  Destroy the triangle in the rasterizer behaviorModel.  
                bmRaster.destroyTriangle(setupID);
        ...
        default:
            ...
            CG_ASSERT("Unknown culling mode.");
            break;
    }
    ...
    if (!dropTriangle)
    {
        ...
        double area = abs(bmRaster.triScreenPercent(setupID)) * screen_size;
        ...
        U32 overlapsX, overlapsY;

        //  Get the triangle Bounding cmoMduBase computed at setup.  
        bmRaster.triangleBoundingBox(setupID, minX, minY, maxX, maxY);
    ...
    {
        bmRaster.selectTwoSidedColor(setupID);
    }
...
void TriangleSetup::sendTriangles(U64 cycle)
{
...
cycle, setupTriangles, setupReserves, triangleCounter, trianglesRequested, trianglesCycle, i);
                    ...
                    CG_ASSERT("Setup triangles received after last triangle mark.");
                }
            )
...
void TriangleSetup::processSetupTriangle(TriangleSetupInput *tsInput, U32 triangleID)
{
    TriangleSetupOutput *tsOutput;
    F64 tArea;
    bool dropTriangle;
    ...
    if (!tsInput->isLast())
    {
        ...
        if (!dropTriangle && (tArea == 0.0f))
        {
            ...
            //  Destroy the triangle in the rasterizer behaviorModel.  
            bmRaster.destroyTriangle(triangleID);
