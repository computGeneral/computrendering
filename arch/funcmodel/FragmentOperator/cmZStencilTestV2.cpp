...
/**
 *
 *  @file cmoDepthStencilTester.cpp
 *
...
cmoDepthStencilTester::cmoDepthStencilTester(U32 numStamps, U32 overWidth, U32 overHeight,
    ...
    U32 numStampUnits_, U32 zBlocks, U32 blocksPerCycle, U32 comprCycles, U32 decomprCycles,
    U32 zInQSize, U32 zFetchQSize, U32 zReadQSize, U32 zOpQSize, U32 zWriteQSize,
    U32 zTestRate, U32 testLat, bool hzUpdDisabled, U32 updateLatency,  U32 blockLatency,
    bmoFragmentOperator &bmFragOp, bmoRasterizer &rastEmul,
    ...
    disableHZUpdate(hzUpdDisabled), hzUpdateLatency(updateLatency),  blockStateLatency(blockLatency), bmRaster(rastEmul),
...

{
    ...
    //  Create fragment and state signals with consumer stage (Fragment FIFO mdu).
    ...
    for(int i = 0; i < freeQueueSize; i++)
    {
        ...
        freeStamp->data = new U08[STAMP_FRAGMENTS * MAX_MSAA_SAMPLES * bytesPixel[0]];
...
void cmoDepthStencilTester::postCacheUpdate(U64 cycle)
{
    U32 hzBlock;
    U32 hzBlockZ;
...
void cmoDepthStencilTester::reset()
{
    ...
    for(U32 b = 0; b < MAX_RENDER_TARGETS; b++)
...
void cmoDepthStencilTester::operateStamp(U64 cycle, ROPQueue *stamp)
{
    Fragment *fr;
    Vec4FP32 *attrib;
    ...
    for(i = 0; i < STAMP_FRAGMENTS; i++)
    {
        ...
        if (modifyDepth)
        {
            ...
            if (!multisampling)
            {
                ...
                inputDepth[i] = bmRaster.convertZ(GPU_CLAMP(attrib[POSITION_ATTRIBUTE][3], 0.0f, 1.0f));
            }
            else
            {
                ...
                //  Convert modified fragment depth to integer format.
                inputDepth[i * msaaSamples] = bmRaster.convertZ(GPU_CLAMP(attrib[POSITION_ATTRIBUTE][3], 0.0f, 1.0f));
                sampleCullMask[i * msaaSamples] = !fragmentCoverage[0];

                //  Copy Z sample values from the fragment z value computed in the shader.
                for(U32 j = 1; j < msaaSamples; j++)
                {
    ...
    //{
        cout << "<< cmoDepthStencilTester::operateStamp >>" << endl;
        ...
        cout << " | Stencil Reference = " << U32(stencilReference) << " | Stencil Test Mask = " << U32(stencilTestMask) << endl;
        cout << "Stencil Update Mask = " << U32(stencilUpdateMask) << " | Stencil Fail Function = " << stencilFail;
    ...
    if (!multisampling)
    {
        ...
        if (validationMode)
        {
            ...
            if (anyNotAlreadyCulled)
            {
                ...
                //  the quad.
                for(U32 qw = 0; qw < ((bytesPixel[0] * 4) >> 3); qw++)
                {
                    ((U64 *) quadMemUpdate.inData)[qw] = ((U64 *) inputDepth)[qw];
                    ((U64 *) quadMemUpdate.readData)[qw] = ((U64 *) stamp->data)[qw];
                }
        ...
        frEmu.stencilZTest(inputDepth, (U32 *) stamp->data, stamp->culled);
        ...
        if (validationMode)
        {
            ...
            if (anyNotAlreadyCulled)
            {
                ...
                for(U32 qw = 0; qw < ((bytesPixel[0] * 4) >> 3); qw++)
                    ((U64 *) quadMemUpdate.writeData)[qw] = ((U64 *) stamp->data)[qw];
                    
                //  Store the write mask for the quad after the write.
                for(U32 b = 0; b < (STAMP_FRAGMENTS * bytesPixel[0]); b++)
    ...
    else
    {
        ...
        for(U32 i = 0; i < msaaSamples; i++)
            frEmu.stencilZTest(&inputDepth[i * STAMP_FRAGMENTS],
                               (U32 *) &stamp->data[i * STAMP_FRAGMENTS * bytesPixel[0]],
...
void cmoDepthStencilTester::postWriteProcess(U64 cycle, ROPQueue *stamp)
{
...
void cmoDepthStencilTester::flush(U64 cycle)
{
...
void cmoDepthStencilTester::swap(U64 cycle)
{
    U32 zStencilBufferScanTiles;
    U32 zStencilBufferBlocks;
    ...
    if (!endFlush)
    {
        ...
        if(!zCache->flush())
        {
            ...
            zStencilBufferScanTiles = (U32)(ceil(vRes / F32(genH * scanH * genH * STAMP_HEIGHT))) *
            ...
            zStencilBufferBlocks = U32(ceil((zStencilBufferScanTiles * (bytesPixel[0] >> 2) *
            ...
            //  Check allocation.
            CG_ASSERT_COND(!(zStencilBlockState == NULL), "Error allocating z stencil block state memory.");
            ...
            //  Send z stencil buffer block state to the DisplayController.
            blockStateDAC->write(cycle, zStencilBlockStateInfo);

            //  Set cycles for copying the z stencil buffer block state to the DisplayController.
            copyStateCycles = (U32) ceil((F32) zStencilBufferBlocks / (F32) blocksCycle);
...
void cmoDepthStencilTester::clear()
{
...
void cmoDepthStencilTester::endBatch(U64 cycle)
{
...
void cmoDepthStencilTester::processCommand(RasterizerCommand *command, U64 cycle)
{
    U32 samples;
    ...
    switch(command->getCommand())
    {
        ...
        case RSCOM_DUMP_STENCIL:
            ...
            //  Check current state.
            CG_ASSERT_COND(!(state != RAST_READY), "DUMP_BUFFER command can only be received in READY state.");
...
void cmoDepthStencilTester::processRegisterWrite(GPURegister reg, U32 subreg,
    GPURegData data)
{
    //  Process register write.
    switch(reg)
...
void cmoDepthStencilTester::getCommandList(std::string &commandList)
{
    commandList.append("tracefragment - Traces the execution of the defined fragment.\n");
}

//  Execute a debug command
void cmoDepthStencilTester::execCommand(stringstream &commandStream)
{
    string command;
...
void cmoDepthStencilTester::saveBlockStateMemory()
{
    zCache->saveBlockStateMemory();
}

void cmoDepthStencilTester::loadBlockStateMemory()
{
    zCache->loadBlockStateMemory();
}

void cmoDepthStencilTester::setValidationMode(bool enable)
{
    validationMode = enable;
}

FragmentQuadMemoryUpdateMap &cmoDepthStencilTester::getZStencilUpdateMap()
{
