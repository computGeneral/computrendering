...
struct TextureStageState
{
    ...
    U32 colorArg0;               //  Third argument for the color operation (triadic operations MAD and LERP).  
    U32 colorArg1;               //  First argument for the color operation.  
    U32 colorArg2;               //  Second argument for the color operation.  
    D3DTEXTUREOP alphaOp;           //  Stage operation for the alpha component.  
    U32 alphaArg0;               //  Third argument for the alpha operation (triadic operations MAD and LERP).  
    U32 alphaArg1;               //  First argument for the alpha operation.  
    U32 alphaArg2;               //  Second argument for the alpha operation.  
    F32 bumpEnvMatrix[2][2];     //  Bump-mapping matrix.  
    F32 bumpEnvLScale;           //  Bump-mapping luminance scale.  
    F32 bumpEnvLOffset;          //  Bump-mapping luminance offset.  
    U32 index;                   //  Index to the texture coordinate to use for the stage.  
    D3DTEXTURETRANSFORMFLAGS transformFlags;        //  Transformation for the texture coordinates.  
    U32 resultArg;               //  Destination register for the result (current or temp).  
...
struct FFState
{
    ...
    F32 tweenFactor;         //  Tween factor for vertex blending.  
...
public:
    ...
    REGISTERID reserve()
    {
        ...
        {
            CG_ASSERT("There are no available registers.");
        }
        else
        {
    ...
    REGISTERID reserveUsage(USAGEID usage)
    {
        ...
        if (isReservedUsage(usage))
            CG_ASSERT("Usage was already reserved for a reserved register.");
    ...
    REGISTERID findUsage(USAGEID usage, bool reserve = false)
    {
        ...
        else
        {
            ...
            if(reserve)
            {
                ...
                CG_ASSERT("Could not reserve register for usage.");
        }
...
private:
    ...
    DWORD genSourceTokenForTextureStageArg(U32 arg, D3DCOLORVALUE constant, D3DRegisterId current,
    ...
    DWORD genDestTokenForTextureStage(U32 stage, D3DRegisterId current, D3DRegisterId tempReg,
    ...
    void genCodeForTextureStageOp(D3DTEXTUREOP op, U32 arg1, U32 arg2, U32 arg3,
