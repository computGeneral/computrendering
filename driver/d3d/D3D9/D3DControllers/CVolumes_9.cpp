...
void CVolumes9::on_add_node_child(StateDataNode* parent, StateDataNode* child) {
    if(child->get_id().name == NAME_VOLUME_9) {
        ...
        // Calculate size
        U32 volume_size = get_volume_size(width, height, depth, format);
        ...
        U32 md = GPUProxy::get_instance()->obtainMemory(volume_size);
        // Initialize memory
        /**@note this is necessary because during simulation accessing
           non initialized areas of the texture raises an error. */
        U08* initial_data = new U08[volume_size];
        for(U32 i = 0; i < volume_size; i ++) initial_data[i] = rand()%256;
